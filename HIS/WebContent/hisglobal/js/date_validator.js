...
DateValidator.setup = function(_containerElementID, _elementName, _value, _valueInFormat, _styleClass, _id, _tabindex, _containerElementIDHidden)
{
	try
	{
		//alert(_containerElementID + "   "+typeof _containerElementID);
		...
		var elemContainerHidden = document.getElementById(_containerElementIDHidden);		
...
/*		if(!(typeof _containerElementID!="undefined")// && typeof _containerElementID!="object" && _containerElementID.canHaveChildren==true))
		...
		var DV = new DateValidator(_elementName, null, elemContainer, _value, _valueInFormat, _styleClass);
		DV.elemContainerHidden = elemContainerHidden;
		DV.elemId = _id;
		DV.create(elemContainer,elem);
			
		var objName = "Pragyas_DateValidator_"+_elementName+"_"+DateValidator.findIndex(DV.element);
		window[objName] = DV;
		//DV.element.tabIndex =_tabindex;
		DV.element.id =_id;
		//alert(DV.element.name);
		//objName.onblur = function(){_funOnblur};
		//objName.addEventListener("blur", _funOnblur);
		
		//DateValidator.addEvent = function(objName, "blur", _funOnblur);
	}
	catch(e)
	{
		alert("Exception in Setup of Date Validator : "+e.message);
...
DateValidator.prototype.create = function(_parent,_text)
{
	...
	DateValidator.addEvent(text, "blur", DateValidator.evtValidateDateBlurReg);
	DateValidator.addEvent(text, "keypress", DateValidator.evtKeyPressDate);//ValidationFunctions.evtValidateDateFormat);

	if(this.elemName.lastIndexOf("_Dup")!=-1)
	{
		this.elemContainerHidden.innerHTML = this.elemContainerHidden.innerHTML + "<input type='hidden' name='" + this.elemName.substring(0, this.elemName.lastIndexOf("_Dup"))+ "' id='"+this.elemId.substring(0, this.elemId.lastIndexOf("_Dup"))+"'/>";
		var hidden = this.elemContainerHidden.lastChild;
		this.elementHidden = hidden;
		//hidden.name = this.elemName.substring(0, this.elemName.lastIndexOf("_Dup"));
	
		/*hidden.Pragya_Control = this;
		hidden.Control_Object = this;
		hidden.Control = this;*/
	}
...
DateValidator.evtValidateDateBlurReg = function(evnt)
{
	try
	{
		var elem = DateValidator.getTargetElement(evnt);
		...
		//alert(elem.value);
		var dt = convertStrToDate(datValObj.element.value,"dd-MM-yyyy");
		//alert(dt);
		var strHidden = convertDateToStr(dt, "dd-Mon-yyyy");
		//alert(strHidden);
		datValObj.elementHidden.value = strHidden;
		//alert(datValObj.elementHidden.value);
		//setMotherValidRule();
		//alert(elem.value);
