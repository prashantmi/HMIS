...
{ 
	...
	switch(command)
	{
		...
		case "dtltet":
		{
			retValues = compare_Date(objValue.value,cmdvalue);
			if(!retValues.cancelKey)
			{
				alert(objValue.name+": Should be a date ");
				return false;
...
function isDate(value) {
	...
	if (dtStr != "")
	{		
		seprator = get_Seperator(dtStr);		//function that returns seperator
		if (seprator != "" && dtStr.length >= 8)
		{
			retValues = parse_Date(dtStr,seprator);
...
{
//alert("date_1>date_2");
alert("Leave From Date Should be Less than or equal To Leave To Date");
return;
}
if(date_1==date_2)
{
