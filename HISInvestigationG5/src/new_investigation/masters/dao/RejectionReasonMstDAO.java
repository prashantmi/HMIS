...
public class RejectionReasonMstDAO extends DataAccessObject implements RejectionReasonMstDAOi
{

	public RejectionReasonMstDAO(TransactionContext _tx)
	{
		super(_tx);
	...
	public void createRejectionReason(RejectionReasonMasterVO rejectionreasonmaster_VO, UserVO _UserVO)
	{
		...
		String queryKey = "INSERT.HIVT_REJECTIONREASON_MST";
		try
		{
		...
		try
		{
			...
			populateMAP.put(sq.next(), rejectionreasonmaster_VO.getAcceptanceType());
			populateMAP.put(sq.next(), rejectionreasonmaster_VO.getRejectionReason());
		...
		{
			throw new HisApplicationExecutionException("RejectionReasonMaster.populateMAP::" + e);
		}
		try
		{
	...
	public void fetchRejectionReason(RejectionReasonMasterVO rejectionreasonmaster_VO, UserVO _UserVO)
	{
		...
		ResultSet rs=null;
		String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
		String queryKey="SELECT.HIVT_REJECTIONREASON_MST";

		Sequence sq= new Sequence();
		Connection conn=super.getTransactionContext().getConnection();

		
		populateMap.put(sq.next(),rejectionreasonmaster_VO.getRejectionreasonId());
		...
		try
		{
			...
			else
			{
				...
				while(rs.next())
				{
					HelperMethods.populateVOfrmRS(rejectionreasonmaster_VO ,rs);
				}
			}
		}
		catch(Exception e)
	...
	public void updateRejectionReason(RejectionReasonMasterVO rejectionreasonmaster_VO, UserVO _UserVO)
	{
		...
		Sequence sq = new Sequence();
		String filename = InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
		String queryKey ="UPDATE.HIVT_REJECTIONREASON_MST";
		try
		{
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		}
	...
	public String checkDuplicateRejectionReason(RejectionReasonMasterVO rejectionreasonmaster_VO, UserVO _UserVO)
	{
		...
		Sequence sq = new Sequence();
		String filename = InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
		String queryKey ="CHECKDUPLICATE.HIVT_REJECTIONREASON_MST";
		try
		{
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		}
		...
		try
		{
			populateMAP.put(sq.next(), rejectionreasonmaster_VO.getRejectionReason().trim());
	...
	public String checkDuplicateModifyRejectionReason(RejectionReasonMasterVO rejectionreasonmaster_VO,UserVO _UserVO)
	{
		...
		Sequence sq = new Sequence();
		String filename = InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
		String queryKey ="CHECKDUPLICITY_MODIFY.HIVT_REJECTIONREASON_MST";
		try
		{
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		}
