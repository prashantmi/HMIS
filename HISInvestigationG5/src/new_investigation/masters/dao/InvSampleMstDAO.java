...
public class InvSampleMstDAO extends DataAccessObject implements InvSampleMstDAOi
{
	...
	public void createTest(TestNewMasterVO testNewMasterVO, UserVO _UserVO)
	{
		...
		try
		{
			...
			populateMAP.put(sq.next(), testNewMasterVO.getIsextracharge());
			
			if(testNewMasterVO.getSugartestcode().equalsIgnoreCase("")||testNewMasterVO.getSugartestcode().equalsIgnoreCase(null))
			{
			populateMAP.put(sq.next(), testNewMasterVO.getMachinetestcode());
			}
			else {
				populateMAP.put(sq.next(), testNewMasterVO.getSugartestcode());
			}
			populateMAP.put(sq.next(), testNewMasterVO.getIssugartest());
			populateMAP.put(sq.next(), testNewMasterVO.getIshidefromrequisitionraising());
			populateMAP.put(sq.next(), testNewMasterVO.getMachinetest());
			
			if(testNewMasterVO.getAnyinstruction()!=null && testNewMasterVO.getAnyinstruction().equals("1"))
			{
				testNewMasterVO.setAnyinstruction(testNewMasterVO.getInstType()==null?"-1":testNewMasterVO.getInstType());
			}
			
			populateMAP.put(sq.next(), testNewMasterVO.getAnyinstruction());
			populateMAP.put(sq.next(), testNewMasterVO.getBlddrTxt());
			populateMAP.put(sq.next(), testNewMasterVO.getFastingtime());
			populateMAP.put(sq.next(), testNewMasterVO.getFastingtype());
			populateMAP.put(sq.next(), testNewMasterVO.getTurnaround_time());
			populateMAP.put(sq.next(), testNewMasterVO.getIsconfidential());
		}
		catch (Exception e)
		{
			throw new HisApplicationExecutionException("AntibodyMethodMstDAO.populateMAP::" + e);
	...
	public List getTestSampleCombo(UserVO _UserVO)
	{
		...
		return departcombo;
			 ...
			 //on modifying the test,  test code is available 
				...
				public List getFastingTypeCombo(UserVO _UserVO)
				{
					String query="";
					Map populateMap= new HashMap();
					ResultSet rs=null;
					List FastingType=new ArrayList();
					String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
					String queryKey="SELECT.getFastingeType";
					...
					try
					{
						...
						{
							rs.beforeFirst();
							FastingType=HelperMethodsDAO.getAlOfEntryObjects(rs);
						}
					}
					catch(Exception e)
					{
				...
				public List getFastingTimeCombo(UserVO _UserVO)
				{
					String query="";
					Map populateMap= new HashMap();
					ResultSet rs=null;
					List FastingTime=new ArrayList();
					String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
					String queryKey="SELECT.getFastingTime";
					...
					try
					{
						...
						{
							rs.beforeFirst();
							FastingTime=HelperMethodsDAO.getAlOfEntryObjects(rs);
						}
					}
					catch(Exception e)
					{
