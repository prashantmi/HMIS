...
import new_investigation.InvestigationpidConfig;
...
public class InvpidDAO extends DataAccessObject
{
	public InvpidDAO(JDBCTransactionContext _tx)
	{
		super(_tx);
	}
	...
	public List LabComboForResultReportPrinting(InvResultReportPrintingVO invresultreportprintingvo,UserVO _UserVO)
	{
		...
		String filename= InvestigationpidConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
		String queryKey="SELECT_LAB_COMBO_FOR_RESULT_REPORT_PRINTING_HIVT_LABORATORY_MST";
		Sequence sq= new Sequence();
		Connection conn=super.getTransactionContext().getConnection();
		
		populateMap.put(sq.next(),_UserVO.getHospitalCode());
		populateMap.put(sq.next(), InvestigationpidConfig.MODULE_ID_INVESTIGATION);
	...
	public List<InvResultReportPrintingVO> setResultReportPrintingEssentialsOnLoad(InvResultReportPrintingVO invresultreportprintingvo, UserVO _UserVO)
	{
		...
		Map populateMAP = new HashMap();
		String filename = InvestigationpidConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
		String queryKey = "SELECT.RESULT_REPORT_PRINTING_ON_LOAD_HIVT_REQUISITION_HEADER_PIDD";
	...
	public void updateResultReportPrintingDetailInRequisitionDtl(InvResultReportPrintingVO invresultreportprintingvo,UserVO _UserVO)
	{
		...
		try
		{
			...
			if(invresultreportprintingvo.getGroupCode()!=null){
				populateMAP.put(sq.next(), InvestigationpidConfig.REQDETAILS_DATATUS_FOR_INV_REPORT_PRINTING);
	...
	public List setLabNoComboEssentials(InvResultReportPrintingVO invresultentryvo,UserVO _UserVO)
	{
		...
		condition1="and hivnum_reqdtl_status ="+InvestigationpidConfig.REPORT_PDF_GEN +" order by labNo";
		else
		...
		if(invresultentryvo.getReportType().equals("0"))
			condition2="and hivnum_reqdtl_status ="+InvestigationpidConfig.REPORT_PDF_GEN_INPROCESS +" order by labNo";
		else
	...
	/*	String filename= InvestigationpidConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
		...
		String condition1="";
		if(invresultentryvo.getReportType().equals("0"))
		condition1="and hivnum_reqdtl_status in ("+InvestigationpidConfig.REPORT_PDF_GEN +",'14') order by testName";
		else
		...
		if(invresultentryvo.getReportType().equals("0"))
			condition2="and hivnum_reqdtl_status ="+InvestigationpidConfig.REPORT_PDF_GEN_INPROCESS +" order by testName";
		else
	...
	public List setSamplNoComboEssentials(InvResultReportPrintingVO invresultentryvo,UserVO _UserVO)
	{
		...
		String condition1="";
		if(invresultentryvo.getReportType().equals("0"))
		condition1="and hivnum_reqdtl_status in("+InvestigationpidConfig.REPORT_PDF_GEN +",'14') order by sampleNo";
		else
		...
		if(invresultentryvo.getReportType().equals("0"))
			condition2="and hivnum_reqdtl_status ="+InvestigationpidConfig.REPORT_PDF_GEN_INPROCESS +" order by sampleNo";
		else
	...
	public List setTestGroupComboEssentials(InvResultReportPrintingVO invresultentryvo,UserVO _UserVO)
	{
		...
		String condition1="";
		if(invresultentryvo.getReportType().equals("0"))
		condition1="and hivnum_reqdtl_status ="+InvestigationpidConfig.REPORT_PDF_GEN +" order by groupName";
		else
		...
		if(invresultentryvo.getReportType().equals("0"))
			condition2="and hivnum_reqdtl_status ="+InvestigationpidConfig.REPORT_PDF_GEN_INPROCESS +" order by groupName";
		else
		 ...
		 * 
		{
		 populateMAP.put(sq.next(), InvestigationpidConfig.REPORT_PDF_GEN);
		}
		
		else
		{
			 populateMAP.put(sq.next(), InvestigationpidConfig.REPORT_PDF_GEN_INPROCESS);
		}
		//populateMAP.put(sq.next(),onlinePatientvo.getPatCRNo());
		populateMAP.put(sq.next(),invresultentryvo.getFromDate());
		populateMAP.put(sq.next(),invresultentryvo.getToDate());
	...
	public Inv_ictc_VO getictcdetails(InvResultReportPrintingVO patVO, UserVO _UserVO) {
		Inv_ictc_VO invReqRaisingVO;
		String query = "";
		Map populateMAP = new HashMap();
		ResultSet rs = null;
		String filename = "new_investigation.transactions.investigationTransactionQuery_pidform";
		String queryKey = "SELECT_ISPID_EXIST_PIDFORM";
		Sequence sq = new Sequence();
        String condition5="";
          if(patVO.getPidno()!=null && !patVO.getPidno().equals(""))
          {
        	  condition5="And hivnum_req_no="+patVO.getReqNo();
          }
          String orderby="order by hivnum_entry_date desc limit 1" ;
	...
	public void insertPid(InvResultReportPrintingVO voLabTest, UserVO _userVO, String piddata,String followup) {
		String query = "";
		Map populateMAP = new HashMap();
		Sequence sq = new Sequence();
		String filename = "new_investigation.transactions.investigationTransactionQuery_pidform";
		String queryKey = "INSERT.HIVT_ICTC_DTL_PIDFORM";
	
		
		String reqno = "";
		String testcode = "";
	
		reqno=voLabTest.getRequisitionNo();
		...
		try {
			...
			populateMAP.put(sq.next(), followup);
			populateMAP.put(sq.next(), voLabTest.getPatGender());

			populateMAP.put(sq.next(), voLabTest.getPatName());

			populateMAP.put(sq.next(), voLabTest.getAddress());
			...
			populateMAP.put(sq.next(), voLabTest.getPatAge().trim());
			...
			populateMAP.put(sq.next(), voLabTest.getPatReqDate());
	...
	public void updatepidbeforeinsert(InvResultReportPrintingVO voLabTest,UserVO _UserVO)
	{
		String query = "";
		Map populateMAP = new HashMap();
		Sequence sq = new Sequence();
		...
		try
		{
			...
			populateMAP.put(sq.next(),voLabTest.getRequisitionNo());
	...
	public void updatepidvaluesresult(String reqno, UserVO _UserVO) {
	    String query = "";
	    Map populateMAP = new HashMap();
	    Sequence sq = new Sequence();
	    String filename = "new_investigation.transactions.investigationTransactionQuery_pidform";
	    String queryKey = "UPDATE_PID_DATA";
