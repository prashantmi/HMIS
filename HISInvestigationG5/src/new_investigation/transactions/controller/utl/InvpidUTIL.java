...
import new_investigation.transactions.controller.data.InvpidDATA;
...
public class InvpidUTIL extends ControllerUTIL
{
	...
	try{	
		...
		WebUTIL.getSession(request).setAttribute(InvestigationConfig.SYSDATEOBJECT,dt);
			...
			/*
			 * mp=InvpidDATA.LabComboForResultReportPrinting(
			 * invresultReportPrintingvo, userVO);
			 * 
			 * 
			 * String tDate =
			 * WebUTIL.getCustomisedSysDate((Date)request.getSession().getAttribute(Config.
			 * SYSDATEOBJECT), "dd-MMM-yyyy"); invresultReportPrintingvo.setFromDate(tDate);
		...
		mpp=InvpidDATA.setResultReportPrintingEssentialsOnLoad(invresultReportPrintingvo, userVO,request);
	...
	public static boolean setResultReportPrintingEssentials(InvResultReportPrintingFB fb, HttpServletRequest _request)
	{
		...
		try
		{
			...
			mp=InvpidDATA.setResultReportPrintingEssentials(invresultReportPrintingvo, userVO);
	...
	public static void showResultEntryPatDetails_viewinv(InvResultReportPrintingFB fb,HttpServletRequest request,HttpServletResponse response)
	{
		...
		try{
			...
			/*int i=0;
				...
				//mp=InvpidDATA.PdfDetails(lstInvResultReportPrintingVO,  userVO);
				...
				filename=filename+".pdf";
					...
					// response.setHeader("Cache-Control", "no-cache");
					 ...
					 // System.out.println("------------------->"+output); 
						//mp=InvpidDATA.saveResultReportPrintingDetails(lstInvResultReportPrintingVO,  userVO);
	...
	public static String isfromFTPorMONGO(HttpServletRequest request,HttpServletResponse response)
	{
		...
		flg=InvpidDATA.isfromFTPorMONGO( userVO);
	...
	public static String getEssentialdata(InvpidFB fb,HttpServletRequest request)
	{
		Status objStatus= new Status();
	...
	try{	
		//Map mp=new HashMap(); 
			 ...
			 * invresultReportPrintingvo.setLabCode("%");
			 */
		invresultReportPrintingvo.setPatCRNo(fb.getPatcrno_pid());
		invresultReportPrintingvo.setReqNo(fb.getReqno_pid());
	
		invresultReportPrintingvo.setPidno(fb.getPid()==null?"":fb.getPid());
	    
		data=InvpidDATA.setResultReportPrintingEssentialsOnLoaddata(invresultReportPrintingvo, userVO);
	...
	public static void savedata(InvpidFB fb,HttpServletRequest request,HttpServletResponse response)
	{
		Status status = new Status();
		HttpSession session=request.getSession();
		StringBuffer sbAjaxRes = new StringBuffer();
		...
		try{
			UserVO userVO = ControllerUTIL.getUserVO(request);
						...
						String data=InvpidDATA.saveResultReportPrintingDetails(fb,  userVO,request);
