...
import new_investigation.reports.dao.MachineTestReportDAO;
import new_investigation.reports.dao.SampleRejectionListingReportDAO;
...
public class MachineTestReportBO 
{
	...
	public Map fetchlab(MachineTestReportVO reqList_VO, UserVO _UserVO)
	{
		...
		{
			tx.begin();
			MachineTestReportDAO reqlist_DAO = new MachineTestReportDAO(tx);
			lstLab=reqlist_DAO.getLab( _UserVO);
			mp.put(InvestigationConfig.LIST_LAB_COMBO,lstLab);
			mp.put(InvestigationConfig.PARAMETER_MACHINE_COMBO,null);
		}
	...
	public Map getLabBasedMachine(MachineTestReportVO reqList_VO, UserVO _UserVO)
	{
		...
		try
		{
			tx.begin();
			MachineTestReportDAO reqlist_DAO = new MachineTestReportDAO(tx);
			lstTest=reqlist_DAO.getLabBasedMachine(reqList_VO, _UserVO);
			mp.put(InvestigationConfig.PARAMETER_MACHINE_COMBO,lstTest);
		}
		catch (HisRecordNotFoundException e)
		{
			tx.rollback();
	...
	public Map fetchTestOnLoad(MachineTestReportVO reqList_VO, UserVO _UserVO)
	{
		JDBCTransactionContext tx = new JDBCTransactionContext();
		Map mp=new HashMap();
		List lstTest=new ArrayList();
