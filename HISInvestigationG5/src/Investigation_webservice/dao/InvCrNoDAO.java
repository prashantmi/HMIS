...
//		}  
	...
	/*
	 * public static String AjaxBilledUnbilledDetails(Inv_SampleCollectionVO vo,
	 * UserVO userVO) {
	 * 
	 * String err = ""; String proc_name1 =
	 * "{call inv_crno_webservice.proc_billing_dtl_based_on_crno(?,?,?,?,?)}"; int
	 * procIndex1 = 0; HisDAO dao = null; WebRowSet ws = null; String status = "0";
	 * JSONObject mainObj = new JSONObject(); //for combined data ArrayList<String>
	 * columnlist = new ArrayList<String>(); ArrayList<JSONObject> jsonolist = new
	 * ArrayList<JSONObject>(); int length = 0; try { dao = new
	 * HisDAO("patient_demographic_data", "AjaxBilledUnbilledDetails()"); procIndex1
	 * = dao.setProcedure(proc_name1); dao.setProcInValue(procIndex1, "reqtype",
	 * vo.getReqType(), 1); dao.setProcInValue(procIndex1,
	 * "hospcode",userVO.getHospitalCode(), 2); dao.setProcInValue(procIndex1,
	 * "crno",vo.getCrNo(), 3); dao.setProcOutValue(procIndex1, "err", 1, 4);
	 * dao.setProcOutValue(procIndex1, "resultset",2, 5);
	 * dao.executeProcedureByPosition(procIndex1); err = dao.getString(procIndex1,
	 * "err"); if (err.equals("")) { ws = dao.getWebRowSet(procIndex1, "resultset");
	 * length = ws.getMetaData().getColumnCount(); for (int i = 1; i <= length; i++)
	 * { columnlist.add(ws.getMetaData().getColumnName(i) .toUpperCase()); }
	 * 
	 * } if (ws != null && ws.size() > 0) { while (ws.next()) { //int j = 0; status
	 * = "1"; JSONObject js = new JSONObject(); for (int i = 1; i <= length; i++) {
	 * String key = columnlist.get(i - 1); String value = ws.getString(i);
	 * js.put(key, value);
	 * 
	 * } jsonolist.add(js);
	 * 
	 ...
	 * 
	 * mainObj.toString(); return mainObj.toString(); }catch (Exception e) {
	 * e.printStackTrace(); return mainObj.toString(); }
	 * 
