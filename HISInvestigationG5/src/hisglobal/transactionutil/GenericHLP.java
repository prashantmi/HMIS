...
public class GenericHLP {
	...
	public String generateData(GenericVO vo, String[] columnHdr,
		...
		try {
			...
			br
					.append("<TABLE BORDER='0' CELLSPACING='1px' CELLPADDING='1px' WIDTH='100%'><TR><TD CLASS='TDCOMBOHEADER'><TABLE WIDTH='100%' CELLSPACING='1px' CELLPADDING='1px'>");
			br.append("<TR>");
			...
			if(vo.getExtraInfoData()!=null && vo.getExtraInfoData().length>0)
			{
				...
				br.append(""+ HdrNote+"</DIV></TD>");
			}
			else
			{
				br.append("<TD CLASS='TDNOTES' WIDTH='50%'><DIV ALIGN='LEFT'>"+ HdrNote	+ "</TD>");
			}
			br.append("<TD WIDTH='50%'><DIV ALIGN='RIGHT'><FONT COLOR='WHITE'>");
			...
			br.append(getTotalBlock(nextpage_no, total_page, max_rownum,
					minrow, blockNo, clr, vo));

			br.append("</FONT></DIV></TD></TR></TABLE></TD></TR></TABLE>");
			//br.append("<TABLE width='100%' bgcolor='white'><TR>");
			//br.append("<td>");
			//br.append("<td>");
			//br.append("</TABLE></TR>");
			//br.append("<div id='scroll' style='width:570px;height:310px; overflow-x:scroll ; overflow-y: hidden; padding-bottom:10px;white-space: nowrap;'>");
			//br.append("<TABLE WIDTH='100%' BORDER='0'> <TR><TD>");
			...
			if (rowsetSize <= 0) {
				...
				br.append("<TABLE  WIDTH='100%' ALIGN='CENTER' CELLPADDING='1' CELLSPACING='1'>"
								+ "<TR class='ROWFONT'><TD width='10%' COLSPAN='"
								+ (columnHdr.length + 1)
								+ "'><b>No Record Found !!</b></TD></TR></TABLE>");
			}
			
			//br.append(getCheckBoxData(rowset, (100 / columnHdr.length), minrow,
				//	eventState, rowStatus, vo,columnHdr,ordrBy));
			br.append(getQueryData(rowset, (100 / columnHdr.length), minrow,
					eventState, rowStatus, vo,columnHdr,ordrBy));
			//br.append("</div>");
			br.append(getSearchRowString(vo, minrow, max_rownum, blockNo,
					total_page, searchFld, columnHdr, pagePerBlock, masterName,
					menuOption, viewHdr));

			br
					.append("####<TABLE BORDER='0' CELLPADDING='1' CELLSPACING='1' ALIGN='center' WIDTH='100%'><TR><TD CLASS='TDHEADER'><TABLE WIDTH='100%'>"); // to
			br.append("<TD CLASS='TDNOTES'>" + footerNote + "</TD>");
			br.append("<TD><DIV ALIGN='RIGHT'>");
			try{
				for (int q = 0; q < rowStatus.length; q += 4) {
					if(!rowStatus[q + 2].equals("")&&!rowStatus[q + 3].equals(""))
					br.append("[<FONT COLOR='" + rowStatus[q + 2] + "'>"
							+ rowStatus[q + 3] + "</FONT>] ");
				}
			}catch(Exception _e){
				
			}
			br.append("</DIV></TD></TABLE></TD></TR></TABLE>");

			br.append("####"
					+ getCombo(vo, no_of_Combo, comboHdr, comboQry, masterName,
							comboData, comboEventState));
			br.append("####" + buttons);

		} catch (Exception e) {
			vo.setStrMsgString("GenericHLP.generateData() --> "
					+ e.getMessage());
			vo.setStrMsgType("1");
		} finally {
			comboQuery = null;
	 ...
	 * @return
	 */
	public String getCombo(GenericVO vo, int no_of_Combo, String[] comboHdr,
			String[] comboQry, String masterName, String[] comboData,
			String comboEventState) {
		...
		try {
			...
			br
					.append("<TABLE WIDTH='100%' ALIGN='CENTER' BORDER='0' CELLSPACING='0px' CELLPADDING='0px'>"
							+ "<TR><TD CLASS='TDHEADER'>"
							+ masterName
							+ "</TD><TD CLASS='TDHEADER'><DIV align='right'>"
							+ util.getASDate("dd-MMM-yyyy/HH:mm:ss")
							+ "  "
							+ (String) vo.getSession().getAttribute("USERNAME")
							+ " <img style='cursor: pointer;' src='../../hisglobal/images/stop.png' " 
							+ "onclick='goToMainPage()'>"
							+ "</TD></TR></TABLE>"
							+ "<TABLE WIDTH='100%' ALIGN='CENTER' BORDER='0' CELLSPACING='1px' CELLPADDING='1px'><TR>"
							+ "<TD  CLASS='TDCOMBOHEADER' >");
			...
			for (int p = 0; p < no_of_combo; p++, i++) {
				...
				if (combo_header[2 * p].equals("1")) {
					...
					for (int k = 0; k < userComboList.length; k++) {
						userComboValue = userComboList[k].replace('^', '#')
								.split("#");
						if (combo != null && combo[p].equals(userComboValue[0]))
							combo_data += "<OPTION SELECTED VALUE='"
									+ userComboValue[0] + "'>"
									+ userComboValue[1] + "</OPTION>\n";
						else
							combo_data += "<OPTION VALUE='" + userComboValue[0]
									+ "'>" + userComboValue[1] + "</OPTION>\n";
					}
				} else {
				...
				/*Changed By Amit Kumar Ateria on 30 Jun 2011 Check if Combo mandatory or not
				as per change request dated 13 Jun 2011 */
				
				if(combo_header[2 * p + 1].replace("^","#").split("#").length>=2 && combo_header[2 * p + 1].replace("^","#").split("#")[1].equals("1"))
				{
				br.append("<B><font color='red'>*</font>&nbsp;" + combo_header[2 * p + 1].replace("^","#").split("#")[0]
						+ "</B>&nbsp;<SELECT NAME='combo' id='comboid" + p
						+ "' TITLE='Select Combo' CLASS='CB1' onChange='"
						+ jsFunc + "'>" + combo_data
						+ "</SELECT>&nbsp;&nbsp;&nbsp;&nbsp;");
				}
				else
				{
					br.append("<B>" + combo_header[2 * p + 1]
							+ "</B>&nbsp;<SELECT NAME='combo' id='comboid" + p
							+ "' TITLE='Select Combo' CLASS='CB1' onChange='"
							+ jsFunc + "'>" + combo_data
							+ "</SELECT>&nbsp;&nbsp;&nbsp;&nbsp;");
	...
	public String getColoumnHeaderString(String[] columnHdr, GenericVO vo,
		...
		try {
			data
					.append("<TD CLASS='TDFONTHEAD'><INPUT TYPE='CHECKBOX' NAME='chkmain' onclick='isCheckedFirst(),chkUserFunc(this);;'></TD>");
	...
	public String getColoumnChecBoxHeaderString(String[] columnHdr, GenericVO vo,
		...
		try {
			//data.append("<TD CLASS='TDFONTHEAD'><INPUT TYPE='CHECKBOX' NAME='chkmain' onclick='isCheckedFirst(),chkUserFunc(this);;'></TD>");
			data.append("<TR>");
				...
				if (temp % rec_per_page == 0) {
					...
					if (idvalue.equals("a1"))
						dataBuffer
								.append("<DIV ID='"
										+ idvalue
										+ "' STYLE='DISPLAY:BLOCK;position:relative'><TABLE width='100%' bgcolor='white'><TR>"+str+"");
					else
						dataBuffer
								.append("<DIV ID='"
										+ idvalue
										+ "' STYLE='DISPLAY:NONE;position:relative'><TABLE width='100%' bgcolor='white'><TR>"+str+"");
			...
			while (rowset.next()) {
				...
				if(coulmn_header[m].replace("^","#").split("#").length>=2)
				{
					...
					if (m == 0) {
						if(count==1)
						{
							dataBuffer1.append("<TR><TD CLASS='TDFONTHEAD'><INPUT TYPE='CHECKBOX' NAME='chkmain' onclick='isCheckedFirst(),chkUserFunc(this);;'></TD></TR>");
							count=0;
						}
						if (flag) {
	...
	public String getSearchRowString(GenericVO vo, int minrow, int max_rownum,
		...
		try {
			...
			{
				br.append("####<TABLE WIDTH = '100%' ALIGN='CENTER' BORDER='0' cellspacing='0' cellpadding='0' >");
			} 
			else 
			{
				br.append("####<TABLE WIDTH = '100%' ALIGN='CENTER' BORDER='0' cellspacing='0' cellpadding='0' >");
			...
			br.append("<TR><TD CLASS='TDSEARCH'> <DIV ALIGN='RIGHT' style='vertical-align: middle'> Search By&nbsp;&nbsp;");
			...
			br.append("&nbsp;&nbsp;&nbsp;<A STYLE='CURSOR:POINTER'><IMG SRC='../../hisglobal/images/search_icon1.gif' alt='Click To Search Records' onClick='searchpage();' tabindex='0' onKeyPress='if(event.keyCode==13) searchpage();'></A></DIV></TD></TR>");
	...
	public String getTotalBlock(int nextpage_no, int total_page,
			int max_rownum, int minrow, int blockNo, String colorNextPrev,
			GenericVO vo) {
		char temp1 = 'a';
		int t = 1;
		int temp = 1;
			...
			for (int k = 1; k <= total_page; k++) 
			{
				if (actual_page_block > 1) 
				{
					...
					{/*
						 ...
						 * STYLE='CURSOR:POINTER' onClick=changeDiv('" + temp1 + "" +
						 * k + "')><B><U>" + (nextpage_no +1) + "</U></B></A>
						 * ");
						 */
						data.append("<A ID='bb"+ temp1+ ""+ k
										+ "' STYLE='CURSOR:POINTER' onClick=changeDiv('"+ temp1 + "" + k + "')><B><U>"
										+ nextpage_no + "</U></B></A> ");
	 ...
	 */
	public String getButtonLayOut1(String[] userDefinedButton,
			String menuOption, String buttonConfig, int minPanelButton,
			int recPerPage, GenericVO vo, String[] dbButtons) {
		...
		try {
			...
			if (userDefinedButton == null) {
				bo.getDBButtons(vo, (String) vo.getSession().getAttribute(
						"SEATID"), (String) vo.getSession().getAttribute(
						"HOSPITAL_CODE"), dbButtons[0]);
				userDefinedButton = vo.getStrDBButton();
				buttonConfig = userDefinedButton[0].split("@")[3];
			}
			if (userDefinedButton == null
					|| userDefinedButton[0].split("@").length == 1) {
				if (userDefinedButton != null) {
				...
				if (buttonConfig.equalsIgnoreCase("left")
						|| buttonConfig.equalsIgnoreCase("right")) {
					for (int i = 0; i < no_of_button; i++) {
						nTableCount = 1;
						sb.append("<div id='buttonL1Id" + (i + 1)
								+ "' style='display:block'>");
						sb
								.append("<TABLE  width=\"100%\"  border=\"1\" cellpadding=\"1\" cellspacing=\"1\" height=\""
										+ btnHeight
										+ "\" bgcolor=\""
										+ btnBgColor
			...
			else//Database Defined Buttons 
			{
				...
				if (buttonConfig.equalsIgnoreCase("left") || buttonConfig.equalsIgnoreCase("right")) 
				{
					for (int i = 0; i < no_of_button; i++) 
					{
						...
						if (buttons[i].split("@")[2].equals("0")) 
						{
							sb.append("<div id=\""
											...
											+ (buttons[i].split("@")[1]
													.equals("") ? "" : ","
													+ buttons[i].split("@")[1])
											+ ";\" onmousedown=\"javascript:this.className ='m0l0oover'\" onmouseup=\"javascript:this.className ='m0l0ooout'\" onmouseup=\"javascript:this.className ='m0l0ooout'\"><div id=\"eo__i\" class=\"logoutOver\">"
											+ buttons[i].split("@")[0]
											+ "</div></div></div>");
							sb
									.append("<div id=\""
						...
						if (userDefinedButton[i].split("@").length == 5
								&& (i == no_of_button - 1 || !(userDefinedButton[i]
										.split("@")[4]
										.equals(userDefinedButton[i + 1]
												.split("@")[4])))) {
							sb.append("</TABLE>");
							sb.append("</div>");
				...
				if (buttonConfig.equalsIgnoreCase("both")) {
					if (minPanelButton >= avgButton) {
						sb.append("<TABLE  width=\"100%\" bgcolor="
								+ btnBgColor + ">");
						for (int i = 0; i < minPanelButton; i++) {
							sb.append("<div id='buttonLL1Id" + (i + 1)
									+ "'  style='display:block'>");
			...
			if (menuOption.equalsIgnoreCase("tiles")) {
				if (nTableCount == 1)
					for (int i = count; i < recPerPage ; i++) {
						footer
								.append("<table width=\"100%\"  border=\"0\" cellpadding=\"1\" cellspacing=\"1\" bgcolor=\""
										+ btnBgColor + "\">");
						footer.append("<TR><td align=\"center\" height=\""
								+ btnHeight + "\">&nbsp;</td></TR></table>");
					}
				else {
					for (int i = 0; i < nTableCount; i++) {
						footer
								.append("<div style='display:none' id='vaccantRowDiv"
										+ userDefinedButton[Integer
												.parseInt(strButtonTableEvent[i])]
												.split("@")[4] + "'>");
						if (i != nTableCount - 1)
							for (int j = Integer
									.parseInt(strButtonTableEvent[i + 1])
									- Integer.parseInt(strButtonTableEvent[i]); j < recPerPage; j++) {
								footer
										.append("<table width=\"100%\"  border=\"0\" cellpadding=\"1\" cellspacing=\"1\" bgcolor=\""
												+ btnBgColor + "\">");
								footer
										.append("<TR><td align=\"center\" height=\""
												+ btnHeight
												+ "\">&nbsp;</td></TR></table>");
							}
						else
							for (int j = no_of_button
									- Integer.parseInt(strButtonTableEvent[i]); j < recPerPage; j++) {
								footer
										.append("<table width=\"100%\"  border=\"0\" cellpadding=\"1\" cellspacing=\"1\" bgcolor=\""
												+ btnBgColor + "\">");
			...
			sb.append("<tr><td CLASS=\"TDCOMBOHEADER\"><div align='center'>No Access Policy Has been defined Please Define the Access Policy</div></td></tr></table>");
			e.printStackTrace();
			vo.setStrMsgString("GenericVO.getButtonLayOut1() --> "
					+ e.getMessage());
			vo.setStrMsgType("1");
		}
		if (flag)
	...
	public String getButtonLayOut2(String[] userDefinedButton,
			String menuOption, String buttonConfig, int minPanelButton,
			int recPerPage, GenericVO vo, String[] dbButtons) {
		StringBuffer sb = new StringBuffer("");
		StringBuffer footer = new StringBuffer("");
		...
		try {
			...
			if (buttonConfig.equalsIgnoreCase("both")) {
				if ((minPanelButton >= no_of_button)
						|| (minPanelButton >= avgButton)) {
					flag = false;
				} else {
					flag = true;
					if (minPanelButton >= avgButton) {
						...
						for (int i = minPanelButton; i < no_of_button; i++) {
							sb.append("<div id='buttonLR1Id" + (i + 1)
									+ "' style='display:block'>");
							sb.append("<tr><td height='23'>");
							if (buttons[i].split("@")[2].equals("0")) {
								sb
						...
						for (int i = avgButton; i < no_of_button; i++) {
							sb.append("<div id='buttonLR2Id" + (i + 1)
									+ "' style='display:block'>");
							sb.append("<tr><td height='23'>");
							if (buttons[i].split("@")[2].equals("0")) {
								sb
			...
			vo.setStrMsgString("GenericVO.getButtonLayOut2() --> "
					+ e.getMessage());
			vo.setStrMsgType("1");
		}
		if (flag)
