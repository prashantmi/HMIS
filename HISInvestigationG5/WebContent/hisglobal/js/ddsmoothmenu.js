...
arrowimages: {down:['downarrowclass', '/HISInvestigationG5/hisglobal/images/down.gif', 23], right:['rightarrowclass', '/HISInvestigationG5/hisglobal/images/right.gif', 6]},
transition: {overtime:200, outtime:700}, //duration of slide in/ out animation, in milliseconds
shadow: false, //enable shadow? (offsets now set in ddsmoothmenu.css stylesheet)
showhidedelay: {showdelay: 100, hidedelay: 200}, //set delay in milliseconds before sub menus appear and disappear, respectively
zindexvalue: 101, //set z-index value for menus
...
repositionv: function($subul, $link, newtop, winheight, doctop, method, menutop){
	//alert('menutop: '+ menutop+ ' newtop: '+ newtop);
	menutop = menutop || 0;
	var topinc = 0, doclimit = winheight + doctop;
	$subul.css({top: newtop, display: 'block'});
	/*while($subul.offset().top < menutop) {
		$subul.css({top: ++newtop});
		++topinc;
	}*/
	$subul.css({top: 25});
	topinc=100;
	//alert('1  topinc'+ topinc+' $link.outerHeight(): '+ $link.outerHeight());
	//alert('topinc: '+ topinc+ ' menutop: '+ menutop+ 'winHeight: '+ winheight+ 'doctop: '+ doctop+ 'newtop: '+ newtop+' $link.offset().top: '+ $link.offset().top+ ' $link.outerHeight(): '+ $link.outerHeight()+ ' $subul.data(height): '+ $subul.data('height')+'$link.parents(ul).last().offset().top: '+$link.parents('ul').last().offset().top);
	if(!topinc && $link.offset().top + $link.outerHeight() < doclimit && $subul.data('height') + $subul.offset().top > doclimit){
		//$subul.css({top: doctop - $link.parents('ul').last().offset().top - $link.position().top});
		$subul.css({top:0});
		//alert('2   top: '+ $subul.css({top:$link.outerHeight()}));
	}
...
buildsubheaders: function($, $headers, setting, method, prevobjs){
	...
	$headers.each(function(){ //loop through each LI header
		var smoothmenu = ddsmoothmenu;
		var $curobj=$(this).css({zIndex: parseInt($(this).parent('ul').css('z-index'))+10 }); //reference current LI header
		...
		$subul.css({top: 0,zIndex: parseInt($curobj.css('z-index'))+10});
		$subul.children().css({zIndex: parseInt($subul.css('z-index'))+10});
		
		$curobj.parent().css({zIndex: parseInt($curobj.parent().css('z-index'))-1});
