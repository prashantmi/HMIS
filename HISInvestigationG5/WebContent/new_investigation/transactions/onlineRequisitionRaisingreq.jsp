...
function setdata_rcmd(setis, valuee)
{
	//alert("setis "+setis);
	//alert("valuee "+valuee);
	//var setis="1024#10001";
	//var valuee="12815^12814^12813^12822^12821^12820^12817^12819^12818^12816#10001#2#1024";
	//var setis="1281410001";
	//var valuee="12814#10001";
	mapdatacheckbox.set(setis,valuee);
	addall();
	//get_codes();
}

function add_rcmd(id){
	//alert(id);
	//id = $(id).closest('tr').index();
	var testcode = testcodelist[id];
	var testname = testnamelist[id];
	//var testname = document.getElementById("rcmd_table").rows[id].cells[0].innerHTML;
	var labcode = labcodelist[id];
	//alert(testcode);
	//alert(testname);
	//alert(labcode);
	if (statecode==1){
	if (testcode.toString()=="1054")
		testcode = "1024";
	if (testcode.toString()=="12745")
		testcode = "12814";
	if (testcode.toString()=="12746")
		testcode = "12815";
	if (testcode.toString()=="11837")
		testcode = "12829";
	if (labcode.toString()=="10003")
		labcode = "10001";
	}
	
	if (testcode.toString().length==4){
		//alert("group");
		group_test_codes = get_codes(testcode);
		//alert(testcode+"#"+labcode+"%%%%"+group_test_codes+"#"+labcode+"#2#"+testcode);
		setdata_rcmd(testcode+"#"+labcode, group_test_codes+"#"+labcode+"#2#"+testcode);
	}
	else{
		//alert("test");
		//alert(testcode+labcode+"%%%%"+testcode+"#"+labcode);
		setdata_rcmd(testcode+labcode, testcode+"#"+labcode);
	}
	//setdata_rcmd(testcode+labcode, testcode+"#"+labcode);
	//setdata_rcmd("1281410001", "12814"+"#"+"10001");
	//setdata_rcmd("1024#10001", "12815^12814^12813^12822^12821^12820^12817^12819^12818^12816#10001#2#1024");
	//var testdatafinal=  ajaxhitforfetchingtestwisedata('12814'+"^"+'10001');	
	//var testdatafinal=  ajaxhitforfetchingtestwisedata(testcode+"^"+labcode);
	//var dd2=maktestinrowtable(testdatafinal);
	//document.getElementById("rcmd_table").deleteRow(parseInt(id));
	testcodelist.splice(id, 1);
	labcodelist.splice(id, 1);
}

function get_codes(for_this_code){
	//alert("hoscode "+for_this_code);
	var return_data = "";
	$.ajax({  
        url:"/HISInvestigationG5/new_investigation/investigationChatBot.cnt?hmode=test_codes&test_codes="+for_this_code, async:false,
        method:"POST",
        success:function(data)
        {
        	return_data = data;
        	//alert("in get codes = "+data);
        },
   });
	return return_data;
}
function get_statecode(){
	var return_data = "";
	$.ajax({  
        url:"/HISInvestigationG5/new_investigation/investigationChatBot.cnt?hmode=statecode", async:false,
        method:"POST",
        success:function(data)
        {
        	return_data = data;
...
function get_ip(){
	var return_data = "";
	$.ajax({  
        url:"/HISInvestigationG5/new_investigation/investigationChatBot.cnt?hmode=ip", async:false,
        method:"POST",
        success:function(data)
        {
        	return_data = data;
...
function get_key(){
	var return_data = "";
	$.ajax({  
        url:"/HISInvestigationG5/new_investigation/investigationChatBot.cnt?hmode=key", async:false,
        method:"POST",
        success:function(data)
        {
        	return_data = data;
...
function get_names_from_this_jsp(codes, names){
	var availableTests =<%=(String)session.getAttribute(InvestigationConfig.ARRAY_TESTNAMES_XRAYPROCESS)%>;
	//console.clear();
/* 	var index = codes.indexOf(1054);
	if (~index) {
		codes[index] = 1024;
	} */
	//availableTests[key].value.split("#")[0]
	//console.log(names);
	//console.log(codes);
	for (var a in codes){
		for (var b in availableTests){
			var checkwiththis = "";
			if (availableTests[b].value.split("#")[0].includes("^"))
				checkwiththis = availableTests[b].value.split("#")[3];
			else
				checkwiththis = availableTests[b].value.split("#")[0];
			//console.log(codes[a]+" checked againt "+checkwiththis);
			if (codes[a]==checkwiththis){
				//console.log(names[a]+" is replaced by "+availableTests[b].label);
				names[a]=availableTests[b].label;
			}
			//console.log(availableTests[b].value.split("#")[0]);
		}
	}
	//console.log(names);
	return (names);
}
window.onpageshow=function openbox() {
 $.ajax({  
        url:"/HISInvestigationG5/new_investigation/investigationChatBot.cnt?hmode=show_rcmd", sync:false,
        method:"POST",
        success:function(data)
        {
       	// alert("show rcmd"+data);
		  if (data=='0')
			  show_rcmd=false;
        },
        error:function(error){
...
var ip = "";
var key = "";
var initial_rcmd = true;
function rcmd_api_call(testdata){
	if (age_code_mll.includes("Y")||age_code_mll.includes("M")||age_code_mll.includes("D")){
		age_code_mll = age_code_mll.split(" ")[0];
	}
	if (show_rcmd){
		var rcmd_api = "";
		if (initial_rcmd){
			rcmd_api =ip+"/?age="+age_code_mll+"&dept="+dept_code_mll+"&gender="+gender_code_mll+"&hosCode="+hos_code_mll+"&secretKey="+key+"&algotype=1&ncodes=1&statecode="+statecode;
			document.getElementById('rcmd_tests_outer').innerHTML  = 'Frequently Raised Test: <div id="rcmd_tests" style="display:inline;"></div>';
			initial_rcmd = false;
		}
		else{
			rcmd_api =ip+"/?age="+age_code_mll+"&dept="+dept_code_mll+"&gender="+gender_code_mll+"&codes="+test_code_mll_string+"&hosCode="+hos_code_mll+"&secretKey="+key+"&algotype=1&ncodes=1&statecode="+statecode;
			document.getElementById('rcmd_tests_outer').innerHTML  = 'Frequently Raised Together: <div id="rcmd_tests" style="display:inline;"></div>';
		}
		$.ajax({
	    ...
	    url: rcmd_api,
	    success: function(data){
	    	//console.log(rcmd_api);
	    	//console.log(data);
	    	//test_code_mll=[];
	    	//$(".rcmd").append("<ul_rcmd></ul_rcmd>");
	    	//console.log(data.codes);
	    	data.names = get_names_from_this_jsp(data.codes, data.names);
	    	//console.log(data.codes);
	    	testcodelist = data.codes;
	    	labcodelist = data.lab_codes;
	    	testnamelist = data.names;
	    	var x = document.getElementById("rcmd_id");
	    	x.style.display = "block";
	    	/* var table = document.getElementById("rcmd_table");
	    	var tableHeaderRowCount = 0;
	    	var rowCount = table.rows.length;
	    	for (var i = tableHeaderRowCount; i < rowCount; i++) {
	    	    table.deleteRow(tableHeaderRowCount);
	    	} */
	    	//console.log("RCMD APPEARED")
	    	var div = document.getElementById("rcmd_tests");
			div.innerHTML = "";
			for(var i in data.codes) {
			    //var li = "<li>";
			    //$("ul_rcmd").append(li.concat(data.codes[i]))
			    if (statecode==1){
			    if (data.names[i].toString().includes("Kidney Function Test")){
			    	//alert("in alert");
			    	data.names[i]="Liver Function Test";
			    }
			    }
			    var div = document.getElementById("rcmd_tests");
			    div.innerHTML += "---";
			    div.innerHTML += "<a class='add' title='Add' onclick='add_rcmd("+i+")'> "+data.names[i]+" </a>";
			    		    
			    /* var row = table.insertRow(i);
		    	var cell1 = row.insertCell(0);
		    	var cell2 = row.insertCell(1);
				cell1.innerHTML = data.names[i];
				cell2.innerHTML = "<a class='add' title='Add' data-toggle='tooltip' ><i class='fa fa-plus' aria-hidden='true' onclick='add_rcmd(this)'></i></a>";
			 */
			}
	    }
		});
...
{
       //alert("in maktestinrowtable "+testdata);      
 ...
 if(tests[5]=="S") // Sample or Slide Based
	...
	// cols += '<td>Freechanks</td>';
     ...
     cols += "<td><a class='deletee "+testco1+isgrp+"' title='Delete' data-toggle='tooltip' id="+testco1+" ><i class='fa fa-trash' aria-hidden='true'></i></a></td><input id='"+tests[2]+"#"+tests[0]+"' type='hidden' name='selectedtestdataatrray' value='"+datattosave+"'/><input id='"+tests[2]+"charge' type='hidden' name='testcharge' value='"+datattosave+"'/><input id='"+tests[2]+"#"+tests[0]+"view' type='hidden' name='selectedtestviews' value=''/>";
	   ...
	   // alert("isapptshw"+isapptshw+" ::"+tests[28]);
       ...
       //abhijat
       //alert("dept_code_mll "+dept_code_mll);
       age_code_mll = age_code_mll.split(" ")[0];
       //alert("age_code_mll "+age_code_mll);
       //alert("gender_code_mll "+gender_code_mll);
       var t1 = null;
       if (testdata.split("#")[10]== 0){
    	   t1 = testdata.split("#")[2];
       }
       else{
       		t1 = testdata.split("#")[10];
       }
       test_code_mll.push(t1);
       test_code_mll = unique(test_code_mll);
       test_code_mll_string = test_code_mll.join(",");
       //alert("test_code_mll "+test_code_mll_string);
       //alert("hos_code_mll "+hos_code_mll);
       rcmd_api_call(testdata);
       return "1" ;
    ...
    */
	...
	$(document).ready(function() {
		...
		//	alert(grp_test);
			...
			var del_ml = "";
			if (newclass1.includes("group")){
				del_ml = newclass1.replace(/group/g, '');
			}
			else if (newclass1.includes("test")){
				del_ml = newclass1.replace(/test/g, '');
			}
			//alert("del_ml= "+del_ml);
			//alert("test_code_mll= "+test_code_mll);
			var index = test_code_mll.indexOf(del_ml);
			test_code_mll.splice(index, 1);
			initial_rcmd = true;
			rcmd_api_call(0);
			//alert("test_code_mll= "+test_code_mll);
		});
...
<logic:present name="<%=InvestigationConfig.PATIENT_VO%>">
              ...
              Inv_RequisitionRaisingPatientVO  lstPatVO=(Inv_RequisitionRaisingPatientVO)session.getAttribute(InvestigationConfig.PATIENT_VO);
           	...
           	String gender_code_ml = lstPatVO.getPatGenderCode(); //abhijat
           	String age_code_ml = lstPatVO.getPatAge(); //abhijat
           	//String hos_code_ml = lstPatVO.getHospitalcode(); //abhijat
           	//String dept_code_ml = lstPatVO.getDepartment();
           	%>
           	<script>
           	gender_code_mll = '<%=gender_code_ml%>'; //abhijat
           	age_code_mll = '<%=age_code_ml%>'; //abhijat
           	<%-- dept_code_mll = '<%=dept_code_ml%>'; --%> 
           	<%-- hos_code_mll = '<%=hos_code_ml%>'; //abhijat --%>
			    	...
			    	for(int k=0;k<sizePatEpisodeVO;k++)
			 		{
			    		...
			    		//abhijat
			    	 	String dept_code_ml=voPatEpisode.getDepartmentcode()==null ? "" : voPatEpisode.getDepartmentcode();
			    	 	String hos_code_ml=voPatEpisode.getHospitalcode()==null ? "" : voPatEpisode.getHospitalcode();
			        	...
			        	for(int kp=0;kp<reason.length;kp++)
			        		...
			        		dept_code_mll='<%=dept_code_ml%>'; //abhijat
			        		hos_code_mll = '<%=hos_code_ml%>';
...
<div class=" container-fluid mt-2 mb-3" id="cont5">
	...
	<div class=" row p-2 shadow rounded bg-white no-guttersNEW">
			...
			<div  class="col-12 col-md-10 rounded">
				...
				<!-- added by abhijat for rcmd -->
				<div class="container" id="rcmd_id" style="display:none">
				<div><div class="p-2 rounded" id="rcmd_tests_outer" style="display:inline;">Frequently Raised Together: <div id="rcmd_tests" style="display:inline;"></div></div></div>
				<!-- <table id="rcmd_table">
				</table> -->
				</div>
				...
				<!-- added by abhijat for rcmd -->
				<!-- <div class="rcmd" id="rcmd_id" style="display:none">
				<div class="p-2 rounded" id="rcmd_tests" style="background: #15658B;"><div class="text-light text-nowrap" align="center"><b>Following tests are also raised along with selected tests.</b></div></div>
				<table id="rcmd_table">
				</table>
				</div> -->
