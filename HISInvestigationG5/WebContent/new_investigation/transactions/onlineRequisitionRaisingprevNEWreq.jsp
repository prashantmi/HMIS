...
function deleteRowPrvReqDtl(labCode,testCode,reqNo,testGroupCode1,testname,viewcount,totalreqviewcount,type,crno,grpname)
 ...
 function checkBillingDetail(labCode,testCode,reqNo)
{
	...
	var objXHR = {url: "/HISInvestigationG5/new_investigation/onlineRequisitionRaisingprevreqNEW.cnt?hmode="+_mode+"&delLabCode="+labCode+"&delTestCode="+testCode+"&requisitionNo="+reqNo, sync:true, postData: "", handleAs: "text",
		load: function(data) 
		{
			//alert(data);
			isBillingDone = data;
...
<body onload="setprevreqall()">
	<html:form action="/onlineRequisitionRaisingprevreqNEW" >
<!-- 
style="border: 7px solid white;border-radius: 2rem !important;background-color:gainsboro;" -->
...
<div
					...
					<table id="preqvreqtbl" class="table table-condensed  table-xs  table-hover table-responsive order-listoftst" style="font-size: smaller;">
  ...
  Map<String,LabTestVO> _mppprevreq=(Map<String,LabTestVO>)session.getAttribute(InvestigationConfig.LIST_PREV_REQ_XRAY);
    ...
    %>
      ...
      <%= voPatEpisodeprev.getTestName().trim()%>
       ...
       <td ><%=voPatEpisodeprev.getTestGroupName().equals("NA")?"-":voPatEpisodeprev.getTestGroupName() %></td>
             <%--  <td><%="Patient Based" %></td> --%>
              <td ><%=voPatEpisodeprev.getSampleCode() %></td>
<td><%=voPatEpisodeprev.getStatus() %></td>
              <%-- <td><span style="background-color:<%=color %>" class="badge badge-pill badge-secondary"><%=voPatEpisodeprev.getStatus() %></span></td> --%>
  ...
  %>
   ...
   <html type="hidden" name="hmode" />
   		...
   		<html:hidden name="InvestigationRaisingDtlreqFB" property="prevreqfromwhichcall" />
      		<html:hidden name="InvestigationRaisingDtlreqFB" property="patCrNo" />
