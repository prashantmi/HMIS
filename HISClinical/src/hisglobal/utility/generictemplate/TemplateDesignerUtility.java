...
public class TemplateDesignerUtility 
{
	public static class Template
	{
		...
		Map<String, Object> mapSessionContent;
		
		
		private Template(){}
		
		public static Template getTemplate(String _tempId, String _tempType, List<TemplateParameterMasterVO> _lstTempParaVO, Map<String , ParameterRangeMasterVO> _mapParaRange, InformationControlBean _infoBean, String _genderType, Map<String, Object> _mapSessionContent)//HttpServletRequest _request)
		{
			...
			temp.mapSessionContent = _mapSessionContent;
		...
		public static Template getTemplate(String _tempId, String _tempType, List<TemplateParameterMasterVO> _lstTempParaVO, Map<String , ParameterRangeMasterVO> _mapParaRange, InformationControlBean _infoBean, String _genderType, Map<String, String> _mapParaValues, Map<String, Object> _mapSessionContent)//, HttpServletRequest _request)
		{
			Template temp =  new Template();
			temp.templateId = _tempId;
			temp.templateType = Integer.parseInt(_tempType);
		...
		public String generateTemplate() throws Exception
		{
			...
			tbl.setAttribute("class","table table-borderless");
	...
	public static class Cell
	{
				...
				case TEMPLATE_TYPE_NORMAL:
					switch(this.parameter.controlType)
					{
						...
						case CONTROL_TYPES_IMAGEVIEW:
							...
							break;
						case CONTROL_TYPES_SUBTEMPLATE:
							td = new TableColumn();
							HisHTMLParserUtil.setParent(td, _parent);
							this.createSubTemplate(td);
							break;
					}		
					break;
		...
		public void createTextBox(TableColumn _parent) throws Exception
		{
			...
			if(this.parameter.isRange.equalsIgnoreCase(YES_NO_VALUE_YES) && this.objTemplate.mapRange!=null)
			{
		...
		public void createCombo(TableColumn _parent) throws Exception
		{
			...
			//class='mul-select12' multiple='multiple'
			/*<script type='text/javascript'>
			$(document).ready(function() { 
				$('.mul-select12').multiselect();
			});
			</script>*/
			/*******************************************By Mukund on 13.07.2017**************************************************/
			if(this.parameter.getHaveDependent().equalsIgnoreCase(YES_NO_VALUE_YES))
			{
				h+="onchange=\"validatePARAMETERDependentFormula(this,event,'"+this.parameter.dependentParaId+"','"+this.parameter.formula+"','"+this.parameter.formulaOutput+"');setPARAMETERChildPresentation(this,event,'"+this.parameter.childPresentation+"','"+this.parameter.childPresentationOn+"');\" ";
			}
			else
			{
				h+="onchange=\"setPARAMETERChildPresentation(this,event,'"+this.parameter.childPresentation+"','"+this.parameter.childPresentationOn+"');\" ";
			...
			if(this.parameter.sourceFlag==SOURCE_FLAG_STATIC)
			{
				...
				for(int j=0;j<options.length;j++)
				{
					String valtemp="";
					String[] arr;
					//System.out.println("xyz"+options[j].split(SEP_IN_PARA_OPTIONS_LABEL_VALUE)[0].split(SEP_IN_PARA_PARENT));
					arr=options[j].split(SEP_IN_PARA_OPTIONS_LABEL_VALUE)[0].split(SEP_IN_PARA_PARENT);
					if(arr.length>2)
						valtemp=options[j].split(SEP_IN_PARA_OPTIONS_LABEL_VALUE)[0].split(SEP_IN_PARA_PARENT)[1];
				//	System.out.println("valtemp"+valtemp);
					else
						valtemp=options[j].split(SEP_IN_PARA_OPTIONS_LABEL_VALUE)[0];
					
					String val=options[j].split(SEP_IN_PARA_OPTIONS_LABEL_VALUE)[0];
					...
					if(valtemp.equalsIgnoreCase(this.parameter.defaultValue))
						h+=" selected='true' ";
		...
		public void createRadio(TableColumn _parent) throws Exception
		{
			...
			if(this.parameter.sourceFlag==SOURCE_FLAG_STATIC)
			{
				...
				for(int j=0;j<options.length;j++)
				{
					...
					if(this.parameter.getHaveDependent().equalsIgnoreCase(YES_NO_VALUE_YES))
					{
						h+="onclick=\"validatePARAMETERDependentFormula(this,event,'"+this.parameter.dependentParaId+"','"+this.parameter.formula+"','"+this.parameter.formulaOutput+"');setPARAMETERChildPresentation(this,event,'"+this.parameter.childPresentation+"','"+this.parameter.childPresentationOn+"');\" ";
					}
					else
					{
						h+="onclick=\"setPARAMETERChildPresentation(this,event,'"+this.parameter.childPresentation+"','"+this.parameter.childPresentationOn+"');\" ";
		...
		{
			String s=TEMPLATE_TD_STARTS_WITH+this.row+SEP_IN_PARA_ROW_COL+this.col;
			...
			h+="<input type='date' tabindex='1' name='"+name+"' ";
			h+="id='C:"+this.parameter.isCompulsory+"&R:"+this.parameter.isRange+"&G:"+this.parameter.genderType+"' ";
		...
		public void createImageView(TableColumn _parent) throws Exception
		{
			...
			//Modified by Vasu dated on 07.Jan.2020 to get template images from ftp server
			String src = ServletsUtilityConfig.SERVLET_DISPLAY_FILE_URL + "?" 
			...
			String filename = Config.GENERIC_TEMPLATE_IMAGE_VIEW_FILE_STORAGE_NAME + this.objTemplate.templateId + "_" + this.row + SEP_IMAGEVIEW_KEY_ROW_COL + this.col + this.parameter.defaultValue;
			...
			ftpServerUserName=ftpServerUserName.replaceAll("%", "%25");
			ftpServerPassword=ftpServerPassword.replaceAll("%", "%25");
			ftpServerUserName=ftpServerUserName.replaceAll("@", "%40");
			ftpServerPassword=ftpServerPassword.replaceAll("@", "%40");
			ftpServerUserName=ftpServerUserName.replaceAll("#", "%23");
			ftpServerPassword=ftpServerPassword.replaceAll("#", "%23");
		...
		public void createSubTemplate(TableColumn _parent) throws Exception
		{
			_parent.setAttribute("id",TEMPLATE_TD_STARTS_WITH+this.row+SEP_IN_PARA_ROW_COL+this.col);
			_parent.setAttribute("align","center");
			_parent.setAttribute("valign","top");
			...
			String h="<div align='center' ";	
			if(this.parameter.presentation.equalsIgnoreCase(PRESENTATION_TYPES_HIDDEN)) 
				h+=" style='display:none;' ";
			h+="><input type='hidden' name='"+name+"' />";

			//GenericTemplateTag objTag= new GenericTemplateTag();
			//objTag.setTemplateId(this.parameter.subTemplateId);
			//objTag.setTemplateId(this.objTemplate.templateId);
			
			//String tempCode= objTag.templateLogicFinal(this.objTemplate.maprequest, this.objTemplate.request.getSession());
			String tempCode= GenericTemplateTag.templateLogicFinal(this.parameter.subTemplateId, (String) this.objTemplate.mapSessionContent.get("mode"), this.objTemplate.mapSessionContent);
			...
			//String newTempcode= tempCode;
			tempCode = tempCode.replaceAll(this.parameter.subTemplateId, this.objTemplate.templateId);
			//Boolean trueFalse = newTempcode.contains(this.parameter.subTemplateId);
...
//edited by Prachi on 19-6-2019
		...
		private static String getStyleParaValue(Parameter _para, ParameterRangeMasterVO _rngVO)
		{
			...
			view+="<font color='#000000' size='2' >";
			if(!withinRange) view += "<font color='#FF0000'>";
			if(_para.printingStyle==null || _para.printingStyle.equals(""))
			{
				view+=_para.paraName  + ": <b>"+ _para.paraValue + "</b>; ";
			}
			else if(_para.printingStyle.equals("#NULL#"))
			{
				// Print Nothing in case of #NULL# 27-8-2019 Pragya
			}
			else 
			{
				String strFinal = _para.printingStyle;
				// PARANAME
				strFinal = strFinal.replaceAll("PARANAME",_para.paraName);
				...
				view += strFinal;// + "; "; // Semicolon will be part of Template Printing Style 27-8-2019 Pragya
			}
			if(_rngVO!=null)
			{
	...
	public static class Parameter
	{
		...
		public Parameter(int r, int c)
		{
			...
			this.subTemplateId = YES_NO_VALUE_NO;
	...
	private static String getDependentValue(String _paraId, Map<String, TempParameter> _mapParaDtl)
	{
		...
		if(_mapParaDtl!=null)
		{
			for(String paraId : _mapParaDtl.keySet())
			{
				...
				if(paraId.equals(_paraId) || (tempPara.getParaParentId()!=null && !tempPara.getParaParentId().trim().equals("") && tempPara.getParaParentId().contains(_paraId)))
				{
	...
	public static String fillParameterValuesIntoHTMLCode(String _htmlCode, Map<String, String> _mapParaValues, Map<String, TempParameter> _mapParaDtl, String _mode, String _tempType, String _genderType)
	{
		try
		{
			...
			//if(_mapParaValues!=null)
			{
				...
				{
					List<Tag> paraTagList; 
					public ParamterNodeVisitor() {	this.paraTagList= new ArrayList<Tag>();	}
				    public void visitTag (Tag tag)
				    {
				    	if(tag.getAttribute("name")!=null && tag.getAttribute("name").startsWith(GenericTemplateConfig.PARAMETER_STARTS_WITH))
				    		this.paraTagList.add(tag);
				...
				List<Tag> allParamTags = myVisitor.getParaTagList();
	...
	{
		Node parent = tag.getParent();
		if(value != null) value="<font color='#000000' size='2' ><b>&nbsp;"+TemplateDesignerUtility.convertSpace(value)+"</b></font>";
		else value="";
	...
	public static final int CONTROL_TYPES_SUBTEMPLATE = 15;
