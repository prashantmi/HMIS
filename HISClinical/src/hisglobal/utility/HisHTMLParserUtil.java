	...
	private static void freezeElement(InputTag node) throws Exception
	{
		...
		{			
			...
			if(node.getAttribute("checked")!=null && (node.getAttribute("checked").equalsIgnoreCase("checked") || node.getAttribute("checked").equalsIgnoreCase("true")))
				html ="<img height=12 width=12 src='/HISClinical"+getPath(ServletsUtilityConfig.imgRadioCheckedPath)+"'>";
			else
				html ="<img height=12 width=12 src='/HISClinical"+getPath(ServletsUtilityConfig.imgRadioPath)+"'>";
			if(node.getParent()!=null)
			{
				Node parent = node.getParent();
				replaceChild(parent,node,new TextNode(html));
		...
		else if (node.getAttribute("type").equalsIgnoreCase("checkbox"))
		{
			...
			if(node.getAttribute("checked")!=null && (node.getAttribute("checked").equalsIgnoreCase("checked") || node.getAttribute("checked").equalsIgnoreCase("true")))
				html ="<img height=12 width=12 src='/HISClinical"+getPath(ServletsUtilityConfig.imgCheckBoxCheckedPath)+"'>";
			else
				html ="<img height=12 width=12 src='/HISClinical"+getPath(ServletsUtilityConfig.imgCheckBoxPath)+"'>";
			if(node.getParent()!=null)
			{
				Node parent = node.getParent();
				replaceChild(parent,node,new TextNode(html));
