...
public class HTMLToPDFServlet extends HttpServlet
{
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws javax.servlet.ServletException, java.io.IOException
	{
		...
		try
		{
			...
			baosPDF = HTMLToPDFUTIL.convertHtmlToPDFDirect(req, strHtmData);
...
/*			// Creating Pdf Writer of iText to write to Pdf as well as to Byte Reader
			...
			/**Added by Vasu on 18.July.2018*/
			String key  = (String) session.getAttribute(ServletsUtilityConfig.PDF_FILE_NAME_KEY);
			req.getSession().setAttribute(ServletsUtilityConfig.PDF_FILE_CONTENT_KEY+key, baosPDF.toByteArray());
			/**End Vasu*/
			
			// Saving To a File at C:\
			/*HisFileControlUtil hfcu = new HisFileControlUtil("abc.pdf","E:\\","/root/");
