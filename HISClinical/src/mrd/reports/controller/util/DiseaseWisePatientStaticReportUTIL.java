...
import mrd.reports.controller.data.DiseaseWisePatientStaticReportDATA;
import mrd.reports.controller.fb.DiseaseWisePatientStaticReportFB;


public class DiseaseWisePatientStaticReportUTIL extends ReportUTIL
{
	public static void getEssential(DiseaseWisePatientStaticReportFB _fb,HttpServletRequest _request)
	{
		Status objStatus= new Status();
		UserVO userVO=new UserVO();
		try
		{
			Map map1=(HashMap)DiseaseWisePatientStaticReportDATA.getAllHospitalEssentials(getUserVO(_request));
			WebUTIL.setMapInSession(map1, _request);
			
			userVO.setHospitalCode("100");
			Map mp=DiseaseWisePatientStaticReportDATA.getDepartment(userVO);
