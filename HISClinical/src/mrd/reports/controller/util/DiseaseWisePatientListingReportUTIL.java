...
import mrd.reports.controller.data.DiseaseWisePatientListingReportDATA;
import mrd.reports.controller.fb.DiseaseWisePatientListingReportFB;


public class DiseaseWisePatientListingReportUTIL extends ReportUTIL
{
	public static void getEssential(DiseaseWisePatientListingReportFB _fb,HttpServletRequest _request)
	{
		Status objStatus= new Status();
		UserVO userVO=new UserVO();
		try
		{
			Map map1=(HashMap)DiseaseWisePatientListingReportDATA.getAllHospitalEssentials(getUserVO(_request));
			WebUTIL.setMapInSession(map1, _request);
			
			userVO.setHospitalCode("100");
			Map mp=DiseaseWisePatientListingReportDATA.getDepartment(userVO);
