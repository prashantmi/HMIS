...
import mrd.reports.controller.util.LabCensusReportUTIL;
...
public class LabCensusReportACTION extends DispatchAction
{
	...
	{
		LabCensusReportFB fb=(LabCensusReportFB)form;
		fb.reset(mapping, request);
		WebUTIL.refreshTransState(request);
		LabCensusReportUTIL.getEssential(fb,request);
	
		return mapping.findForward("NEW");
	}
	public ActionForward GETLABORATORYLIST_AJAX(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) throws Exception {
		LabCensusReportFB fb = (LabCensusReportFB) form;
		StringBuffer strBuff = LabCensusReportUTIL.getLaboratoryandTimeMap(request,fb);
...
//        StringBuffer sb=null;
		...
		try 
		{	
			byte[] byteArray =LabCensusReportUTIL.generateTextReport(fb,request,getServlet().getServletContext(),response);
			OutputStream os;
			try {
