...
import inpatient.transaction.controller.data.IpdDoctorDeskLoginDATA;
...
public class IpdDoctorDeskLoginUTL extends ControllerUTIL
{
	/**
	 ...
	 */ 
	public static boolean getWardListBasedOnRole(IpdDoctorDeskLoginFB fb, HttpServletRequest request)
	{
		...
		try
		{
			...
			listWard=IpdDoctorDeskLoginDATA.getWardListBasedOnRole(getUserVO(request));
	...
	// Getting Department Unit List
	public static boolean getDeptUnitList(IpdDoctorDeskLoginFB fb,HttpServletRequest request)
	{
		Status objStatus = new Status();
		boolean flag = true;		
		try
		{
			...
			List lstUnit=IpdDoctorDeskLoginDATA.getDeptUnitList(getUserVO(request));
	...
	// Getting Ward List By Unit Code
	public static boolean getWardOnBasisOfUnitCode(IpdDoctorDeskLoginFB fb,HttpServletRequest request)
	{
		Status objStatus = new Status();
		boolean flag = true;
		List listWard = new ArrayList();
		try
		{
			...
			listWard=IpdDoctorDeskLoginDATA.getWardOnBasisOfUnitCode(unitCode,getUserVO(request));
	...
	public static boolean getRoomOnBasisOfWardCode(IpdDoctorDeskLoginFB fb,HttpServletRequest request)
	{
		Status objStatus = new Status();
		boolean flag = true;
		...
		try
		{
			if(InpatientConfig.IPD_DOCTOR_DESK_ROOM_SELECTION.equals(InpatientConfig.ROOM_SELECTION_NO)
					|| InpatientConfig.IPD_DOCTOR_DESK_UNIT_SELECTION.equals(InpatientConfig.UNIT_SELECTION_NO))
			{
			...
			else
			{
				...
				fb.setRoomCode("-1");
				listRoom=IpdDoctorDeskLoginDATA.getRoomOnBasisOfWardCode(unitCode,fb.getWardCode(),getUserVO(request));
	...
	// Going To IPD Doctor Desk
	public static void goToIpdDoctorDesk(IpdDoctorDeskLoginFB _fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
		...
		try
		{
			...
			if(InpatientConfig.IPD_DOCTOR_DESK_UNIT_SELECTION.equals(InpatientConfig.UNIT_SELECTION_NO))
			{
				...
				DynamicDeskUTIL.setAttributeInSession(_request, DynamicDeskConfig.DYNAMIC_DESK_TYPE, DynamicDeskConfig.DESK_TYPE_IPD_DOCTOR_DESK);
...
//			Map mp = null;
//			mp = IpdDoctorDeskLoginDATA.getICDAllEssentials(getUserVO(_request));
//			DynamicDeskUTIL.setMapInSession(mp, _request);
	...
	// Going To IPD Doctor Desk
	public static void getIPDDoctorDeskEssentialForFooter(IpdDoctorDeskLoginFB _fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
		try
		{
