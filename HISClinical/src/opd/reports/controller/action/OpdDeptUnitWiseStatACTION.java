...
import opd.reports.controller.util.OpdDeptUnitWiseStatUTIL;
...
public class OpdDeptUnitWiseStatACTION extends DispatchAction
{
	...
	{
		OpdDeptUnitWiseStatFB fb=(OpdDeptUnitWiseStatFB)form;
		fb.reset(mapping,request);
		WebUTIL.refreshTransState(request);
		OpdDeptUnitWiseStatUTIL.getEssential(fb,request);
	...
	{
		OpdDeptUnitWiseStatFB fb=(OpdDeptUnitWiseStatFB) form;
		OpdDeptUnitWiseStatUTIL.setUnitBasedOnDepartment(fb,request);	
		return mapping.findForward("NEW");
	}
	...
	public ActionForward TEXT(ActionMapping mapping,ActionForm form,HttpServletRequest request,HttpServletResponse response) throws HisReportDataNotFoundException
	{
		...
		try 
		{			
			byte[] byteArray =OpdDeptUnitWiseStatUTIL.generateTextReport(fb,request,getServlet().getServletContext(),response);
