...
public class OpdPatientDeskUTIL extends ControllerUTIL
{
	...
	public static void getTodayOpdPatientList(OpdPatientDeskFB _fb, HttpServletRequest _request)
	{
		...
		try
		{
			...
			dailyPatientVOs = OpdPatientDeskDATA.getTodayPatientList(getUserVO(_request), _fb.getDepartmentUnitCode(), roomCode);
	...
	*/
	public static void saveOpdPatientEpisode(OpdPatientDeskFB opdPatientDeskFB, HttpServletRequest request)
	{
		...
		try
		{
			...
			if (flag) OpdPatientDeskDATA.saveOpdPatientEpisode(patientDetailVO.getPatCrNo(), patientDetailVO
	...
	public static boolean getOpdDeskEssentials(OpdPatientDeskFB opdPatientDeskFB,HttpServletRequest request) 
	{
		...
		{	
			UserVO userVO = getUserVO(request);
			setSysdate(request);
		    opdPatientDeskFB.setHospitalCode(userVO.getHospitalCode());
		    
		    //Added by Vasu on 28.01.2019
		   ...
		   // SimpleDateFormat _sdf4 =new SimpleDateFormat("dd-MMM-yyyy HH:mm");
		    ...
		    String currDate =_sdf3.format(_sdf1.parse((String)request.getSession().getAttribute(Config.SYSDATE)));
		   // Date date = DateUtils.addDays(new Date(), -1);
		    //SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		    //return sdf.format(date);
		  
		    
			opdPatientDeskFB.setEpisodeDate(currDate);

			
			List list = OpdPatientDeskDATA.getOpdDeskEssentials(getUserVO(request));
	...
	{	
		...
		try
		{
			_fb.setDepartmentUnitCode((String) session.getAttribute(DynamicDeskConfig.DYNAMIC_DESK_UNIT_CODE));			
			String roomCode = (String) session.getAttribute(DynamicDeskConfig.DYNAMIC_DESK_ROOM_CODE);
			dailyPatientVOs = OpdPatientDeskDATA.getTodayAllPatientList(getUserVO(_request), _fb.getDepartmentUnitCode(), roomCode);
...
/**
## 		Modification Log		: Taking Desk Type from Session , bec same file is used for BAY DESK Also
##		Modify Date				: 15-01-2015
##		Reason	(CR/PRS)		: CR
##		Modify By				: Akash Singh
*/
	...
	{		
		...
		try
		{
			...
			patientDetailVO.setDepartmentUnitCode(opdPatientDeskFB.getDepartmentUnitCode());
			patientDetailVO.setEpisodeVisitType(opdPatientDeskFB.getVisitType());
			patientDetailVO.setRoomCode(opdPatientDeskFB.getRoomCode());
			patientDetailVO.setEpisodeDate(opdPatientDeskFB.getEpisodeDate());
			
			
			/* if(opdPatientDeskFB.getPatCrNo().equals(userVO.getHospitalCode()))
			{
				patientDetailVO.setPatCrNo("");
			}
			else
			{
				
				patientDetailVO.setPatCrNo(opdPatientDeskFB.getPatCrNo());
			} */
			
			if(opdPatientDeskFB.getSrchCriteria()!=null)
			{
				if(opdPatientDeskFB.getSrchCriteria().equals("1"))
					patientDetailVO.setPatCrNo(opdPatientDeskFB.getSrchValue());
				if(opdPatientDeskFB.getSrchCriteria().equals("2"))
					patientDetailVO.setPatName(opdPatientDeskFB.getSrchValue());
			}
			//String deskType1 = DynamicDeskConfig.DESK_TYPE_OPD_DOCTOR_DESK;
			...
			Map<String, Object> mapObj=OpdPatientDeskDATA.getAllPatientList_AJAX(patientDetailVO, userVO,i_page,i_limit,_sidx,_sord,_where,deskType);
