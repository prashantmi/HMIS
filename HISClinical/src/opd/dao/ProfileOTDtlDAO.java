...
public class ProfileOTDtlDAO extends DataAccessObject implements ProfileOTDtlDAOi
{
	public ProfileOTDtlDAO(TransactionContext _tx)
	{
		super(_tx);
	}
	...
	public void create(ProfileOTDtlVO _profileOTDtlVO, UserVO _userVO)
	{
		...
		String queryKey = "INSERT.HPMRT_PROFILE_OT_DTL";
		try
		{
		...
		try
		{
			populateMAP.put(sq.next(), _profileOTDtlVO.getProfileId());
			populateMAP.put(sq.next(), _profileOTDtlVO.getOtReqNo());
	...
	public ProfileOTDtlVO[] fetchOTProfileDetails(PatientProfileDetailVO _patientProfileDtlVO, UserVO _userVO)
	{
		...
		ProfileOTDtlVO[] profileOTDetailVO=new ProfileOTDtlVO[0];
		String query = "";
		Map _populateMap = new HashMap();
		String filename = OpdConfig.QUERY_FILE_FOR_OPD_DAO;
		String queryKey = "SELECT_OT_PROFILE_DTL.HPMRT_PROFILE_OT_DTL";
		...
		{
			throw new HisApplicationExecutionException("ProfileOTDtlDAO.populateMAP::" + e);
		}
		...
		try
		{
			...
			{
				rs.beforeFirst();
				vo = HelperMethods.populateVOfrmRS(ProfileOTDtlVO.class, rs);
				profileOTDetailVO = new ProfileOTDtlVO[vo.length];
				for (int i = 0; i < vo.length; i++)
				{
					profileOTDetailVO[i] = (ProfileOTDtlVO) vo[i];
				}
			}
		}
		catch (Exception e)
		{
			...
			else throw new HisDataAccessException("ProfileOTDtlDAO:fetchOTProfileDetails::OT Profile Details:: " + e);
		}
		return profileOTDetailVO;
	}
	...
	public void updateIsValidStatus(ProfileOTDtlVO _profileOTDtlVO, UserVO _userVO)
	{
		...
		Sequence sq = new Sequence();
		String filename = OpdConfig.QUERY_FILE_FOR_OPD_DAO;
		String queryKey = "UPDATE_IS_VALID.HPMRT_PROFILE_OT_DTL";
		try
		{
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		}
		...
		{
			throw new HisDataAccessException("ProfileOTDtlDAO:populateMap::" + e);
		}
