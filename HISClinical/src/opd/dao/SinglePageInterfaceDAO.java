...
public class SinglePageInterfaceDAO extends DataAccessObject {

	public SinglePageInterfaceDAO(TransactionContext _transactionContext) {
...
/*			populateMAP.put(sq.next(), usablity);
	 ...
	 * @param _userVO
	 */
	public void create(SinglePageInterfaceMasterVO singlePageInterfaceMasterVO, UserVO _userVO) {

		String query = "";
		Map populateMAP = new HashMap();
		String filename = OpdConfig.QUERY_FILE_FOR_OPD_MASTERSDAO;
		String queryKey = "INSERT_EHRTS_SECTION_TEMPLATES_MST";
		...
		try {
			...
			populateMAP.put(sq.next(), singlePageInterfaceMasterVO.getInterfaceId());
			populateMAP.put(sq.next(), singlePageInterfaceMasterVO.getInterfaceName());
			populateMAP.put(sq.next(), singlePageInterfaceMasterVO.getDesc());
			populateMAP.put(sq.next(), singlePageInterfaceMasterVO.getInterfaceId());
			populateMAP.put(sq.next(), singlePageInterfaceMasterVO.getInterfaceId());
			populateMAP.put(sq.next(), singlePageInterfaceMasterVO.getClinicalSectionCode());
			populateMAP.put(sq.next(), singlePageInterfaceMasterVO.getKeyCode());
	...
	public SinglePageInterfaceMasterVO fetchRecord(SinglePageInterfaceMasterVO singlePageInterfaceMasterVO,
			UserVO userVO) {
		String query = "";
		...
		String filename = OpdConfig.QUERY_FILE_FOR_OPD_MASTERSDAO;
		String queryKey = "SELECT.EHRTS_SECTION_INTERFACES_MST";
...
//			populateMAP.put(sq.next(), Config.IS_VALID_ACTIVE);
			populateMAP.put(sq.next(), Config.IS_VALID_ACTIVE);
			populateMAP.put(sq.next(), singlePageInterfaceMasterVO.getClinicalSection());
			/*populateMAP.put(sq.next(), Config.SUPER_USER_HOSPITAL_CODE); //addedby:NehaRajgariya date:7sept2016
		...
		try {
			...
			if (!rs.next()) {
				...
				HelperMethods.populateVOfrmRS(singlePageInterfaceMasterVO, rs);
			}
		} catch (Exception e) {
			if (e.getClass() == HisRecordNotFoundException.class) {
				throw new HisRecordNotFoundException(e.getMessage());
	...
	public void modify(SinglePageInterfaceMasterVO singlePageInterfaceMasterVO, UserVO userVO) {
		String query = "";
		Map populateMAP = new HashMap();
		Sequence sq = new Sequence();
		String filename = OpdConfig.QUERY_FILE_FOR_OPD_MASTERSDAO;
		String queryKey = "UPDATE.ehrts_section_interfaces_mst";
	...
	public boolean checkDuplicateBeforeSave(SinglePageInterfaceMasterVO singlePageInterfaceMasterVO, UserVO userVO) {
	...
	Sequence sq = new Sequence();
	String filename = OpdConfig.QUERY_FILE_FOR_OPD_MASTERSDAO;
	String queryKey = "DUPLICATE_CHECK_BEFORE_SAVE.EHRTS_SECTION_TEMPLATES_MST";
//	String profileName = null;
	...
	public boolean checkDuplicateBeforeModifySave(SinglePageInterfaceMasterVO singlePageInterfaceMasterVO, UserVO userVO) {
		...
		Sequence sq = new Sequence();
		String filename = OpdConfig.QUERY_FILE_FOR_OPD_MASTERSDAO;
		String queryKey = "DUPLICATE_CHECK_BEFORE_MODIFY_SAVE.ehrts_section_interfaces_mst";
		try {
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		} catch (Exception e) {
			throw new HisApplicationExecutionException(
