...
public class EmployeeDutyRosterUTL extends ControllerUTIL
{
	...
	public static boolean getRosterCategoryList(HttpServletRequest _request,EmployeeDutyRosterFB _fb)
	{
		...
		try
		{
			...
			EmployeeDutyRosterUTLFunctions.getYearList(_request,_fb);
			EmployeeDutyRosterUTLFunctions.getMonthsList(_request,_fb);
			
			
			//currentYear=WebUTIL.getSession(_request).getAttribute(arg0)
			
			dutyRosterCategoryList=EmployeeDutyRosterDATA.getRosterCategoryList(_userVO);			
	...
	public static boolean getEmpTypeMonthWiseDutyRostersOnTheBasisOfRosterCategory(HttpServletRequest _request,EmployeeDutyRosterFB _fb)
	{
		Status objStatus = new Status();
		...
		try
		{
			...
			dutyRosterList=EmployeeDutyRosterDATA.getEmpTypeMonthWiseDutyRostersOnTheBasisOfRosterCategory(_fb.getRosterCategory().split("@")[0],_userVO);			
			...
			if(dutyRosterList.size()==1)
			{
				...
				EmployeeDutyRosterUTL.getDutyAreaWithCapacityAndDesignationBasedOnRosterType(_request, _fb);
			}
		...
		public static boolean getDutyAreaWithCapacityAndDesignationBasedOnRosterType(HttpServletRequest _request,EmployeeDutyRosterFB _fb)
		{
			Status objStatus = new Status();
			...
			try
			{
				...
				EssentialMap=EmployeeDutyRosterDATA.getDutyAreaWithCapacityAndDesignationBasedOnRosterType( _rosterId,_userVO);
				...
				if(dutyAreaList.size()==1 )//&& desigList.size()==1)
					{
					EmployeeDutyRosterUTL.getEmployeesCalendarBasedOnDutyAreaAndDesignation(_request, _fb);
					}
			 ...
			 * @return
			 */
			public static boolean getEmployeesCalendarBasedOnDutyAreaAndDesignation(HttpServletRequest _request,EmployeeDutyRosterFB _fb)
			{
				Status objStatus = new Status();
			...
			//	List empRosterList=new ArrayList();
				...
				try
				{
					...
					System.out.println("manisha"+_fb.getAreaCode());
					
					String _month=_fb.getMonth();
					String _year=_fb.getYear();
					String _areaId=_fb.getAreaCode().split("@")[5];//splitValues[1];
...
//month,rosterId,areaTypeCode and areaCode		
					...
					EssentialMap=EmployeeDutyRosterDATA.getEmployeesWithRoster(_desigId,_year, _month,_areaTypeCode, _areaCode, _rosterId,  _areaId,_userVO);
		...
		//setting all the essential things present in map to session
				...
				EmployeeDutyRosterUTLFunctions.getEmpDutyRosterCalendar(_request,_fb);
...
public static boolean saveEmpDutyRoster(EmployeeDutyRosterFB _fb, HttpServletRequest _request)
	{
		Status objStatus = new Status();
...
if(arrayOfEmployees!=null && arrayOfEmployees.length!=0)
			_rosterDtlVO=EmployeeDutyRosterUTLFunctions.getRosterDtlVOsToBeSaved(arrayOfEmployees,_request,_fb);		
...
if(_oldRosterDtlVO==null && _rosterDtlVO!=null)			
			EmployeeDutyRosterDATA.saveEmpDutyRoster(_rosterDtlVO, userVO);
			else//else we will save and modify the old  records
				{
					
					String empListToBeUpdated=_fb.getEmpListToBeUpdated();
					String daysListToBeUpdated=EmployeeDutyRosterUTLFunctions.getDaysListToBeUpdated(_request, _fb);
		...
		if(_rosterDtlVO!=null){						
				...
				EmployeeDutyRosterDATA.saveAndModifyEmpDutyRoster(_rosterDtlVO, userVO,empListToBeUpdated,daysListToBeUpdated,_year,_month,_rosterId,_areaTypeCode,_areaCode);
				}
...
public static void setTotalEmpRoster(EmployeeDutyRosterFB _fb, HttpServletRequest _request)
{
	Status objStatus = new Status();
	boolean hasFlag = true;
...
Map reliverEmpMap=EmployeeDutyRosterUTLFunctions.getMapOfEmpToBeRelived(_request, _fb);
