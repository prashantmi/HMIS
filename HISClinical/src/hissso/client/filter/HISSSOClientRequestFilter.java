...
public class HISSSOClientRequestFilter implements Filter
{
		...
		try
		{
			...
			else
			{
				...
				if (authenticate)
				{
					...
					{
						if(strURI.equalsIgnoreCase("/HISClinical/hisglobal/filetransfer/attachUploadFile.action"))
							chain.doFilter(request, response);
	...
	private boolean setCredentialsInSession(HttpSession objSession_p, HISServiceTicket objST_p)
	{
		...
		{
			//System.out.println("HISSSO::ClientREquestFilter: UserVO Hospital VO Set Problem");
			ee.printStackTrace();
			flg = false;
		}
		return flg;
