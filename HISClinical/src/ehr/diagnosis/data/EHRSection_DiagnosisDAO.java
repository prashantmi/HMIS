...
public class EHRSection_DiagnosisDAO extends DataAccessObject 
{
	...
	public List<EHRSection_DiagnosisVO> getPatientDiagnosisDetail(String _pmode, EHR_AccessPermissionVO _voEHRAccess, EHR_PatientEncounterVO _voEHREncounter, EHRSection_DiagnosisVO _voEHRDiagnosis)
	{
		int nProcedureIndex;
		...
		try
		{
			nProcedureIndex = hisDAO_p.setProcedure(EHRConfig.GET_EHR_PATIENT_DIAGNOSIS);
			// Setting and Registering In and Out Parameters 
			hisDAO_p.setProcInValue(nProcedureIndex, "p_mode", _pmode,1);
			hisDAO_p.setProcInValue(nProcedureIndex, "p_crno", _voEHRDiagnosis.getPatCrNo(),2);
			hisDAO_p.setProcInValue(nProcedureIndex, "p_episodecode", _voEHRDiagnosis.getEpisodeCode(),3);
		...
		ValueObject vo[] = null;
		List<EHRSection_DiagnosisVO> lst = new ArrayList<EHRSection_DiagnosisVO>(); 
		try
		{
			vo = HelperMethods.populateVOfrmRS(EHRSection_DiagnosisVO.class, objResSet);
			for(ValueObject v :vo)
				lst.add((EHRSection_DiagnosisVO)v);
		}
		catch (Exception e)
		{
	...
	public void create(EHRSection_DiagnosisVO _episodediagVO, UserVO _userVO)
	{
		...
		hisDAO_p.setProcInValue(nProcedureIndex, "p_somedICDMapped", (_episodediagVO.getSnomedCTICDCode()==null) ? "": _episodediagVO.getSnomedCTICDCode(),20);
