...
public class EHRSection_CaseSummaryUTL extends ControllerUTIL{

	public static void getEssentials(EHRSection_CaseSummaryFB _fb,HttpServletRequest _request, HttpServletResponse response) {
		...
		try
		{
			...
			EHRSection_CaseSummaryVO summaryVO = new EHRSection_CaseSummaryVO();
			...
			summaryDtlVO = EHRSection_CaseSummaryDATA.getEssentials(summaryVO, userVO);	
				
			if(summaryDtlVO.getCaseSummary()!=null)
			{
				_fb.setCaseSummary(summaryDtlVO.getCaseSummary());
			}
				
				
		   EHRVOUtility.setCaseSummaryVO(_request, _fb.getPatCrNo(), summaryDtlVO); //Added by Vasu on 05.Mar.2018
	...
	public static String saveEhrDischargedetails(HttpServletRequest _request,HttpServletResponse response, EHRSection_CaseSummaryFB _fb) {
		...
		try
		{
			...
			summaryVO.setDeptCode(selectedPatientVO.getDepartmentCode());
			summaryVO.setDeptUnitCode(selectedPatientVO.getDepartmentUnitCode());
			summaryVO.setCaseSummary(_fb.getCaseSummary());
			summaryVO.setSnomdPTCaseSummary(_fb.getSnomdPTCaseSummary());
			summaryVO.setSnomdCIDCaseSummary(_fb.getSnomdCIDCaseSummary());
			
			EHRSection_CaseSummaryDATA.saveDetails(summaryVO, getUserVO(_request));
			...
			//patencountervo.setListSaveAllDischarge(dischargeVO);
	...
	public static void saveDetails(HttpServletRequest _rq,HttpServletResponse response, EHRSection_CaseSummaryFB _fb) 
	{
		String flag=EHRSection_CaseSummaryUTL.saveEhrDischargedetails(_rq,response,_fb);
