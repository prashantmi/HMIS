...
public class EHRSection_Medication_AdviceDAO extends DataAccessObject
{
	public EHRSection_Medication_AdviceDAO(TransactionContext _tx)
	{
		super(_tx);
	}
	...
	public EHRSection_Medication_AdviceVO getEssentials(EHRSection_Medication_AdviceVO summaryVO, UserVO userVO) {
		int nProcedureIndex;
		HisDAO hisDAO_p = new HisDAO("EHR", "EHRSection_Medication_AdviceDAO");
		String pmode="1";
		String strDBErr;
		ResultSet objResSet;
		ValueObject vo[] = null;
		...
		try
		{
			nProcedureIndex = hisDAO_p.setProcedure(EHRConfig.GET_MEDICATION);
			...
			hisDAO_p.setProcInValue(nProcedureIndex, "p_admNo", (summaryVO.getAdmissionNo()==null) ? "0": summaryVO.getAdmissionNo(),6);
		...
		try
		{
			...
			vo = HelperMethods.populateVOfrmRS(EHRSection_Medication_AdviceVO.class, objResSet);
			summaryDtlVO = new EHRSection_Medication_AdviceVO();
			
			if(vo.length>0)
				summaryDtlVO = (EHRSection_Medication_AdviceVO) vo[0];
	...
	public void saveDetails(EHRSection_Medication_AdviceVO summaryVO, UserVO userVO) {
		int nProcedureIndex;
		HisDAO hisDAO_p = new HisDAO("EHR", "EHRSection_Medication_AdviceDAO");
		String pmode="1";
		...
		try
		{
		nProcedureIndex = hisDAO_p.setProcedure(EHRConfig.SAVE_MEDICATION);
		// Setting and Registering In and Out Parameters 
		hisDAO_p.setProcInValue(nProcedureIndex, "p_mode", pmode,1);
		hisDAO_p.setProcInValue(nProcedureIndex, "pat_cr_no", (summaryVO.getPatCrNo()==null) ? "":summaryVO.getPatCrNo(),2);
		hisDAO_p.setProcInValue(nProcedureIndex, "p_episodecode", summaryVO.getEpisodeCode(),3);
		hisDAO_p.setProcInValue(nProcedureIndex, "p_adm_no", (summaryVO.getAdmissionNo() ==null)? "0": summaryVO.getAdmissionNo(),4);
		...
		hisDAO_p.setProcInValue(nProcedureIndex, "p_summary", (summaryVO.getMedication()==null) ? "": summaryVO.getMedication(),10);
		hisDAO_p.setProcInValue(nProcedureIndex, "p_ids_summary", (summaryVO.getSnomdCIDMedication()==null) ? "": summaryVO.getSnomdCIDMedication(),11);
		hisDAO_p.setProcInValue(nProcedureIndex, "p_pterms_summary", (summaryVO.getSnomdPTMedication()==null) ? "": summaryVO.getSnomdPTMedication(),12);
