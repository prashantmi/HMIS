...
{
	//alert("Inside monthWiseDutyReport.js -->> SubmitPage()-> mode :"+mode);
	if(mode=="GET_DUTY_AREA")
	{
		...
		if(document.forms[0].rosterId.value=="-1"){
			document.getElementById("divAreaCodeId").innerHTML="<select class='textbox' onchange='' tabindex='1' name='areaCode'>" +
															"<option value='-1'>Select Value</option></select>";
			return false;
		}
	}
	...
	if(mode=="GET_REPORT")
	{
		//alert("inse mod=GET_REPORT");
		if(validateGeneration())
		{	
			var rosterIdObj=document.forms[0].rosterId;
			var rosterName=rosterIdObj.options[rosterIdObj.selectedIndex].text;
			document.forms[0].rosterName.value=rosterName
...
function validateFinalSubmit(){
 ...
 //   These All Fields are Mandatory
  ...
  if(comboValidation(document.getElementsByName('year')[0],"Year")
