...
if (validateFinalSubmit() && validateMaxContinousOff() && validateMaxOff() && validateNoOfHrsPerWeek() && validateShiftCapacity() ) 
  {
	...
	//alert("ddd---"+document.getElementsByName("concatedValueToBeSaved")[0].value);
...
if(document.getElementsByName("generatedRosterId")[0].value!="")	
		submitPage("GENERATE_ROSTER");
		else
		alert("Roster Cannot be Generated ,Since the Roster Details are not Saved Yet ")
	}
}
...
function validateGeneration()
{
    ...
    if(validateFinalSubmit()&& isEmpty(document.getElementsByName('startDate')[0],"Generation From Date ") && isEmpty(document.getElementsByName('endDate')[0],"Generation To Date"))
    	{
    	var flag=true;
		...
		if(year!=startDateArray[2])
			{
			flag=false;
			alert("Please Select The  Year in Generation From Date,Same as the Year in Combo");
		...
		if(year!=endDateArray[2])
			{
			flag=false;
			alert("Please Select The  Year in Generation To Date,Same as the Year in Combo");
		...
		if(monthArray[month-1]!=startDateArray[1])
			{
			flag=false;
			alert("Please Select The  Month in Generation From Date,Same as the Month in Combo");
			document.getElementsByName('startDate')[0].focus();
			}
			else
		if(monthArray[month-1]!=endDateArray[1])
			{
			flag=false;
			alert("Please Select The  Month in Generation To Date,Same as the Month in Combo");
			document.getElementsByName('endDate')[0].focus();
			}
			else
...
var check=confirm("Do You Want to Drop The Complete Roster");
	...
	getEmpListToBeUpdated();
			...
			if(check==true)
				submitPage("DROP_ROSTER");
		}
		else
		alert("Roster Cannot be Dropped ,Since the Roster Details are not Saved Yet ")
...
//through all the days of the  month	
	...
	//if button is not disabled and button id is greater than pattern repeatAfter		
			if(document.getElementById(buttonId).disabled!=true && j >=(parseInt(patternRepeatFrom)+parseInt(patternRepeatAfter)) )
							{
							...
							var modulo=(j-patternRepeatFrom)%patternRepeatAfter+parseInt(patternRepeatFrom);
...
if(document.getElementById(checkBoxId).checked==true)
   {    
	...
	//alert("morningCapacityFlag----"+morningCapacityFlag);
