...
<table id="tbInvestigationResults" width="70%" >
</table>
</form>
</body>
...
createAdviceonDischarge('ENCINVESTIGATION',ehrObj);
 ...
 function createAdviceonDischarge(key,jobj)
{
	...
	//alert(jobj.patCompositionSelectData.ENCINVESTIGATION.list_selected);
	var obj=[];
	if(jobj.patCompositionSelectData.ENCINVESTIGATION.comp_select == "true")
	{
		obj=jobj.patCompositionSelectData.ENCINVESTIGATION.list_selected;
	
	//alert(obj.length);
	//alert(key);
	//  var content = '<div id= div_ADVICEONDISCHARGE>';
	//alert(jobj.patCompositionSelectData.ENCINVESTIGATION.list_selected.stringify);
	var content = '';
	   ...
	   for(var i=0; i < obj.length; i++)
		{
		   ...
		   content += '<tr><td width="40%">' +((obj[i].patvisitdate == undefined) ? '' : obj[i].patvisitdate)+'</td><td width="40%">'+ ((obj[i].testName == undefined) ? '' : obj[i].testName)+ '</td><td width="10%"></td><td width="5%"></td><td width="5%"></td></tr>';
		   
		  	 
		   
		  // if(visitDate!=prevvisitDate && testName!=prevtestName)
		   content += '<tr><td width="40%"></td><td width="40%">&nbsp;&nbsp;' + ((obj[i].parameterName == undefined) ? '' : obj[i].parameterName)+'<td width="10%">'+ ((obj[i].testValue == undefined) ? '' : obj[i].testValue)+ '</td><td width="5%">'+ ((obj[i].testUnit == undefined) ? '' : obj[i].testUnit)  + '</td><td width="5%"></td></tr>';
	...
	//alert(content);
	$('#tbInvestigationResults').append(content);   
	
	window.opener.jsonSelect.patCompositionSelectData.ENCINVESTIGATION.html = content;
