	...
	<body>
		<html:form action="/master/UserDeskMenuTempMapping.cnt">
			...
			<his:ContentTag>
				...
				<logic:empty name="<%=OpdConfig.LIST_MAPPED_UNITS_FOR_WARD%>" >		
						...
						<td width="35%"  class="tdfonthead">
							<div align="center"  >
								<html:select name="UserDeskMenuTempMapFB" property="mainWardsList" multiple="true" size="6" >
										...
										<logic:present name="<%=OpdConfig.EssentialBO_LIST_ALL_WARDS_NOT_ASSIGNED_SEAT_FORWARDWISE %>">
										<html:options collection="<%=OpdConfig.EssentialBO_LIST_ALL_WARDS_NOT_ASSIGNED_SEAT_FORWARDWISE%>" property="value" labelProperty="label" />
										</logic:present>
								</html:select>
							</div>
...
<%} %>
			...
			<his:statusTransactionInProcess>	
				<logic:equal name="UserDeskMenuTempMapFB" property="isGoPressed" value="<%=OpdConfig.STEP1 %>">
					<img class="button" src='<his:path src="/../HIS/hisglobal/images/buttons/btn-sv.png"/>'  style="cursor:pointer" tabindex="1" onclick ="if(validateAdd()) submitPage('SAVEBYUNITWARDWISE')" onkeypress="if(event.keyCode==13)if(validateAdd()) submitPage('SAVEBYUNITWARDWISE')">
				</logic:equal>	
