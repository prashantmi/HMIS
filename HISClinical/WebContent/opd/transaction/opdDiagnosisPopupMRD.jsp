...
function setIcdDiseaseCode(diseaseCode,diseaseName)
{
   //alert(diseaseCode);
   //alert(diseaseName);
}

function populateSelectedIcdCodes()
{
	var sections="";
	  for(var i=0;i<document.getElementsByName("selectedCode").length;i++)
		{
			var chk = document.getElementsByName("selectedCode")[i];
			if(chk.checked == true)
			{
				sections = chk.value 
				//alert(sections);
				var icdEssential = sections.split("^")
				var icdCode = icdEssential[0];
				var diseaseName = icdEssential[1];
                 
				//alert(icdCode);
				//alert(diseaseName);
				opener.document.getElementById('icdCodeListId').value = icdCode;
				opener.document.getElementsByName('diagnosisName')[0].value = diseaseName; 

             	//var icdcode =document.getElementById('icdCodeListId').value;
             	//var diseaseName=document.getElementsByName('diagnosisName')[0].value;
                            
                 
				opener.AddRowToTable(false);
...
<tr>
		...
		<div align="center">
		<img src='<his:path src="/hisglobal/images/forward3.gif"/>' tabindex="1" onClick="if(validateSearch(1)) submitForm21('SEARCHMRD');" onkeypress="if(event.keyCode==13) if(validateSearch(1)) submitForm21('SEARCHMRD');">
		</div>
		</td>
		<td width="30%" class="tdfonthead">
		<div align="center">
		...
		<td width="10%" class="tdfonthead">
		<div align="center">
		<img src='<his:path src="/hisglobal/images/forward3.gif"/>' tabindex="1" onClick="if(validateSearch(2)) submitForm21('SEARCHMRD');" onkeypress="if(event.keyCode==13) if(validateSearch(2)) submitForm21('SEARCHMRD');">
		</div>
		</td>
		</tr>
	</table>
	...
	<table width="100%" cellpadding="0" cellspacing="1">
		<logic:iterate id="icdDiseaseaVO" name="<%=OpdConfig.EssentialBO_DIAGNOSIS_CODE_LIST_UNIT_WISE%>" type="hisglobal.vo.IcdDiseaseMasterVO">
			<tr>
				<td width="10%" class="tdfonthead">
					<div align="center">
						...
						<%-- <html:checkbox name="OpdDiagnosisFB" property="selectedCode" value="<%=Integer.toString(x)%>" onclick="validateCode()"/> --%>
						<input type="checkbox" name="selectedCode" value="<%=icdDiseaseaVO.getDiseaseCode()+"^"+icdDiseaseaVO.getFullDiseaseName()%>" onclick="setIcdDiseaseCode('<%=icdDiseaseaVO.getDiseaseCode()%>','<%=icdDiseaseaVO.getFullDiseaseName()%>')"/>
						<% x++; %>
					</div>
				</td>
            ...
            <his:ButtonToolBarTag>
					<img class="button" src='<his:path src="/../HIS/hisglobal/images/buttons/btn-ok.png"/>'  style=cursor:pointer tabindex="1" onclick ="populateSelectedIcdCodes()" onkeypress="if(event.keyCode==13) validateSave()">
