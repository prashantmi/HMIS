...
function activateTab(tabObj){
	document.getElementById('tabFlag').value=tabObj.id;
	<%
...
window.onload=function()
{
	...
	//activateTab(document.getElementById('<bean:write name="PatientTreatmentDetailFB" property="activeTab"/>'));
	 ...
	 if(document.getElementById("tabFlag").value=="tabDrugAdvice"){
		 	
		                activateTab(document.getElementById("tabDrugAdvice"));
		 	
		        }
		 	
		        
			
		        if(document.getElementById("tabFlag").value=="tabLAAdvice"){
		 	
		                activateTab(document.getElementById("tabLAAdvice"));
		 	
		        }
		 	
		        
		 	
		        if(document.getElementById("tabFlag").value=="tabDietAdvice"){
		 	
		                activateTab(document.getElementById("tabDietAdvice"));
...
{
	var path='/HISClinical/opd/patTreatmentDetailTile.cnt?hmode=REVOKEREMARKS&drugInstructionIndex='+index;
	child = window.open(createFHashAjaxQuery(path),'popupWindow','status=yes,scrollbars=yes,height='+400+',width='+800+',left=10,top=10');  
	child.moveTo(250,250);
	child.focus(); 
	if(!child.opener)
	...
	<table width="100%" border="0" cellspacing="1" cellpadding="0">
		<tr>
			...
			%>
				...
				<td id="tabLAAdvice" onclick="activateTab(this)" width="25%" class="tdfonthead"><div align="center">Other/Pre-Operative Advice</div></td>
				...
				<tr id="trSpecimen">
						...
						<div align="center">
							<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
								<b>
									...
									<html:hidden name="PatientTreatmentDetailFB" property="prevDrugBrandId" value="<%=drugDtlVO.getDrugBrandId() %>"/>
								</b>
							</font>
						</div>
					</td>
					...
					<td width="5%" class="tdfont">
						<div align="center">
							<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
								<b>
									<img name="prevDrugSchedule" class="button"  src='<his:path src="/hisglobal/images/scheduler.png"/>' style='cursor:pointer' title="Drug Schedule " onclick ="showPrevDrugSchedule(<%=ind %>,'PREVDRUGSHEDULE')" onkeypress="if(event.keyCode==13) showDrugSchedule(validateSheduleRow(findIndex(this)),event,findIndex(this));">
								</b>
							</font>
						</div>
					</td>
...
<!--									<bean:write name="drugDtlVO" property="remarks"/>-->
					...
					<% String callonRevoke="showRevokeRemarks(this,"+idx+")"; %>
					<td width="5%" class="tdfont">
						<div align="center">
							<logic:notEqual value="before" name="drugDtlVO" property="date">
								<html:checkbox property="revoke" name="PatientTreatmentDetailFB" value='<%=drugDtlVO.getDrugId()+"$"+ind +"$"+drugDtlVO.getFrequencyId()+"$"+drugDtlVO.getDoseId()%>' onclick='<%=callonRevoke%>' ></html:checkbox>      <!-- onclick="validateCheck(this)" -->
								<html:hidden property="revokeHidden" name="PatientTreatmentDetailFB"/>
								<html:hidden property="revokeRemarks" name="PatientTreatmentDetailFB"/>
							</logic:notEqual>
						</div>
					</td>
					<td width="5%" class="tdfont">
		...
		<his:SubTitleTag name="Drug Advice Detail">
			...
			<tr>
				...
				<td width="5%" class="tdfonthead">
					<div align="right">
						<img class="button" style="cursor: pointer" alt="Search" title="Search"	src='<his:path src="/hisglobal/images/btn-search.png"/>' onclick="openPopup(createFHashAjaxQuery('/HISClinical/opd/patTreatmentDetailTile.cnt?hmode=SEARCHDRUG'),event,500,800);">
					</div>
				</td>
			</tr>
		</table>
		...
		<table id="tblDrugDetailId" width="100%" border="0" cellspacing="1" cellpadding="0" style="display:block;">
			<tr>
				<td width="15%" class="tdfonthead" nowrap="nowrap" colspan="1">
					<div align="center">
						<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
							<b>
			...
			<%countNew++;%>
			<!-- <td width="0.5%" class="tdfont"> -->
			<td width="14.5%" class="tdfont">
				<img id="stk_avl<%=i%>" style="height: 12; width: 12; vertical-align: center; display:none;" src='<his:path src="/hisglobal/images/gDot.png"/>'>
				<img id="stk_unavl<%=i%>" style="height: 12; width: 12; vertical-align: center; display:none;" src='<his:path src="/hisglobal/images/Red_Dot.png"/>'>
			
				
				<html:hidden name="PatientTreatmentDetailFB" property="selStock" value='<%=drugDtlVO.getSelStock() %>'/>
			<!-- </td> -->
					...
					<html:hidden name="PatientTreatmentDetailFB" property="selDrugBrandId" value='<%=drugDtlVO.getDrugId()%>' />
				...
				<td width="10%" class="tdfont">
					<div align="center">
						...
						<% } else { %>
							<html:select name="PatientTreatmentDetailFB" property="selDoseId" tabindex="1" value="<%=drugDtlVO.getDoseId() %>" style="width: 100px;"  onchange="<%=showqty%>">
				...
				<td width="10%" class="tdfont">
					<div align="center">
						...
						<html:select name="PatientTreatmentDetailFB" property="selFrequencyId" tabindex="1" 
							value="<%=drugDtlVO.getFrequencyId() %>" onchange="<%=showqty%>" >
				...
				<td width="25%" class="tdfont">
					<div align="center">
						...
						<html:hidden name="PatientTreatmentDetailFB" property="selDrugItemTypeId"
							value='<%=drugDtlVO.getDrugId().split("#")[1]%>' />
						<html:hidden name="PatientTreatmentDetailFB" property="selDrugBrandId" value='<%=drugDtlVO.getDrugBrandId()%>'  />
			...
			<%-- 	<td class="tdfont" width="10%" id="firstEmptyStomach">  // commented to remove empty stomach checkbox
					<div align="center">
				...
				<td width="20%" class="tdfont">
					<div  align="center">
						<html:select name="PatientTreatmentDetailFB" property="selDrugAdminId" tabindex="1" value="<%=drugDtlVO.getDrugAdminId() %>"   onchange="setDrugAdminName(findIndex(this))" >
							<html:option value="-1">Select</html:option >
						...
						<html:hidden name="PatientTreatmentDetailFB" property="selDrugAdminName" value="<%=drugDtlVO.getDrugAdminName()%>"></html:hidden>	
																
				</td>
				
				
				
				<!-- shows selected admin Id only -->
			<%-- <td width="20%" class="tdfont"> 
					<div  align="center">
						<html:select name="PatientTreatmentDetailFB" property="selDrugAdminId" tabindex="1" value="<%=drugDtlVO.getDrugAdminId() %>"  onchange="setDrugRouteName(findIndex(this))" style="width: 100px;" >	
						<logic:notEmpty name="drugDtlVO" property="drugAdminId">
							
							<html:option value="<%=drugDtlVO.getDrugAdminId() %>">
								<bean:write name="drugDtlVO" property="drugAdminName"/> 
							</html:option>	
						</logic:notEmpty>
					<logic:empty name="drugDtlVO" property="drugAdminId">
					...
					<div align="center">
					 ...
					 %>
						<html:text name="PatientTreatmentDetailFB" property="quantity" maxlength="2" size="3" tabindex="1" 
							 onkeypress="return validateIntegerOnly(this,event)" readonly="<%=readonly %>" value="<%=drugDtlVO.getQuantity() %>"></html:text>
					</div>
				</td>
				<td width="5%" class="tdfont" id="tdButtonPlus">
					<%
					//System.out.println("Shweta==========="+countNew);
					if(countNew==countmaxlength){%>
						<img class="button" id="addButton" style="cursor: pointer" src='<his:path src="/../HIS/hisglobal/images/Pl_Green_Sqr.png"/>' alt="Add Treatment" title="Add Treatment" onkeypress="if(event.keyCode==13) submitFormOnValidate1(validateRows(),'ADDDRUGROW');" onclick="submitFormOnValidate1(validateRows(),'ADDDRUGROW');">
					<% } else { %>
						<img class="button" id="deleteButton" style="cursor: pointer" src='<his:path src="/../HIS/hisglobal/images/minus.gif"/>' alt="Add Treatment" title="Add Treatment" onkeypress="if(event.keyCode==13) deleteDrugRow('DELETEDRUGROW',<%=i %>)" onclick="deleteDrugRow('DELETEDRUGROW',<%=i %>)">
					<%} %>
				</td>
			</tr>
			</logic:iterate>
...
<his:ButtonToolBarTag>
	<img class='button' src='<his:path src="/../HIS/hisglobal/images/buttons/btn-sv.png"/>' style="cursor: pointer" tabindex="1" onclick="submitFormOnValidate1(validateRowsSave(),'SAVE');" onkeypress="if(event.keyCode==13)submitFormOnValidate1(validateRowsSave(),'SAVE');")>
	...
	<select name="drugList" id="drugList" multiple="multiple" size="4">
		<logic:present name="<%=OpdConfig.ESSENTIALS_LIST_ALL_DRUGS%>">
			<logic:iterate id="objEntry" name="<%=OpdConfig.ESSENTIALS_LIST_ALL_DRUGS%>" type="hisglobal.utility.Entry">
				<option value="<%=objEntry.getValue()%>"> <%=objEntry.getLabel()%>[Qty:<%=objEntry.getValue().split("#")[5]%>]</option>
				<%System.out.print("Chk:"+objEntry.getValue()); %>
			</logic:iterate>
		</logic:present>								
	</select>
</div>
...
<html:hidden name="PatientTreatmentDetailFB" property="isPregnantFlag" />
<html:hidden name="PatientTreatmentDetailFB" property="drugAdminId" />
<html:hidden name="PatientTreatmentDetailFB" property="drugAdminName" />
<input type="hidden" value="<%=request.getContextPath()%>" name="path">
<html:hidden name="PatientTreatmentDetailFB"  styleId="tabFlag" property="tabFlag" />
</form>
