...
{	
	//alert("inside submitFormOnValidateButton");
	...
	if(flag)
	{
		...
		activateSelectedCRNO(crNOElement,frm,false);
		unclickAllMenus();
		//alert("frm.mode.value :"+frm.mode.value);
		//alert("frm.deskMenuId.value :"+frm.deskMenuId.value);
		clickMenu(frm.mode.value, frm.deskMenuId.value);
		//alert("inside submitFormOnValidateButton just before submit");
		frm.submit();
	}
	else
	{
...
function activateSelectedCRNO(_elemCrNo,_frm, flg)
{	
	//alert("inside activateSelectedCRNO");
	var selectedCRNo = null;	
	...
	if(selectedCRNo==null || (_elemCrNo!=null && selectedCRNo!=_elemCrNo.value))
		activitiesOnChangeCrNo(flg);
}

function activitiesOnChangeCrNo(flg)
{
	//alert("inside activitiesOnChangeCrNo");
	...
	if(flg)	submitFormOnValidateButton(window.event,"OPDNEXTVISITDETAIL","","0",null);
}
