...
{	 
    ...
    var count=1;
	$("#list1").jqGrid({
		url:createFHashAjaxQuery('/HISClinical/opd/opdDeskPatientList.cnt?hmode=AJX_G_PATIENTLIST'+"&departmentUnitCode="+departmentUnitCode[0]+"&visitType="+visitType),    	
    	datatype: "json",
         colNames:['','','Que No', 'Patient Name', 'CR Number', 'Gender/Age', 'Category', 'Visit Reason', 'Department/Unit','Room'],
	     colModel:[
	              	...
	              	{name:'queNo',index:'queNo',width:(pageWidth*(6/100)),search:false, sorttype: 'number', align:'center'},
         ...
         pager:'#pager1',
         multiSort: true,
         sortname: 'to_number(replace(queNo,\'C-\',\'\'))',//;//to_number(replace(queNo,''C-'',''''))',
         viewrecords: true,
         height: (height-115),
         autowidth: true,
         autoheight: true,
...
//modified  by manisha gangwar, date : 28.06.2016  
...
var color="black"; //color for attented patient row

//added by manisha gangwar, date : 02.Nov.2015   reason: to highlight cell by pink color when balance is 0.0

function highlight2 (cellValue, options, rowObject) {
	...
	if(cellValue.charAt(15)=="@")
	{
		var cellId=new Array();
		cellId = cellValue.toString().split("@");
		if(cellId[2] !='undefined' && cellId[2] !='null' && cellId[2] !='')
		{
		if(cellId[2] == 1)
		{color = "blue";
		}
...
function filterPatientList()
{
	...
	if((persistRowFlag != "0") && (jQuery("#list1").jqGrid().setGridParam({url : '/HISClinical/opd/opdDeskPatientList.cnt?hmode=AJX_G_PATIENTLIST'+"&departmentUnitCode="+departmentUnitCode[0]+"&visitType="+visitType+"&roomCode="+roomCode+"&patCrNo="+crNo}).trigger("reloadGrid"))>0)
	{	    	
		alert("loadComplete..");
...
function getRoom()
{
		...
		var unit = document.getElementsByName("departmentUnitCode")[0].value;
		var urlNew= "/HISClinical/opd/opdDeskPatientList.cnt?hmode="+_mode+'&unit='+unit;	
