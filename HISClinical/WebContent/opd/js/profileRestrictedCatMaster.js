function moveRightSingle(listNo)
{
	...
	if(listNo=="1")
	{
		source  = document.forms[0].patientCategoryCode;
		target = document.forms[0].selectedPatientCategoryCode;	
	}
...
function moveLeftSingle(listNo)
{
	...
	if(listNo=="1")
	{
		target  = document.forms[0].patientCategoryCode;
		source = document.forms[0].selectedPatientCategoryCode;	
	}
...
function MoveToSelected()
{
	...
	if(document.forms[0].selectedPatientCategoryCode)
		for(var i=0;i<document.forms[0].selectedPatientCategoryCode.length;i++)
			document.forms[0].selectedPatientCategoryCode.options[i].selected=true;
	
	// Unselect Remaining 
	if(document.forms[0].patientCategoryCode)
	{	
		for(var i=0;i<document.forms[0].patientCategoryCode.options.length;i++)
			document.forms[0].patientCategoryCode.options[i].selected=false;
	}
}


function submitToSave(mode)
{
	MoveToSelected();
...
function clearForm(){
	...
	document.getElementsByName('selectedPatientCategoryCode')[0].length=0;
	document.getElementsByName('patientCategoryCode')[0].length=0;
}
 
function getFetchedList(){

	var fetchedList=document.forms[0].selectedPatientCategoryCode;
...
function getCategory(obj){
	...
	var profileType=obj.value;
	if(profileType=="-1"){
		document.getElementsByName('selectedPatientCategoryCode')[0].length=0;
		document.getElementsByName('patientCategoryCode')[0].length=0;
	}
	else{
		document.getElementsByName("hmode")[0].value="GETPATCATEGORY"
		document.forms[0].submit()
	}	
}