...
function convertConsetToPdf(tbl,e)
{
	...
	var url="/HISClinical/opd/consentPrint?templateId="+id;
...
function controlCode()
{
	...
	this.color="<td width='25%' class='tdfonthead'> Color </td>"+
				...
				"<div style='width: 103px; width /* */: /**/ 100px; width: /**/ 100px;'>"+
				"<input id='color' type='text' value='#000000' "+
				"onchange=\"document.getElementById('colorTabView').style.backgroundColor=this.value;\" "+
				"maxlength='7' size='10' style='width: 80px; font-size: 12px; height: 17px; float: left; border: 0px; padding-top: 2px'>"+
				...
				"onclick=\"ColorPicker.setup({ inputField : 'color', button : 'color1', singleClick : true }); eval(this.onclick());\">"+	
				"</div>"+
				"<label id='colorTabView'>&nbsp;&nbsp;&nbsp;&nbsp;</label></td>";
...
function generateTemplate()
{
	...
	var paraList=TemplateProp.paraArray;
		for(var i=0;i<TemplateProp.totalRows;i++)
		{
			...
			for(var j=0;j<9;j++)
			{
				var m=getViewControlCode(paraList[i][j]);
				//alert(m);
				h+=m;
			}
			h+="\n\t</tr>";
	...
	//alert(h);	
	//alert(document.getElementById('tblTemplate').tagName);
...
function getViewControlCodeNormal(pObj)
{
	...
	switch(pObj.paraType)
	{
		case '-1':
					...
					break;
		case '0' :
					h+="\n\t\t<td align='center' width='"+Math.floor(100/9)+"%'>";
					...
					break;
		case '1':
					h+="\n\t\t<td align='center' width='"+Math.floor(100/9)+"%' colspan='"+pObj.colspan+"'>";
					h+="\n\t\t\t<table width='100%' id='"+pObj.row+"&"+pObj.col+"' onclick='CellOnFocus(this)' border='0' cellspacing='1' cellpadding='0'>";
					h+="\n\t\t\t\t<tr>";
...
function CellOnFocus(sC) // sC -> selectCell
{
	...
	{
		//alert("Name:"+e.name+" Number:"+e.number+" Prototype:"+e.prototype);
...
function SetTypeProperties(val)
{
	...
	if(val=='2' || val=='3' || val=='4' || val=='5' || val=='6' || val=='7')
	{
		...
		// Hiding Paramter Name when not a Label in Matrix
...
function renderTemplateForModification()
{
	...
	var h="\n<div id='tblTemplate'><table width='100%' border='0' cellspacing='1' cellpadding='0'>\n</table></div>";
