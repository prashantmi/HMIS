...
if(eventObj.type=="click" || eventObj.keyCode==13)
 {
   	child = window.open(createFHashAjaxQuery(url),'popupWindow','status=yes,scrollbars=yes,height='+height+',width='+width+',left=10,top=10,dependent=yes,resizable='+resize+'');  
  	child.moveTo(250,250);
 	child.focus(); 
...
{
   	//child = window.open(createFHashAjaxQuery(url),'popupWindow','status=yes,scrollbars=yes,height=300,width=600,left=10,top=10');  
