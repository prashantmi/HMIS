...
function moveRightSingle(listNo)
{
	...
	if(listNo=="1")
	{
		source  = document.forms[0].unMappedDocument;
		target = document.forms[0].mappedDocument;	
	}
...
function moveLeftSingle(listNo)
{
	...
	if(listNo=="1")
	{
		source  = document.forms[0].mappedDocument;
		target = document.forms[0].unMappedDocument;	
	}
...
function MoveToSelected()
{
	...
	if(document.forms[0].mappedDocument)
		for(var i=0;i<document.forms[0].mappedDocument.length;i++)
			document.forms[0].mappedDocument.options[i].selected=true;
	
	// Unselect Remaining 
	if(document.forms[0].unMappedDocument)
	{	
		for(var i=0;i<document.forms[0].unMappedDocument.options.length;i++)
			document.forms[0].unMappedDocument.options[i].selected=false;
	}
...
{
	if(comboValidation(document.forms[0].patientPrimaryCategory,"Patient Primary Category") && validateRole())
	{
		return true;
	}
	else{
...
function validateRole(){
	if(document.forms[0].mappedDocument.options.length==0)
	{
		alert("Please Select Some Verification Document");
		document.forms[0].unMappedDocument.focus();
		return false;
	}
	else{
		return true;
...
function clearForm(){
   	...
   	document.getElementsByName('patientPrimaryCategory')[0].value="-1";
	document.getElementsByName('mappedDocument')[0].length=0;
	document.getElementsByName('unMappedDocument')[0].length=0;
...
function getVerificationDocs(){
	...
	submitPage('GET_VERIFICATION_DOCS');
