		...
		processGeneralNode:function(elementName,elementNode)
		  ...
		  showDivDate:function(){
			  ...
			  $(function() {
				 ...
				 var _date=new Date(new Date().getTime()).toLocaleDateString("en-au", {year: "numeric", month: "short",day: "2-digit"}).replace(/ /g,"-").replace('.', ""); 
		  ...
		  checkFromDateValid:function(){
			  $("#patFromDateId").validatebox({required: true, validType: ['date[\'fromDate\',\'dd-mmm-yyyy\']','dtltetlastdt[\'From Date must be lesser than Today\']','dtltet[\'toDate\',\'FromDate should be less than or equal to ToDate\']', 'dtltetctdt[\'Date Should Be Less Than or Equal to Current Date \']']});
			  
		  },
		  checkToDateValid:function(){
			  $("#patToDateId").validatebox({required: true, validType: ['date[\'toDate\',\'dd-mmm-yyyy\']','dtltetlastdt[\'To Date must be lesser than Today\']','dtgtet[\'fromDate\',\'ToDate should be greater than or equal to FromDate\']', 'dtltetctdt[\'Date Should Be Less Than or Equal to Current Date \']']});
