...
<his:javascript src="/registration/js/validation.js"/>
...
function departmentWiseRegPatReportHandler(){	 
	       if(document.getElementsByName('choice')[0].checked){
              document.getElementsByName('jrxmlName')[0].value="<%=RegistrationConfig.DEPARTMENT_WISE_REG_PAT_TODAY%>";
	          }
	        else{
	                    
	          document.getElementsByName('jrxmlName')[0].value="<%=RegistrationConfig.DEPARTMENT_WISE_REG_PAT_DATEWISE%>";         
	         }	   		
	   success=false;
	   if(validateGraphOrText()){ 
           ...
           if(document.getElementsByName('graphOrText')[0].selectedIndex==2){
	         document.getElementsByName('reportMode')[0].value="<%=Config.GRAPH%>";
	           if(validateGraphForDepartmentWiseRegPatReport()){
		               if(validateTodayOrDate())        
		                success=true;	                
	                }
...
<his:TitleTag name="Registered Patient Listing Report">
  ...
  <%
      ...
      <logic:equal name="departmentWiseRegPatFB" property="choice" value="<%=Config.CHOICE_TODAY%>">
      ...
      <logic:equal name="departmentWiseRegPatFB" property="choice" value="<%=Config.CHOICE_DATE_WISE%>">
        <% fromDateLabel="" ;
               toDateLabel="" ;
               fromDateControl="" ;
               toDateControl="" ;
	   ...
	   <bean:define name="departmentWiseRegPatFB" property="fromDate" id="frDate" type="java.lang.String"/>
	   <bean:define name="departmentWiseRegPatFB" property="toDate" id="tDate" type="java.lang.String"/>          
	   <%
...
<tr>            
 <td class="tdfonthead" nowrap width="25%">       	      
		      ...
		      <html:radio name="departmentWiseRegPatFB" property="choice" tabindex="1" value="<%=Config.CHOICE_TODAY%>" onclick="showdivtoday()"/> 
		         <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
		       <bean:message key="dateWise"/></font> 
		        <html:radio name="departmentWiseRegPatFB" property="choice" tabindex="1" value="<%=Config.CHOICE_DATE_WISE%>" onclick="showdivdatewise()"/>
 </td> 
 </tr>
 <tr>
 ...
 <td class="tdfont" width="25%">
	    <div id='divfromDateControl' style='<%=fromDateControl %>'align="left">	               		 
		 ...
		 <span id='divfromhrcontrol' style='<%=fromControl %>'align="left">	            
		   <html:text name="departmentWiseRegPatFB" tabindex="1" property="fromHour"  maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>	
		   <b>	<bean:message key="colon"/></b>
		</span>
		 <span id='divfromMinControl' style='<%=fromControl %>' align="left">         
			<html:text name="departmentWiseRegPatFB" tabindex="1" property="fromMin"   maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>				
 ...
 <td class="tdfont" width="25%">
		 <div id='divtoDateControl' style='<%=toDateControl %>' align="left">    
    	  ...
    	  <span id='divtohrcontrol' style='<%=toControl%>' align="left">	            
		   <html:text name="departmentWiseRegPatFB" tabindex="1" property="toHour"  maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>	
		   	<b><bean:message key="colon"/>	  </b>
		</span>
		 <span id='divtoMinControl' style='<%=toControl%>' align="left">         
			<html:text name="departmentWiseRegPatFB" tabindex="1" property="toMin" maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>				
				<bean:message key="timeFormat"/>	  
    	 </span>
   ...
   <td class="tdfonthead">
    ...
    <td class="tdfont">
       <div id="deptControl" align="left"  style='<%=deptcontrol%>'>
     		 <html:select name="departmentWiseRegPatFB"  property="departmentCode"  tabindex="1"  styleClass ="regcbo" >   
   ...
   <table width="100%" cellspacing="1" cellpadding="0">
       <tr>       
        ...
        <td width="25%" class="tdfont" > 
	        <html:select name="departmentWiseRegPatFB"  property="graphOrText" tabindex="1" styleClass="regcbo" onchange='showpdfrtfdiv()'>
		        <html:option value="-1">Select Value</html:option>
		        <html:option value="<%=Config.TEXT%>">Textual</html:option>
		      <%--   <html:option value="<%=RegistrationConfig.GRAPH%>">Graphical</html:option>	    --%>
			</html:select> 		
        </td>
       
       
        <logic:equal name="departmentWiseRegPatFB" property="graphOrText" value="<%=Config.TEXT%>">
           <%
             pdfrtflabel="" ;
             pdfrtfcontrol="" ;
        ...
        <logic:equal name="departmentWiseRegPatFB" property="graphOrText" value="<%=Config.GRAPH%>">
           <%
	   ...
	   <td class="tdfont" nowrap width="25%">       
		      ...
		      <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
			     <html:select name="departmentWiseRegPatFB"  property="reportType" tabindex="1" styleClass="regcbo">
		        <html:option value="-1">Select Value</html:option>
	 ...
	 <%--   <div id="divGraph" style='<%=category%>' align="right">
       ...
       <td width="17%" class="tdfont">
	       <html:select name="departmentWiseRegPatFB" property="patGenderCode" tabindex="1" styleClass="regcbo">
	       <html:option value="-1">Select Value</html:option>
	  ...
	  <tr>
        ...
        <font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"> 
          <html:select name="departmentWiseRegPatFB"  property="patPrimaryCatCode"  tabindex="1"  styleClass ="regcbo" onchange="if(this.value!='-1') submitPage('GETSECCAT')">   
		  <html:option value="-1">Select Value</html:option>
        ...
        <td width="25%" class="tdfont">
        <html:select name="departmentWiseRegPatFB" property="patSecondaryCatCode" tabindex="1" styleClass ="regcbo">
...
<input type="hidden"  name="mode" value="DEPARTMENTWISEREGPATREPORT">
<html:hidden name="departmentWiseRegPatFB" property="jrxmlPath" value="<%=RegistrationConfig.REGISTRATION_JRXML_PATH %>"/>
<html:hidden name="departmentWiseRegPatFB" property="jrxmlName"/>
<html:hidden name="departmentWiseRegPatFB" property="sysDate" value="<%=tDate%>"/>
