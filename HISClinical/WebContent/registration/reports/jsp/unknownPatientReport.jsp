...
function unknownPatientReportHandler()
{
	if(document.getElementsByName('choice')[0].checked)
	{
    	document.getElementsByName('jrxmlName')[0].value="<%=RegistrationConfig.UNKNOWN_PATIENT_REPORT_TODAY%>";
	}
	else 
	{
		document.getElementsByName('jrxmlName')[0].value="<%=RegistrationConfig.UNKNOWN_PATIENT_REPORT_DATEWISE%>";         
	}	
	success=false;
...
<his:TitleTag name="Unknown Patient Report">
</his:TitleTag>
	<%
		String pdfrtflabel="" ;
	...
	%>
	  ...
	  <logic:equal name="emergencyFB" property="choice" value="<%=Config.CHOICE_TODAY%>">
      ...
      <logic:equal name="emergencyFB" property="choice" value="<%=Config.CHOICE_DATE_WISE%>">
        	<% fromDateLabel="" ;
               toDateLabel="" ;
               fromDateControl="" ;
               toDateControl="" ;
      ...
      <bean:define name="emergencyFB" property="fromDate" id="frDate" type="java.lang.String"/>
	  <bean:define name="emergencyFB" property="toDate" id="tDate" type="java.lang.String"/>          
      <%
    ...
    <tr>            
 		<td class="tdfonthead" nowrap width="25%">       	      
		    ...
		    <html:radio name="emergencyFB" property="choice" tabindex="1" value="<%=Config.CHOICE_TODAY%>" onclick="showdivtoday()"/> 
		    <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
		    <bean:message key="dateWise"/></font> 
		    <html:radio name="emergencyFB" property="choice" tabindex="1" value="<%=Config.CHOICE_DATE_WISE%>" onclick="showdivdatewise()"/>
 		</td> 
 	...
 	<tr>
        ...
        <td class="tdfont" width="25%">
	    	<div id='divfromDateControl' style='<%=fromDateControl %>'align="left">	               		 
		 	...
		 	<span id='divfromhrcontrol' style='<%=fromControl %>'align="left">	            
		   		<html:text name="emergencyFB" tabindex="1" property="fromHour"  maxlength="2" size="3"  onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)" />	
		   		<b>	<bean:message key="colon"/></b>
			</span>
		 	<span id='divfromMinControl' style='<%=fromControl %>' align="left">         
				<html:text name="emergencyFB" tabindex="1" property="fromMin"   maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>				
        ...
        <td class="tdfont" width="25%">
			<div id='divtoDateControl' style='<%=toDateControl %>' align="left">    
    	  	...
    	  	<span id='divtohrcontrol' style='<%=toControl%>' align="left">	            
		   		<html:text name="emergencyFB" tabindex="1" property="toHour"  maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>	
		   		<b><bean:message key="colon"/>	  </b>
			</span>
		 	<span id='divtoMinControl' style='<%=toControl%>' align="left">         
				<html:text name="emergencyFB" tabindex="1" property="toMin" maxlength="2" size="3"  onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>				
				<bean:message key="timeFormat"/>	  
    	 	</span>
 		</td>                 			
	</tr>  
      ...
      <td width="25%" class="tdfonthead">
		...
		<td class="tdfont" width="25%">
			<div >
			<html:select
				name="emergencyFB" property="multipleHospitalCode" tabindex="1"   multiple="true" size="5">
				<%if(((List)pageContext.findAttribute(RegistrationConfig.HOSPITAL_COMBO_SEATID_WISE_LIST)).size()!=1){
					%>
					<html:option value="%">All</html:option>
				<%}%>
					<logic:present name="<%=RegistrationConfig.HOSPITAL_COMBO_SEATID_WISE_LIST%>">
					<html:options collection="<%=RegistrationConfig.HOSPITAL_COMBO_SEATID_WISE_LIST%>"
	...
	<table width="100%" cellspacing="1" cellpadding="0">     
		<tr>       
        	...
        	<td  width="25%" nowrap="nowrap" class="tdfont" > 
	        	<html:select name="emergencyFB"  property="graphOrText" tabindex="1" styleClass="regcbo" onchange='showpdfrtfdiv()'>
		        	<html:option value="-1">Select Value</html:option>
		        	<html:option value="<%=Config.TEXT%>">Textual</html:option>
		        </html:select> 		
        	</td>  
        	
        	<logic:equal name="emergencyFB" property="graphOrText" value="<%=Config.TEXT%>">
           		<%
             		pdfrtflabel="" ;
             		pdfrtfcontrol="" ;
        	...
        	<logic:equal name="emergencyFB" property="graphOrText" value="<%=Config.GRAPH%>">
            	<%
	         ...
	         <td  width="25%"  class="tdfonthead">
	   		...
	   		<td  width="25%"  class="tdfont" >  
	       		...
	       		<div id='divrtfpdf' style='<%=pdfrtfcontrol%>' align="left">
		      		<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
			    		<html:select name="emergencyFB"  property="reportType" tabindex="1" styleClass="regcbo">
		        			<html:option value="-1">Select Value</html:option>
 				...
 				<div id='divpiebar' style='<%=chartcontrol%>' align="left">
		      		<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
			     		<html:select name="emergencyFB"  property="chartType" tabindex="1" styleClass="regcbo">
		        			<html:option value="-1">Select Value</html:option>
...
<his:status/>
<html:hidden name="emergencyFB" property="reportMode"/>
<html:hidden name="emergencyFB" property="mode" value="UNKNOWNPATIENTREPORT"/>
<html:hidden name="emergencyFB" property="jrxmlPath" value="<%=RegistrationConfig.REGISTRATION_JRXML_PATH %>"/>
<html:hidden name="emergencyFB" property="jrxmlName"/>
<html:hidden name="emergencyFB" property="sysDate" value="<%=tDate%>"/>
