...
function showpdfrtfdiv(){
  ...
  if(val==1){
    ...
    document.getElementById("barline").style.display="none";  
  ...
  if(val==2){
    	...
    	document.getElementById("barline").style.display="";  
    	
    	
  }
}

function groupWiseCashCollReportHandler(){	 
//alert("inside report handler");
	       if(document.getElementsByName('choice')[0].checked){
	       		if(document.getElementsByName('patListing')[0].selectedIndex==1){
               		document.getElementsByName('jrxmlName')[0].value="<%=RegistrationConfig.GROUP_WISE_CASH_COLLECTED_TODAY_OLD%>";
	          }
	          else
	              document.getElementsByName('jrxmlName')[0].value="<%=RegistrationConfig.GROUP_WISE_CASH_COLLECTED_TODAY_NEW%>";          
	         }        
	          
	        if(document.getElementsByName('choice')[1].checked){
	            if(document.getElementsByName('patListing')[0].selectedIndex==1){
  		               document.getElementsByName('jrxmlName')[0].value="<%=RegistrationConfig.GROUP_WISE_CASH_COLLECTED_DATEWISE_OLD%>";           	            
	            }
	            else
	              document.getElementsByName('jrxmlName')[0].value="<%=RegistrationConfig.GROUP_WISE_CASH_COLLECTED_DATEWISE_NEW%>";          
	         }                 	          
	      ...
	      if(document.getElementsByName('graphOrText')[0].selectedIndex==1){
           ...
           if(document.getElementsByName('graphOrText')[0].selectedIndex==2){
	          document.getElementsByName('reportMode')[0].value="<%=Config.GRAPH%>";
	           if(validateGraphForGroupWiseCashCollReport()){
		               if(validateTodayOrDate()){
		               	if(validiatePatListing())
		               		{
	           	     		return true;           	   
...
<his:TitleTag name="Group Wise Cash Collection  Report">
  ...
  <%
      ...
      <logic:equal name="groupWiseCashCollFB" property="choice" value="<%=Config.CHOICE_TODAY%>">
      ...
      <logic:equal name="groupWiseCashCollFB" property="choice" value="<%=Config.CHOICE_DATE_WISE%>">
        <% fromDateLabel="" ;
               toDateLabel="" ;
               fromDateControl="" ;
               toDateControl="" ;
	   ...
	   <bean:define name="groupWiseCashCollFB" property="fromDate" id="frDate" type="java.lang.String"/>
	   <bean:define name="groupWiseCashCollFB" property="toDate" id="tDate" type="java.lang.String"/>          
	   <%
...
<tr>            
 <td class="tdfonthead" nowrap width="25%">       	      
		      ...
		      <html:radio name="groupWiseCashCollFB" property="choice" tabindex="1" value="<%=Config.CHOICE_TODAY%>" onclick="showdivtoday()"/> 
		         <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
		       <bean:message key="dateWise"/></font> 
		        <html:radio name="groupWiseCashCollFB" property="choice" tabindex="1" value="<%=Config.CHOICE_DATE_WISE%>" onclick="showdivdatewise()"/>
 </td> 
 </tr>
 <tr>
 ...
 <td class="tdfont" width="25%">
	    <div id='divfromDateControl' style='<%=fromDateControl %>'align="left">	               		 
		 ...
		 <span id='divfromhrcontrol' style='<%=fromControl %>'align="left">	            
		   <html:text name="groupWiseCashCollFB" tabindex="1" property="fromHour"  maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>	
		   <b>	<bean:message key="colon"/></b>
		</span>
		 <span id='divfromMinControl' style='<%=fromControl %>' align="left">         
			<html:text name="groupWiseCashCollFB" tabindex="1" property="fromMin"   maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>				
 ...
 <td class="tdfont" width="25%">
		 <div id='divtoDateControl' style='<%=toDateControl %>' align="left">    
    	  ...
    	  <span id='divtohrcontrol' style='<%=toControl%>' align="left">	            
		   <html:text name="groupWiseCashCollFB" tabindex="1" property="toHour"  maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>	
		   	<b><bean:message key="colon"/>	  </b>
		</span>
		 <span id='divtoMinControl' style='<%=toControl%>' align="left">         
			<html:text name="groupWiseCashCollFB" tabindex="1" property="toMin" maxlength="2" size="3" onkeydown="setPreviosValue(this, event)" onkeyup="moveToRightBox(this, event)" onkeypress="return validateNumeric(event)"/>				
				<bean:message key="timeFormat"/>	  
    	 </span>
 </td>                 			
</tr>
...
<td class="tdfont" >				  
   <div id="patListingcontrol" align="left"  style='<%=patListingcontrol%>'>
     		 <html:select name="groupWiseCashCollFB"  property="patListing" tabindex="1" styleClass="regcbo" onchange='showpdfrtfdiv()'>
   ...
   <table width="100%" cellspacing="1" cellpadding="0">
       <tr>       
        ...
        <td width="25%" class="tdfont" > 
	        <html:select name="groupWiseCashCollFB"  property="graphOrText" tabindex="1" styleClass="regcbo" onchange='showpdfrtfdiv()'>
		        <html:option value="-1">Select Value</html:option>
        ...
        <logic:equal name="groupWiseCashCollFB" property="graphOrText" value="<%=Config.TEXT%>">
           <%
             pdfrtflabel="" ;
             pdfrtfcontrol="" ;
        ...
        <logic:equal name="groupWiseCashCollFB" property="graphOrText" value="<%=Config.GRAPH%>">
           <%
	   ...
	   <td class="tdfont" nowrap width="25%">       
		      ...
		      <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
			     <html:select name="groupWiseCashCollFB"  property="reportType" tabindex="1" styleClass="regcbo">
		        <html:option value="-1">Select Value</html:option>
     	  ...
     	  <div id='barline' style='<%=chartcontrol%>' align="left">
		      <font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
			     <html:select name="groupWiseCashCollFB"  property="chartType" tabindex="1" styleClass="regcbo">
		        <html:option value="-1">Select Value</html:option>
	...
	<%--  
	  ...
	  <tr>
        ...
        <font color="#000000" size="1" face="Verdana, Arial, Helvetica, sans-serif"> 
          <html:select name="groupWiseCashCollFB"  property="patPrimaryCatCode"  tabindex="1"  styleClass ="regcbo" onchange="if(this.value!='-1') submitPage('GETSECCAT')">   
		  <html:option value="-1">Select Value</html:option>
...
<input type="hidden"  name="mode" value="GROUPWISECASHCOLLECTIONREPORT">
<html:hidden name="groupWiseCashCollFB" property="jrxmlPath" value="<%=RegistrationConfig.REGISTRATION_JRXML_PATH %>"/>
<html:hidden name="groupWiseCashCollFB" property="jrxmlName"/>
<html:hidden name="groupWiseCashCollFB" property="sysDate" value="<%=tDate%>"/>
