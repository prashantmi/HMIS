...
function  closeWindow(){
	...
	opener.document.getElementsByName('isDuplicatePatientPopup')[0].value="";
...
function  submitFormForNewDeptVisit(mode,obj){
	...
	<body>
		<%String action ="/RegistrationDeskDuplicate"; 
		  String desk=(String)request.getAttribute(RegistrationConfig.REGISTRATION_DESK_TYPE);
		if(desk!=null && desk.equals(RegistrationConfig.REGISTRATION_DESK_TYPE_ROOM_WISE)){
			action="/RoomWiseRegistrationDesk";
		}
		%>
			...
			<his:ContentTag>
			 <table  width="100%" border="0" cellspacing="1" cellpadding="0">
				<tr>
					...
					<b><bean:message key="select"/></b></font>
					</div>
		  			</td>
				...
				<logic:iterate id="patientVo" name="<%=RegistrationConfig.ALL_PATIENT_VO_LIST %>" type="hisglobal.vo.PatientVO">	
				  <tr>
					...
					<div align="center">	           
						<b><html:radio name="offlineRegistrationFB" property="prevCrNo" value="<%=patientVo.getPatCrNo() %>" onclick="submitFormForNewDeptVisit('SAVEPATIENTNEWDEPTVISIT',this)">
						</html:radio> </b>
					</div>
		  			</td>
...
<!--		 		<img class="button" src='<his:path src="/../HIS/hisglobal/images/buttons/btn-sv.png"/>' style=cursor:pointer onkeypress="if(event.keyCode==13) submitForm('SAVEASNEWPATIENT');" onclick ="submitForm('SAVEASNEWPATIENT');"> 
		 	...
		 	<html:hidden name="offlineRegistrationFB" property="hmode"/>
	 		</html:form>
	    <his:status/>
	</body>
</html>