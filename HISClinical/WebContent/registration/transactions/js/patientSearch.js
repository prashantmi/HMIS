...
var patientSearch={		
		...
		showSearchTile:function(){
			...
			if($('[name="isDemographicSearch"]')[1].checked){
				...
				$("#searchId").show();		
				$("#submitId").hide();	
				$("#divCatCardNoLabelId").html("<font color='red'>*</font>&nbsp;"+getFieldLabelBasedOnUniqueIdSearchBy($('[name="uniqueIdType"]')[0]));
		...
		onchange_patAddCountryCode:function()
		  {
			...
			var action 		= "/HISClinical/registration/transactions/getStatePatientSearch.action?countryCode="+countryCode;
		  ...
		  onchange_patAddStateCode:function()
		  {
			...
			var action 			= "/HISClinical/registration/transactions/getDistrictPatientSearch.action?countryCode="+countryCode+"&stateCode="+stateCode;
		  ...
		  searchPatientDtl:function()
		  {
			  	...
			  	var action 	= "/HISClinical/registration/transactions/savePatientSearch.action?patSearchById="+patSearchID+"&patUniqueIdType="+patUniqueSearchTypeID+"&patDocType="+patDocType+"&hospitalCode="+hospitalCode;
				//alert(action);
...
function createPatientEmpRow(arrStrPatJsonObj)
{
	...
	var empHeaderRow =	"<div class='div-table-listing rounded width100'><div class='div-table-row listHeader'>"+
							...
							"<div class='div-table-col width10 alignCenter'>Age/Gender</div>"+
							"<div class='div-table-col width20 alignCenter'>Father Name</div>"+
							"<div class='div-table-col width20 alignCenter'>Address</div>"+
	...
	for (i in arrPatJsonObj)
	{
		...
		if(arrPatJsonObj[i]["patGuardianName"]=="")
			...
			empDtlRow +=	"<div class='div-table-row listData'>"+
							"<div class='div-table-col width10 alignCenter'>"+
								"<input type='radio' name='patIndex' value='"+arrPatJsonObj[i]["patCrNo"]+"' onclick='callOpenerFetchPatDtlBasedOnPatCatCardNo(this);' />"+
							...
							"<div class='div-table-col width10  alignCenter'>"+arrPatJsonObj[i]["patAgeWithUnit"]+"/ "+arrPatJsonObj[i]["patgender"]+"</div>"+
							/* Start:  Sheeldarshi
							 * Reason: Bug 10480 - Address details displays under column father name for that patient at search window.
							 * "<div class='div-table-col width20  alignCenter'>"+arrPatJsonObj[i]["patGuardianName"]+"</div>"+*/
							"<div class='div-table-col width20  alignCenter'>"+guardianName+"</div>"+
							"<div class='div-table-col width20  alignCenter'>"+arrPatJsonObj[i]["patadddistrict"]+" "+arrPatJsonObj[i]["pataddstate"]+" "+arrPatJsonObj[i]["pataddcountry"]+"</div>"+
						"</div>";
	}	
			empDtlRow +="</div>";
		...
		empDtlRow +=	"<div class='div-table-listing rounded width100'><div class='div-table-col width100  alignCenter'><img id='imgNoDataId' align='center' src='/HIS/hisglobal/images/NoData.png' /></div></div>";
...
{
	//alert(obj.value);
	window.opener.document.forms[0].patCrNo.value=obj.value;
	window.opener.submitFormOnValidate(true,'GETPATDTL');
	window.close();
}
...
{
$("#divCatCardNoLabelId").html("<font color='red'>*</font>&nbsp;"+getFieldLabelBasedOnUniqueIdSearchBy(Obj));
}
