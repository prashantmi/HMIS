/*function addTab(title, url, deskMenuId, deskMenuType)
{
	...
	else 
	{
		...
		if(getDeskPatInfoStr())	deskPatInfo = getDeskPatInfoStr();
		
		if(url=='DESKPATLIST')
			targetURL = "/HISClinical/hisglobal/utility/dynamicdesk/list.cnt";
		else
			targetURL = "/HISClinical/hisglobal/utility/dynamicdesk/centerNew.cnt?mode="+url+"&deskMenuId="+deskMenuId+"&hmode=NEW"+deskPatInfo;
		alert(targetURL);
