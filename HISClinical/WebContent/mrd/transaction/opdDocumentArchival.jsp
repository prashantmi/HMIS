 ...
 <table width="100%" cellspacing="1" cellpadding="0">
 	...
 	<td width="20%"  class="tdfonthead">
 		...
 		<td width="20%"  class="tdfonthead">
  		...
  		<logic:iterate id="docArchivalId" name="<%=OpdConfig.DOCUMENT_DTL_VO_ARRAY%>" indexId="index"  type="hisglobal.vo.DocumentUploadDtlVO">
  			...
  			<%
  			String path ="";
  					if(docArchivalId.getFileType().equals(RegistrationConfig.DOCUMENT_TYPE_DOC))
  					{
							 path = ServletsUtilityConfig.SERVLET_DISPLAY_FILE_URL + "?" 
									+ ServletsUtilityConfig.FILE_PATH_WINDOWS + "=" + Config.OPD_DOCUMENT_FILE_STORAGE_PATH_WINDOWS + "&" 
									+ ServletsUtilityConfig.FILE_PATH_LINUX + "=" + Config.OPD_DOCUMENT_FILE_STORAGE_PATH_LINUX + "&" 
									+ ServletsUtilityConfig.FILE_NAME + "=" + docArchivalId.getDocumentName(); 
  					}
  					else if(docArchivalId.getFileType().equals(RegistrationConfig.DOCUMENT_TYPE_AUDIO_VIDEO))
  					{
  						 //path ="file:///"+Config.OPD_DOCUMENT_FILE_STORAGE_PATH_WINDOWS+"/"+docArchivalId.getDocumentName(); 
  						 path =Config.URL_OF_AUDIO_VIDEO_FILE_ON_SERVER + docArchivalId.getDocumentName();
  					}
  						%>
  			<tr>
  			<td width="5%"  class="tdfont">
				<div align="center">	           
				<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
				<html:checkbox name="OpdDocumentArchivalFB" property="revoke" value="<%=String.valueOf(index.intValue()) %>" onclick="enableReason(this)" tabindex="1" ></html:checkbox>
				</font>
				</div>
	  		</td>
  			...
  			<td width="20%"  class="tdfont">
				...
				<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
				<b>
				<logic:equal name="docArchivalId" property="fileType" value="<%=RegistrationConfig.DOCUMENT_TYPE_DOC %>" >
				<%=RegistrationConfig.DOCUMENT_TYPE_DOC_LABEL %>
				</logic:equal>
				<logic:equal name="docArchivalId" property="fileType" value="<%=RegistrationConfig.DOCUMENT_TYPE_AUDIO_VIDEO %>" >
  			...
  			<td width="20%"  class="tdfont">
					<div align="center">	           
				<font color="#000000" size="2" face="Verdana, Arial, Helvetica, sans-serif">
				<a  style="cursor:pointer" onclick="getUploadedeFile(event,'<%=path%>',<%=docArchivalId.getFileType() %>)" ><b><bean:write name="docArchivalId" property="documentTitle"/></b></a>
				<html:hidden name="OpdDocumentArchivalFB"  property="prevAddDocTitle" value="<%=docArchivalId.getDocumentTitle() %>"/>
				</font>
				</div>
	  		</td>
	  		...
	  		<td width="20%"  class="tdfont">
				...
				<html:text name="OpdDocumentArchivalFB" property="removeReason" disabled="true" tabindex="1" value="<%=(String)removeReasonId[index.intValue()]%>" ></html:text>  </b>
				</font>
				</div>
	  		</td>
  			</tr>
 	...
 	<his:ContentTag>
 		<table width="100%" cellspacing="1" cellpadding="0">
 			<tr >
	  		...
	  		<td width="20%"  class="tdfont">
					<div align="left">	           
						<html:select name="OpdDocumentArchivalFB" property="fileType" tabindex="1"  >
	  		...
	  		<tr>
				...
				<bean:message key="uploadFile"/>
	  			...
	  			<div align="left">
				<img class="button"  class="button" src="../hisglobal/images/icn-add.png"  border="0" tabindex="1" style="cursor:pointer" onkeypress="if(event.keyCode==13)submitToUpload(event);" onClick="submitToUpload(event);" >
				</div>
			</td>	
	...
	<logic:iterate id="documentDeatail" indexId="index" name="<%=OpdConfig.OPD_ADD_DOCUMENT_DETAIL_MAP%>" type="java.util.List">
 	...
 	<table width="100%" cellspacing="1" cellpadding="0">
 		...
 		<td width="20%"  class="tdfont" >
			...
			<td width="20%"  class="tdfont">
					<div align="center">	           
				<img class="button" border="0" src='<his:path src="/hisglobal/images/icn-min.png"/>' tabindex="1" style="cursor:pointer" onkeypress="if(event.keyCode==13)deleteRow('<%=indexVal%>')" onClick="deleteRow('<%=indexVal%>')">
				</div>
			</td>
	  		</tr>
 	</table>
