...
" 	(		select initcap(GSTR_MODULE_NAME)"+		
 "	 from GBLT_METATABLE_TYPE_MST		where GNUM_MODULE_ID = a.GNUM_MODULE_ID	) module_name,"+	
 " 	 	( 		select initcap(gstr_role_name)		from gblt_role_mst		where GNUM_ROLE_ID=a.GNUM_ROLE_ID"+	
 "			and  GNUM_MODULE_ID = a.gnum_module_id	) rolename,"+		
 " 				(		select initcap(GSTR_MENU_NAME) menuname		from GBLT_MENU_MST"+		
 "				where GNUM_MENU_ID=a.GNUM_MENU_ID	) menuname "+
 " 					 from GBLT_ROLE_MENU_MST a where GNUM_ISVALID = '1'";
...
query += "ORDER BY GNUM_MENU_ID,GNUM_MODULE_ID,GNUM_ROLE_ID";

List dataList = beanObj.getDetails(query,3);

System.out.println("query_Report "+query);
...
<a style=cursor:hand><img src='../../images/Print.gif' border=0 tabindex='1' onKeyPress='callMe()' onClick='callMe();'></a>
    ...
    <tr> 		
      <td colspan='5' align='center'><b>Role Menu Detail Report , Status	:<u><%=currentStatus%></u></b></td>	  
    </tr>
      <tr> 		
  ...
  for(int i=0;i<dataList.size();i+=3)
  {
	  ...
	  String currMenu 	= dataList.get(i+2).toString();
	  ...
	  else if( !currModule.equals(prevModule) || !currRole.equals(prevRole) || !currMenu.equals(prevSeat) )
	  {
	  ...
	  if(displayHeading)
	  {
		  if(displayLine)
		  {
			  ...
			  %> 			  
			   ...
			   <td colspan='1' align='left'><b>Menu : <%=currMenu%></b></td>
