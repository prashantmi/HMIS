...
public class seatRoleAuditLogReport extends FuncLib {
	...
	{	
		String html = "";
		System.out.println("module length ++"+this.module.length());
...
System.out.println("slno-------"+slno+"-----role id to be deleted ---"+old_roleid.get(i));
	...
	public String getListOfUnSelectedRoles(HttpServletRequest request)
	{
		...
		if(!(this.seat==null) && !this.seat.equals("") && !this.seat.equals("0")) 
		{
			...
			String query1 =" SELECT gnum_role_id,(SELECT gstr_role_name  FROM gblt_role_mst WHERE gnum_role_id = a.gnum_role_id  AND gnum_hospital_code = a.gnum_hospital_code) as roleName, to_char(gdt_effective_frm,'dd-Mon-yyyy') as effectiveFrom, "+
			  "  to_char(gdt_effective_to,'dd-Mon-yyyy') as effectiveTo,'InActive'  FROM gblt_seat_role_mst a  WHERE gnum_seatid = '"+this.seat+"'  and GNUM_HOSPITAL_CODE='"+this.hospitalcode+"'  and GBL_ISVALID in (0,2)  " +
			...
			System.out.println("getListOfUnSelectedRoles query------"+query1);
			try
			{
				rs = super.getRecord(query1,true);
				String str1 = "";
		  ...
		  if(rs!=null)
		      {	
				...
				while(rs.next())
					...
					html+="<td class='adddatavalueNew' width='25%'>"+str2+"</td>";          
					html+="<td class='adddatavalueNew' width='25%'>"+str3+"</td>";
					html+="<td class='adddatavalueNew' width='25%'>"+str4+"</td>";          
					html+="<td class='adddatavalueNew' width='25%'>"+str5+"</td>";
...
String query =" SELECT gnum_role_id,(SELECT gstr_role_name  FROM gblt_role_mst WHERE gnum_role_id = a.gnum_role_id  AND gnum_hospital_code = a.gnum_hospital_code) as roleName, to_char(gdt_effective_frm,'dd-Mon-yyyy') as effectiveFrom, "+
			  " to_char(gdt_effective_to,'dd-Mon-yyyy') as effectiveTo,'Active'  FROM gblt_seat_role_mst a  WHERE gnum_seatid = '"+this.seat+"'  and GNUM_HOSPITAL_CODE='"+this.hospitalcode+"'  and GBL_ISVALID=1  " +
			  " ORDER BY effectiveFrom desc,effectiveTo desc ";
			
			HisResultSet rs = null;
			
			
			System.out.println("getListOfSelectedRoles query---->"+query);
			try
			{
				rs = super.getRecord(query,true);
				String str1 = "";
