...
public class SeatRoleAddMstAdvancedBean extends FuncLib {
...
String query1 ="SELECT a.gnum_role_id||'#'||GNUM_MODULE_ID, (SELECT gstr_role_name"+
            ...
            " ||'('|| (select initcap(GSTR_MODULE_NAME) from GBLT_METATABLE_TYPE_MST where GNUM_MODULE_ID=a.GNUM_MODULE_ID and GNUM_HOSPITAL_CODE=a.GNUM_HOSPITAL_CODE and GBL_ISVALID=1)||')' as Role " +            
            " FROM gblt_group_role_mst a"+
            " WHERE gnum_group_code ='"+this.group+"'"+
            " AND a.gnum_hospital_code = '"+this.hospitalcode+"'"+
            " AND gnum_isvalid = '1'"+
	...
	public boolean insertRecord(HttpServletRequest req)
	{
		...
		List effecDate=new ArrayList();
		List effecTo=new ArrayList();
        ...
        "(?,(select GNUM_MODULE_ID from GBLT_GROUP_ROLE_MST where GNUM_GROUP_CODE='"+this.group+"' and GNUM_ROLE_ID=? and gnum_isvalid ='1'"+
             ...
             " AND (TO_DATE (NVL (gdt_effective_to, SYSDATE), 'DD-MM-YY') >= TO_DATE (SYSDATE, 'DD-MM-YY')"+
             " )),'"+this.seat+"','1','"+this.seatId+"','"+this.hospitalcode+"',?,sysdate,?,? )";
		
		String updateQuery ="UPDATE GBLT_SEAT_ROLE_MST set GBL_ISVALID='2',GDT_EFFECTIVE_TO=sysdate where gnum_role_id=? and GNUM_SEAT_ROLE_SLNO=? and GNUM_SEATID='"+this.seat+"'" +
							"AND gnum_hospital_code = '"+this.hospitalcode+"' ";
		
		
		String updateQueryForDates ="UPDATE GBLT_SEAT_ROLE_MST set GDT_EFFECTIVE_FRM=?, GDT_EFFECTIVE_TO=? where gnum_role_id=? and GNUM_SEAT_ROLE_SLNO=? and GNUM_SEATID='"+this.seat+"'" +
							"AND gnum_hospital_code = '"+this.hospitalcode+"' ";
...
//		System.out.println("updateQuery query"+updateQuery);
		...
		{	
			...
			ps3 = conn.prepareStatement(updateQueryForDates);
			...
			while(stOne.hasMoreElements())
			{
				...
				String[] arrayOfValues=nextElement.split("@");
				
				
				
				new_roleid.add(arrayOfValues[0]);
				effecDate.add(arrayOfValues[1]);
				effecTo.add(arrayOfValues[2]);
				
		if(arrayOfValues[2].equals("NA"))
			arrayOfValues[2]="";
					
				boolean flag = old_roleid.contains(arrayOfValues[0]);
				
				//if the new role id is not in the old role list 
				//hence it should be inserted as a new row 
				if(!(flag))
				{
					System.out.println("------new  Insertion of a role-------"+insertQuery);
					String slno = getSerialNumber(arrayOfValues[0]);
					
					ps.setInt(1,Integer.parseInt(arrayOfValues[0]));
					ps.setInt(2,Integer.parseInt(arrayOfValues[0]) );
					ps.setInt(3,(Integer.parseInt(slno))+1);
					ps.setString(4, arrayOfValues[1]);
					ps.setString(5, arrayOfValues[2]);
					

System.out.println("slno-------"+slno+"-----role id to be deleted ---"+arrayOfValues[0]);
System.out.println("effective from date"+arrayOfValues[1]+"-----effective to date---"+arrayOfValues[2]);
										
					ps.addBatch();
					ps.executeBatch();
					
				}
				else//else if it is found it is only updated here 
					if(flag)
					{
				
	System.out.println("updateQueryForDates------"+updateQueryForDates);
							ps3 = conn.prepareStatement(updateQueryForDates);
							String slno = getSerialNumber(arrayOfValues[0]);
							
							ps3.setString(1,arrayOfValues[1]);
							ps3.setString(2,arrayOfValues[2]);
							ps3.setInt(3,Integer.parseInt(arrayOfValues[0]));
							ps3.setInt(4,Integer.parseInt(slno));
	System.out.println("slno-------"+slno+"-----role id to be deleted ---"+arrayOfValues[0]);
	System.out.println("effective from date"+arrayOfValues[1]+"-----effective to date---"+arrayOfValues[2]);
			...
			for(int i=0; i<old_roleid.size();i++)
			{
				...
				//if the new role id List doesn't contains any value form the old 
				//role list .hence it is to be in validated 
				if(!(flag))
				{
					/*String isValid= getIsValid((String)old_roleid.get(i));
					if(isValid.equals("1"))
					{*/	
	System.out.println("update isValid -------"+updateQuery);
						ps1 = conn.prepareStatement(updateQuery);
...
System.out.println("slno-------"+slno+"-----role id to be deleted ---"+old_roleid.get(i));
	...
	public String getListOfUnSelectedRoles(HttpServletRequest request)
	{
		String html = "";
		int i=0;
		if(!(this.seat==null) && !this.seat.equals("") && !this.seat.equals("0")) 
		  ...
		  if(rs!=null)
		      {	
				...
				while(rs.next())
					...
					/*System.out.println("Unselected  effectFromdate----------"+effectFromdate);
					System.out.println("Unselected  effectTodate----------"+effectTodate);
					System.out.println("Unselected  effect__Fromdate----------"+effect_Fromdate);
					System.out.println("Unselected  effect__Todate----------"+effect_Todate);
					*/
...
html+="<tr>";					
html+="<td class='adddatalabel' width='10%'><input type=\"checkbox\" id=\""+checkBoxName+"\" value=\""+str1+"\"></td>";          
html+="<td class='adddatalabel' width='30%'>"+str2+"</td>";
html+="<td class='adddatalabel' width='30%'><input type=\"text\" name=\""+effectFromdate+"\" id=\""+effectFromdate+"\" readonly value='"+super.getSysdate()+"'> 	<img src=\"../../images/iconPicDate.gif\" id=\""+effect_Fromdate+"\" style=\"cursor: pointer; border: 1px solid red;\"  	title=\"Date selector\"  	onmouseover=\"this.style.background='red';\"  	 onmouseout=\"this.style.background=''\"> 	<script language=\"JavaScript\" > Calendar.setup( { inputField     :    \""+effectFromdate+"\", ifFormat       :    \"%d-%b-%Y\", button         :    \""+effect_Fromdate+"\", singleClick    :    true });	</script></td>";
html+="<td class='adddatalabel' width='30%'><input type=\"text\" name=\""+effectTodate+"\" id=\""+effectTodate+"\" readonly value=''> 	<img src=\"../../images/iconPicDate.gif\" id=\""+effect_Todate+"\" style=\"cursor: pointer; border: 1px solid red;\"  	title=\"Date selector\"  	onmouseover=\"this.style.background='red';\"  	 onmouseout=\"this.style.background=''\"> 	<script language=\"JavaScript\" > Calendar.setup( { inputField     :    \""+effectTodate+"\", ifFormat       :    \"%d-%b-%Y\", button         :    \""+effect_Todate+"\", singleClick    :    true });	</script></td>";
html+="</tr>";
		...
		html+="<input type='hidden' id='noOfUnSelectedRoles' value='"+i+"' >";
	...
	public String getListOfSelectedRoles(HttpServletRequest request)
	{
		ArrayList old_roleid=new ArrayList();
...
String query ="SELECT a.gnum_role_id||'#'||GNUM_MODULE_ID, (SELECT gstr_role_name"+
            " FROM gblt_role_mst where gnum_role_id=a.gnum_role_id and gnum_hospital_code=a.gnum_hospital_code) "+
            " ||'('|| (select initcap(GSTR_MODULE_NAME) from GBLT_METATABLE_TYPE_MST where GNUM_MODULE_ID=a.GNUM_MODULE_ID and GNUM_HOSPITAL_CODE=a.GNUM_HOSPITAL_CODE and GBL_ISVALID=1)||')' as Role, " +            
            " (SELECT to_char(gdt_effective_frm,'dd-Mon-yyyy') "+
            " FROM gblt_seat_role_mst "+
           " WHERE gnum_seatid = '"+this.seat+"' "+
            " AND gbl_isvalid = '1'  "+
            " AND gnum_hospital_code = '"+this.hospitalcode+"' "+
            " and GNUM_ROLE_ID=a.GNUM_ROLE_ID "+
            "  ) as effectiveFrom, "+
            " (SELECT to_char(gdt_effective_to,'dd-Mon-yyyy') "+
          	"  FROM gblt_seat_role_mst "+
          	" WHERE gnum_seatid =  '"+this.seat+"' "+
          	" AND gbl_isvalid = '1' "+
          	" AND gnum_hospital_code = '"+this.hospitalcode+"' "+
			...
			{	i=1;
				...
				while(rs.next())
				{
					effectFromdate="selectedFromDate"+i;
					effectTodate="selectedToDate"+i;
					effect_Fromdate="selected_FromDate"+i;
					effect_Todate="selected_ToDate"+i;
					checkBoxName="selectedCheckBox"+i;
					
					java.sql.Date currentDate=new java.sql.Date(111111);
					java.sql.Date effeFromDate=new java.sql.Date(111111);
					java.sql.Date effeToDate=new java.sql.Date(111111);
				...
				//	currentDate=super.getSqlDate(super.getSysdate());
				//	effeFromDate=super.getSqlDate(str3);
				//	effeToDate=super.getSqlDate(str4);
...
html+="<td class='adddatalabel' width='10%'><input type=\"checkbox\" id=\""+checkBoxName+"\" value=\""+str1+"\" checked=\"checked\"></td>";          
html+="<td class='adddatalabel' width='30%'>"+str2+"</td>";

//if(effeFromDate.compareTo(currentDate) < 0)
	html+="<td class='adddatalabel' width='30%'><input type=\"text\" name=\""+effectFromdate+"\" id=\""+effectFromdate+"\" readonly value='"+str3+"'></td>";
//else
//	html+="<td class='adddatalabel' width='30%'><input type=\"text\" name=\""+effectFromdate+"\" id=\""+effectFromdate+"\" readonly value='"+str3+"'><img src=\"../../images/iconPicDate.gif\" id=\""+effect_Fromdate+"\" style=\"cursor: pointer; border: 1px solid red;\"  	title=\"Date selector\"  	onmouseover=\"this.style.background='red';\"  	 onmouseout=\"this.style.background=''\"> 	<script language=\"JavaScript\" > Calendar.setup( { inputField     :    \""+effectFromdate+"\", ifFormat       :    \"%d-%b-%Y\", button         :    \""+effect_Fromdate+"\", singleClick    :    true });	</script></td>";

	html+="<td class='adddatalabel' width='30%'><input type=\"text\" name=\""+effectTodate+"\" id=\""+effectTodate+"\" readonly value='"+str4+"'> 	<img src=\"../../images/iconPicDate.gif\" id=\""+effect_Todate+"\" style=\"cursor: pointer; border: 1px solid red;\"  	title=\"Date selector\"  	onmouseover=\"this.style.background='red';\"  	 onmouseout=\"this.style.background=''\"> 	<script language=\"JavaScript\" > Calendar.setup( { inputField     :    \""+effectTodate+"\", ifFormat       :    \"%d-%b-%Y\", button         :    \""+effect_Todate+"\", singleClick    :    true });	</script></td>";
html+="</tr>";
...
html+="<input type='hidden' id='noOfSelectedRoles' value='"+i+"' >";		
