...
public class Page extends usermgmt.FuncLib
{
...
//		html = "<tr class='ShadedTitleTagImage'>\n";
		...
		html += "<tr>\n";
		html += "<td height='19' colspan='" + (no_of_fields + 1) + "' >\n";
...
//		html += "<tr class ='ShadedSubTitleTagImage' >\n";
		...
		html += "'>Total - <strong>" + totRec + "</strong> record(s)</font>\n</td>\n";
		...
		if (no_of_pages > 0)
		{
			...
			for (i = j; i <= (page_per_block + j - 1); i++)
			{
				...
				if (i == j && block > 1)
				{
					html += "<a tabindex='1' style='cursor:pointer;' onClick=\"pagination(event,'" + pagename + "','" + (i - page_per_block) + "','"
							+ (block - 1) + "');\">prev</a>\n";
				...
				if (i == page_no) 
					html += "<font color=''>" + i + "</font>\n";
				else 
					html += "<a tabindex='1' style='cursor:pointer;' onClick=\"pagination(event,'" + pagename + "','" + i + "','"
					+ (block == 0 ? 1 : block) + "');\">" + i + "</a>\n";
			}
			...
			{
				// By Preeti
				html += "<a tabindex='1' style='cursor:pointer;' onClick=\"pagination(event,'" + pagename + "','" + i + "','" + (block == 0 ? 2 : block + 1)
						+ "');\">next</a>\n";
			}
	...
	public String getDataRows(int page_no, List l1, List primary)
	{
		...
		for (i = 0; i < record_per_page; i++)
		{
			...
			{
				html += "<td align = 'left' nowrap> <font size=2 >\n";
	...
	public String toolbar(String previouspage, String dataPage_width)
	{
		...
		page_html += "<table width='" + dataPage_width + "' cellspacing='0' cellpadding='0' align = 'center' style='margin-left: 25%;'>\n";
		page_html += "<tr>\n";
		page_html += "<td height='23' align = 'center'>\n";

		page_html += "<a style='cursor:pointer;'  tabindex='1' onKeyPress='add(event);' onClick='add(event);' class='button'><span class='add'> Add </span> </a>\n";
		page_html += "<a style='cursor:pointer;' tabindex='1' onKeyPress='edit(event);' onClick='edit(event);' class='button'><span class='modify'> Modify </span> </a>\n";
		page_html += "<a style='cursor:pointer;'  tabindex='1' onKeyPress='del(event);' onClick='del(event);' class='button'><span class='delete'> Delete </span> </a>\n";
		page_html += "<a style='cursor:pointer;' tabindex='1' onKeyPress='view(event);' onClick='view(event);' class='button'><span class='view'> View </span> </a>\n";
		page_html += "<a style='cursor:pointer;' tabindex='1' onKeyPress='report(event);' onClick='report(event);' class='button'><span class='reports'> Report </span> </a>\n";
	...
	public String HospMsttoolbar(String previouspage, String dataPage_width)
	{
		...
		page_html += "<tr>\n";
		page_html += "<td height='23' align = 'center' style='padding-left: 30%;'>\n";

		page_html += "<a class='button' style=cursor:pointer;  tabindex='1' onKeyPress='add(event);' onClick='add(event);'><span class='add'>Add</span></a>\n";
		page_html += "<a class='button' style=cursor:pointer; tabindex='1' onKeyPress='edit(event);' onClick='edit(event);' ><span class='modify'>Modify</span></a>\n";
		page_html += "<a class='button' style=cursor:pointer; tabindex='1' onKeyPress='view(event);' onClick='view(event);'><span class='view'>View</span></a>\n";
		page_html += "<a class='button' style=cursor:pointer; tabindex='1' onKeyPress='report(event);' onClick='report(event);'><span class='reports'>Report</span></a>\n";
	...
	public String DefaultToolbar(String previouspage, String dataPage_width)
	{
		String page_html = "";
		...
		page_html += "<a class='button' style=cursor:pointer;  tabindex='1' onKeyPress='add(event);' onClick='add(event);'><span class='cancel'>Cancel</span></a>\n";
	...
	public String getFooter(Map searchmap, List headlist)
	{
		...
		html += "<tr class='ShadedSubTitleImage'>\n";
		html += "<td align='right' width='90%' style='vertical-align: middle' >\n";
		...
		if (!searchmap.isEmpty())
		{
			...
			html += "' tabindex='1' onKeyPress=\" pagination(event,'" + pagename + "','1','1');\"></td>\n"; // change by Pradeep 24-04-2006
																							// :: for tracking Enter key

			html += "<td><a class='button' style='cursor:pointer;;' tabindex='1' ";
			html += "  onClick=pagination(event,'" + pagename + "',1,1);  onKeyPress=pagination(event,'" + pagename + "',1,1);>";
			html += "<span class='find'> Search </span></a>\n";
	...
	public String getViewRows(List AL_List, List viewHdr, Map viewControl)
	{
		...
		for (i = 0; i < fields; i++)
		{
			...
			while (index != -1)
			{
				if (flag == 0)
				{
					html += "<td height='20' colspan = '2' class='LABEL' align = 'center'>\n";
					flag = 1;
				}
	...
	public String getViewHeader(String mstName)
	{
		...
		html = "<table class='formbg' width='100%' border='0' cellspacing='1' cellpadding='0' align = 'center'>\n";
		html += "<tr>\n";
		html += "<td colspan='2' class='ShadedSubTitleTagImage'>\n";
		html += "\n";
	...
	public String getViewToolBar()
	{
		...
		html += "<tr class='footer'><td> </td> </tr>\n";
		html += "<tr>\n";
		html += "<td colspan = '2' height='23' align='center'><div class='control_button'>\n";
		html += "<a style=cursor:pointer;>\n";
		html += "<a class='button' style=cursor:pointer;  tabindex='1'  onKeyPress='submitpage2(event,form1,\"DEFAULT\");' onClick='submitpage2(event,form1,\"DEFAULT\");'>\n";
		html += "<span class='cancel'>Cancel </span> </a>\n";
		html += "</div></td>\n";
		html += "</tr>\n";
		html += "</table>\n";
			...
			String currentStatusValue) throws Exception
	{
		...
		else
		{
			...
			htmlStr += "<tr><td style='float:right;'><div  id = 'id1' align='center'>";
			htmlStr += "<a class='button' style=cursor:pointer; tabindex='1' onKeyPress='submitpage1(event,\"DEFAULT\");' border=0 onClick='submitpage1(event,\"DEFAULT\");'><span class='cancel'>Cancel</span></a>\n";
			htmlStr += "<a class='button' style=cursor:pointer; border=0 tabindex='1' onKeyPress='callMe1(event)' onClick='callMe1(event);'><span class='print'>Print </span> </a>\n";
	...
	private String getPrintData(List AL_List, List rptWidth, int noFields, List colIndex, int totColumn)
	{
		...
		for (i = 0; i < noRows; i++)
		{
			...
			html += "<td align = 'left' width = '5%'><font size='2'face='Verdana, Arial, Helvetica, sans-serif'>" + (i + 1) + ".</font></td>\n";
			...
			for (j = 0; j < noFields; j++)
			{
				...
				html += "<td align = 'left' width = '" + rptWidth.get(j) + "'><font size='2'face='Verdana, Arial, Helvetica, sans-serif'>";
