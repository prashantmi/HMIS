...
public class Auth extends HisGlobal.HisMethods 
{
	...
	public String authorizeUser(String salt) throws Exception
	{   
		...
		String hashPassword=this.pass;

		try
		{
			String strQuery ="select GSTR_PASSWORD from gblt_hospital_mst  where GSTR_USER_NAME like '"+this.user+"' AND GNUM_ISVALID=1" ;
			String	hashPasswordDb = super.getField(strQuery );

			String strQueryPasHash ="SELECT sha1('"+salt+hashPasswordDb+"') from dual ";
			hashPasswordDb = super.getField(strQueryPasHash );
			
			boolean a =PasswordHash.slowEquals(hashPasswordDb.getBytes("UTF-16LE"), hashPassword.getBytes("UTF-16LE"));
			...
			//	ps.setString(2,encryptpass);//comment ths
		if(a)
