...
public class ReferMandatoryTag extends SimpleTagSupport
{
	private String property = "";
	private String isUnitBased = "";
	...
	public void doTag() throws JspException, IOException
	{
		...
		if (deptList != null && deptList.size() > 0)
		{
			...
			while (iterator.hasNext())
			{
				...
				if(this.getIsUnitBased().equals("")){
					mandatoryValue = WebUTIL.getDepartmentCodeFromPropertiesFile(Config.PROPERTIES_FILE_FOR_REFER_DEPARTMENTS, deptCode);
				}
				else{
					mandatoryValue = WebUTIL.getDepartmentCodeFromPropertiesFile(Config.PROPERTIES_FILE_FOR_REFER_DEPARTMENTS, deptCode.substring(0,3));
				}
				...
				else
				{
					...
					if(this.getIsUnitBased().equals("")){
						...
						validationString = validationString + " && ((document.getElementsByName('" + this.getProperty() + "')[0].value == " + deptCode
								+ ") ? (( true ";
					}
					else{
						validationString = validationString + " && ((document.getElementsByName('" + this.getProperty() + "')[0].value == " + deptCode.substring(0,3)
						+ ") ? (( true ";
					}
					...
					for (int i = 0; i < control.length; i++)
					{
						...
						if (controlType.equalsIgnoreCase("checkbox")) validationString = validationString + " && isChecked(document.forms[0]."
	...
	public String getIsUnitBased() {
		return isUnitBased;
	}

	public void setIsUnitBased(String isUnitBased) {
