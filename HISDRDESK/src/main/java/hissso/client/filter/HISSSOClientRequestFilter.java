...
public class HISSSOClientRequestFilter implements Filter
{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException
	{
		...
		try
		{
			...
			//System.out.println("HBIMS URI HISSSOClientRequestFilter "+strURI);
			
			//System.out.println("Condition ...."+strURI.contains("HISDRDESK"));
			...
			/*if(!strURI.contains("rest") && !(objRequest.getHeader("Referer") != null && objRequest.getHeader("Referer").trim().length() > 0))
            {
                ...
                System.out.println("Inside referrer ...."+strURI.contains("/HISDRDESK/new_opd/transaction/DoctorDeskAction.cnt"));
                rd = objRequest.getRequestDispatcher(HISSSOConfig.SSO_AUTHENTICATION_ERROR_PAGE_URL);
                rd.forward(objRequest, objResponse);
               
                return;
            }*/
			if (strURI.contains(HISSSOClientConfig.SSO_ST_SERVICE_URL) || strURI.contains("rest") ||strURI.contains("service") || strURI.contains("QMS/QMS.jsp") || strURI.contains("websocket") )
			{
				// Forward as-it-is
