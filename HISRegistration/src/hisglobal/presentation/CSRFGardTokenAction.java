...
public class CSRFGardTokenAction extends DispatchAction
{
	...
	public String validateToken(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		String retVal = "0";
		String checkflag="";
		if(request.getSession().getAttribute("resultEntryFlag")!=null)
		{
			System.out.println(request.getSession().getAttribute("resultEntryFlag").toString());
		 checkflag=	request.getSession().getAttribute("resultEntryFlag").toString() ; // for result entry
		}
		try
		{
		...
		if (retVal.equals("0"))
		{
			response.sendRedirect("/AHIMS/hisglobal/errorPage/token_error_page.jsp");
