 ...
 public static  String DATE_FORMAT_NOW = "dd-MMM-yyyy";
  ...
  return a;
	...
	public String generateData(GenericVO vo, String[] columnHdr, int pagePerBlock, List<String> viewHdr,
			...
			String[] CmbData, String[] ordrBy, String clr,String cnt ) {
		...
		try {
			...
			br.append("<div class='div-table'><div class='div-table-row '><DIV  ALIGN='RIGHT' ID='m^"+ b+ "' STYLE='DISPLAY:BLOCK;position:relative' TITLE='Click to View More Data'  >"
					+ "<div class='div-table-col label' ><b>"+ tempArr[0]+ " >></b></div></div></div>");
			// TO ADD HEADER NOTE & PAGINATION IN SAME ROW BY RAHUL ON 27 AUG,07
			
			br.append("<div class='div-table-row title '><div class='div-table-col width100'>");
			br.append("<DIV ALIGN='RIGHT'><FONT COLOR='white'>");
			/*
			 * getTotalBlock method
			 *
			 */
			br.append(getTotalBlock(nextpage_no, total_page, max_rownum,minrow, blockNo, clr, vo,cnt));
			br.append("</FONT></div></div></div>");
			br.append("<div class='div-table-row emptyBar '><div class='div-table-col width100'></div></div>");
			...
			br.append("<div class='div-table-row title '>"+ "<div class='div-table-col width100 label' >");

			
			for (int p = 0; p < no_of_combo; p++, i++) {
				if (i % 4 == 0)
					br.append("</div><div class='div-table-row '><div class='div-table-col label width100'>");	// CSS CHANGED FROM TDFONTCOMBO TO TDCOMBOHEADER BY RAHUL ON 27 AUG,07
				...
				br.append("<B>"	+ combo_header[2*p+1]+ "&nbsp;</B><SELECT NAME='combo' TITLE='"+ combo_header[2*p+1]+"'  onChange=fetchRecordsCombo("
								+ p + ")>" + combo_data + "</SELECT>&nbsp;&nbsp;&nbsp;");

			}
			
			br.append("</div></div> </div>"
					+ "<div class='div-table-listing rounded'><div class='div-table-row listHeader ' >");
			br.append(getColoumnHeaderString(columnHdr, ordrBy, vo));
			br.append("</div>");
			if (rowsetSize <= 0) {
				br.append("<div class='div-table-row '><div class='div-table-col' style='width: "+10*(columnHdr.length + 1)+"'><b>No Record Found</b></div></div></div>");
			}
			br.append("<div class='div-table-row'><div class='div-table-col' style='width:100%'>");

			br.append(getQueryData(rowset,(95 / columnHdr.length), minrow, vo,masterName));
			br.append("</div></div>");
...
public String getTotalBlock(int nextpage_no, int total_page,int max_rownum, int minrow, int blockNo, String colorNextPrev, GenericVO vo,String cnt)
    {
		char temp1 = 'a';
		int t = 1;
		int temp = 1;
		...
		if (blockNo != 1)
			...
			data.append("<A STYLE='CURSOR:POINTER' onClick=fetchRecords('m^"
					+ (blockNo - 1) + "^0'," + (minrow)
					+ ",'2',\""+cnt+"\")><B><FONT COLOR='" + colorNextPrev
					+ "'>Previous</FONT></B></A> ");
		...
		for (int k = 1; k <= total_page; k++)
		{
			if (actual_page_block > 1)
			{
				...
				//	System.out.println("Inside If");
					data.append("<A  STYLE='CURSOR:POINTER' onClick=fetchRecords('m^"
									...
									+ colorNextPrev
									+ "'>Next</FONT></B></A> ");
				//	System.out.println("Inside If ->"+data.toString());
				}
				else
				{
				//	System.out.println("Step II");
					/*
					data.append("<A ID='bb" + temp1 + "" + k
							+ "' STYLE='CURSOR:POINTER' onClick=changeDiv('"
							+ temp1 + "" + k + "')><B><U>" + (nextpage_no +1)
							+ "</U></B></A> ");
					*/
					data.append("<A ID='bb" + temp1 + "" + k
							+ "' STYLE='CURSOR:POINTER' onClick=changeDiv('"
							+ temp1 + "" + k + "')><B><U>" + nextpage_no
							+ "</U></B></A> ");
			...
			{
			//	System.out.println("Insdie else 2");
				data.append("<A ID='bb" + temp1 + "" + k
						+ "' STYLE='CURSOR:POINTER' onClick=changeDiv('"
						+ temp1 + "" + k + "')><B><U> " + nextpage_no
						+ "</U></B></A> ");
	...
	public String getColoumnHeaderString(String[] columnHdr,
		...
		int size_percent = 95 / coulmn_header_length;
		...
		try{
			if(strDragDrop.equals("1"))
				data.append("<div class='div-table-col width100'  >#</div>");
			data.append("<div class='div-table-col'  style='width:5%'><INPUT TYPE='CHECKBOX' NAME='chkmain' onkeypress='return keypress(event);' onclick='isCheckedFirst();'></div>");
			...
			for (int columnHead = 0; columnHead < coulmn_header_length; columnHead++) {
				...
				if (flag == 1) {
						...
						if(vo.getOrderby().equals(order_by_Array[orderHead + 1]+ " ASC")) {
							...
							data.append("<div class='div-table-col' style='width:"
									...
									+ order_by_Array[orderHead + 1] + " DESC\");'></div>");
					...
					data.append("<div class='div-table-col' style='width:" + size_percent+ "%'><B>" + coulmn_header[columnHead] + "</B></div>	");
				}
			...
			while (rowset.next()) {
				...
				if (temp % rec_per_page == 0)
				{
					...
					if (idvalue.equals("a1"))
						dataBuffer.append("<DIV ID='"+ idvalue	+ "' STYLE='DISPLAY:BLOCK;position:relative'><div class='div-table'>");

					else
						dataBuffer.append("<DIV ID='"+ idvalue+ "' STYLE='DISPLAY:NONE;position:relative'><div class='div-table'>");

//					System.out.println("Data-->"+dataBuffer.toString());
				}
				dataBuffer.append("<div class='div-table-row listData' ID='tr" + k	+ "' "+rowClassName+">");				
				ResourceBundle utilRes = ResourceBundle.getBundle("hisglobal.hisconfig.hisProperties");
				...
				if(strDragDrop.equals("1"))
					dataBuffer.append("<div class='div-table-col'><img class='drag' title='Drag this Tag to Required Process' style='cursor: pointer;' " +
							" name='"+tempArray[0]+"' src='../hisglobal/images/Drag_Row_1.gif'></div>");
				
				
				dataBuffer.append("<div class='div-table-col width5'><INPUT TYPE='CHECKBOX' onkeypress='return keypress(event)' VALUE='"	+ tempArray[0] + "$" + (k) + "' NAME='chk'></div>");
				k++;

				for (int m = 0; m < no_of_column; m++)
				{
					dataBuffer.append("<div class='div-table-col' style='width:" + size_percent + "%'>&nbsp;"+ tempArray[m + 1] + "</div>");
				}
	...
	public String getSearchRowString(GenericVO vo,int pagePerBlock, List<String> viewHdr,
		...
		String search ="";
		if (vo.getSearch()!=null) search = vo.getSearch().replace("$", "%");
		...
		try{
			...
			br.append("####<div class='div-table' ><div class='div-table-row'>"	+
					"<div class='div-table-col  ' style='width: 98%; text-align: right;' >  Search By&nbsp;&nbsp;&nbsp;");
			br.append(" "	+ "<SELECT  name='searchColumn' " +
					">");
			...
			br.append("</select>");
			br.append("<input type='text' " +
					" name='search' value='"	+ selectSearchValue+ "' onkeyup='validateSearchField(this);' onchange='validateSearchField(this);' onKeyPress='if(event.keyCode==13) return searchpage();' >");
			br.append(" &nbsp;&nbsp;<a  onClick='searchpage();' class='button' href='#'><span class='search'>Search</span></a></div>");
			...
			if(temp.length>=2)
			{
				...
				{
					br.append("<div class='div-table-row'><div class='div-table-col' style='width:100%'><div align='right'>" +
							"[<FONT COLOR='" + rowStatus[3] + "'>"
							+ rowStatus[2] + "</FONT>]" +
									"</div></div></div></div>");
				}
				else
					br.append("<div class='div-table-row'><div class='div-table-col' style='width:100%'>&nbsp;</div></div></div>");	// LOWER HEADER ROW IS ADDED HERE BY RAHUL ON 27 AUG,07
			}
			else
				br.append("<div class='div-table-row'><div class='div-table-col' style='width:100%'>&nbsp;</div></div></div>");	// LOWER HEADER ROW IS ADDED HERE BY RAHUL ON 27 AUG,07
	...
	*/
	...
	public String getReportInterface(GenericVO vo, String[] colmnHeader,String strRecordPerPage,String cnt) {
		...
		try {
			...
			reportQuerydata.append("<tr><td colspan=2 width=100% align=center>" +
					...
					"onclick='generateReport(\""+cnt+"\");'><img onkeypress='onPressingEnter(this,event)' tabindex='1' " +
					"style='cursor: pointer;' src='../hisglobal/images/back_tab.png' " +
					"onclick='submitCancelAction(\""+cnt+"\");'></td></tr>");
	...
	public String getReports(GenericVO vo,
			String[] comboHeader, int record_per_page, int page_per_block, String[] colmnHeader,
		...
		//	int maxRow = (blockNo* (record_per_page * page_per_block) + 1);
			...
			if(vo.getStrRepeat()==null || !vo.getStrRepeat().equals("yes")){
				...
				reportQuerydata.append((colmnHeader.length + 1)*no_of_combo + "'><div align=right class='style_16'>Date:&nbsp;"+date+"</div></TD>");
				reportQuerydata.append("</TR>");
				for (int k = 1, i = 0; i < no_of_combo; i++, k++) {
					if (k % 3 == 0)
						reportQuerydata.append("<TR>");

					reportQuerydata.append("<TD WIDTH='20%'><div class='style_16'>" + comboHeader[2*i+1] // changed on 12 july.. change is 2*i+1 instead of i
							+ ":</div></TD><TD class='style_19'> " + temp[i] + "</TD> ");
				}
				reportQuerydata.append("</TR><TR><TD class='style_19' COLSPAN='"
						+ (colmnHeader.length + 1) *no_of_combo+ "' ALIGN='RIGHT'>");
				prev = blockNo;
				if (blockNo > 1)
					reportQuerydata.append("<A  STYLE='CURSOR:POINTER' tabIndex=1 " +
					"onClick=fetchNextPrev('"+ (--prev)+ "')><B><font color='black'>Previous</FONT></B></A>");
				...
				if (webRs.size() > length_min)
					reportQuerydata.append("<a style='cursor:pointer' tabIndex=1 onClick=fetchNextPrev('"+ (++next)
									+ "')><b><font color='black'>Next</font></b></a>");
			...
			reportQuerydata
					.append("<TABLE ALIGN='CENTER' id='reportTableId' width='100%'  " +
			...
			if(vo.getStrRepeat()==null || !vo.getStrRepeat().equals("yes")){
				reportQuerydata.append("<TR class='style_14' valign='top'><TD align=center WIDTH='5%'  " +
						" style='border: 1px solid rgb(0, 0, 0); font-weight: normal;' class='style_15'>" +
						"<div class='style_16'>S.No.</div></TD> ");
			}
			/*****************testcodestart***********/
			...
			if(vo.getStrRepeat()==null || !vo.getStrRepeat().equals("yes")){
				for (int i = 0; i < colmnHeader.length; i++) {
					...
					if(fPathClearGo){
						reportQuerydata.append("<TD align=left WIDTH=" + strColsWidth[i] + "%   " +
								" style='border: 1px solid rgb(0, 0, 0); font-weight: normal;' class='style_15'>" +
								" <div class='style_16'>"
								+ colmnHeader[i] + "</div></TD> ");
					}
				}
				reportQuerydata.append("</TR>");
			}
				...
				if (length_loop <= (length_min)) {
					...
					for(int nTmpI=0;strGroupByCols!=null && nTmpI<strGroupByCols.length; nTmpI++){
						...
						if(strTmpData[nTmpI]==null || !strTmpData[nTmpI].equals(tempArray[nTmpJ])){
							...
							if(!strConcat.equals("") && fPrint && vo.getStrConcatString().equals("1")){
								...
								reportQuerydata.append("<TR> <TD valign='top' align=center   class='style_19' WIDTH='5%' >&nbsp; "
										+ (++nNewSlNo) + ".</TD>");
								reportQuerydata.append("<TD class='style_19' valign='top' ALIGN='"+align+"'  WIDTH="
										+ width + "% >&nbsp; " + strConcat + " </TD></tr>");
								fPrint = false;
								strConcat="";
							//	fPrintInLoop = true;
							}
					...
					for (int i = 0; i < colmnHeader.length; i++) {
						...
						if(fPathClearGo && vo.getStrConcatString().equals("0")){
							...
							/*****************testcodeend***********/
							reportQuerydata.append("<TD class='style_19' valign='top' ALIGN='"+strAlign+"'  WIDTH="
								+ strColsWidth[i] + "% >&nbsp; " + tempArray[i + 1] + " </TD>");
							/*****************testcodestart***********/
	...
	public String getNextPrevRows(GenericVO vo,	String mstName, String nextprevcolor,String cnt) {
		...
		try{
			viewBuffer.append("<div class='div-table'>");
			viewBuffer.append("<div class='div-table-row title'>"
							+ "<div class='div-table-col control width25'>"
							+ master
							+ "</div><div class='div-table-col label' style='width:73%'>");

			viewBuffer.append("<a style='cursor:pointer' onClick=fetchViewPrevious('"
							+ (--prev) + "','"+ (cnt) + "')><B><font color='"
							+ nextprevcolor
							+ "'>Previous</font></B></A>");

			viewBuffer.append("&nbsp;&nbsp; || &nbsp;&nbsp;");
			viewBuffer.append("<a  style='cursor:pointer' onClick=fetchViewNext('"+ (next) + "','"+ (cnt) + "')><B> <font color='"
					+ nextprevcolor	+ "'>Next </font></B></A></div></div>");
		} catch(Exception e){
			e.printStackTrace();
			vo.setStrMsgString("GenericHLP.getNextPrevRows -->"+e.getMessage());
			vo.setStrMsgType("1");
		}
		System.out.println(viewBuffer.toString());
	...
	//	viewBuffer.append("<div class='div-table'>");
		...
		if (vwHdr.size() < 0) {
			...
			for (int i = 0, j = 0; i < vwHdr.size(); j++, i += 2) {
				...
				viewBuffer.append("<div class='div-table-row'><div class='div-table-col width30 label'>"
						+ vwHdr.get(i)
						+ " </div><div class='div-table-col  width55 control' >"
						+ getControl((String) AL_List.get(j), fieldName, p	.charAt(0)) + "</div></div>");

			}
		}
		viewBuffer.append("<div class='div-table-row '><div class='div-table-col  '></div></div>");
		viewBuffer.append("<div id='footer' style='display: none'></div><div class='div-table-button' >	<div class='div-table-row'><div class='div-table-col footerBar'></div></div><div class='div-table-row'><div class='div-table-col emptyBar'></div></div><div class='div-table-row' align='center'>");
		
		viewBuffer.append("<a onClick='window.opener.closePopUp_master();' class='button' href='#'><span class='cancel'>Cancel</span></a>");
		viewBuffer.append("</div></div></div>");
		} catch(Exception e){
