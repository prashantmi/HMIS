...
public class GenericData {
	...
	public static void LIST(HttpServletRequest request,
			MasterInterface masterObj, String cnt_page,String module_id,Map sessionMap)

	{
		 //HttpSession session=ServletActionContext.getRequest().getSession(false); 
		HttpSession htSession = request.getSession();
		 request.getSession().setAttribute("cnt_page", cnt_page); 
		 request.getSession().setAttribute("module_id", module_id); 


		sessionMap.put("MSTCOMBO",  request.getParameterValues("combo"));
		sessionMap.put("USERVALUE",  userVal);
		htSession.setAttribute("MSTCOMBO", request.getParameterValues("combo"));
		htSession.setAttribute("USERVALUE", userVal);

		masterObj.setHttpSessionMap(sessionMap);
		// Setting Request
		Class classObj=masterObj.getClass();
		try
		{
	...
	public static void LISTPAGE(HttpServletRequest request,
		...
		try {
			...
			if(vo.getBlockNo()==null){vo.setBlockNo("1");}
			vo.setPrevNext(request.getParameter("prevNext"));
			if(vo.getPrevNext()==null){vo.setPrevNext("1");}

			vo.setRowNum(request.getParameter("rowNum"));
			if(vo.getRowNum()==null){vo.setRowNum("0");}
			if(vo.getMax_rownum()==null){vo.setMax_rownum("10");}
			...
			response.getWriter().write(hlp.generateData(vo, masterObj.getColumnHeader(),
					...
					MasterInterface.nextprevcolor,	(String)request.getSession().getAttribute("cnt_page"))); 
	...
	public static void REPORTDATA(HttpServletRequest request,
		...
		PrintWriter out = null;
		String hospCode=Config.SUPER_HOSPITAL_CODE ;
		try {
...
//			out.print(hlp.getReportInterface(vo, masterObj.getColumnHeader(), (masterObj.getRecord_per_page()*masterObj.getPage_per_block())+"",(String)request.getSession().getAttribute("cnt_page"))+interFace);
//		} catch (Exception e) {
//			vo.setStrMsgString(e.getMessage());
//			vo.setStrMsgType("1");
//			HisException eObj = new HisException(masterObj.getMasterName(),
