...
public class PatientVisitUTL extends ControllerUTIL
{	
	public static void setPatAmount(PatientVisitSUP objPatVisitSUP_p,PatientVO objPatientVO_p,UserVO objUserVO_p, HttpServletRequest objRequest_p ){
		...
		System.out.println("PatPrimaryCatCode"+objPatientVO_p.getPatPrimaryCatCode());
		/*System.out.println("PatPrimaryCatCode"+objPatientVO_p.getPatPrimaryCatCode()+""+objPatVisitSUP_p.getPaymentModeCode().split("#")[2]);*/
		/* #Start					:Sheeldarshi 
		...
		EpisodeRefDtlVO[] episodeReferalDtlVO=(EpisodeRefDtlVO[])objRequest_p.getSession().getAttribute(RegistrationConfig.ARR_EPISODE_REFER_OPD_PAT_VO);
			...
			switch(key)
			{
				case 1: case 2:
					...
					else
					{
						...
						{
							if(registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_Only)||registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_and_Special_Both))
							{
								objUserVO_p.setTariffID(RegistrationConfig.OPD_CROSS_CONSULTATION_TARIFF_ID);
								strAmountCollected=PatientVisitDATA.getBillAmount(objPatientVO_p.getPatPrimaryCatCode(),objUserVO_p);
					...
					if(!objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY) && 
						...
						//if(objPatientVO_p.getIsReferred().equals("1"))
						//	objPatVisitSUP_p.setPatAmountCrConsultation("0.00");
				...
				case 3: case 4:
					...
					if(objPatVisitSUP_p.getNewDepartmentVisit().equalsIgnoreCase("on")){
						...
						else
						{
							...
							//objPatVisitSUP_p.setPatActualAmount(strAmountCollected);
							objPatVisitSUP_p.setPatActualAmount(strAmountWOCrConsultation);
							/* #Start					:Sheeldarshi 
							#Modify Date(CR/PRS)	:28thMay'15 
							#Reason					: UAT Test Report Requirements:Requirement 8
							 */
	...
	public static void setPatientDtlByCrno( PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p){
		...
		System.out.println("PatientVisitUTL :: setPatientDtlByCrno()");
		...
		try{
			if(objPatVisitSUP_p.getOtherHospitalFlag().equals("0"))
			{
			objPatientVO=PatientVisitDATA.getPatientDtl(objPatientVO,objUserVO, visitType);
			
			//added by manisha gangwar date: 28.3.2018
			WebUTIL.setAttributeInSession(objRequest_p,FileTransferConfig.KEY_UPLOADED_FILE_NAME, objPatientVO.getImageFileName());
			WebUTIL.setAttributeInSession(objRequest_p,FileTransferConfig.KEY_UPLOADED_FILE_CONTENT, objPatientVO.getImageFile());
			
			//HelperMethods.populate(objPatVisitSUP_p, objPatientVO);
			objPatVisitSUP_p.setPatAge(objPatientVO.getPatAge());
			objPatVisitSUP_p.setPatGender(objPatientVO.getPatGender());
			//delete below line
			...
			PatientVisitUTL.getAddmittedDetailsOnCRNO(objPatVisitSUP_p, objRequest_p);
			...
			if(objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY_WITHOUT_REFERENCE) ||
				...
				PatientVisitUTL.getCreditBeneficiaryDtlForPatCatOnCRNO(objPatVisitSUP_p,objRequest_p);
			}
			...
			if(objRequest_p.getSession().getAttribute(RegistrationConfig.ESSENTIALBO_MAP_OF_RENEWEL_CONFIG_VO)==null)
				PatientVisitUTL.getMapOfRenewalConfigDtlOnKeyPatCat(objPatVisitSUP_p,objRequest_p);
			...
			#Reason					: UAT Test Report Requirements:Requirement 8
			 */
			setReferEpisodeDtl(objPatVisitSUP_p, objRequest_p);
			//end
			// Setting Patient Amount
			setPatAmount(objPatVisitSUP_p, objPatientVO, objUserVO, objRequest_p);
			//setting the patient qr code
			setQrCode(objPatVisitSUP_p, objPatientVO, objUserVO, objRequest_p);
			...
			EpisodeRefDtlVO[] episodeReferalDtlVO=(EpisodeRefDtlVO[])objRequest_p.getSession().getAttribute(RegistrationConfig.ARR_EPISODE_REFER_OPD_PAT_VO);
			if(registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_Only)||registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_and_Special_Both)){
			//if(registrationConfigVO.getIsCrossConsulation().equals("1")){
			objPatVisitSUP_p.setPatIsCrossConsultantWithReferal("1");
			if(episodeReferalDtlVO!=null&&episodeReferalDtlVO.length>0)
			  {
					objPatVisitSUP_p.setIsReferred("on");
					objPatVisitSUP_p.setIsRefferInOut("I");
					objPatVisitSUP_p.setReferInternalExternal("I");
					objPatVisitSUP_p.setReferingRowIndex("0");
			  }
			}
			//End
			status.add(Status.TRANSINPROCESS);
		}
		else if(objPatVisitSUP_p.getOtherHospitalFlag().equals("1"))
		{
			setExtPatientDtlByCrno(objPatVisitSUP_p, objRequest_p);
		}
		}
	...
	#Reason					: UAT Test Report Requirements:Requirement 8
	 */
	public static void setReferEpisodeDtl(PatientVisitSUP objPatVisitSUP_p,
		...
		try{			
			...
			//if(objPatVisitSUP_p.getIsPatReferByList().equals("1"))
			//{
				EpisodeRefDtlVO[] arrEpisodeRefDtlVO=null;
				if(objPatVisitSUP_p.getOtherHospitalFlag().equals("0"))
				arrEpisodeRefDtlVO=PatientVisitDATA.getReferEpisodeDtl(objPatVisitSUP_p.getPatCrNo(),"",userVO);
				else if(objPatVisitSUP_p.getOtherHospitalFlag().equals("1"))
				{
					//userVO.setHospitalCode(objPatVisitSUP_p.getFromHospital());
					arrEpisodeRefDtlVO=PatientVisitDATA.getExtReferEpisodeDtl(objPatVisitSUP_p,"",userVO);
				}
				WebUTIL.setAttributeInSession(request,RegistrationConfig.ARR_EPISODE_REFER_OPD_PAT_VO, arrEpisodeRefDtlVO);
	...
	public static void setAllInitialOldPatientVisitEssentials(HttpServletRequest objRequest_p){
		...
		try{
			...
			Map mp=PatientVisitDATA.getAllInitialOldDeptVisitEssentials();
			WebUTIL.setMapInSession(mp, objRequest_p,"PatientVisitACT");
	 ...
	 */
		public static void setAllInitialNewDeptVisitEssentials(HttpServletRequest objRequest_p,PatientVisitSUP objPatVisitSUP_p){
			...
			try{
			Map mp=PatientVisitDATA.getAllInitialNewDeptVisitEssentials();
	...
	public static void setOldDeptVisitEssentials(PatientVisitSUP objPatVisitSUP_p, HttpServletRequest objRequest_p){
		...
		Status status = new Status();
		try{
			Map mp=PatientVisitDATA.getOldDeptVisitEssentials(objPatVisitSUP_p, getUserVO(objRequest_p));
			WebUTIL.setMapInSession(mp, objRequest_p,"PatientVisitACT");
	 ...
	 * @param objRequest_p HttpServletRequest
	 */
	public static void setEpisodeDetails( PatientVisitSUP objPatVisitSUP_p,String strPatCatCode,HttpServletRequest objRequest_p ){		
	Status status = new Status();
		try{
			System.out.println("PatientVisitUTL :: setEpisodeDetails()");
			UserVO objUserVO =getUserVO(objRequest_p);		
			...
			if(objPatientVO.getPatIsDead().equals(RegistrationConfig.PATIENT_IS_DEAD)==false){
				objArrEpisodeVO=PatientVisitDATA.getOldPatientEpisodes(objPatVisitSUP_p.getPatCrNo(),strPatCatCode, RegistrationConfig.EPISODE_VISIT_TYPE_OPD, objPatientVO.getRenewalConfig().getStrRenewalType(), objUserVO);
	...
	public static RegistrationSlip preapareSlip(EpisodeVO objArrEpisodeVO[], PatientVO objPatientVO_p, PatientVisitSUP objPatVisitSUP_p, HttpServletRequest objRequest_p){		
		...
		regSlip.setDependentName(objPatientVO_p.getPatStaffName());
		regSlip.setPatMemRelationName(objPatientVO_p.getPatStaffRelationName());
		regSlip.setHiddenPatIdNo(objPatientVO_p.getPatStaffNo());
		regSlip.setPatMemDeptName(objPatientVO_p.getPatStaffDeptName());
	...
	public static boolean saveOldPatientVisit(HttpServletRequest objRequest_p, PatientVisitSUP objPatVisitSUP_p,String strMode_p){
		...
		try
		{
			System.out.println("PatientVisitUTL :: saveOldPatientVisit()");
			...
			String[] str=objPatVisitSUP_p.getArrsnomdPTVisitReason().split(",");
			String[] str1=objPatVisitSUP_p.getArrsnomdPTVisitReason().split(",");
			String[] str2=objPatVisitSUP_p.getArrsnomdCIdVisitReason().split(",");
			...
			for(int i=0; i<y; i++)
			{
				for(int j=0;j<x;j++){
					...
					if(arrEpisodeVOSes[i].getEpisodeCode().equals(deptToVisit[j])){
						arrEpisodeVOSes[i].setPatVisitReason(objPatVisitSUP_p.getArrPatVisitReason()[i]);
						arrEpisodeVOSes[i].setCreditBillFlag(objPatVisitSUP_p.getArrCreditBillFlag()[i]);
						arrEpisodeVOSes[i].setSnomdCIdVisitReason(str2[i]);
						arrEpisodeVOSes[i].setSnomdPTVisitReason(str1[i]);
						...
						if(objPatVisitSUP_p.getPatPrimaryCatGrpCode()!=null && objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY) )
						{
							if("1".equals(objPatVisitSUP_p.getArrCreditBillFlag()[i])){
								...
								arrEpisodeVOSes[i].setCreditLetterRefNo(objPatVisitSUP_p.getCreditLetterRefNo());
								arrEpisodeVOSes[i].setCreditLetterDate(objPatVisitSUP_p.getCreditLetterDate());
							...
							else
							{
								...
								/*if(arrEpisodeVOSes[i].getPatPrimaryCatCode()!=null && arrEpisodeVOSes[i].getPatPrimaryCatCode().equals(objPatVisitSUP_p.getPatPrimaryCatCode()))
								{
									arrEpisodeVOSes[i].setCreditLetterRefNo(objPatVisitSUP_p.getArrCreditLetterRefNo()[i]);
									arrEpisodeVOSes[i].setCreditLetterDate(objPatVisitSUP_p.getArrCreditLetterDate()[i]);
								}
							...
							{
								objPatientVO.setCreditLetterDate(objPatVisitSUP_p.getCreditLetterDate());
								objPatientVO.setCreditLetterRefNo(objPatVisitSUP_p.getCreditLetterRefNo());
								objPatientVO.setCreditLimit(objPatVisitSUP_p.getCreditLimit());
								objPatientVO.setClientCode(objPatVisitSUP_p.getClientCode());
								objPatientVO.setClientName(objPatVisitSUP_p.getClientName());
								objPatientVO.setStaffCardNo(objPatVisitSUP_p.getStaffCardNo());
								objPatientVO.setStaffCardName(objPatVisitSUP_p.getStaffCardName());
								objPatientVO.setLetterType(objPatVisitSUP_p.getLetterType());
								objPatientVO.setCardvalidityDate(objPatVisitSUP_p.getCardvalidityDate());
								objPatientVO.setRelationWithStaff(objPatVisitSUP_p.getRelationWithStaff());
								objPatientVO.setRelationNameWithStaff(objPatVisitSUP_p.getRelationNameWithStaff());
								
							}
							
						}
						if(objPatVisitSUP_p.getPatPrimaryCatGrpCode()!=null && objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY_WITHOUT_REFERENCE) )
						{
							if(arrEpisodeVOSes[i].getRenewalRequired().equalsIgnoreCase("1"))
							{
									objPatientVO.setClientCode(objPatVisitSUP_p.getClientCode());
									objPatientVO.setClientName(objPatVisitSUP_p.getClientName());
									objPatientVO.setAgsNo(objPatVisitSUP_p.getAgsNo());
									objPatientVO.setAgsDistrictCode(objPatVisitSUP_p.getAgsDistrictCode());
									objPatientVO.setAgsCounterNo(objPatVisitSUP_p.getAgsCounterNo());
									objPatientVO.setCreditLimit(objPatVisitSUP_p.getAgsCreditLimit());
				...
				for(int i=0;i<objArrEpisodeVO1.length;i++)
				{
					...
					if(strRenewalTypeHospOrEpisode.equals("E")){
						if(objArrEpisodeVO1[i].getRenewalRequired().equalsIgnoreCase("1")){
						 ...
						 } else{/*Block for handling the old dept visit case*/
							 objUserVO.setTariffID(RegistrationConfig.OLD_DEPT_VISIT_TARIFF_ID);
							 selectedEpisodeVO[i].setTariffId(RegistrationConfig.OLD_DEPT_VISIT_TARIFF_ID);
							 selectedEpisodeVO[i].setPatAmountCollected(RegistrationConfig.PAT_CAT_FREE_FEES);/////////////
						 }
			...
			*/
			...
			objArrEpisodeVO2=PatientVisitDATA.saveOldPatientVisit(objPatientVO, selectedEpisodeVO, objUserVO,episodeRefVO);
...
/**
 * sets Patient Status
 * calls  getPatientStatus(objPatientVO, objUserVO) of PatientVisitDATA
 * @param objRequest_p HttpServletRequest
...
//		String patStatus="";
//		try{
//	    patStatus=PatientVisitDATA.getPatientStatus(objPatientVO_p, objUserVO);
//	    
//		}
//		catch(HisRecordNotFoundException e){
	...
	public static void getOpenEpisodeDtl(HttpServletRequest objRequest_p, PatientVisitSUP objPatVisitSUP_p){
		...
		try{
			if(objPatVisitSUP_p.getIsReferred().equals(RegistrationConfig.IS_REFERRED_TRUE)){
				if(objPatVisitSUP_p.getOtherHospitalFlag().equals("0"))
				{
			EpisodeRefDtlVO[] arrOPDOpenEpisodeVO=PatientVisitDATA.getOpenEpisodeOPD(objPatientVO.getPatCrNo(), objUserVO,"3");
			...
			objPatVisitSUP_p.setIsRefferInOut("I");
					...
					EpisodeRefDtlVO[] arrOPDOpenEpisodeVO=PatientVisitDATA.getExtOpenEpisodeOPD(objPatVisitSUP_p,objUserVO, "3");
					for(int i=0;i<arrOPDOpenEpisodeVO.length;i++)
					{
						if(arrOPDOpenEpisodeVO[i].getFromDepartment()==null)
							arrOPDOpenEpisodeVO[i].setFromDepartment("");
					...
					WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.ARR_OPD_OPEN_EPISODE_VO,arrOPDOpenEpisodeVO);
					objPatVisitSUP_p.setIsReferred("1");
					objPatVisitSUP_p.setIsRefferInOut("O");
	...
	//	String isVisitOnRequest=objPatVisitSUP_p.getOnRequestVisit();
		try{
			...
			{	
				...
				arrayEpisodeForStamping=PatientVisitDATA.saveDeptWiseRenewalAndStaming(objPatientVO,selectedEpisodeVO,arrRenewalEpisodeVO,objArrEpisodeVO,episodeRefVO,sysDate,objUserVO);
...
//	}
	...
	public static void getOldPatReferDtl(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			System.out.println("PatientVisitUTL :: getOldPatReferDtl()");
			EpisodeRefDtlVO[] arrEpisodeOldPatRefDtlVO=PatientVisitDATA.getReferPat(objUserVO,"1");
	...
	public static void getReferPatient(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p,String strMode_p)
	{
		...
		try
		{
			...
			arrEpisodeRefDtlVO=PatientVisitDATA.getReferPat(objUserVO,strMode_p);
	...
	/*public static void setRefDepartment(PatientVisitSUP fb,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			List refDeptList=PatientVisitDATA.setRefDepartment(objUserVO);
	...
	public static void setNewDepartmentVisitDtl(PatientVisitSUP objPatVisitSUP_p, HttpServletRequest objRequest_p) 
	{
		...
		try{
			System.out.println("PatientVisitUTL :: setNewDepartmentVisitDtl()");
			UserVO objUserVO =getUserVO(objRequest_p);
			...
			try{
				if(objPatVisitSUP_p.getIsPatReferByList()!=null && objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)){
					...
					mp1=PatientVisitDATA.getAllNewDeptVisitEssentials(objPatVisitSUP_p.getPatCrNo(),objUserVO);	    
				}
				else{
				...
				WebUTIL.setMapInSession(mp1,objRequest_p,"PatientVisitACT");
			}
			...
			objSession.setAttribute(RegistrationConfig.PATIENT_VO,objPatientVO);
			//String patStatus=PatientVisitUTL.setPatientStatus( objRequest_p,objPatientVO);
		
		//	if(objPatVisitSUP_p.getIsPatReferByList()!=null && !(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)))
				if(objPatVisitSUP_p.getIsPatReferByList()!=null)
			{
				try{
					EpisodeRefDtlVO[] arrOPDOpenEpisodeVO=PatientVisitDATA.getOpenEpisodeOPD(objPatientVO.getPatCrNo(), objUserVO,"3");//procedure created
					WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.ARR_OPD_OPEN_EPISODE_VO,arrOPDOpenEpisodeVO);
				}
				catch(HisRecordNotFoundException e){
	 ...
	 * calls getAllNewDeptVisitEssentials() from PatientVisitDATA
	...
	public static Map setAllNewDeptVisitEssentials( PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p){
		...
		try{
			...
			System.out.println("PatientVisitUTL :: setAllNewDeptVisitEssentials()");
		UserVO objUserVO =getUserVO(objRequest_p);		
	    mp=PatientVisitDATA.getAllNewDeptVisitEssentials(objPatVisitSUP_p.getPatCrNo(),objUserVO);	    
	...
	public static void setDeptOptions(HttpServletRequest objRequest_p, PatientVisitSUP objPatVisitSUP_p){
		System.out.println("PatientVisitUTL :: setDeptOptions()");
	...
	public static void getRefDept_AJAX(HttpServletRequest objRequest, HttpServletResponse objResponse) {
		...
		try{
			...
			List lstRefDept=PatientVisitDATA.getRefDept_AJAX(userVO, strRefHospCode);
	...
	public static void setOldDepartmentVisitDtl(PatientVisitSUP objPatVisitSUP_p,
		...
		try{
			...
			System.out.println("PatientVisitUTL:: setOldDepartmentVisitDtl()");
			HttpSession objSession=objRequest_p.getSession();
			EpisodeRefDtlVO[] arrEpisodeOldPatRefVO=(EpisodeRefDtlVO[])objSession.getAttribute(RegistrationConfig.ARR_EPISODE_OLD_PAT_REFER_VO);
			System.out.println("IsPatReferByList :"+objPatVisitSUP_p.getIsPatReferByList());
			if(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))
			{
				...
				{
					objArrEpisodeVO=PatientVisitDATA.getOldPatientEpisodesByDept(objPatVisitSUP_p.getPatCrNo(),arrEpisodeOldPatRefVO[indexID].getToDepartmentCode(),
														objPatientVO.getRenewalConfig().getStrRenewalType(),objUserVO, RegistrationConfig.EPISODE_VISIT_TYPE_OPD);
				}
				else
				{
					objArrEpisodeVO=PatientVisitDATA.getOldPatientEpisodesByUnit(objPatVisitSUP_p.getPatCrNo(),arrEpisodeOldPatRefVO[indexID].getToDepartmentUnitCode(), 
														objPatientVO.getRenewalConfig().getStrRenewalType(),objUserVO, RegistrationConfig.EPISODE_VISIT_TYPE_OPD);
				}
				WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.REGISTRATIONDESK_EPISODEVO_ARR, objArrEpisodeVO);
			...
			if(!(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)))
			{
				...
				setEpisodeDetails(objPatVisitSUP_p,objPatientVO.getPatCatCode(),objRequest_p);
				Map referEssDept=PatientVisitDATA.referedEssentialDepartment(objUserVO);  
	...
	public static void savePatientNewDepartmentVisit(HttpServletRequest objRequest_p, PatientVisitSUP objPatVisitSUP_p){
		...
		try{
			...
			System.out.println("PatientVisitUTL :: savePatientNewDepartmentVisit()");
			...
			System.out.println("----Dept To Visit---"+objPatVisitSUP_p.getToDepartmentCode()+"-----------");
			//add to check Category flag and Category Code, By Raj Kumar  
			...
			if(objPatientVO.getDepartment()==null||objPatientVO.getDepartment().equals(""))
				objPatientVO.setDepartment(objPatVisitSUP_p.getToDepartment());
			//To set Credit Limit for Beneficiary Categories
			if(objPatientVO.getPatPrimaryCatGrpCode()!=null && objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY) )
			{
				objPatientVO.setCreditLimit(objPatVisitSUP_p.getCreditLimit());
				objPatientVO.setOldCreditLetterRefNo(objPatVisitSUP_p.getOldCreditLetterRefNo());//By Mukund on 09.05.2017
			}
			if(objPatientVO.getPatPrimaryCatGrpCode()!=null && objPatientVO.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY_WITHOUT_REFERENCE) )
			{
				objPatientVO.setCreditLimit(objPatVisitSUP_p.getAgsCreditLimit());
				objPatientVO.setCreditBillFlag("1");
			}
...
//			String patPriCatLable=objPatientVO.getPatPrimaryCat();
			objUserVO.setModuleId(RegistrationConfig.MODULE_ID_REGISTRATION);
			//edited by Vasu on 30/5/18 for apmnt details
			if(!objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)||(objPatVisitSUP_p.getIsPatFromAppointList().equals(RegistrationConfig.IS_PAT_REFER_BY_APMNT_TRUE))){
				...
				objPatientVO.setRoomCode(objPatVisitSUP_p.getDepartmentCode().split("#")[2]);

			}/*else{
				objPatientVO.setDepartmentCode(objPatVisitSUP_p.getDepartmentCode().split("#")[0]);
				//System.out.println("DepartmentCode----"+objPatVisitSUP_p.getDepartmentCode().split("#")[0]);
				objPatientVO.setDepartmentUnitCode(objPatVisitSUP_p.getDepartmentCode().split("#")[1]);
				//System.out.println("DepartmentCode------"+objPatVisitSUP_p.getDepartmentCode().split("#")[1]);
				objPatientVO.setRoomCode(objPatVisitSUP_p.getDepartmentCode().split("#")[2]);
				//System.out.println("DepartmentCode----"+objPatVisitSUP_p.getDepartmentCode().split("#")[2]);
				objPatVisitSUP_p.setToDepartmentCode(objPatientVO.getDepartmentCode());
				//System.out.println("DepartmentCode--------"+objPatientVO.getDepartmentCode());
				objPatVisitSUP_p.setToDepartmentUnitCode(objPatientVO.getDepartmentUnitCode());
				//System.out.println("DepartmentCode---------"+objPatientVO.getDepartmentUnitCode());
			}*/
			//End Vasu
			...
			//String patAmountCollected=PatientVisitDATA.getBillAmount(objPatientVO.getPatPrimaryCatCode(),objUserVO);
			...
			if(objPatVisitSUP_p.getIsReferred().equals("1")&&(registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_Only)||registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_and_Special_Both)))
			{
				objUserVO.setTariffID(RegistrationConfig.OPD_CROSS_CONSULTATION_TARIFF_ID);
			}
					...
					!objPatVisitSUP_p.getOtherHospitalDataFound().equals("1"))
			{
				...
				objPatientVO.setPatPrimaryCat(objPatVisitSUP_p.getPatPrimaryCatCode().split("#")[1]);
			...
			if(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)){
				objPatientVO.setRoundRobinUnitFlag("1");
			...
			if(!objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))
			{
				...
				if(arrOpenOPDEpisodeVO!=null){
					for(int i=0;i<arrOpenOPDEpisodeVO.length;i++)
						if(episodeRefVO.getEpisodeCode()!=null && ((episodeRefVO.getEpisodeCode().split("###")[0]).equals(arrOpenOPDEpisodeVO[i].getEpisodeCode()))){
							episodeRefVO.setFromDepartmentCode(arrOpenOPDEpisodeVO[i].getFromDepartmentCode());
							episodeRefVO.setFromDepartmentUnitCode(arrOpenOPDEpisodeVO[i].getFromDepartmentUnitCode());
							episodeRefVO.setEpisodeVisitNo(arrOpenOPDEpisodeVO[i].getEpisodeVisitNo());
							episodeRefVO.setMlcNo(arrOpenOPDEpisodeVO[i].getMlcNo());
							...
							episodeRefVO.setEpisodeExpiryDate(arrOpenOPDEpisodeVO[i].getEpisodeExpiryDate());
			...
			else
			{
				...
				objPatientVO.setDepartmentCode(objPatVisitSUP_p.getDepartmentCode());
				objPatVisitSUP_p.setIsReferred(RegistrationConfig.IS_REFERRED_TRUE);
				episodeRefVO.setIsRefferInOut(RegistrationConfig.IS_REFERRED_IN_OUT_ACCEPT_INTERNAL);
				objPatientVO.setIsPatReferByList(objPatVisitSUP_p.getIsPatReferByList());
				//episodeRefVO.setEpisodeExpiryDate(objPatVisitSUP_p.getRefferringOPDEpisode().split("###")[1]);
				//episodeRefVO.setEpisodeExpiryDate(objPatVisitSUP_p.getRefferringOPDEpisode());
				//objPatientVO.setUnitRosterType(RegistrationConfig.ROSTERTYPE_OPD);
			}
			...
			if(deptToVisit==null || deptToVisit.length==0)
			{
				...
				objArrEpisodeVO1[0].setPaymentModeCode(objPatVisitSUP_p.getPaymentModeCode().split("#")[0]);
				if(!objArrEpisodeVO1[0].getPaymentModeCode().equals("1"))
				...
				if(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))
				{
					...
					deptName=PatientVisitUTL.getEntryLabel(col,objPatVisitSUP_p.getDepartmentUnitCode());
				}
			...
			else
			{
				...
				for(int i=0; i<objArrEpisodeVO1.length; i++)
				{
					...
					if(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))
					{
					 ...
					 deptName=PatientVisitUTL.getEntryLabel(col,deptToVisit[i]);
					}
	...
	if(objPatVisitSUP_p.getPatPrimaryCatGrpCode()!=null && objPatientVO.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY) )
	{
		HelperMethods.populate(objCreditAvailDtlVO, objPatientVO);
		objCreditAvailDtlVO.setTariffId(objUserVO.getTariffID());
		if(objPatVisitSUP_p.getIsPatReferByList()!=null&&!objPatVisitSUP_p.getIsPatReferByList().equals("1"))
			strUniqueIdDuplicyFlag = NewRegistrationDATA.checkBeneficiaryIdDuplicy(objUserVO, objCreditAvailDtlVO);
	...
	objPatientVO.setUnitRosterType(RegistrationConfig.ROSTERTYPE_OPD);
			/////////////////////////////////////////////////////////////////////////////
			EpisodeVO [] objArrEpisodeVO2=PatientVisitDATA.newDepartmentVisit(objPatientVO, objArrEpisodeVO1, episodeRefVO, objUserVO,oldPatientVO);
			...
			if(objPatientVO.getPatNationalId()!=null && !objPatientVO.getPatNationalId().equals(""))
			{
				String strPatAadhaarNo =NewRegistrationDATA.getAadhaarDecrypted(objPatientVO.getPatNationalId(), objPatientVO.getPatCrNo());
				objPatientVO.setPatNationalId(strPatAadhaarNo.split("#@#")[0]);
			...
			if(jsonStr!=null && !(jsonStr.equals("")))
				...
				objQRC.createQRCode(objPatientVO.getPatCrNo(), objRequest_p);
			/***/
			...
			objPatVisitSUP_p.setPrint("2");//1 =with OPD card; 2 = only Bar code
			...
			//Bill Receipt Printing 
			//if((objPatVisitSUP_p.getPatAmountCollected()!=null)&&!(objPatVisitSUP_p.getPatAmountCollected().equals("0"))&&!(objPatVisitSUP_p.getPatAmountCollected().equalsIgnoreCase("0.00")))
			if(false)
			{
		...
		try
		{
			...
			if(objPatientVO!=null){
				...
				if(Integer.parseInt(objPatVisitSUP_p.getEpisodeVisitNo())>1)
					entryDate="";
				//billAmount=PatientVisitDATA.getBillAmountByDeptGrouping(objPatientVO.getPatPrimaryCatCode(), objPatVisitSUP_p.getSelectedFromDept(), objPatVisitSUP_p.getDepartmentCode(), objUserVO);
				billAmount=PatientVisitDATA.getBillAmountByDeptGrouping(objPatientVO.getPatPrimaryCatCode(), objPatVisitSUP_p.getSelectedFromDept(), objPatVisitSUP_p.getDepartmentCode(),entryDate, objUserVO);
...
//						 } //Above Code For Calculating PatAmount is commented, because for HospitalWise Amount is already calculated while populating episodeVo at NewDeptVisit (i.e in method populateNewDepartmentVisitEpisode)
			...
			if((objPatVisitSUP_p.getIsReferred()!=null && objPatVisitSUP_p.getIsReferred().equals(RegistrationConfig.IS_REFERRED_TRUE)) && !(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)) && !(objPatVisitSUP_p.getIsRefferInOut()!=null && objPatVisitSUP_p.getIsRefferInOut().equals(RegistrationConfig.IS_REFERRED_IN_OUT_ACCEPT_EXTERNAL))){
				...
				/*  ## 		Modification Log							
		 		...
		 		##		Modify By				:Sheeldarshi */
		/*
		 * Commneted to remove letter duplicacy check for AIIMS on 27 Dec 2018 
		 */
				/*String strUniqueIdDuplicyFlag = "0";
		CreditAvailDetailVO objCreditAvailDtlVO=new CreditAvailDetailVO();
		objCreditAvailDtlVO.setCreditLetterRefNo(objPatVisitSUP_p.getCreditLetterRefNo());
		...
		*/
		//End:sheeldarshi
				episodeMap=PatientVisitDATA.savePatientVisit(objPatientVO, selectedEpisodeVO, objUserVO,episodeRefVO,arrNewDeptEpisodeVO, objPatVisitSUP_p);
				...
				if(_oldEpisodeVO!=null && _oldEpisodeVO.length>0)
				{
					...
					//objPatVisitSUP_p.setPrintDivContent(strPrintDivContent1+strPrintDivContent2);
					...
					{
						strBillPrintDivContent1=NewRegistrationSlip.printBillReceipt(preapareSlip(_newEpisodeVO,objPatientVO,objPatVisitSUP_p,objRequest_p), tmpFileName,objRequest_p,"NDV");
					...
					if(_oldEpisodeVO!=null){
						...
						{
							strBillPrintDivContent2=NewRegistrationSlip.printBillReceipt(preapareSlip(_oldEpisodeVO,objPatientVO,objPatVisitSUP_p,objRequest_p), tmpFileName,objRequest_p,"ODV");
	...
	private static EpisodeVO[] populateNewDepartmentVisitEpisode(
		...
		try
		{
			...
			System.out.println("PatientVisitUTL :: populateNewDepartmentVisitEpisode()");
			Collection col=(Collection)objRequest_p.getSession().getAttribute(RegistrationConfig.REGISTRATIONDESK_OPTION_DEPARTMENT);
			String deptName="";
	...
	public static void getMapOfRenewalConfigDtlOnKeyPatCat(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			System.out.println("PatientVisitUTL :: getMapOfRenewalConfigDtlOnKeyPatCat()");
			setSysdateAndDefaultCrNoFormat(objRequest_p);
			mapOfRenewalVoOnKeyPatCatGroup=PatientVisitDATA.getMapOfRenewalConfigDtlOnKeyPatCat(objUserVO);
			registrationConfigVO=PatientVisitDATA.getRegistrationConfigDtl(objUserVO);
			//objPatVisitSUP_p.setStrRenewalType(objRenewalConfigVO.getStrRenewalType());
	...
	public static void getCreditBeneficiaryDtlForPatCatOnCRNO(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			...
			if(objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY))	
			{
				...
				objPatVisitSUP_p.setLetterType(creditBeneficiaryDtl_VO.getLetterType());//by mukund on 08.05.2017
			}
			else
			{
			...
			/*if(objPatVisitSUP_p.getPatPrimaryCatGrpCode()!=null && objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY))
			{
					...
					dNowStr =dateFormat.format(cldr.getTime());
					dNow = dateFormat.parse(dNowStr);//dNow = sysdate in dd-MM-yyyy format
					dt1=dateFormat.parse(creditBeneficiaryDtl_VO.getCardvalidityDate());//dt1 = card valid upto date in dd-MM-yyyy format
					//compare two dates
					int judge = dt1.compareTo(dNow);//dt1 is credit letter expiry date and dNow is today's date
					if(judge<0)// dt1 is earlier than dNow
					{
						objPatVisitSUP_p.setFourteenDayRenewal("1");//need renewal
					}
					else if(judge>=0)//dt1 is later than dNow
					{
						objPatVisitSUP_p.setFourteenDayRenewal("0");//no renewal
					}
	...
	public static void getInitEssentials(PatientVisitSUP objPatVisitSUP_p, HttpServletRequest objRequest, HttpServletResponse objResponse) {
		...
		try{
			...
			//end
			/***/
			if(objPatVisitSUP_p.getIsDuplicateRegistered()!=null && objPatVisitSUP_p.getSelectedDuplicatePatCrNo()!=null && !objPatVisitSUP_p.getIsDuplicateRegistered().equals("") && !objPatVisitSUP_p.getSelectedDuplicatePatCrNo().equals(""))
			{
				continueWithExistingDuplicateRecord(objPatVisitSUP_p, objRequest, userVO);
			}
			/***/
		}
	...
	public static void getAddmittedDetailsOnCRNO(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			System.out.println("PatientVisitUTL :: getAddmittedDetailsOnCRNO()");
			isAdmitted=PatientVisitDATA.checkPatientIsAdmitted(objPatVisitSUP_p, objUserVO);
	 ...
	 */
	public static void getCashCollectionDetail(HttpServletRequest request,HttpServletResponse response, PatientVisitSUP objPatSup_p) {
	...
	public static void getExtReferPatient(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p,String strMode_p)
	{
		UserVO objUserVO =getUserVO(objRequest_p);
		Status objStatus=new Status();
		EpisodeRefDtlVO[] arrEpisodeRefExtDtlVO=null;
		try
		{
			System.out.println("patientVisitUTL :: getReferPatient");
			arrEpisodeRefExtDtlVO=PatientVisitDATA.getReferPat(objUserVO,strMode_p);
			//arrEpisodeRefDtlVO=PatientVisitDATA.getReferPat(objUserVO,"6");
			WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.ARR_EPISODE_REFER_EXT_PAT_VO,arrEpisodeRefExtDtlVO);
			objStatus.add(Status.LIST);
			objPatVisitSUP_p.setIsRefferInOut("2");
			System.out.println("arrEpisodeRefExtDtlVO.length :"+arrEpisodeRefExtDtlVO!=null?arrEpisodeRefExtDtlVO.length:0);
		}
		catch(HisRecordNotFoundException e)
		{
			e.printStackTrace();
...
public static void saveExtPatientNewDepartmentVisit(HttpServletRequest objRequest_p, PatientVisitSUP objPatVisitSUP_p){
		...
		try{
			...
			/*objPatVisitSUP_p.setToDepartmentCode(objPatVisitSUP_p.getDepartmentCode().split("#")[0]);
			objPatVisitSUP_p.setToDepartmentUnitCode(objPatVisitSUP_p.getDepartmentUnitCode().split("#")[1]);*/
...
//			String patPriCatLable=objPatientVO.getPatPrimaryCat();
					...
					!objPatVisitSUP_p.getOtherHospitalDataFound().equals("1"))
			{
				...
				objPatientVO.setPatPrimaryCat(objPatVisitSUP_p.getPatPrimaryCatCode().split("#")[1]);
				objPatientVO.setPatIdNo(objPatVisitSUP_p.getPatIdNo());
				objPatientVO.setPatPrimaryCatGrpCode(objPatVisitSUP_p.getPatPrimaryCatCode().split("#")[9]);
			}
			//objPatientVO.setPatStatusCode(RegistrationConfig.PATIENT_STATUS_CODE_OUTPATIENT);
			String patPriCatLable=objPatientVO.getPatPrimaryCat();
			Collection colDept		=(Collection)objSession.getAttribute(RegistrationConfig.REGISTRATIONDESK_OPTION_DEPARTMENT);
			...
			{
				EpisodeRefDtlVO[] episodeRefDtlVO=(EpisodeRefDtlVO[])objSession.getAttribute(RegistrationConfig.ARR_EPISODE_REFER_EXT_PAT_VO);
				int i=0;
				...
				episodeRefVO.setMlcFlag(episodeRefDtlVO[index].getMlcFlag());
				episodeRefVO.setFromDepartment(episodeRefDtlVO[index].getFromDepartment());
				episodeRefVO.setFromDepartmentCode(episodeRefDtlVO[index].getFromDepartmentCode());
				episodeRefVO.setFromDepartmentUnit(episodeRefDtlVO[index].getFromDepartmentUnit());
				episodeRefVO.setFromDepartmentUnitCode(episodeRefDtlVO[index].getFromDepartmentUnitCode());
				episodeRefVO.setFromHospital(episodeRefDtlVO[index].getFromHospital());
				episodeRefVO.setToDepartment(episodeRefDtlVO[index].getToDepartment());
				episodeRefVO.setToDepartmentCode(episodeRefDtlVO[index].getToDepartmentCode());
				objPatVisitSUP_p.setDepartmentCode(objPatVisitSUP_p.getToDepartmentCode());
				objPatientVO.setDepartmentCode(objPatVisitSUP_p.getDepartmentCode());
				objPatVisitSUP_p.setIsReferred(RegistrationConfig.IS_REFERRED_TRUE);
				episodeRefVO.setIsRefferInOut(RegistrationConfig.IS_REFERRED_IN_OUT_ACCEPT_EXTERNAL);
	...
	*/
	//End:sheeldarshi	
			...
			/////////////////////////////////////////////////////////////////////////////
			EpisodeVO [] objArrEpisodeVO2=PatientVisitDATA.newExtDepartmentVisit(objPatientVO, objArrEpisodeVO1, episodeRefVO, objUserVO,oldPatientVO);
 ...
 * Reason : to get the episode details of the externally referred patients
 * date : 7th oct 2016 */
public static void setExtEpisodeDetails( PatientVisitSUP objPatVisitSUP_p,String strPatCatCode,HttpServletRequest objRequest_p ){		
	Status status = new Status();
		try{
			System.out.println("PatientVisitUTL :: setEpisodeDetails()");
			UserVO objUserVO =getUserVO(objRequest_p);		
			...
			if(objPatientVO.getPatIsDead().equals(RegistrationConfig.PATIENT_IS_DEAD)==false){
				objArrEpisodeVO=PatientVisitDATA.getExtOldPatientEpisodes(objPatVisitSUP_p,objPatVisitSUP_p.getPatCrNo(),strPatCatCode, RegistrationConfig.EPISODE_VISIT_TYPE_OPD, objPatientVO.getRenewalConfig().getStrRenewalType(), objUserVO);
				//System.out.println("objArrEpisodeVO.length :"+objArrEpisodeVO.length);
				WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.REGISTRATIONDESK_EPISODEVO_ARR, objArrEpisodeVO);
			}
...
/*Start : Surabhi
 * Reason : to get the new dept visit details of the externally referred patients
 * date : 7th oct 2016 */
public static void setExtNewDepartmentVisitDtl(PatientVisitSUP objPatVisitSUP_p, HttpServletRequest objRequest_p) 
{
	Status  objStatus=new Status();
	String amountCollected="";
	String amount="";
	...
	try{
		...
		HttpSession objSession=objRequest_p.getSession();
		EpisodeRefDtlVO[] arrEpisodeRefPatVO=(EpisodeRefDtlVO[])objSession.getAttribute(RegistrationConfig.ARR_EPISODE_REFER_EXT_PAT_VO);
		Map mp1=new HashMap();
		try{
			if(objPatVisitSUP_p.getIsPatReferByList()!=null && objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)){
				int index=Integer.parseInt(objPatVisitSUP_p.getOnlineReferedIndex());
		...
		if(objPatVisitSUP_p.getIsPatReferByList()!=null && objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))
		{
			...
			objPatVisitSUP_p.setFromDepartment(arrEpisodeRefPatVO[index].getFromDepartment());
			objPatVisitSUP_p.setFromDepartmentCode(arrEpisodeRefPatVO[index].getFromDepartmentCode());
			objPatVisitSUP_p.setFromDepartmentUnit(arrEpisodeRefPatVO[index].getFromDepartmentUnit());
			objPatVisitSUP_p.setFromDepartmentUnitCode(arrEpisodeRefPatVO[index].getFromDepartmentUnitCode());
			objPatVisitSUP_p.setToDepartment(arrEpisodeRefPatVO[index].getToDepartment());
			objPatVisitSUP_p.setToDepartmentCode(arrEpisodeRefPatVO[index].getToDepartmentCode());
			objPatVisitSUP_p.setToDepartmentUnit(arrEpisodeRefPatVO[index].getToDepartmentUnit());
			objPatVisitSUP_p.setToDepartmentUnitCode(arrEpisodeRefPatVO[index].getToDepartmentUnitCode());
		...
		HelperMethods.populatetToNullOrEmpty(objPatVisitSUP_p,objPatientVO);
		objPatVisitSUP_p.setDepartmentCode(objPatVisitSUP_p.getFromDepartmentCode());
		//setPatAmount(objPatVisitSUP_p, objPatientVO, objUserVO, objRequest_p);
		objPatVisitSUP_p.setPatBillAmountWithoutGrouping(objPatVisitSUP_p.getPatAmountCollected());
		objSession=WebUTIL.getSession(objRequest_p);
		objSession.setAttribute("objPatientVO",objPatientVO);
	...
	//	if(objPatVisitSUP_p.getIsPatReferByList()!=null && !(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)))
		...
		{
			try{
				EpisodeRefDtlVO[] arrOPDOpenEpisodeVO=PatientVisitDATA.getExtOpenEpisodeOPD(objPatVisitSUP_p,objUserVO,"1");//procedure created
				WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.ARR_OPD_OPEN_EPISODE_VO,arrOPDOpenEpisodeVO);
			}
			catch(HisRecordNotFoundException e){
				String msg=e.getMessage();
...
/*Start : Surabhi
 * Reason : to get the old dept visit details of the externally referred patients
 * date : 7th oct 2016 */
public static void setExtOldDepartmentVisitDtl(PatientVisitSUP objPatVisitSUP_p,UserVO objUserVO_p,
		HttpServletRequest objRequest_p) {
	Status status=new Status();
	UserVO objUserVO =getUserVO(objRequest_p);
	PatientVO objPatientVO=(PatientVO)objRequest_p.getSession().getAttribute(RegistrationConfig.PATIENT_VO);
	...
	try{
		...
		if(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))
		{
			...
			if(arrEpisodeOldPatRefVO[indexID].getToDepartmentUnitCode()==null ||arrEpisodeOldPatRefVO[indexID].getToDepartmentUnitCode()=="")
			{
				objArrEpisodeVO=PatientVisitDATA.getExtOldPatientEpisodesByDept(objPatVisitSUP_p,objPatVisitSUP_p.getPatCrNo(),arrEpisodeOldPatRefVO[indexID].getToDepartmentCode(),
													objPatientVO.getRenewalConfig().getStrRenewalType(),objUserVO, RegistrationConfig.EPISODE_VISIT_TYPE_OPD);
			}
			else
			{
				objArrEpisodeVO=PatientVisitDATA.getExtOldPatientEpisodesByUnit(objPatVisitSUP_p,objPatVisitSUP_p.getPatCrNo(),arrEpisodeOldPatRefVO[indexID].getToDepartmentUnitCode(), 
													objPatientVO.getRenewalConfig().getStrRenewalType(),objUserVO, RegistrationConfig.EPISODE_VISIT_TYPE_OPD);
			}
			WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.REGISTRATIONDESK_EPISODEVO_ARR, objArrEpisodeVO);
		...
		{
			// Has to implement (Aadil)
			setExtEpisodeDetails(objPatVisitSUP_p,objPatientVO.getPatCatCode(),objRequest_p);
			Map referEssDept=PatientVisitDATA.referedEssentialDepartment(objUserVO);  
			WebUTIL.setMapInSession(referEssDept,objRequest_p,"PatientVisitACT");
		}
		...
		if(!(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))){
			try{
				EpisodeRefDtlVO[] arrOPDOpenEpisodeVO=PatientVisitDATA.getExtOpenEpisodeOPD(objPatVisitSUP_p,objUserVO_p, "2");//procedure created
				WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.ARR_OPD_OPEN_EPISODE_VO,arrOPDOpenEpisodeVO);
			}
			catch(HisRecordNotFoundException e){
				String msg=e.getMessage();
...
/*Start : Surabhi
 * Reason : to get the patient details of the externally referred patients
 * date : 7th oct 2016 */

public static void setExtPatientDtlByCrno( PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p){
	...
	PatientVO objPatientVO=new PatientVO();
	objPatientVO.setPatCrNo(objPatVisitSUP_p.getPatCrNo());
	objPatientVO.setOtherHospitalFlag(objPatVisitSUP_p.getOtherHospitalFlag());
	//String hospitalCode=objUserVO.getHospitalCode();
	String visitType=RegistrationConfig.EPISODE_VISIT_TYPE_OPD;
	try{
		objPatVisitSUP_p.setFromHospital(objPatientVO.getPatCrNo().substring(0,5));
		objPatientVO=PatientVisitDATA.getPatientDtl(objPatientVO,objUserVO, visitType);
		//HelperMethods.populate(objPatVisitSUP_p, objPatientVO);
		objPatVisitSUP_p.setPatAge(objPatientVO.getPatAge());
		objPatVisitSUP_p.setPatGender(objPatientVO.getPatGender());
		...
		//	objPatientVO.setOtherHospitalDataFound("1");
		objPatVisitSUP_p.setOtherHospitalDataFound(objPatientVO.getOtherHospitalDataFound());
		objPatVisitSUP_p.setStrHospName(objPatientVO.getStrHospName());
		/////////////
		...
		//end
		// Setting Patient Amount
		setPatAmountForExt(objPatVisitSUP_p, objPatientVO, objUserVO, objRequest_p);
		
		if(objPatVisitSUP_p.getPatAmountNCrConsultation().equals("-1") && objPatVisitSUP_p.getPatActualAmount().equals("-1"))
		{
			objPatVisitSUP_p.setPatAmountNCrConsultation("0.00");
			objPatVisitSUP_p.setPatActualAmount("0.00");
		}
		...
		if(objPatVisitSUP_p.getModeCase().equals("0"))
		{
	 		...
	 		objPatVisitSUP_p.setOldDepartmentVisit("On");
	 		setExtOldDepartmentVisitDtl(objPatVisitSUP_p,objUserVO,objRequest_p);
		}
		else if(objPatVisitSUP_p.getModeCase().equals("1"))
		{
			
			objPatVisitSUP_p.setNewDepartmentVisit("On");
			setExtNewDepartmentVisitDtl(objPatVisitSUP_p, objRequest_p);
		}
		else if(objPatVisitSUP_p.getModeCase().equals("2"))
		{
			if(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))
		...
		if(episodeReferalDtlVO!=null&&episodeReferalDtlVO.length>0)
		  {
				objPatVisitSUP_p.setIsReferred("on");
				objPatVisitSUP_p.setIsRefferInOut("2");
				objPatVisitSUP_p.setReferInternalExternal("E");
				objPatVisitSUP_p.setReferingRowIndex("0");
		  }
		}
		//End
...
/*Start : Surabhi
 * Reason : to get the amount details of the externally referred patients
 * date : 7th oct 2016 */
public static void setPatAmountForExt(PatientVisitSUP objPatVisitSUP_p,PatientVO objPatientVO_p,UserVO objUserVO_p, HttpServletRequest objRequest_p ){
	String	strRenewalType =objPatientVO_p.getRenewalConfig().getStrRenewalType();
	String strAmountCollected="";
	String strAmountWOCrConsultation="0.00";
	...
	objUserVO_p.setTariffID(RegistrationConfig.RENEWAL_TARIFF_ID);
	String strRenewalAmountCollected=PatientVisitDATA.getBillAmountForExt(objPatientVO_p,objUserVO_p);
	/* #Start					:Sheeldarshi 
	#Modify Date(CR/PRS)	:28thMay'15 
	#Reason					: UAT Test Report Requirements:Requirement 8
	 */
		...
		switch(key)
		{
			case 1: case 2:
				...
				if(objPatientVO_p.getOpdRenewalRequired()!=null && objPatientVO_p.getOpdRenewalRequired().equalsIgnoreCase(RegistrationConfig.RENEWAL_REQUIRED_TRUE)){
					...
					objUserVO_p.setTariffID(RegistrationConfig.NEW_DEPT_VISIT_TARIFF_ID);
					strAmountCollected=PatientVisitDATA.getBillAmountForExt(objPatientVO_p,objUserVO_p);
				}
				else
				{
					strAmountCollected="0.00";
					...
					if(episodeReferalDtlVO!=null&&episodeReferalDtlVO.length>0)
					{
						if(registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_Only)||registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_and_Special_Both))
						{
							...
							//objUserVO_p.setTariffID(RegistrationConfig.SPECIAL_NEW_DEPT_VISIT_TARIFF_ID);
							objUserVO_p.setTariffID(RegistrationConfig.NEW_DEPT_VISIT_TARIFF_ID);
							strAmountWOCrConsultation=PatientVisitDATA.getBillAmountForExt(objPatientVO_p,objUserVO_p);
						}
						else
							strAmountCollected="0.00";
					}
			...
			case 3: case 4:
				...
				if(objPatVisitSUP_p.getNewDepartmentVisit().equalsIgnoreCase("on")){
					...
					if(!objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY) && 
						...
						#Reason					: UAT Test Report Requirements:Requirement 8
						 */
						objPatVisitSUP_p.setPatAmountCollected(strAmountCollected.equals("-1")?"0.00":strAmountCollected);
						objPatVisitSUP_p.setPatAmountNCrConsultation(strAmountWOCrConsultation);
						objPatVisitSUP_p.setPatAmountCrConsultation(strAmountCollected);
						//End
					}
				...
				if(objPatVisitSUP_p.getOldDepartmentVisit().equalsIgnoreCase("on")){
					...
					if(!objPatVisitSUP_p.getPatPrimaryCatGrpCode().equals(RegistrationConfig.PATIENT_REG_CATEGORY_GROUP_BENEFICIARY) && 
						 ...
						 */
						objPatVisitSUP_p.setPatAmountNCrConsultation(strAmountWOCrConsultation);
						objPatVisitSUP_p.setPatAmountCrConsultation(strAmountCollected.equals("-1")?"0.00":strAmountCollected);
						//End
...
// end
	 ...
	 * By Mukund
	 * On 16.May.2018
	 * To update the duplicate patient search log
	 * **/
	public static void continueWithExistingDuplicateRecord(PatientVisitSUP objPatVisitSUP_p, HttpServletRequest objRequest, UserVO objUserVO_p)
	{
		 PatientVO objPatVO = new PatientVO();
		 //uhid_for_duplicate_search_frm_proc
		 String dupSrchSlno = (String)objRequest.getSession().getAttribute("duplicate_search_sno_frm_proc");
		 String strUHID = (String)objRequest.getSession().getAttribute("uhid_for_duplicate_search_frm_proc");
		 if(strUHID!=null)
			 objPatVO.setPatSecUHID(strUHID);
		 objPatVO.setPatCrNo(objPatVisitSUP_p.getPatCrNo());
		 
		 PatientVisitDATA.continueWithExistingDuplicateRecord(objUserVO_p, objPatVO, dupSrchSlno);
	}
	
	/*By Vasu on 25.05.2018 to get Online Appointed Patient*/
	public static void getAppointedPatient(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p,String strMode_p)
	{
		...
		try
		{
			...
			arrEpisodeAptDtlVO=PatientVisitDATA.getReferPat(objUserVO,strMode_p);
	...
	public static void setQrCode(PatientVisitSUP objVisitSUP_p, PatientVO objPatientVO, UserVO objUserVO, HttpServletRequest objRequest_p)
	{
