...
import registration.transactions.controller.data.CreditPatientVisitDATA;
...
public class CreditPatientVisitUTL extends ControllerUTIL
{	
	public static void setPatAmount(PatientVisitSUP objPatVisitSUP_p,PatientVO objPatientVO_p,UserVO objUserVO_p, HttpServletRequest objRequest_p ){
		...
		String strRenewalAmountCollected=CreditPatientVisitDATA.getBillAmount(objPatientVO_p.getPatPrimaryCatCode(),objUserVO_p);
		/* #Start					:Sheeldarshi 
			...
			switch(key)
			{
				case 1: case 2:
					...
					if(objPatientVO_p.getOpdRenewalRequired()!=null && objPatientVO_p.getOpdRenewalRequired().equalsIgnoreCase(RegistrationConfig.RENEWAL_REQUIRED_TRUE)){
						...
						strAmountCollected=CreditPatientVisitDATA.getBillAmount(objPatientVO_p.getPatPrimaryCatCode(),objUserVO_p);
					}
					else
					{
						strAmountCollected="0.00";
						...
						if(episodeReferalDtlVO!=null&&episodeReferalDtlVO.length>0)
						{
							if(registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_Only)||registrationConfigVO.getIsCrossConsulation().equals(RegistrationConfig.Cross_Consultation_For_OPD_and_Special_Both))
							{
								...
								objUserVO_p.setTariffID(RegistrationConfig.NEW_DEPT_VISIT_TARIFF_ID);
								strAmountWOCrConsultation=CreditPatientVisitDATA.getBillAmount(objPatientVO_p.getPatPrimaryCatCode(),objUserVO_p);
							}
							else
								strAmountCollected="0.00";
						}
	...
	public static void setPatientDtlByCrno( PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p){
		...
		try{
			objPatientVO=CreditPatientVisitDATA.getPatientDtl(objPatientVO,objUserVO, visitType);
	...
	public static void setReferEpisodeDtl(PatientVisitSUP objPatVisitSUP_p,
		...
		try{			
			...
			//{
				...
				arrEpisodeRefDtlVO=CreditPatientVisitDATA.getReferEpisodeDtl(objPatVisitSUP_p.getPatCrNo(),"",userVO);
	...
	/*public static void setPatientDtlByName( PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p){
		...
		objPatientVO=CreditPatientVisitDATA.getPatientDtlByName(searchName,objUserVO);
	 ...
	 * calls getAllInitialOldDeptVisitEssentials of  CreditPatientVisitDATA
	 * 
	 * @param objRequest_p HttpServletRequest
	 */
	public static void setAllInitialOldPatientVisitEssentials(HttpServletRequest objRequest_p){
		...
		try{
			...
			Map mp=CreditPatientVisitDATA.getAllInitialOldDeptVisitEssentials();
	 ...
	 * for the NEW mode
	 * calls getAllInitialNewDeptVisitEssentials() of CreditPatientVisitDATA
	 ...
	 */
		public static void setAllInitialNewDeptVisitEssentials(HttpServletRequest objRequest_p,PatientVisitSUP objPatVisitSUP_p){
			...
			try{
			Map mp=CreditPatientVisitDATA.getAllInitialNewDeptVisitEssentials();
	 ...
	 * getOldDeptVisitEssentials(objPatVisitSUP_p, getUserVO(objRequest_p)) from CreditPatientVisitDATA
	 * @param objPatVisitSUP_p PatientVisitSUP form bean
	 * @param objRequest_p HttpServletRequest
	 */
	public static void setOldDeptVisitEssentials(PatientVisitSUP objPatVisitSUP_p, HttpServletRequest objRequest_p){
		...
		Status status = new Status();
		try{
			Map mp=CreditPatientVisitDATA.getOldDeptVisitEssentials(objPatVisitSUP_p, getUserVO(objRequest_p));
			WebUTIL.setMapInSession(mp, objRequest_p,"PatientVisitACT");
	...
	Status status = new Status();
		try{
			...
			if(objPatientVO.getPatIsDead().equals(RegistrationConfig.PATIENT_IS_DEAD)==false){
				objArrEpisodeVO=CreditPatientVisitDATA.getOldPatientEpisodes(objPatVisitSUP_p.getPatCrNo(),strPatCatCode, RegistrationConfig.EPISODE_VISIT_TYPE_OPD, objPatientVO.getRenewalConfig().getStrRenewalType(), objUserVO);
	 ...
	 * calls saveOldPatientVisit(objPatientVO, objArrEpisodeVO, getUserVO(objRequest_p)) of CreditPatientVisitDATA
	 * @param objRequest_p HttpServletRequest
	 * @param objPatVisitSUP_p PatientVisitSUP form bean
	 */
	public static boolean saveOldPatientVisit(HttpServletRequest objRequest_p, PatientVisitSUP objPatVisitSUP_p,String strMode_p){
		...
		try
		{
			...
			objArrEpisodeVO2=CreditPatientVisitDATA.saveOldPatientVisit(objPatientVO, selectedEpisodeVO, objUserVO,episodeRefVO);
...
/**
 * sets Patient Status
 * calls  getPatientStatus(objPatientVO, objUserVO) of CreditPatientVisitDATA
 * @param objRequest_p HttpServletRequest
...
//		String patStatus="";
//		try{
//	    patStatus=CreditPatientVisitDATA.getPatientStatus(objPatientVO_p, objUserVO);
//	    
//		}
//		catch(HisRecordNotFoundException e){
...
//	}
	...
	public static void getOpenEpisodeDtl(HttpServletRequest objRequest_p, PatientVisitSUP objPatVisitSUP_p){
		...
		try{
			...
			EpisodeRefDtlVO[] arrOPDOpenEpisodeVO=CreditPatientVisitDATA.getOpenEpisodeOPD(objPatientVO.getPatCrNo(), objUserVO,"3");
	...
	public static void getCreditBenDtl(HttpServletRequest objRequest_p, PatientVisitSUP objPatVisitSUP_p){
		PatientVO objPatientVO=(PatientVO)WebUTIL.getSession(objRequest_p).getAttribute(RegistrationConfig.PATIENT_VO);
		UserVO objUserVO=getUserVO(objRequest_p);
		Status status=new Status();
		try{
			
			BeneficiaryPatientVO[] _arrBeneficiaryPatientVO=CreditPatientVisitDATA.getCreditBenDtl(objPatientVO.getPatCrNo(), objUserVO,"1");
			
			WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.ARR_BENEFICIARY_PAT_VO,_arrBeneficiaryPatientVO);
	...
	//	String isVisitOnRequest=objPatVisitSUP_p.getOnRequestVisit();
		try{
			...
			{	
				...
				arrayEpisodeForStamping=CreditPatientVisitDATA.saveDeptWiseRenewalAndStaming(objPatientVO,selectedEpisodeVO,arrRenewalEpisodeVO,objArrEpisodeVO,episodeRefVO,sysDate,objUserVO);
...
//	}
	...
	public static void getOldPatReferDtl(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			...
			EpisodeRefDtlVO[] arrEpisodeOldPatRefDtlVO=CreditPatientVisitDATA.getReferPat(objUserVO,"1");
	...
	public static void getReferPatient(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p,String strMode_p)
	{
		...
		try
		{
			...
			arrEpisodeRefDtlVO=CreditPatientVisitDATA.getReferPat(objUserVO,strMode_p);
	...
	/*public static void setRefDepartment(PatientVisitSUP fb,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			List refDeptList=CreditPatientVisitDATA.setRefDepartment(objUserVO);
	...
	public static void setNewDepartmentVisitDtl(PatientVisitSUP objPatVisitSUP_p, HttpServletRequest objRequest_p) 
	{
		...
		try{
			...
			try{
				if(objPatVisitSUP_p.getIsPatReferByList()!=null && objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)){
					...
					mp1=CreditPatientVisitDATA.getAllNewDeptVisitEssentials(objPatVisitSUP_p.getPatCrNo(),objUserVO);	    
				}
				else{
		...
		//	if(objPatVisitSUP_p.getIsPatReferByList()!=null && !(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)))
			...
			{
				try{
					EpisodeRefDtlVO[] arrOPDOpenEpisodeVO=CreditPatientVisitDATA.getOpenEpisodeOPD(objPatientVO.getPatCrNo(), objUserVO,"3");//procedure created
					WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.ARR_OPD_OPEN_EPISODE_VO,arrOPDOpenEpisodeVO);
				}
				catch(HisRecordNotFoundException e){
	 ...
	 * calls getAllNewDeptVisitEssentials() from CreditPatientVisitDATA
	...
	public static Map setAllNewDeptVisitEssentials( PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p){
		...
		UserVO objUserVO =getUserVO(objRequest_p);		
	    mp=CreditPatientVisitDATA.getAllNewDeptVisitEssentials(objPatVisitSUP_p.getPatCrNo(),objUserVO);	    
	...
	public static void getRefDept_AJAX(HttpServletRequest objRequest, HttpServletResponse objResponse) {
		...
		try{
			...
			List lstRefDept=CreditPatientVisitDATA.getRefDept_AJAX(userVO, strRefHospCode);
	...
	public static void setOldDepartmentVisitDtl(PatientVisitSUP objPatVisitSUP_p,
		...
		try{
			...
			if(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE))
			{
				...
				{
					objArrEpisodeVO=CreditPatientVisitDATA.getOldPatientEpisodesByDept(objPatVisitSUP_p.getPatCrNo(),arrEpisodeOldPatRefVO[indexID].getToDepartmentCode(),
														objPatientVO.getRenewalConfig().getStrRenewalType(),objUserVO, RegistrationConfig.EPISODE_VISIT_TYPE_OPD);
				}
				else
				{
					objArrEpisodeVO=CreditPatientVisitDATA.getOldPatientEpisodesByUnit(objPatVisitSUP_p.getPatCrNo(),arrEpisodeOldPatRefVO[indexID].getToDepartmentUnitCode(), 
														objPatientVO.getRenewalConfig().getStrRenewalType(),objUserVO, RegistrationConfig.EPISODE_VISIT_TYPE_OPD);
				}
				WebUTIL.setAttributeInSession(objRequest_p,RegistrationConfig.REGISTRATIONDESK_EPISODEVO_ARR, objArrEpisodeVO);
			...
			if(!(objPatVisitSUP_p.getIsPatReferByList().equals(RegistrationConfig.IS_PAT_REFER_BY_LIST_TRUE)))
			{
				...
				Map referEssDept=CreditPatientVisitDATA.referedEssentialDepartment(objUserVO);  
...
//			String patPriCatLable=objPatientVO.getPatPrimaryCat();
			...
			//String patAmountCollected=CreditPatientVisitDATA.getBillAmount(objPatientVO.getPatPrimaryCatCode(),objUserVO);
	...
	//End:sheeldarshi	
			...
			/////////////////////////////////////////////////////////////////////////////
			EpisodeVO [] objArrEpisodeVO2=CreditPatientVisitDATA.newDepartmentVisit(objPatientVO, objArrEpisodeVO1, episodeRefVO, objUserVO,oldPatientVO);
		...
		try
		{
			...
			if(objPatientVO!=null){
				...
				if(Integer.parseInt(objPatVisitSUP_p.getEpisodeVisitNo())>1)
					entryDate="";
				//billAmount=CreditPatientVisitDATA.getBillAmountByDeptGrouping(objPatientVO.getPatPrimaryCatCode(), objPatVisitSUP_p.getSelectedFromDept(), objPatVisitSUP_p.getDepartmentCode(), objUserVO);
				billAmount=CreditPatientVisitDATA.getBillAmountByDeptGrouping(objPatientVO.getPatPrimaryCatCode(), objPatVisitSUP_p.getSelectedFromDept(), objPatVisitSUP_p.getDepartmentCode(),entryDate, objUserVO);
...
//						 } //Above Code For Calculating PatAmount is commented, because for HospitalWise Amount is already calculated while populating episodeVo at NewDeptVisit (i.e in method populateNewDepartmentVisitEpisode)
		...
		//End:sheeldarshi
				episodeMap=CreditPatientVisitDATA.savePatientVisit(objPatientVO, selectedEpisodeVO, objUserVO,episodeRefVO,arrNewDeptEpisodeVO);
	...
	public static void getMapOfRenewalConfigDtlOnKeyPatCat(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			...
			mapOfRenewalVoOnKeyPatCatGroup=CreditPatientVisitDATA.getMapOfRenewalConfigDtlOnKeyPatCat(objUserVO);
			registrationConfigVO=CreditPatientVisitDATA.getRegistrationConfigDtl(objUserVO);
			//objPatVisitSUP_p.setStrRenewalType(objRenewalConfigVO.getStrRenewalType());
	...
	public static void getCreditBeneficiaryDtlForPatCatOnCRNO(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			...
			creditBeneficiaryDtl_VO=CreditPatientVisitDATA.getCreditBeneficiaryEssentials(objPatVisitSUP_p, objUserVO);
	...
	public static void getInitEssentials(HttpServletRequest objRequest, HttpServletResponse objResponse) {
		...
		try{
			...
			List lstDistrict=CreditPatientVisitDATA.getDistrict_AJAX(userVO, strStateCode, strCountryCode);
			WebUTIL.setAttributeInSession(objRequest, RegistrationConfig.ESSENTIAL_BO_OPTION_DISTRICT_LIST_STATEWISE,lstDistrict);
			
			List lstRelation=CreditPatientVisitDATA.getRelationsList(userVO);
			 ...
			 */
			List lstClient=CreditPatientVisitDATA.getClientList(userVO);
	...
	public static void getAddmittedDetailsOnCRNO(PatientVisitSUP objPatVisitSUP_p,HttpServletRequest objRequest_p)
	{
		...
		try
		{
			...
			isAdmitted=CreditPatientVisitDATA.checkPatientIsAdmitted(objPatVisitSUP_p, objUserVO);
