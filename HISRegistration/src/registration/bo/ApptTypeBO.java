...
import registration.dao.ApptTypeDAO;
...
public class ApptTypeBO {
	...
	/** To Save the AppointmentType Master Details,**/	
	public boolean saveApptTypeDtl(ApptTypeVO objModelRoom_p,String strMode_p,UserVO uservo) 
	{
		HisDAO hisDAO = new HisDAO("Reg","regbo");
		boolean bExistStatus=ApptTypeDAO.chkApptTypeDuplicate(objModelRoom_p,strMode_p,hisDAO,uservo);
		...
		if (bExistStatus == true) 
		{			
			ApptTypeDAO.saveApptTypeDetails(objModelRoom_p,strMode_p,hisDAO,uservo);
			if (objModelRoom_p.getStrMsgType()!=null && objModelRoom_p.getStrMsgType().equals("1")) 
			{
				String strErr = objModelRoom_p.getStrMsgString();
				objModelRoom_p.setStrMsgString("UnitMstBO.insertQuery() --> " + strErr);
	...
	/**To show Appointment Details in Modify,Feb'19 **/
	public ApptTypeVO modifyRecordApptTypeMst(ApptTypeVO objModelRoom_p)
	{
		ApptTypeVO RoomVO_p = null;
		try
		{
			HisDAO hisDAO = new HisDAO("Reg","regbo");
			RoomVO_p=ApptTypeDAO.modifyDetails(objModelRoom_p,hisDAO);
	...
	/**To Update the AppointmentType Details,**/
	public boolean updateApptTypeDtl(ApptTypeVO objModelRoom_p,String strMode_p,UserVO uservo ) {
		HisDAO hisDAO = new HisDAO("Reg","regbo");
		boolean bExistStatus=true;
		//boolean bExistStatus=ApptTypeDAO.chkApptTypeDuplicate(objModelRoom_p,strMode_p,hisDAO,uservo);
		...
		if (objModelRoom_p.getStrMsgType()!=null && objModelRoom_p.getStrMsgType().equals("1")) {
			...
			//System.out.println("updateQuery()--->"+regMasterBO.updateQuery());
		}
		if (bExistStatus == true) {
			ApptTypeDAO.updateApptTypeDetails(objModelRoom_p,hisDAO, uservo );
	...
	/** To List AppointmentType Details in DeptUnitRoomMst, Added by Singaravelan on 08-Jan-14 **/
	public List getApptTypeList(DeptUnitRoomVO objModelDeptUnitRoom_p,UserVO uservo) {
		HisDAO hisDAO = new HisDAO("Reg", "regbo");
		List lstLocation = null;
		try {
			//lstLocation = DeptUnitRoomDAO.getApptTypeList(hisDAO,objModelDeptUnitRoom_p,uservo);
		} catch (HisRecordNotFoundException e) {
			throw new HisRecordNotFoundException(e.getMessage());
		} catch (HisApplicationExecutionException e) {
			throw new HisApplicationExecutionException();
	...
	public List getRoomList(String deptUnitCode,UserVO uservo) {
		...
		List lstUnit = null;
		try {
			//lstUnit = ApptTypeDAO.getRoomList(deptUnitCode,hisDAO, uservo);
		} catch (HisDataAccessException e) {
			throw new HisDataAccessException();
		} catch (HisRecordNotFoundException e) {
			throw new HisRecordNotFoundException(e.getMessage());
