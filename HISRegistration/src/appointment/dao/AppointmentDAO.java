	 ...
	 // PROCEDURE proc_cancel_appt(appointmentno numeric,hospitalcode character varying, seatId character varying, appointmentstatus character varying, remarks character varying, OUT err character varying);
	  ...
	  public RescheduleCancelAppointmentVO saveRescheduleCancelAppointment(RescheduleCancelAppointmentVO _objRescheduleCancelAppointmentVO, UserVO _userVO)
	  {
		/*  PROCEDURE proc_reschedule_appt_save(hospitalcode character varying, appointmentno numeric, appointmentdate character varying, seatid character varying, 
					...
					OUT err character varying, OUT prevApptDate character varying, OUT prevApptTime character varying*/

		    HisDAO daoObj = null;
			String strProcName = "{call pkg_appointment_Transaction.proc_reschedule_appt_save(?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?::character varying,?,?,?,?)}";//2 args added for prev apt date and time
			int nProcIndex = 0;
			int sq=0;
			String strErr = "";
			...
			daoObj.setProcOutValue(nProcIndex, "appointmentqueueno", 1,++sq);
			//By Mukund
			daoObj.setProcOutValue(nProcIndex, "prevApptDate", 1,++sq);
			daoObj.setProcOutValue(nProcIndex, "prevApptTime", 1,++sq);
			
			daoObj.executeProcedureByPosition(nProcIndex);
			_objRescheduleCancelAppointmentVO.setErrorMessage(daoObj.getString(nProcIndex, "err"));
			_objRescheduleCancelAppointmentVO.setAppointmentQueueNo(daoObj.getString(nProcIndex, "appointmentqueueno"));
			//By Mukund to get the value of previous appointment slot
			_objRescheduleCancelAppointmentVO.setPrevApptDate(daoObj.getString(nProcIndex, "prevApptDate"));
			_objRescheduleCancelAppointmentVO.setPrevApptTime(daoObj.getString(nProcIndex, "prevApptTime"));
	  ...
	  public Map<String,RescheduleCancelAppointmentVO> getAppointmentDtl(																																																																																																												
			...
			try
			{
				...
				while(rs.next()){
					...
					/*Start: Surabhi
					 * reason: for adding the appointmentFor Id in rechedule and cancel appointment
					 * date : 28-7-2016*/
					obj.setAppointmentForId(rs.getString(24));
					System.out.println("Print "+obj.getAppointmentForId());
					//End
					...
					obj.setDepartmentUnit(rs.getString(33));//By Mukund on 09.03.2017
					if(rs.getString(34)!=null && !rs.getString(34).equals("")){
						obj.setTestProcName(rs.getString(34));//By Mukund on 25.01.2019
					}else{
						obj.setTestProcName("-NA-");
					}
