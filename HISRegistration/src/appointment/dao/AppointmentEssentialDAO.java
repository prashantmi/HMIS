...
public class AppointmentEssentialDAO extends DataAccessObject
{
	...
	public String getActualParaRefernceId(UserVO userVO,String appointmentForId, String allActualParameterId) 
	{
		...
		{	//By Mukund on 01.03.2017 as new allActualParameterId = 12103@1004@LocationName
			if(allActualParameterId.contains("@"))
			{
				String []arrSplit = allActualParameterId.split("@");
				allActualParameterId = arrSplit[0];
			}//End:  01.03.2017
			if(allActualParameterId.contains("#"))
	...
	public Map<String, RescheduleCancelAppointmentVO> getPreviousAppointmentList(																																																																																																												
		...
		try
		{
			...
			while(rs.next()){
				...
				obj.setDepartmentUnit(rs.getString(33));//By Mukund to get departmentUnit
				mp.put(obj.getAppointmentNo(), obj);				
		...
		{	
			...
			if(strErr.equals("")){
				...
				while(rs.next()){
					...
					jo.put("title", rs.getString(3)+" ("+rs.getString(4)+"/"+rs.getString(5)+")");
	...
	//by Mukund to get data for IPD OPD Booked Appointment Slots
	public static WebRowSet getIpdOpdBookedApptSlotsCount(UserVO _userVO,	AppointmentParameterVO appParaVO)
	{		
		WebRowSet ws=null;	   	
	   	HisDAO daoObj = null;
		String strErr="";
		...
		{			
			daoObj=new HisDAO("Appointment", "AppointmentEssentialDAO");
			strProcName = "{call pkg_appointment_transaction.proc_get_opd_ipd_booked_slot_dtl(?,?,?,?,? ,?,?,?,?)}";
			nProcIndex = daoObj.setProcedure(strProcName);
			daoObj.setProcInValue(nProcIndex, "modeval", "1", 1);
			daoObj.setProcInValue(nProcIndex, "aptfor", appParaVO.getAppointmentForId(), 2);
			daoObj.setProcInValue(nProcIndex, "actutalrefid", appParaVO.getActualParameterReferenceId(), 3);
