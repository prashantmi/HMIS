	...
	public static void SaveNewAppointment(	NewAppointmentSUP objNewAppointmentSUP_p,HttpServletRequest objRequest) {
		...
		try
		{
			...
			if(objNewAppointmentSUP_p.getActualParameterId()!=null &&  objNewAppointmentSUP_p.getActualParameterId().length>0){
				...
				//if(objNewAppointmentSUP_p.getAppointmentForId()!=null && objNewAppointmentSUP_p.getAppointmentForId()!="" && objNewAppointmentSUP_p.getAppointmentForId().equals("1"))
				String input_date=objNewAppointmentVO.getAppointmentDate();//"28-Feb-2017"
				SimpleDateFormat readFrmt=new SimpleDateFormat("dd-MMM-yyyy");//"EEE, MMM d, ''yy"
				Date dt1=readFrmt.parse(input_date);
				SimpleDateFormat wrtFrmt=new SimpleDateFormat("EEE, MMM dd");//Tues, Feb 28
				...
				String shiftStartTime = objNewAppointmentSUP_p.getShiftST().split("\\,")[0];
				String shiftEndTime = objNewAppointmentSUP_p.getShiftET().split("\\,")[0];
				...
				//By Mukund on 02.03.2017
				String message="";
				if(objNewAppointmentSUP_p.getAppointmentForId()!=null && objNewAppointmentSUP_p.getAppointmentForId()!="" )
				{
					if(objNewAppointmentSUP_p.getAppointmentForId().equals("1"))//Special Clinic
					{
						message = "Appointment confirmed with "+objNewAppointmentSUP_p.getDepartmentUnitName()+" on "+finalDay+", "+finalTime+" with Reference No. "+objNewAppointmentVO.getAppointmentNo()+" at "+objNewAppointmentSUP_p.getAllActualParameterId().split("@")[2]+"\n-"+AppointmentConfig.MSG_HOSP_NAME;//as allActualParameterId = 10213@1004@MICAMPUS = deptUnitCode/LocCode/LocName
					}else if(objNewAppointmentSUP_p.getAppointmentForId().equals("2")){//Investigation Lab Test Wise
						message = "Appointment confirmed with "+objNewAppointmentSUP_p.getDepartmentUnitName()+" for "+objNewAppointmentSUP_p.getTestProcName()+" on "+finalDay+", between "+finalShiftStartTime+" to "+finalShiftEndTime+" with Reference No. "+objNewAppointmentVO.getAppointmentNo()+". Please report atleast 15 min prior to avoid any inconvenience.\n-"+AppointmentConfig.MSG_HOSP_NAME;
					}else if(objNewAppointmentSUP_p.getAppointmentForId().equals("3")){//Service Area
						message = "Appointment confirmed with "+objNewAppointmentSUP_p.getDepartmentUnitName()+" for "+objNewAppointmentSUP_p.getTestProcName()+" on "+finalDay+", "+finalTime+" with Reference No. "+objNewAppointmentVO.getAppointmentNo()+"\n-"+AppointmentConfig.MSG_HOSP_NAME;
					}
					else if(objNewAppointmentSUP_p.getAppointmentForId().equals("4")){//Investigation LabWise
						message = "Appointment confirmed with "+objNewAppointmentSUP_p.getDepartmentUnitName()+" on "+finalDay+", between "+finalShiftStartTime+" to "+finalShiftEndTime+" with Reference No. "+objNewAppointmentVO.getAppointmentNo()+". Please report atleast 15 min prior to avoid any inconvenience.\n-"+AppointmentConfig.MSG_HOSP_NAME;
					}else{
						message  = "You have successfully booked an appointment on "+objNewAppointmentVO.getAppointmentDate()+ " at " +objNewAppointmentVO.getAppointmentTime()+" with Reference No. "+objNewAppointmentVO.getAppointmentNo();
					}
					System.out.println("New Appointment Msg:\n"+message);
					final String   smsmessage  = message;
					final String patMobileNo=objNewAppointmentVO.getMobileNo();
					...
					//String message  = "You have successfully booked an appointment in "+objNewAppointmentSUP_p.getDepartmentUnitName()+objNewAppointmentSUP_p.getAllActualParameterId()+" on "+objNewAppointmentVO.getAppointmentDate()+ " at " +objNewAppointmentVO.getAppointmentTime()+" with Reference No. "+objNewAppointmentVO.getAppointmentNo();
					//message  = "You have successfully booked an appointment in "+objNewAppointmentSUP_p.getActualParameterId()[0]+ " for "+objNewAppointmentSUP_p.getActualParameterId()[1]+" on "+objNewAppointmentVO.getAppointmentDate()+ " at " +objNewAppointmentVO.getAppointmentTime()+" with Reference No. "+objNewAppointmentVO.getAppointmentNo();
					...
					//SMSHttpsNICPostClient.sendTextSMSThroughNICSMSGateway(objSMSConfig.nic_sms_username, objSMSConfig.nic_sms_password,objSMSConfig.nic_sms_senderId,objSMSConfig.nic_sms_url, objNewAppointmentVO.getMobileNo(),message);
				}
				//End:Sheeldarshi 
				...
				message=message + "\n\n\nDISCLAIMER: PLEASE DO NOT REPLY TO THIS MAIL. THIS IS AN AUTO GENERATED MAIL AND REPLIES TO THIS EMAIL ID ARE NOT ATTENDED TOO. FOR ANY QUERIES OR CLARIFICATIONS PLEASE CALL US AT THE CONTACT NUMBERS PROVIDED AT OUR WEBSITE nims.edu.in OR VISIT NIMS.";

				if(objNewAppointmentVO.getEmailId()!=null && !objNewAppointmentVO.getEmailId().trim().equalsIgnoreCase(""))
				{
				    ...
				    //emailPost.sendSingleMailthroughGateway(_mailSession, objEmailConfig.SMTP_FROM_MAILI_ID, objNewAppointmentVO.getEmailId(), "Appointment Confirmation In NIMS", message);
					//Code for sending mail by NIC SMS Gateway
					 //emailNICPost.sendEmail(objNewAppointmentVO.getEmailId(), "Appointment Confirmation In NIMS", message);
				}
				
				/* End of Mail Code appended by Garima */
