...
public class HISSSOClientRequestFilter implements Filter
{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException
	{
		...
		try
		{
			...
			if (strURI.contains(HISSSOClientConfig.SSO_ST_SERVICE_URL))
			{
				// Forward as-it-is
				//chain.doFilter(new RequestWrapper((HttpServletRequest) request), response);
				chain.doFilter(request, response);
				System.out.println("Out of Request wrapper");
			}
			else
			{
				...
				if (session != null)
				{
					...
					System.out.println("Session Alreday exists in HISRegistration with session id in Client request Filter"+session.getId());
					...
					else
					{
						...
						if(reqGrantingTicketId!=null && !reqGrantingTicketId.equals("") && !reqGrantingTicketId.equals(objHISServiceTicket.getGrantingTicketId()))
						{
							authenticate = false;
							authenticatioRequired = true;
							System.out.println("Inside if (session != null) and !reqGrantingTicketId.equals(objHISServiceTicket.getGrantingTicketId()");
						}
						else
						{
							...
							{
								// create new session
								System.out.println("Inside if (session != null) and creating new session and invalidating session "+ objRequest.getSession().getId());
				...
				if (authenticatioRequired)
				{
					...
					if (objGrantTicket != null)
					{
						...
						if (objST != null)
						{
							System.out.println("Inside If ticket exist, logout and delete the old ticket");
							objST.logout();
				...
				/*if (authenticate)
				{
					...
					if(strURI.contains("PatientReferral.action") || strURI.contains("PatientDeathDetail.action"))
					{
						
						chain.doFilter(request, response);
					}
					else if (objHISServiceTicket.getAuthorization().isAuthorizedURL(strURI))
					{
					...
					{
						RequestDispatcher rd = null;
						rd = objRequest.getRequestDispatcher(HISSSOConfig.SSO_AUTHORIZATION_ERROR_PAGE_URL);//not authorized to access page
						rd.forward(objRequest, objResponse);
					}
				}*/
				if (authenticate)
				...
				{
					RequestDispatcher rd = null;
					rd = objRequest.getRequestDispatcher(HISSSOConfig.SSO_AUTHENTICATION_ERROR_PAGE_URL);//session expired page
					rd.forward(objRequest, objResponse);
				}
			}
		...
		catch (IllegalStateException e)
		{
			e.printStackTrace();
			RequestDispatcher rd = null;
			rd = request.getRequestDispatcher(HISSSOConfig.SSO_ILLEGAL_ACTIVITY_ERROR_PAGE_URL); //HISSSOConfig.SSO_ILLEGAL_ACTIVITY_ERROR_PAGE_URL
	...
	private boolean setCredentialsInSession(HttpSession objSession_p, HISServiceTicket objST_p)
	{
		...
		try
		{
			...
			System.out.println("Setting voGlobalUser in Session in HISRegistration WAR with session id"+objSession_p.getId());
			...
			System.out.println("Setting voGlobalHospital in Session in HISRegistration WAR with session id"+objSession_p.getId());
