...
<s:form action="PatientListReportByUser">
...
<!-- 			</div> -->
			...
			<div class="div-table-row ">
				...
				<div class="div-table-col" style="width: 25%;"> <s:select cssStyle="width: 75%" key="strUnitCode" value="%{strUnitCode}" headerKey="-1" headerValue="All" 
			...
			<div class="div-table-row ">
				<div class="div-table-col label" style="width: 25%;"><font color="#FF0000">*</font>&nbsp;<s:text name="global.userName"/>&nbsp;</div>
				<div class="div-table-col" style="width: 25%;">
				 <div class="div-table-col" style="width: 50%;">
				 <s:select cssStyle="width: 100%" key="struserList" value="%{struserList}" headerKey="0" headerValue="All" 
				 			list="%{#session.userList}" listKey="label" listValue="value" name="struserList"> </s:select> 
				<%--  <select style="width: 100%" name="strUserList">
				 <%
				    List<String> userList = (List) session.getAttribute("userList");
				 System.out.println("UserLST1 :::::::::: :::::::::::: "+userList); 
				 for(String s:userList){  
					 System.out.println("UserLST :::::::::: :::::::::::: "+s); 
				%>
...
<s:hidden name="title" value="User Wise Patient Listing Report"></s:hidden>
...
$('[name="struserList"]').validatebox({required: true, validType: ['selectCombo[-1]']});

window.onload = function (e) {
	 $('[name="strChoice"]')[0].checked=true;
	  report.showTimeTile();
	  /* var objUser =document.getElementByName("struserList")[0]; */
	 
	  $('select[name=struserList] option').each(function(index){
		  if($('select[name=struserList]').find('option').eq(index).text()=="<%= session.getAttribute("MyCurUserName") %>")
			  $('select[name=struserList]').val($('select[name=struserList]').find('option').eq(index).val());
	  });
		  
	  console.log('struserList ::::::: >>>>>>>  <%= session.getAttribute("curUser") %>');
	  <%-- objUser.value = "<%= session.getAttribute("curUser") %>"; --%>
	  <% System.out.println("current user from jsp.:::::::::::::::::::::::::::::::::::::::::"+session.getAttribute("curUser"));%>
		...
		onchange_strDeptCode:function()
		  {
			...
			var action 		= "/HISRegistration/registration/reports/getUnitlistReport.action?deptCode="+deptCode;
		  ...
		  clearTile:function(){
			  document.forms[0].action="cancellistReportByUser.action";
			  document.forms[0].submit();
		  },
		  showReport:function(){
			  if($('#PatientListReport').form('validate')){
				  document.forms[0].action="showReportlistReportByUser.action";
			  	  document.forms[0].submit();
			  }
			  else{
				  return true;
