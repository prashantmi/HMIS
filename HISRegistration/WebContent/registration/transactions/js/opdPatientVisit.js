...
$(document).ready(function(){
	...
	if($("#afterGoId").val()=="1"){
		...
		document.getElementsByName("refferringOPDEpisode")[0].checked=true;
			...
			//By Default New Dept Visit Stamp to be Unchecked, Added by Singaravelan on 20-Jul-2015
			{
				...
				if(oldDepartmentVisitFlag!='undefined' && oldDepartmentVisitFlag.checked)
					{
				     $('[name="newDepartmentVisit"]')[0].checked=false;
				      opdPatientVisitJsObj.showHideNewAndOldVisit();
					}
	...
	//alert("document.ready--> PatientVisit JS"+ $('[name="isDuplicateRegistered"]').val());
	
	if($('[name="isDuplicateRegistered"]').val()=='1'){
		$('[name="isDuplicateRegistered"]').val('0');
		opdPatientVisitJsObj.showDuplicateDetail();
	}
...
function setValidForDate(obj)
{
	   
	var letterdatevalue=obj.value;
	
		var _dateStr=letterdatevalue.replace(/-/g,' ');
...
showhidePatIdOnload:function()
{
  ...
  /*Start : Surabhi
	 * Reason : for externally referred patients
	 * date : 7th oct 2016 */
  showExternal:function(objIntrnlExtrnl)
  {
	  //alert("inside showInternalExternal() -->> objIntrnlExtrnl :"+objIntrnlExtrnl.value);
	  if(objIntrnlExtrnl.value=="I"){
		  $("#divInternalReferId").show();
		 ...
		 // }
		  ...
		  if(patPrCatGroup!=PATIENT_REG_CATEGORY_GROUP_BENEFICIARY &&
		   ...
		   if(document.getElementsByName("patIsCrossConsultantWithReferal")[0].value=="1"){
				...
				document.getElementsByName("patActualAmount")[0].value=amtWithoutCrConsultation;
			}
		  /*if(refIndex!="")
  ...
  /*Start : Surabhi
	 * Reason : for externally referred patients
	 * date : 7th oct 2016 */
  checkIsReferredForExt : function(obj)
  {
	// alert("inside checkIsReferred()");
	  if(!opdPatientVisitJsObj.checkForRefer()){
		  return false;
	...
	//  alert("is corss--"+document.getElementsByName("patIsCrossConsultantWithReferal")[0].value);
	  if(obj.checked){
		  ...
		  $("#divRefDtlChkboxId").show();
		  $('[name="isRefferInOut"]')[0].checked=true;
		 // alert($('[name="isRefferInOut"]')[0].value);
		  opdPatientVisitJsObj.showInternalExternal($('[name="isRefferInOut"]')[0]);
		 // if( $('[name="isPatReferByList"]')[0].value=="1"){
		   var amtWithCrConsultation=document.getElementsByName("patAmountCrConsultation")[0].value;
		  // alert("amtWithCrConsultation---"+amtWithCrConsultation);
  ...
  validateDepartmentNewDeptVisit : function(field)
  {
  	...
  	if($('[name="isPatReferByList"]')[0].value==IS_PAT_REFER_BY_LIST_TRUE){
      	...
      	isValid=true;   
  	}//added by Vasu on 30 May,2018
  	else if($('[name="isPatFromAppointList"]')[0].value==1){
  		if(PATIENT_REG_CATEGORY_GROUP_BENEFICIARY == $("#patPrimaryCatGrpCodeId").val() && 
	    		opdPatientVisitJsObj.validateCatBeneficiary("new")==false)
	  	{
	    	return false;
  ...
  validateDepartmentOldDeptVisit : function(callerChecker) 
  {
	 ...
	 // console.log("patPrimaryCatGrpCode :"+$("#patPrimaryCatGrpCodeId").val());
		...
		/*if($('[name="otherHospitalFlag"]')[0].value=='0')
			{
			if($('[name="patPrimaryCatGrpCode"]')[0].value== PATIENT_REG_CATEGORY_GROUP_PAID)
		{
  ...
  showHideNewAndOldVisit : function(){
	  ...
	  var today=moment().format('DD-MMM-YYYY');//new Date().toLocaleFormat('%d-%b-%Y');
   	 ...
   	 // alert("_todayPlus30Days: "+_todayPlus30Days+"\ncreditLetterDate"+document.getElementsByName("creditLetterDate")[0].value);
   		...
   		else
   		{
   			 /* var today=new Date();
   			  var _todayPlus30Days=new Date();*/
   			   ...
   			   today = moment().format('DD-MMM-YYYY');//new Date().toLocaleFormat('%d-%b-%Y');
   				_todayPlus30Days=moment().add(30, 'days').format('DD-MMM-YYYY');//new Date();
   				/*_todayPlus30Days.setDate(_todayPlus30Days.getDate() + 29);
	  ...
	  if(newDepartmentVisitFlag!='undefined' && newDepartmentVisitFlag.checked){
		...
		for(var w=0; w<paymentModeLength; w++){
			...
			if(paymentModeCodeVal.includes("#1")){
				...
				opdPatientVisitJsObj.setPaymentModeRefId(document.getElementsByName("paymentModeCode")[0]);
				break;
			}
			/*if(PrimaryCatName.toUpperCase() === paymentModeName.toUpperCase()){
				...
				opdPatientVisitJsObj.setPaymentModeRefId(document.getElementsByName("paymentModeCode")[0]);
				//opdPatientVisitJsObj.setPaymentModeRefId(document.getElementsByName("paymentModeCode")[0].selectedIndex=0);
	...
	/*  if(!newDepartmentVisitFlag.checked && !oldDepartmentVisitFlag.checked)
	  ...
	  if($("#divCatGroupArogyaShreeBeneficiaryId").show())
	  {
	   $("#agsCounterNoId").validatebox({
			required : false,
			validType : 'alphaNumeric'//by Mukund from numeric to alphaNumeric
		});
		$("#agsNoId").validatebox({
			required : true,
  ...
  validateIsReferred : function(){
	  ...
	  var objIsRef = $('[name="isReferred"]');
		if(typeof objIsRef != undefined && objIsRef.length>0 && objIsRef[0].checked){
			...
			if($('[name="otherHospitalFlag"]')[0].value=='1')
				{
				if ($('[name="isRefferInOut"]')[0].checked)
				{	
					$('[name="isRefferInOut"]')[0].value = IS_REFERRED_IN_OUT_ACCEPT_EXTERNAL;
				}
				}
			
		else if($('[name="otherHospitalFlag"]')[0].value=='0'){
  ...
  validateOnRequest : function (e){
	...
	validateCatBeneficiary : function(mode){
		//By Mukund on 11.05.2017
		//alert("DISABLE\n"+$('[name="letterType"]').prop('disabled')+"\n"+$('[name="creditLetterDate"]').prop('disabled'));
		//
		
		//alert("patamountcollected"+$('[name="patAmountCollected"]')[0].value);
		//alert("grandtotal"+$('[name="grandTotal"]')[0].value);
		...
		if(mode=="new"){
			...
			//alert("patAmountCollected Condition......."+($('[name="patAmountCollected"]')[0].value!="0" || $('[name="patAmountCollected"]')[0].value!="0.00"));
			
			//alert("New Amount COndition"+(parseFloat($('[name="patAmountCollected"]')[0].value)>0));
			 //return false;
			 if(parseFloat($('[name="patAmountCollected"]')[0].value)>0)
				 {
			...
			//alert("grandTotal Condition......."+($('[name="grandTotal"]')[0].value!="0" || $('[name="grandTotal"]')[0].value!="0.00"));
			//alert("Old Amount Condition"+(parseFloat($('[name="grandTotal"]')[0].value)>0));
			//return false;
			var departmentsToVisitStamp = $('[name="departmentsToVisitStamp"]');
			var cntDept=0;
			for(var i=0; i< departmentsToVisitStamp.length; i++){
				...
				if(departmentsToVisitStamp[i].checked && $('[name="renewalRequired"]')[i].value=="1"){
					//alert("Renewal"+$('[name="renewalRequired"]')[i].value+"index..."+i);
					if(parseFloat($('[name="grandTotal"]')[0].value)>0)
						{
					if($('[name="creditLetterRefNo"]')[0].value==""){
						alert("Please Enter Letter Ref No.");
						$('[name="creditLetterRefNo"]')[0].focus();
						return false;
			...
			if(cntDept>1)
				{
				  alert("More than one department renewal can not be done against one letter.");
				}
		}
		return true;
	},
	...
	//Added by Vasu on 28.May.18 for Online Appointment
	showAptDtl: function(obj){
		var patCrNoWithIndex= obj.value;
		 alert(patCrNoWithIndex);
		 $('[name="isPatFromAppointList"]')[0].value	= "1";
		 //alert("$([name=isPatFromAppointList)[0].value:"+$('[name="isPatFromAppointList"]')[0].value);
		 $('[name="patCrNo"]')[0].value			= patCrNoWithIndex.substring(patCrNoWithIndex.indexOf('$')+1);
		 $('[name="onlineReferedIndex"]')[0].value = patCrNoWithIndex.substring(0,patCrNoWithIndex.indexOf('#'));
		 //alert( $('[name="onlineReferedIndex"]')[0].value);
		 $('[name="indexID"]')[0].value			= patCrNoWithIndex.substring(0,patCrNoWithIndex.indexOf('#'));
		 $('[name="selectedReferal"]')[0].value	= patCrNoWithIndex.substring(patCrNoWithIndex.indexOf('#')+1,patCrNoWithIndex.indexOf('&'));
		 alert($('[name="selectedReferal"]')[0].value);
		 $('[name="toDepartment"]')[0].value	= patCrNoWithIndex.substring(patCrNoWithIndex.indexOf('&')+1,patCrNoWithIndex.indexOf('%'));
		 $('[name="departmentCode"]')[0].value	= patCrNoWithIndex.substring(patCrNoWithIndex.indexOf('%')+1,patCrNoWithIndex.indexOf('$'));
		 alert( $('[name="departmentCode"]')[0].value);
		 if( $('[name="selectedReferal"]')[0].value	=="0")
			 {
			 $('[name="selectedReferal"]')[0].value	="N";
			 }
		 else
			 {
			 $('[name="selectedReferal"]')[0].value	="O";
			 }
		 	//alert($('[name="selectedReferal"]')[0].value);

		 //alert($('[name="patCrNo"]')[0].value+"\n"+$('[name="onlineReferedIndex"]')[0].value+"\n"+$('[name="indexID"]')[0].value+"\n"+$('[name="selectedReferal"]')[0].value);
		  submitForm("GETPATDTL"); 
		  
	},
	
	onchange_departmentCode : function(deptObj)
	{
		
		var deptCode = deptObj.options[deptObj.selectedIndex].value;
		var deptVal = deptObj.options[deptObj.selectedIndex].text;
		$('[name="toDepartment"]').val(deptVal);
		//var primaryCatObj = $('[name="patPrimaryCatCode"]')[0].value;
		//var PrimaryCatObj = $('[name="patPrimaryCatCode"]')[0];
		var primaryCatGrp = $('[name="patPrimaryCatGrpCode"]')[0].value;
		//alert(primaryCatObj);
		/*var PrimaryCatCode= PrimaryCatObj.options[PrimaryCatObj.selectedIndex].value;
		alert(PrimaryCatCode);*/

		if(deptCode!='-1')
		{
			var arrDeptCode= deptCode.split('#');
			var rosterType=arrDeptCode[8];
			var deptName=arrDeptCode[9];
			//alert($('[name="patNewDeptVisitAmount"]')[0].value);			alert($('[name="patNewDeptVisitAmountSpl"]')[0].value);
			if(rosterType=="2")
			{	
				if (primaryCatGrp != PATIENT_REG_CATEGORY_GROUP_BENEFICIARY && primaryCatGrp != PATIENT_REG_CATEGORY_GROUP_BENEFICIARY_WITHOUT_REFERENCE)			
					{$('[name="patActualAmount"]')[0].value = $('[name="patNewDeptVisitAmountSpl"]')[0].value
					$('[name="patAmountCollected"]')[0].value = $('[name="patNewDeptVisitAmountSpl"]')[0].value}
				else
					{$('[name="patActualAmount"]')[0].value = $('[name="patNewDeptVisitAmountSpl"]')[0].value
					$('[name="patAmountCollected"]')[0].value = "0.00";//$('[name="patNewDeptVisitAmountSpl"]')[0].value
					}
			}
			else
			{
				if (primaryCatGrp != PATIENT_REG_CATEGORY_GROUP_BENEFICIARY && primaryCatGrp != PATIENT_REG_CATEGORY_GROUP_BENEFICIARY_WITHOUT_REFERENCE)			
					{$('[name="patAmountCollected"]')[0].value =  $('[name="patNewDeptVisitAmount"]')[0].value
					$('[name="patActualAmount"]')[0].value = $('[name="patNewDeptVisitAmount"]')[0].value}
				else
					{$('[name="patAmountCollected"]')[0].value =  "0.00";//$('[name="patNewDeptVisitAmount"]')[0].value
				$('[name="patActualAmount"]')[0].value = $('[name="patNewDeptVisitAmount"]')[0].value}
			}			
		}
	}, //End:Mukund
	
	//End Vasu on 28.05.18
...
$("#submitNewId").click(function(){
	sortandEncodebase64($("#PatientVisit"));
	$("#submitNewId").hide();
...
$("#submitBothId").click(function(){
	...
	//$("#submitBothId").hide();
	/**Added by Vasu on 30.May.18*/
	 if ($('[name="isPatFromAppointList"]')[0].value==1)
	{
		//alert("inside appointment");	
		$('[name="newDepartmentVisit"]')[0].checked=true;
		if( $('[name="selectedReferal"]')[0].value=="O"||$('[name="selectedReferal"]')[0].value=="o")
		{
			$('[name="newDepartmentVisit"]')[0].checked=false;
			$('[name="oldDepartmentVisit"]')[0].checked=true;
		}
	}
	 //End Vasu
	...
	{     //alert("submit .old dept is one js..");
		  $('[name="departmentCode"]').hide();
		// $("#patVisitReasonId").hide();
		 $('[name="departmentCode"]').css("display","none");
...
function submitForm(mode) {
	document.forms[0].action = mode + "PatientVisit.action";
...
function submitFormonRadio(mode,obj) {
	document.forms[0].action = mode + "PatientVisit.action?crno="+obj.value;
	document.forms[0].submit();
