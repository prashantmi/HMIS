...
/*****************************************************/
$('#submitIdBtn').click(function(e){
	//alert("inside submitClick");
	$('#submitIdBtn').hide();
	$("#PatientReferral").attr('action',"/HISRegistration/registration/transactions/SAVEDRDESKPatientReferral.action");
	if(ValidateBeforeSubmitDrDesk())
	{
		if($('#PatientReferral').form('validate')){
			if(typeof $('[name="aptFor"]')[0] != "undefined"){
				if(createAppointment()){
				...
				//alert("isRefferInOut final: "+$('[name="isRefferInOut"]').val());
				$('#PatientReferral').submit();
				$('#submitIdBtn').show();
			}
...
function ValidateBeforeSubmitDrDesk(){
	//var flagYes=false;
		
	var flag=0;
	//swal("ValidateBeforeSubmitDrDesk: "+$('input[name=isRefferInOut]').val()+"\n"+$('input[name=isRefferInOut]').is(':checked'));
	if($('input[name=isRefferInOut_chk]').is(':checked')){
		//alert("1");
		//alert($('input[name=isRefferInOut]').val());
		$('[name="isRefferInOut"]').val("1");
		if($('input[name=choice]').is(':checked')){
			//alert("2");
			var deptCode=$('select[name="departmentCode"]').val();
			if(deptCode=="-1"){
			...
			//alert("3");
			var deptSpclCode=$('select[name="departmentUnitCode"]').val();
			if(deptSpclCode=="-1"){
				swal("Please select Special Clinic Name.");
				return false;
			}
		}
	}
		...
		$('[name="isRefferInOut"]').val("0");
		//alert($('input[name=isRefferInOut]').val());
		if($('input[name="referringInstType"]').is(':checked')){
			$('input[name="isAssociated"]').val("0");
		}
		else{
			 $('input[name="isAssociated"]').val("1");
		}
		//alert("referInstTyp: "+$('input[name="referringInstType"]').is(':checked'));
		if($('input[name="referringInstType"]').is(':checked')){
			var instName=$("#patRefGnctdHospitalCodeId").val();
			if(instName=="-1"){
				swal("Please select Institute Name.");
				return false;
			}
			else{
				if($('select[name="patRefGnctdHospitalDeptUnit"]').val()=="-1"){
					swal("Please enter name for Referring Institute Department.");
					return false;
				}
				else if($('select[name="patRefGnctdHospitalDeptUnit"]').val()=="0"){
					var otherReferDeptName=$('input[name=patRefHospitalDeptOther]').val();
					if(otherReferDeptName==""){
						swal("Please enter name for Other Referred Department.");
						return false;
					}
				}
			}
			...
			//$("#patRefGnctdHospitalCodeId").validatebox({validType: null});
			var hospName=$('[name="patRefHospitalName"]').val();
			if(hospName==""){
				swal("Please enter Other Institution Name for referral.");
				return false;
			}
			else {
				if($('select[name="patRefGnctdHospitalDeptUnit"]').val()=="0"){
					var otherReferDeptName=$('input[name=patRefHospitalDeptOther]').val();
					if(otherReferDeptName==""){
						swal("Please enter name for Other Referred Department.");
						return false;
