/**
 * Modified By: Mukund  Vinayak on 23.05.2016
 */
...
$(document).ready(function(){
	...
	if($("#afterGoId").val()=="1"){
		...
		{
			$('[name="arrCreditLetterDate"]').DatePicker({
		...
		//end
		//Start:Sheeldarshi
		//Reason:Referred details not displayed 
	...
	/*if( $('[name="isPatReferByList"]')[0].value!="1")
	{
	 if(!$("#patPrimaryCatCodeId").is(':visible')){
		if(typeof $('[name="newDepartmentVisit"]')[0] != 'undefined')
			//edited by sandip naik on 26 May,2017
...
var opdPatientVisitJsObj={
  ...
  onchange_patRefGnctdHospitalDept:function()
  {
	...
	var action 	= "/HISRegistration/registration/transactions/getRefDeptPatientUnitWiseVisit.action?refHospCode="+refHospCode+"&flafRefHospOrInst="+flafRefHospOrInst;
  ...
  },onchange_patPrimaryCategory:function()
  {
		...
		var primaryCatGrp=arrPrimaryCatCode[10];//arrPrimaryCatCode[9];//the index value for arrPrimaryCatCode is need to be incremented by 1 because of introduction of spl charges at index 3
		var patCatCharge=arrPrimaryCatCode[2];
		var patCatSplCharge=arrPrimaryCatCode[3];
	    ...
	    if(arrPrimaryCatCode[4]=="1")
		{
			$("#divCatCardId").show();
			$("#shownPatIdNoId").attr('maxlength',arrPrimaryCatCode[5] );
  ...
  showDetail:function(obj)
  {
	...
	//alert("opdPatientVisitJsObj.showDetail() -->> onlineReferedIndex:"+$('[name="onlineReferedIndex"]')[0].value);
	  $('[name="indexID"]')[0].value			= patCrNoWithIndex.substring(0,patCrNoWithIndex.indexOf('#'));
	//alert("opdPatientVisitJsObj.showDetail() -->> indexID :" +$('[name="indexID"]')[0].value);
	  $('[name="selectedReferal"]')[0].value	= patCrNoWithIndex.substring(patCrNoWithIndex.indexOf('#')+1,patCrNoWithIndex.indexOf('$'));
	//alert("opdPatientVisitJsObj.showDetail() -->> selectedReferal: " +$('[name="selectedReferal"]')[0].value);  
	  $('[name="patCrNo"]')[0].value			= patCrNoWithIndex.substring(patCrNoWithIndex.indexOf('$')+1);
	//alert("opdPatientVisitJsObj.showDetail() -->> patCrNo "  +$('[name="patCrNo"]')[0].value);  
  ...
  validateDepartmentNewDeptVisit : function(field)
  {
  	...
  	if($('[name="isPatReferByList"]')[0].value==1){
      	...
      	isValid=true;   
  	}//added by sandip naik on 29 May,2017
  	...
  	else
  	{
  		...
  		var pMCV = $('[name="paymentModeCode"]').val();
	  	if( (depCode=="") || (depCode=="-1") )
	  	{
	  		alert("Please Select Department");
	  		isValid=false;
	  	}else if(pMCV == -1){
	  		alert("Please Select Payment Mode"); isValid=false;
	  	}else{
			...
			if(typeof objIsRef != undefined && objIsRef.length>0 && objIsRef[0].checked==false)
		  	...
		  	if(isValid){
		  		...
		  		//Changes done by Surabhi for wrong message is displayed when a patient registered to a bounded department
			  	var defaultGenderName;
			  	var defaultGenderCode=depCodeArray[6];
			  	if(defaultGenderCode=="M")
			  		{
			  		defaultGenderName="Male";
			  		}
			  	else if(defaultGenderCode=="F")
			  		{
			  		defaultGenderName="Female";
			  		}
			  	else if(defaultGenderCode=="I")
			  		{
			  		defaultGenderName="Indeterminate";
			  		}
			  	else if(defaultGenderCode=="N")
			  		{
			  		defaultGenderName="Not Stated";
			  		}
			  	else if(defaultGenderCode=="T")
			  		{
			  		defaultGenderName="Transgender";
			  		}
			  	//alert(defaultGenderName);
  ...
  checkForRefer : function()
  {
	  ...
	  if( (countDeptVisit>1) && (isRef.checked)){
		  ...
		  //alert("inside else-if  check for refer");
		  if($('[name="newDepartmentVisit"]')[0].checked){
			  isRef.checked=false;
			  alert("Please Do Either New Depart Visit or Old Depart Visit For Visit Stamp With Refer insideUNITWISE");
			  return false;
		  }
	  }
	  flagTrue=true;
  ...
  validateNewAndOldSave: function()
  {
	  ...
	  var flagSave = true;
	  var finalTotal=0;
	  if(!$('#PatientUnitWiseVisit').form('validate'))
		  return false;
  ...
  validateOnRequest : function (e){
	...
	onchange_departmentCode : function(deptObj)
	{
		...
		if(deptCode!='-1')
		{
			...
			$('[name="toDepartment"]').val(deptVal);//deptVal = deptName(unitname)
			//alert(deptName+"\n"+deptVal);
 ...
 //edited by sandip naik on 24 May,2017 for patient through appointment in unit wise registration
...
$("#submitBothId").click(function(){
	...
	{
		//alert(" inside refer");
		//alert("inside submitboth ID FIRST IF");	
		$('[name="newDepartmentVisit"]')[0].checked=true;
		//Start:Mukund Vinayak 27.06.2016
...
function submitForm(mode) {
	document.forms[0].action = mode + "PatientUnitWiseVisit.action";
...
function submitFormonRadio(mode,obj) {
	document.forms[0].action = mode + "PatientUnitWiseVisit.action?crno="+obj.value;
	document.forms[0].submit();
