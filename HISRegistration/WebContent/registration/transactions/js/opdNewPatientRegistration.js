				...
				function() {
					...
					if ($("#flagHasActionErrorId").val() == 'false') {
						...
						$('[name="patAddStreet"]').validatebox({
							validType : 'alphaNumSpecialChar'//'alphaWithSpace'//'alphaNumSpecialChar'
						});
...
var opdregistration = {
	...
	fetchDefaultValues : function() {
		var action = "/HISRegistration/registration/transactions/populateformvaluesNewRegistration.action";
				...
				.ajax({
					...
					success : function(data) {
						...
						for ( var i = 0; i < rootNode.childNodes.length; i++) {
							...
							} else if (elementName == 'defaultpatAddPIN') {
	...
	processCategoryNode : function(elementName, elementNode) {
		...
		if (document.getElementsByName(elementName).length != 0){
			document.getElementsByName(elementName)[0].innerHTML = optionText;
			opdregistration.setCategoryCmboSearchable();
			opdregistration.setOrganizationCmboSearchable();
		}
	...
	setDepartmentDependents : function(deptObj) {
		...
		for ( var i = 0; i < departmentJSONArray.length; i++) {
			if (departmentJSONArray[i].departmentCode == deptCode) {
				...
				//alert("Gender Code Value"+$('[name="patGenderCode"]')[0].value+"\n"+departmentJSONArray[i].defaultGenderCode);
				if($('[name="patGenderCode"]')[0].value=="-1" && departmentJSONArray[i].defaultGenderCode!="-1"){
	...
	setdefaultVariables : function(elementNode) {
		...
		//Start:  Sheeldarshi
		//Reason: NIMS Bug 8980 - pin code does not changes in accordance with district name at Patent Registration Page 
		$("#patAddDistrictCodeId").bind("change",
				opdregistration.onchange_patAddDistrictCode);
		//End
	},
	onchange_patAddCountryCode : function() {
		...
		var action = "/HISRegistration/registration/transactions/getStateNewRegistration.action?countryCode="
				+ countryCode;
	...
	onchange_patAddStateCode : function() {
		...
		var action = "/HISRegistration/registration/transactions/getDistrictNewRegistration.action?countryCode="
	...
	//Start:  Sheeldarshi
	//Reason: NIMS Bug 8980 - pin code does not changes in accordance with district name at Patent Registration Page 
	onchange_patAddDistrictCode : function() {
		/*if($('[name="patAddDistrictCode"]')[0].value == $('[name="defaultpatAddDistrictCode"]')[0].value)
			$('[name="patAddPIN"]')[0].value=$('[name="defaultpatAddPIN"]')[0].value;
		else*/
			$('[name="patAddPIN"]')[0].value="";
	...
	onchange_patRefGnctdHospitalDept : function() {
		...
		var action = "/HISRegistration/registration/transactions/getRefDeptNewRegistration.action?refHospCode="
	...
	onchange_patPrimaryCategory : function() {
		...
		console.log("PrimaryCatCode#################"+PrimaryCatCode);
		...
		if (PrimaryCatCode == "-1") {
			...
			// Added by Raj Kumar For Automatic Pay Mode Selection
		...
		for ( var i = 0; i < categoryJSONArray.length; i++) {
			if (categoryJSONArray[i].patPrimaryCatCode == PrimaryCatCode) {
				...
				if (categoryJSONArray[i].patCatGroup == PATIENT_REG_CATEGORY_GROUP_BENEFICIARY_WITH_REFERENCE) {
					/*var tds = document.getElementById("divClient").getElementsByTagName("select");
					if(tds[1]!=undefined)
					{
					var divrem = document.getElementById("divClient").getElementsByTagName("div");
					if(tds[2]!=undefined)
						{
						tds[2].remove();						
						}
					if(divrem[3]!=undefined)
						{
						divrem[3].remove();
						}
					}*/
				...
				if (categoryJSONArray[i].idReqFlag == "0") {
					...
					$("#shownPatIdNoId").validatebox({
						required : true,
						validType : [ catIdValidType, docValidTypeNLength, 'NotAllZero' ]//By Mukund from "NotAllZeroWithNumber" to "NotAllZero" on 15.04.2017 on recommendation from Dharam sir
					});
...
fetchPatDtlBasedOnPatCatCardNo : function(patientJSONObject) {
		...
		opdregistration.setPatientDistrictDtlFromWhiteCardService(patientJSONObject);
		/*
	...
	savePatientDtl : function() {
		...
		// For Submission
		isSearchable=false;
		sortandEncodebase64($("#NewRegistration"));//By Tanayjit on 21.05.18 for STH(Para Pollution)
		var action = "/HISRegistration/registration/transactions/saveNewRegistration.action";
		$('#submitId').hide();
		$.ajax({
			url : action,
			type : "POST",
	...
	saveAsNewPatientDtl : function() {
		...
		var action = "/HISRegistration/registration/transactions/saveAsNewPatientNewRegistration.action";
		sortandEncodebase64($("#NewRegistration"));//By Tanayjit on 21.05.18 for STH(Para Pollution)
		$('#submitId').hide();
		$.ajax({
			url : action,
	...
	initializeAfterSave : function(saveMsgElementName, saveMsgElementNode) {
		...
		var tokenToSet = saveMsgElementNode.getAttribute("tkn");
		...
		if (isSavedSuccussful == "1") {
			...
			document.getElementsByName('token')[0].value = tokenToSet;//By Mukund
		} else {
		...
		var $dialog = $('<div></div>').html(
			...
			buttons:{
				"Save As New Patient": function() {
					...
					//alert("Not Disabled2: "+selecteCRToRevisit);
					//return false;
	...
	setPaymentModeRefId: function(obj){
		...
		//if(obj.value!="" && obj.value!=1 && obj.value!=13 && obj.value!=10)
		var paymentModeCode=obj.value.split("#")[0];
		if(paymentModeCode!="" && paymentModeCode!=-1 && paymentModeCode!=1 && paymentModeCode!=13 && paymentModeCode!=10)
		{//1 for cash, 10 for cm relief fund, 13 for jan arogya
			$('#divFeeVal3').show();
...
$("#alreadyRegisteredFlagId")
		.click(
				function() {
					...
					var patPrimarCatId = $("#patPrimaryCatCodeId").val();
					var action = "/HISRegistration/registration/transactions/openAlreadyRegPatPopupNewRegistration.action?patPrimarCatId="
							+ patPrimarCatId
...
.click(
		function() {
			...
			var patPrimarCatId = $("#patPrimaryCatCodeId").val();
			var action = "/HISRegistration/registration/transactions/openMobileSearchPatPopupNewRegistration.action?patPrimarCatId="
					+ patPrimarCatId
					+ "&alreadyRegisteredFlag="
					+ alreadyRegisteredFlag;
...
/*$('#submitId').on('focus',function(e) {
	$(window).keypress(function(event) {
	     if (event.which == 13 && enterCount==0)
	   	 {
	    	 enterCount++;
	    	 return true;
	   	 }
	     else if(event.which == 13 && enterCount!=0)
	    {
	    	return false; 
	    }
	   });
...
$('#submitId').click(function(e) {
	...
	if ($('#NewRegistration').form('validate')) {
		$('#submitId').hide();
		...
		//var first = tds[0].firstChild.value;
	//	var second = tds[1].firstChild.value;
...
function changeTabIndex()
{
	...
	alert(align);*/
	 ...
	 $('[name="patEmgCntNo"]').attr('tabindex', '0');
}
...
function showHideCatGroupBeneficiaryTile(mode) {
	...
	if (mode == "show") {
		...
		//By Mukund on 13.04.2017 on recommedation of Dharam sir
		...
		$("#creditLetterRefNoId").validatebox({
			...
			/*validType : ['alphaNumSpecialCharCannotStartZero']*/
			validType : ['alphaNumSpecialChar','NotAllZeroWithSpclChar','consecutiveSpecialCharOnly']
		});
		$('[name="letterType"]').validatebox({
			validType: ['selectCombo[-1]']
		});
		...
		$("#creditLetterDateId")
				.validatebox(
						{
							validType : ['dtltetctdt[\'It should not be greater than Current Date.\']','dtNotGtGvnNoDay[\''+today+'\',\'30\',\'Letter Date must be within 30 days\']']
						});
		$("#cardvalidityDateId")
				.validatebox(
						{
		...
		$("#staffCardNoId").validatebox({
			...
			/*validType : ['alphaNumericCannotStartZero']*/
		...
		$("#agsCounterNoId").validatebox({
			required : false,
			validType : 'alphaNumeric'//alphanumeric by Mukund on demand from dharamendra sir on 12.04.2017
			/*validType : 'numeric'*/
		});
		$("#agsNoId").validatebox({
			required : true,
			validType : ['alphaNumeric','NotAllZero']
			/*validType : ['alphaNumericNotAllZero']*/
		});
		...
		$("#clientCodeId").validatebox({
			required : false,
			validType : null
		});
		$("#letterTypeId").validatebox({
			required : false,
			validType : null
		});
...
function setRelation() {
	...
	if ($('[name="relationNameWithStaff"]')[0].value == "Self"
			...
			|| $('[name="relationNameWithStaff"]')[0].value == "Mother"|| $('[name="relationNameWithStaff"]')[0].value == "Spouse") {
...
function setPatNameSelf() {
	...
	if ($('[name="relationNameWithStaff"]')[0].value == "Spouse") {
		$('[name="patHusbandName"]')[0].value = $('[name="staffCardName"]')[0].value;
	}
...
{
	var action = "/HISRegistration/registration/transactions/openWhiteCardPopupNewRegistration.action";
openURLInPopupWithoutClose(action, '600', '200');
}
