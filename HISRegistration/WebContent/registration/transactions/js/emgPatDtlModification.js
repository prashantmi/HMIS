...
function getDocType(docType)
{
	...
	for(var i=0; i<veriyDocJSONArray.length; i++){
		if(veriyDocJSONArray[i].patDocType==docType){
			...
			docValidType= emgPatDtlModification.getDocValidtype(veriyDocJSONArray[i].patDocValidationType);
...
var emgPatDtlModification={fetchDefaultValues:function (){

	var action 			= "/HISRegistration/registration/transactions/populateformvaluesEmgPatDetailMod.action";
	...
	$.ajax({url: action,type:"POST",async:false,dataType:"xml" ,success:function(data)
		...
		for(var i=0;i<rootNode.childNodes.length;i++ )
		{
			...
			{
				emgPatDtlModification.processVerifyDocNode(elementName,elementNode);
			}
			...
			//Reason:Bug 10166 - System should be able to validate senior citizen category
					...
					if (elementNode
						...
						$('[name="isImageStoredFTP"]')[0].value = elementNode
						 .getAttribute('isImageStoredFTP');
					}
				}
			//End
			else if(elementName == 'broughtByDeclared')
			{
				emgPatDtlModification.processGeneralNode(elementName,elementNode);
			}
			else
			{ 
				//alert(elementName);
				emgPatDtlModification.processGeneralNode(elementName,elementNode);
				/*if(elementName == 'broughtByDeclared')
				{
					for(var i=0;i<elementNode.childNodes.length;i++ )
					{
						...
						//alert(optionNode.getAttribute("label"));
		...
		emgPatDtlModification.setdefaultVariables(rootNode.getElementsByTagName("defaults")[0]);
...
//	showState();
	...
	emgPatDtlModification.onchange_patAddCountryCodeDefault();
...
},setdefaultVariables:function(elementNode)
{
	for(var i=0;i<elementNode.attributes.length;i++ )
	{
		...
		if(document.getElementsByName((elementNode.attributes[i].name.substring(7)))[0]!=undefined){
			//alert("var "+elementNode.attributes[i].name+"='"+elementNode.attributes[i].value+"'");
	...
	$("#patAddCountryCodeId").bind("change",emgPatDtlModification.onchange_patAddCountryCode);
...
onchange_patAddCountryCode:function()
{
	...
	$.ajax({url: action,type:"POST",async:true,dataType:"xml" ,success:function(data)
		...
		if(flagStateNDIstComboReqd)
		{
			...
			$('[name="patAddStateCode"]').validatebox({validType: ['selectCombo[0]']});
			$('[name="patAddDistrictCode"]').validatebox({validType: ['selectCombo[0]']});
			$("#patAddStateCodeId").bind("change",emgPatDtlModification.onchange_patAddStateCode);
		}
		else
		{
...
}};// end of emgPatDtlModification Object Method
...
function setMotherValidRule(){
	...
	var fatherName = $('[name="patGuardianName"]').val();
...
$("#patDocTypeId").change(function(){
	emgPatDtlModification.onchange_patDocType();
	/*var docTypeObj = $('[name="patDocType"]')[0];
	var docType= docTypeObj.options[docTypeObj.selectedIndex].value;
	for(var i=0; i<veriyDocJSONArray.length; i++){
		if(veriyDocJSONArray[i].patDocType==docType){
...
$('#submitId').click(function(e){
	...
	$("#EmgPatDetailMod").attr('action',"/HISRegistration/registration/transactions/SAVEEmgPatDetailMod.action");
	//document.forms[0].action = "saveVerificationDoc.action";
	if($('#EmgPatDetailMod').form('validate'))
		{
		sortandEncodebase64($("#EmgPatDetailMod"));
		$('#EmgPatDetailMod').submit();
		}
	//e.preventDefault();
}); 
