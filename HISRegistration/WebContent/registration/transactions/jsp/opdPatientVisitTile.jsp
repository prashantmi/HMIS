...
<!--By Surabhi on 24-11-16 for SNOMED CT implementation  -->
	 ...
	 if(trgt=="2")
		{
		 ...
		 document.getElementsByName("arrPatVisitReason")[0].value="";
		}
...
function setValue(selectedSNOMEDTerm,status)
{	
	//alert(status);
	...
	if(selectedSNOMEDTerm !="undefined" && selectedSNOMEDTerm!=null && selectedSNOMEDTerm!=""){
		...
		if(target == "patVisitReason"){ // Visit Reason
		...
		}else if(target == "arrPatVisitReason"){ // Arr Visit Reason
			//alert("inside patVisitReason of setValue()");
			 targetPrefferedTerm=document.getElementsByName("arrsnomdPTVisitReason")[0];
			 targetConceptId=document.getElementsByName("arrsnomdCIdVisitReason")[0];    //concept Id
			...
			/*  targetPrefferedTerm=document.getElementById("arrsnomdPTVisitReasonId"+status)[0];
			 alert(targetPrefferedTerm);
			 targetConceptId=document.getElementsByName("arrsnomdCIdVisitReasonId"+status)[0];
...
/* function fun(elmtid,ev)
{
	var str=document.getElementById("txt-snomed-ct-search_"+elmtid).value;	
	//alert(status);
	//alert(str);
...
<script>
	...
	$(window).on("load.loading1", function() {
		if($("#afterGoId").val()=="1"){
			...
			//opdPatientVisitJsObj.showhidePatIdOnload();
			...
			if($('[name="patPrimaryCatGrpCode"]').val()==PATIENT_REG_CATEGORY_GROUP_BENEFICIARY)
			{
				...
				/*if($('[name="fourteenDayRenewal"]').val()==IS_REFERRED_TRUE)
				{	//alert($('[name="fourteenDayRenewal"]').val());
					$('[name="creditLetterRefNo"]').val('');
					...
					var _todayPlus30Days=moment().add(30, 'days').format('DD-MMM-YYYY');//new Date();
...
function paymentModeCodeOnChangeValidate(e){ 
	 ...
	 console.log($(e).attr('oldVal')+":::::::::::::: $(e).attr('oldVal')");
}
/*
$(document).ready(function(){
	...
	$('input[name=newDepartmentVisit]').on('change', function(){
		 alert($('#patPriCat').val()+" ---"); 
		 var val=$('select[name=paymentModeCode]').val();  
		 console.log(val + ':::::::::::>> '+val.split('#')[1]);
		 if(val.split('#')[2]!=$('#patPriCat').val())
		 {
			 $('select[name=paymentModeCode]').find('option').eq(0).prop('selected', true);
...
*/
//alert(document.getElementById('patPriCat').value);
</script>
<body>
		<s:form action="PatientVisit">
			...
			<div class="wrapper rounded" id="nonprintableDiv1">
				...
				<div class="div-table">
					<div class="div-table-row ">
						<div class="div-table-col title width85"><s:text name="global.patient"/>&nbsp;<s:text name="global.revisit"/></div>
							<div id="divCashCollection" class="div-table-col title width15" >
					<!-- <a id="cashCollection" style="cursor: pointer;color: white;display: none;" onclick="openDependentPopup('/HISRegistration/registration/transactions/CASHCOLLECTIONPOPUPPatientVisit.action',event,250,600);" 
		...
		<input type="hidden" id="patPriCat" value="<%= session.getAttribute("PatsPrimaryCatCode") %>" />
				...
				<s:hidden name="patIsCrossConsultantWithReferal" value="%{patIsCrossConsultantWithReferal}"/>
				<s:hidden name="patAmountCrConsultation" value="%{patAmountCrConsultation}" />
				<s:hidden name="patAmountNCrConsultation" value="%{patAmountNCrConsultation}" />
				...
				<!-- <html:hidden name="patIsCrossConsultantWithReferal" property="patIsCrossConsultantWithReferal" /> -->
				...
				<!-- For New Start External -->
			<s:set name="voPatExternalRefer" value="#session[@registration.config.RegistrationConfig@ARR_EPISODE_REFER_EXT_PAT_VO]"></s:set>
				<s:if test="%{#afetrGoForJsp==0 && #voPatExternalRefer!= null && #voPatExternalRefer.length>0}">
					<div class="div-table-listing rounded">
						<div class="div-table-row ">
							<div class="div-table-col title width100 "><s:text name="external"/>&nbsp;<s:text name="global.patient"/>&nbsp;<s:text name="referral"/>&nbsp;<s:text name="global.detail"/> </div>
						</div>
						<div class="div-table-row listHeader">
							<div class="div-table-col alignCenter" style="width: 4%;"><s:text name="select"/></div>
							<div class="div-table-col alignCenter" style="width: 13%;"><s:text name="crNO"/></div>
							<div class="div-table-col alignCenter" style="width: 13%;"><s:text name="global.name"/></div>
							<div class="div-table-col alignCenter" style="width: 13%;"><s:text name="referFromHosp"/></div>
							<div class="div-table-col alignCenter" style="width: 13%;"><s:text name="referFromDept"/></div>
							<div class="div-table-col alignCenter" style="width: 13%;"><s:text name="referFromDeptUnit"/></div>
							<div class="div-table-col alignCenter" style="width: 13%;"><s:text name="referto"/>&nbsp;<s:text name="global.department"/> </div>
							<div class="div-table-col alignCenter" style="width: 13%;"><s:text name="refer"/>&nbsp;<s:text name="date"/></div>
						</div>
						<s:iterator id="voPatExternalReferId"  value="voPatExternalRefer" status="statusExtRefrVO">
							<div class="div-table-row listData">
								<div class="div-table-col alignCenter" style="width: 4%;">
									<%-- <s:if test="%{deptUnitIsClosed == @registration.config.RegistrationConfig@DEPT_UNIT_IS_CLOSED_FALSE}">
										<s:if test="%{deptUnitIsOnDuty == @registration.config.RegistrationConfig@DEPT_UNIT_IS_ON_DUTY_TRUE}"> --%>
											<s:set name="strSelectedReferCrNo" value="%{#statusExtRefrVO.index + '#N$' + patCrNo}"></s:set>
											<input type="radio" name="selectedReffereCrNo" value='<s:property value="%{strSelectedReferCrNo}"/>' onclick="opdPatientVisitJsObj.showDetail(this)">
										<%-- </s:if>
										<s:else>
											<!-- <input type="radio" name="selectedReffereCrNo1" value="%{#statusRefrVO.index}"> -->
								...
								<div class="div-table-col alignCenter" style="width: 13%;"><s:property value="patCrNo"/> </div>
								<div class="div-table-col alignCenter" style="width: 13%;"><s:property value="patName"/> </div>
								<div class="div-table-col alignCenter" style="width: 13%;"><s:property value="fromHospital"/> </div>
								<div class="div-table-col alignCenter" style="width: 13%;"><s:property value="fromDepartment"/> </div>
								<div class="div-table-col alignCenter" style="width: 13%;"><s:property value="fromDepartmentUnit"/> </div>
								<div class="div-table-col alignCenter" style="width: 13%;"><s:property value="toDepartment"/> </div>
								<div class="div-table-col alignCenter" style="width: 13%;"><s:property value="reffDateTime"/> </div>
							</div>
						<s:set name="isReferInOut" value="O" />
						</s:iterator>
					</div>
				</s:if>
				
				
				<!--By Vasu on 28.05.2018 for showing patient list for appoinment  -->
					...
					<div class="div-table-listing rounded">
						...
						<s:iterator id="voPatApptmntId"  value="voPatApptmnt" status="statusRefrVO">
							<div class="div-table-row listData">
								<div class="div-table-col alignCenter" style="width: 4%;">
									<s:if test="%{deptUnitIsClosed == @registration.config.RegistrationConfig@DEPT_UNIT_IS_CLOSED_FALSE}">
										<%-- <s:if test="%{deptUnitIsOnDuty == @registration.config.RegistrationConfig@DEPT_UNIT_IS_ON_DUTY_TRUE}"> --%>
											 ...
											 <s:set name="toDepartment" value="%{toDepartment}"></s:set> 
											 <s:set name="toDepartmentCode" value="%{toDepartmentCode}"></s:set>
											 <s:set name="toDepartmentUnitCode" value="%{toDepartmentUnitCode}"></s:set>
											 <s:set name="strSelectedAppointCrNo" value="%{#statusRefrVO.index + '#'+isOldDeptVisitFlag+'&' + toDepartment+'%' +toDepartmentCode+'#' +toDepartmentUnitCode+'$' + patCrNo}"></s:set> 
											 <%-- <s:set name="strSelectedAppointCrNo" value="%{#statusRefrVO.index + '#0$'+ patCrNo}"></s:set>  --%>
											<%--  <s:hidden name="toDepartment" value="%{toDepartment}"/> --%>
								...
								<%-- <s:hidden name="toDepartment" value=""/>  --%>
								<s:hidden name="toDepartmentUnit" value="%{toDepartmentUnit}"/>
								<%-- <s:hidden name="toDepartment" value="%{toDepartmentCode}" />
								<s:hidden name="toDepartmentUnit" value="%{toDepartmentUnitCode}"/> --%>
							</div>
						</s:iterator>
					</div>
				</s:if>
...
<%-- 					</s:action> --%>
				...
				<%-- <s:include value="/registration/transactions/jsp/patientDetailsTile.jsp"/> --%>
					<s:if test="%{#otherHospitalFlagForJsp==1 && #otherHospitalDataFoundForJsp!=1}">
						<div class="div-table">
							<div class="div-table-row">
								...
								<div class="div-table-col width control" style="width: 35%">
									...
									<s:if test="%{#optionsPriCatDtl!= null}">
										<s:select id="patPrimaryCatCodeId" name="patPrimaryCatCode" 
										   ...
										   headerKey="-1"  headerValue="Select Value" value="%{patPrimaryCatCode}" />
								...
								<div id="divCatCardId" class="div-table-col" style="width: 40%; display: none;">
									<div  class="div-table" >
										<div class="div-table-row">
					...
					<s:if test="%{#otherHospitalFlagForJsp!=1}">
					<div class="div-table" >
							<%-- 'modeCaseForJsp:<s:property value="%{#modeCaseForJsp}"/>' --%>
							<!-- Edited by Vasu on 29.05.18 for Online Appointed Patients -->
							...
							<div class="div-table-col width23 control" >
								<s:if test="%{(newDepartmentVisit=='On' || newDepartmentVisit=='on')}">
									<s:if test="#isPatReferByListForJsp == @registration.config.RegistrationConfig@IS_PAT_REFER_BY_LIST_TRUE">
										<input type="checkbox" name="newDepartmentVisit" id="newDepartmentVisitID" checked="checked" disabled="disabled" />
									</s:if>
								...
								<%-- <s:if test="%{#patPrimaryCatGrpCodeForJsp == @registration.config.RegistrationConfig@PATIENT_REG_CATEGORY_GROUP_BENEFICIARY}"> --%>
									<input type="hidden" name="creditBillFlag" id="creditBillFlagId" value="1">
										...
										<div class="div-table-col width16 control" >
											...
											<input type="hidden" name="oldCreditLetterRefNo" id="oldCreditLetterRefNoId" value="<s:property value="creditLetterRefNo" />" >
										</div>
					...
					<div class="div-table">
						...
						<div class="div-table-row" >
							...
							<div class="div-table-col width16 label"><font color="red" size="2">*</font><s:text name="arogyasri"/>&nbsp;<s:text name="number"/>
							</div>
							<div class="div-table-col width16 control" >
								<input type="text" name="agsNo" id="agsNoId" value="<s:property value="agsNo" />" maxlength="15" size="20">
							</div>
							<!--By Mukund on 12.04.2017 after recommedations of Dhramendra sir The changes further proceeded to the backend -->
						<div class="div-table-col width16 label"><%-- <s:text name="arogyamitra"/>&nbsp; --%><s:text name="global.counter"/>&nbsp;<s:text name="number"/>/<s:text name="TG"/>&nbsp;<s:text name="number"/>
						</div>
						<div class="div-table-col width16 control" >
							<input type="text" name="agsCounterNo" id="agsCounterNoId" class="input75prcnt" maxlength="15" tabindex="1">
						</div>
					<!--End Mukund 12.04.2017  -->				
						</div>
						<div class="div-table-row">
						<div class="div-table-col width16 label"><s:text name="credit"/>&nbsp;<s:text name="Limit"/>
						</div>
					...
					<s:if test="%{newDepartmentVisit=='On' || newDepartmentVisit=='on' || #goWithoutDeptVisitOnForJsp==1}">
						<div class="div-table" id="divNewDeptVisitId">
							...
							<s:set name="newDeptVisitWidth" value="'width12'"></s:set><!--By warish from 16 to 12-->
							<div class="div-table-row">
...
<!-- 					edited by Vasu on 29 May,2018 for patient through appointment in patient Revisit -->
									<s:if test="%{#isPatReferByListForJsp == @registration.config.RegistrationConfig@IS_PAT_REFER_BY_LIST_TRUE}">
									<div class="div-table">
											<div class="div-table-row">
													...
													<%--<s:set name="deptOption" value="#session[@registration.config.RegistrationConfig@REGISTRATIONDESK_OPTION_DEPARTMENT]"></s:set>
													 <s:if test="%{#deptOption!= null}">
														<s:select name="departmentCode" id="departmentCodeId" cssClass="select90prcnt" list="deptOption" 
																listKey="value" listValue="label" tabindex="1"
																headerKey="-1" headerValue="Select Value" />
													</s:if>
													<s:else>
														<select name="departmentCode" id="departmentCodeId" tabindex="1" class="select90prcnt"><option value="-1">Select Value</option> </select>
													</s:else> --%>
												</div>
								...
								<%-- <s:if test="%{#patPrimaryCatGrpCodeForJsp == @registration.config.RegistrationConfig@PATIENT_REG_CATEGORY_GROUP_BENEFICIARY}">
									 <input type="hidden" name="creditBillFlag" id="creditBillFlagId" value="1"> 
										...
										<input id="creditLetterDateId" type="text" name="creditLetterDate" class="input60prcnt" onblur="dateNotInRange(this)">
										<!-- End -->
										</div>
								</s:if>
								<s:else>
								...
								<div class="div-table-col control <s:property value="#newDeptVisitWidth"/>" style="height:28px;">
									...
									<div align="left" id="dialog-form_1" >
										...
										<div id="snomed-ct-search">
											...
											<input placeholder="Enter 3 characters to search..." id="txt-snomed-ct-search_1" class="clearable ui-autocomplete-input tooltipClass" name="txt-snomed-ct-search_1"  style="width:65%;color:#000000;" type="text" onfocus="load1('1','')"/>
										</div>
								...
								<s:set name="paymentMode" value="#session[@registration.config.RegistrationConfig@PAYMENT_MODE_OPTION_LIST_PROCESSED]"></s:set>
													<s:if test="%{#paymentMode!= null}">
														<s:select name="paymentModeCode" id="divFeeVal2" cssClass="select90prcnt" list="paymentMode" 
																listKey="value" listValue="label" 
																headerKey="1" onfocus="$(this).attr('oldVal',$(this).val());" onchange='paymentModeCodeOnChangeValidate(this)'/>
													</s:if>
				                        </div>
				                     <div class="div-table-col control" id="divFeeVal3" maxlength="20" size="10" style="width:15%;display:none;">
					                <input tabindex="1" name="paymentModeRefId"  type="text" />
				                 </div> 
							</div>
								<%-- <div class="div-table-row">
								<s:if test="%{#patPrimaryCatGrpCodeForJsp == @registration.config.RegistrationConfig@PATIENT_REG_CATEGORY_GROUP_BENEFICIARY}">
									<input type="hidden" name="creditBillFlag" id="creditBillFlagId" value="1">
										...
										<div class="div-table-col <s:property value="#newDeptVisitWidth"/> control" >
											<!-- <input type="date" name="creditLetterDate" id="creditLetterDateId" value="Today" tabindex="1" class="input60prcnt letterDate" onblur="dateNotInRange(this)" style="text-align: left;"> -->
											<input id="creditLetterDateId" tabindex="1" type="text" name="creditLetterDate" onchange="dateNotInRange(this);" class="input60prcnt">&nbsp;
										</div>
										<div class="div-table-col <s:property value="#newDeptVisitWidth"/> label"><font color="red">*</font><s:text name="Letter"/>&nbsp;<s:text name="type"/>
										</div>
										<div class="div-table-col <s:property value="#newDeptVisitWidth"/> control" id="divLetterType">
										...
										<div class="div-table-col <s:property value="#newDeptVisitWidth"/> label"><s:text name="validfor"/>
										</div>
										<div class="div-table-col <s:property value="#newDeptVisitWidth"/> control" >
										<input type="text" name="cardvalidityDate" id="cardvalidityDateId"  class="input60prcnt">
										</div>
										<div class="div-table-col <s:property value="#newDeptVisitWidth"/> label"><s:text name="credit"/>&nbsp;<s:text name="Limit"/>
										</div>
										<div class="div-table-col <s:property value="#newDeptVisitWidth"/> control" >
					...
					<s:if test="%{oldDepartmentVisit=='On' || oldDepartmentVisit=='on' || #goWithoutDeptVisitOnForJsp==1}">
						<div class="div-table" id="divOldDeptVisitId1">
							<div class="div-table-row">
								<div class="div-table-col title width100 "><s:text name="old"/>&nbsp;<s:text name="global.department"/>&nbsp;<s:text name="global.visit"/>&nbsp;<s:text name="global.details"/></div>
							</div>
						</div>
										...
										<div class='div-table-row listData <s:property value="backgroundColor"/>'>
													...
													<s:if test="%{deptUnitIsOnDuty == @registration.config.RegistrationConfig@DEPT_UNIT_IS_ON_DUTY_TRUE}">
														<s:if test="%{#renewalTypeForJsp== 1 || #renewalTypeForJsp==2}">
															<s:if test="%{#opdRenewalRequiredForJsp == @registration.config.RegistrationConfig@RENEWAL_REQUIRED_TRUE}">
																...
																<s:hidden name="renewalRequired" value="%{opdRenewalRequiredForJsp}" id='renewalRequired<s:property value="%{#statusArrEpisodeVO.index}"/>'/>
																<s:hidden name="regRenewalRequired" value="%{opdRenewalRequiredForJsp}"/>																
																<input type="checkbox" name="departmentsToVisitStamp" tabindex="1" value="<s:property value="episodeCode"/>" onclick='opdPatientVisitJsObj.calculateGrandtotal(this,<s:property value="%{#statusArrEpisodeVO.index}"/>);'/>
															</s:if>
															<s:else>
																<s:hidden name="renewalRequired" value="%{opdRenewalRequiredForJsp}" id='renewalRequired<s:property value="%{#statusArrEpisodeVO.index}"/>'/>
																<s:hidden name="regRenewalRequired" value="%{opdRenewalRequiredForJsp}"/>	
														...
														<s:elseif test="%{#renewalTypeForJsp== 3 || #renewalTypeForJsp==4}">
															<s:if test="%{renewalRequired == @registration.config.RegistrationConfig@RENEWAL_REQUIRED_TRUE}">
																...
																<input type="checkbox" name="departmentsToVisitStamp"  tabindex="1" onclick='opdPatientVisitJsObj.calculateGrandtotal(this,<s:property value="%{#statusArrEpisodeVO.index}"/>);' value="<s:property value="episodeCode"/>"/>
															</s:if>
															<s:else>
																<input type="checkbox" name="departmentsToVisitStamp" tabindex="1" value="<s:property value="episodeCode"/>" onclick="opdPatientVisitJsObj.calculateGrandtotal(this,<s:property value="%{#statusArrEpisodeVO.index}"/>);" />
															</s:else>
											...
											<s:if test="%{#patPrimaryCatGrpCodeForJsp == @registration.config.RegistrationConfig@PATIENT_REG_CATEGORY_GROUP_BENEFICIARY
												...
												<s:if test="%{#fontColor=='colorRed'}">
													...
													<!-- <div class='div-table-col alignCenter width10'>
														<input type="text" name="arrCreditLetterRefNo" id="arrCreditLetterRefNoId" class="input90prcnt letterNo" maxlength="50" tabindex="1">
													</div>
													<div class='div-table-col alignCenter width10'>
														<input type="date" name="arrCreditLetterDate" id="arrCreditLetterDate" value="Today" tabindex="1"  class="input90prcnt letterDate" onblur="dateNotInRange(this)" style="text-align: left;">
													</div> -->
											...
											<div class='div-table-col alignCenter width<s:property value="%{16- #reducedWidth}"/>'>
												...
												<!--By Surabhi on 24-11-16 for Snomed CT implementation-->
													...
													<div align="left" id="dialog-form_2" readonly <s:property value="fontColor"/>>
													<s:set name="status" value='<s:property value="%{#statusArrEpisodeVO.index}"/>'></s:set>
															...
															<input type="hidden" name="arrPatVisitReason" value='<s:property value="arrPatVisitReason" />' id='arrPatVisitReasonId<s:property value="%{#statusArrEpisodeVO.index}"/>'/>
															
															<div id="snomed-ct-search" align="center">
																	<span class="ui-helper-hidden-accessible" aria-live="polite" role="status"></span>
													<s:if test="%{visitedToday == @registration.config.RegistrationConfig@DEPT_UNIT_VISITED_TODAY_TRUE}">
													<div style="width:100%" align="center"><input type="text" readonly value='<s:property value="arrPatVisitReason" />' /></div></s:if>
													<s:else>
																	<%-- <s:property value="arrPatVisitReason" /> --%>
																	<input placeholder="Enter 3 characters to search..." id='txt-snomed-ct-search_2' value='<s:property value="arrPatVisitReason" />' class="clearable ui-autocomplete-input x onX" name="txt-snomed-ct-search_2"  style="width:70%;color:#000000;" type="text" onfocus="load1('2','');" />
																<%--<textarea name="txt-snomed-ct-search_2" id="txt-snomed-ct-search_2" value='<s:property value="arrPatVisitReason" />' class="searchText ui-autocomplete-input tooltipClass" style="width:60%;color:#000000" autocomplete="off">
																	<s:property value="arrPatVisitReason" />
																	</textarea> --%>
															</s:else>
															</div>
				...
				<s:if test="%{#otherHospitalFlagForJsp==1}">
					...
					<!--------------------------------For refer details when internally referred ---------------------- -->
							...
							<s:if test="%{#afetrGoForJsp!=0 && #voArrOpenEpisode!= null}">
								<div class="div-table-listing rounded" >
									...
									<s:iterator id="voArrOpenEpisodeId"  value="voArrOpenEpisode" status="statusArrOpenEpisodeRefr">
										...
										<%--By Mukund on 02.05.2017  --%>
										<s:set name="episodeExpiryDateForJsp" value="episodeExpiryDate"></s:set>
										<input type="hidden" name="episodeExpiryDate" id="episodeExpiryDate" value='<s:property value="episodeExpiryDateForJsp"/>'>
										<!-- End Mukund on 02.05.2017 -->
										
										<div class="div-table-row listData">
											<div class="div-table-col alignCenter" style="width: 4%;">
												<input type="radio" name="refferringOPDEpisode" id="refferringOPDEpisodeId" value='<s:property value="epCode"/>###<s:property value="episodeExpiryDateForJsp"/>'>
							...
							<div class="div-table-col width100">
												...
												<div class="div-table-row" >
													...
													<div class="div-table-col control" style="width: 50%;">
														...
														<s:if test="%{#optionsRefHospital!= null}">
															<s:select id="patRefGnctdHospitalCodeId" name="patRefGnctdHospitalCode" tabindex="1"
																cssStyle="width:145px" list="optionsRefHospital"  
																 listKey="value" listValue="label" 
															   headerKey="-1"  headerValue="Select Value" />
															   <s:set name="frmHosp" value="fromHospital"></s:set>
														</s:if>
														<s:else>
								...
								<div class="div-table" id="divReferred" style="display: none;">
									<div class="div-table-row">
										...
										<div class="div-table-col control" style="width: 25%;">
											<input type="text" 	name="patRefDoctor" maxlength="60" tabindex="1">
										</div>
									</div>
				...
				<div class="div-table-col" style="width: 50%;">
					...
					<!-- --------------------------- for refer details when externally referred--------------------------------- -->
					...
					<div class="div-table" >
						<div class="div-table-row title">
							...
							<div class="div-table-col label" style="width: 20%">
									<s:text name="isreferred"/>
									<!-- <input type="checkbox" name="isReferred" value="0" onclick="opdPatientVisitJsObj.checkIsReferred(this)"> -->
									<s:checkbox name="isReferred" value="%{isReferred}" onclick="opdPatientVisitJsObj.checkIsReferredForExt(this)"> </s:checkbox>
							</div>
						</div>
					</div>
					<div id="divRefDtlChkboxId" class="div-table" style="display: none;">
						<div id="divCheckReferralId" class="div-table-row" >
							<div class="div-table-col width100">&nbsp;&nbsp;&nbsp;
							<table><tr><td>
								<s:text name="referExternal"/> <input type="radio" name="isRefferInOut" value="I" onclick="opdPatientVisitJsObj.showExternal(this)" />&nbsp;&nbsp;</td>
						    	<td id="tdExtRefer"><%-- <s:text name="referInternal"/> <input type="radio" name="isRefferInOut" value="O" onclick="opdPatientVisitJsObj.showInternalExternal(this)" /> --%></td>
						    	</tr>
						    	</table>
							</div>
						</div>
						
						<div id="divInternalReferalId" >
							<s:set name="voArrOpenEpisode" value="#session[@registration.config.RegistrationConfig@ARR_OPD_OPEN_EPISODE_VO]"></s:set>
							<s:if test="%{#afetrGoForJsp!=0 && #voArrOpenEpisode!= null}">
								<div class="div-table-listing rounded" >
									<div class="div-table-row listHeader" >
										<div class="div-table-col alignCenter" style="width: 5%"><s:text name="select"/></div>
										<div class="div-table-col alignCenter" style="width: 35%"><s:text name="referFromHospital"/> </div>
										<div class="div-table-col alignCenter" style="width: 30%"><s:text name="referFromDept"/> </div>
										<div class="div-table-col alignCenter" style="width: 30%"><s:text name="referFromDeptUnit"/></div>
									</div>
									<s:iterator id="voArrOpenEpisodeId"  value="voArrOpenEpisode" status="statusArrOpenEpisodeRefr">
										<s:set name="epCode" value="episodeCode"></s:set>
										<s:set name="frmHosp" value="strHospCode"></s:set>
										<s:set name="frmDep" value="fromDepartment"></s:set>
										<s:set name="frmDepUnit" value="fromDepartmentUnit"></s:set>
										<s:set name="frmDepCode" value="fromDepartmentCode"></s:set>
										<s:set name="frmDepUnitCode" value="fromDepartmentUnitCode"></s:set>
										...
										<div class="div-table-row listData">
											<div class="div-table-col alignCenter" style="width: 5%;">
												<input type="radio" name="refferringOPDEpisode" id="refferringOPDEpisodeId" value='<s:property value="epCode"/>'>
												<s:hidden name="episodeCode" value="%{epCode}"></s:hidden>
											</div>
											
											<div class="div-table-col alignCenter" style="width: 35%">
												<s:property value="strHospName"/>
											</div>
											
											<div class="div-table-col alignCenter" style="width: 30%">
												<s:property value="frmDep"/>
												<s:hidden name="frmDepCode" value="%{frmDepCode}"></s:hidden>
											</div>
				...
				<div class="div-table-button" id="nonprintableDiv2">
					...
					<div class="div-table-row" align="center">
						<s:if test="%{#afetrGoForJsp!=0}">
							...
							<%-- <s:if test="%{#otherHospitalFlagForJsp==1}">
							<s:if test="%{(newDepartmentVisit=='On' || newDepartmentVisit=='on') && oldDepartmentVisit==''}">
							...
							<s:if test="%{(newDepartmentVisit=='On' || newDepartmentVisit=='on') || (oldDepartmentVisit=='On' || oldDepartmentVisit=='on')}">
							     <a class="button" id="submitBothId" style="cursor: pointer;"><span class="save"><s:text name="save"/></span></a>
							</s:if>
							<%-- </s:else> --%>
						</s:if>
			...
			<div class="div-table" id="divPrintId" style="font-weight: normal;color: black;" align="center">
			...
			<s:hidden name="fromHospital" value="%{patRefGnctdHospitalCode}" />
			...
			<s:hidden name="fourteenDayRenewal" value="%{fourteenDayRenewal}"/>
		...
		<script type="text/javascript"
			src="./../../registration/transactions/js/opdPatientVisit.js" /></script>
</body>
</html>