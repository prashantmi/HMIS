...
//	alert('fecg');
	...
	var params= './../'+module_id+'/list'+cnt+'.action?searchColumn='+searchColumn+"&search="+search+"&struts.token.name=token&token="+token+"&blockNo="+blockNo+"&prevNext="+prevNext+"&rowNum="+rowNum+"&orderby="+order+str;
	...
	xmlHttp.onreadystatechange = function()
	{
		...
		if(xmlHttp.readyState == 4)
		{
			if(xmlHttp.status =='200')
			{
				...
				//alert("response data"+responseData[6]);
				//check for error
				if (responseData[0] == "ERROR") {
					...
					document.getElementsByName("token")[0].value = responseData[6];
...
function deleteRecords()
{
	...
	var len			= 	0;
	var no_of_combo	=	0;
	//alert(document.getElementsByName("token")[0].value);
	...
	//alert(document.forms[0].token.value);
	...
	var params= './../'+module_id+'/delete'+document.forms[0].cnt_page.value+'.action?divisionId='+divisionId+"&struts.token.name=token&token="+token+"&searchColumn="+searchColumn+"&chk="+primarykey+"&prevNext"+prevNext+"&rowNum=0&minrow="+minrow+"&blockNo="+blockNo+"&max_rownum="+max_rownum+"&search="+search+"&searchColumn="+searchColumn;
		...
		if(xmlHttp.readyState ==4 )
		{
			if(xmlHttp.status == 200 || xmlHttp.status=='complete' )
			{
				...
				if (responseData[0] == "ERROR") {
					...
					document.getElementsByName("token")[0].value = responseData[8];
					document.getElementById("footer").style.display	=	'block';
...
function cancelList(mode)
{
	//document.forms[0].hmode.value=mode;
	//document.forms[0].action="cancelList"+mode+".action";
	//document.forms[0].submit();
	callMenu();
...
function callMenu()
{
	...
	var targetURL = "/AHIMSG5/hislogin/transactions/jsp/st_desk_background.jsp";// + "?varSSOTicketGrantingTicket=" + document.getElementsByName("varSSOTicketGrantingTicket")[0].value;
...
function add(cnt)
{
	//alert("Inside Add Method: "+cnt);
	//alert("Token Length is"+document.getElementsByName("token").length);
	//alert("Token Name is"+document.getElementsByName("struts.token.name")[0].value);
	//alert("Token Value is"+document.getElementsByName("token")[0].value);
	if(checkforAdd())
	{
