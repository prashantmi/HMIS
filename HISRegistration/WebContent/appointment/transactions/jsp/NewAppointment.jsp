...
$(document).ready(function() {
	...
	changePatientType();
	validateFields();
	if(document.getElementsByClassName("DatePicker_Icon_Inside")[0]!=undefined)
	{
	document.getElementsByClassName("DatePicker_Icon_Inside")[0].style.marginTop="8px";
	}
	$('#submitId').click(function(){


		/* alert(": "+$('[name="departmentUnitName"]').val()+"\n: "+$('[name="allActualParameterId"]').val());
		return false;  */

		var flagPass = true;
		var apptType = $('[name="appointmentTypeId"]').val();
		var opdSlotsAll = $('[name="opdSlotsAll"]').val();
		var ipdSlotsAll = $('[name="ipdSlotsAll"]').val();
		var opdSlotsBooked = $('[name="opdSlotsBooked"]').val();
		var ipdSlotsBooked = $('[name="ipdSlotsBooked"]').val();
		//alert(opdSlotsAll+"\n"+ipdSlotsAll+"\n"+opdSlotsBooked+"\n"+ipdSlotsBooked+"\n"+apptType);
...
if(flagPass){/***/
		...
		/***/
		if(undefined!=document.getElementsByName("parameterId")[1]){
			var parameterId =document.getElementsByName("parameterId")[1].value;
			var concatId2= '_'+parameterId;
			var testProcedure = "#ACTUALPARAMETERID"+concatId2;
			var testProcName = $(testProcedure).find(":selected").text();
			$('[name="testProcName"]').val(testProcName);
		...
		/*By Mukund on 10.04.2017 for Getting appointmetn saved on either father name or spouse name*/
		var fthName = $('[name="patGuardianName"]').val();
		var spsName = $('[name="patHusbandName"]').val();
		//alert("aftergo: "+document.getElementsByName("afterGo")[0].value +"\nspsName: "+spsName+"\nfthName: "+fthName);
	...
	document.getElementsByName("mobileNo")[0].value=document.getElementsByName("crMobileNo")[0].value;
	document.getElementsByName("emailId")[0].value=document.getElementsByName("crEmail")[0].value;
...
<s:form action="NewAppointment">
    ...
    <div class="wrapper rounded" style="width:95%" >
	    <div class="div-table">
			...
			<%String display="none"; %>
			<input type="hidden" name="crMobileNo" value="<s:property value="mobileNo"/>">
			<input type="hidden" name="crEmail" value="<s:property value="emailId"/>">
						...
						<div class="div-table-row">
							...
							<div class="div-table-col control" style="width: 25%">
							       ...
							       <s:if test="%{#session.LSTAGETYPE}">
										<s:select list="%{#session.LSTAGETYPE}" listKey="value" listValue="label" name="patAgeUnit" tabindex="1" cssStyle="width :75px"  > </s:select>
									</s:if>
									<s:else>
							...
							<div class="div-table-col control" style="width: 25%;">
								<s:if test="%{#session.LSTGENDER}">
									<s:select 	list="%{#session.LSTGENDER}"  tabindex="1"  listKey="value" listValue="label" name="patGenderCode" cssStyle="width:145px;" > </s:select>
								</s:if>
								<s:else >
		...
		<div id='div_appointment'  style="display:none;">
			...
			<div class="div-table">
				<div class="div-table-row ">
					...
					<div class="div-table-col control" style="width: 25%;">
					      ...
					      <input name="appointmentDate"  tabindex="1" id="appointmentDate" maxlength="10" type="text" style="width: 95px;"  onchange="getSlotDetails_withCapping();">   
					...
					<div class="div-table-col label" style="width: 25%" ><font color="red">*</font><s:text name="Apt_appointment"/>&nbsp;<s:text name="global.type"/></div>
					<div class="div-table-col control" style="width: 25%;">
					   <select name="appointmentTypeId"  tabindex="2" id="appointmentTypeId" style="width :145px" onchange="getSlotDetails_withCapping();">
				...
				<div class="div-table-row ">
					...
					<div class="div-table-col control" style="width: 25%;">
					   <input name="mobileNo"  tabindex="1" maxlength="10" type="text" onblur="showMobileNo();">    
					</div>
					<div class="div-table-col label" style="width: 25%" ><s:text name="global.email"/></div>
					<div class="div-table-col control" style="width: 25%;">
					   <input name="emailId"  tabindex="2"  maxlength="50" type="text" >
					</div>	
				</div>			
				<div class="div-table-row ">
					...
					<div class="div-table-col control" style="width: 25%;">
					   <textarea rows="2"   tabindex="2" cols="20" name="remarks" maxlength="200" ></textarea>
					 </div>	
				</div>						
			</div>	
			<div class="div-table-button">
				...
				<div class="div-table-row" align="center">
					...
					<a href="#"  tabindex="1" class="button" id="cancelId" onclick="submitForm('CANCEL');"><span class="cancel"><s:text name="cancel"/></span></a>
				</div>				
			</div>
		</div>
