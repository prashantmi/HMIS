					...
					<xsl:if test="number(@type) = 2">
						<xsl:if test="rowDetails">            
							...
							<xsl:element name="table">
								...
								<xsl:attribute name="style">border-spacing: 0px 10px;border-collapse: separate;padding:1px;font-family:Arial</xsl:attribute>
							...
							<xsl:element name="td">
								<xsl:choose>
									...
									-->
										...
										<xsl:attribute name="width">
											<xsl:value-of select="number($maxwidth)"/>%</xsl:attribute>
									</xsl:otherwise>
...
<!--								<xsl:element name="div">-->
								...
								<xsl:if test="current()/table/tr/td/element/@align ='0'">
									
										<xsl:attribute name="align">left</xsl:attribute>
									
								</xsl:if>
								
								<xsl:if test="current()/table/tr/td/element/@align ='1'">
									
									<xsl:attribute name="align">center</xsl:attribute>
									
								</xsl:if>
								
								<xsl:if test="current()/table/tr/td/element/@align ='2'">
    ...
    <xsl:for-each select="options">
     ...
     <xsl:value-of select="current()/table/tr/td/element/@value"/> -->
            ...
            <xsl:attribute name="align">center</xsl:attribute>-->
					...
					<!-- start xsl when for autocomplete-->

					<xsl:when test="current()/table/tr/td/element/@idC = 'autocomplete'">
						<xsl:element name="td">
							<xsl:choose>
								<xsl:when test="$maxColumns = @colNo">
									<xsl:if test="current()/table/tr/td/element/@displayParam = '0'">
    ...
    <xsl:for-each select="options">
     ...
     <xsl:value-of select="current()/table/tr/td/element/@value"/> -->
            ...
            <xsl:attribute name="align">center</xsl:attribute>-->
					...
					<!-- end xsl when for autocomplete-->




					<!-- start xsl when for listbox-->

					<xsl:when test="current()/table/tr/td/element/@idC = 'listbox'">
						<xsl:element name="td">
							<xsl:choose>
								<xsl:when test="$maxColumns = @colNo">
									<xsl:if test="current()/table/tr/td/element/@displayParam = '0'">
							...
							<xsl:choose>
								...
								<xsl:otherwise>
									<!--<xsl:call-template name="listboxValues">
											<xsl:with-param name="concatValue" select="substring-before($selectval,'$')"></xsl:with-param>
									</xsl:call-template>-->
									<!--<xsl:choose>
										<xsl:when test="contains($selectval,'$')">-->

									<xsl:for-each select="current()/table/tr/td/element/options">
										<xsl:for-each select="option">
											<xsl:if test="@value != ''">

												<xsl:if test="contains($selectval, @value)">
									...
									<!--<xsl:call-template name="listboxValue"></xsl:call-template>-->
								</xsl:otherwise>
							</xsl:choose>
...
<!--							<xsl:attribute name="style">padding-left:20px</xsl:attribute>-->
	...
	<xsl:template name="listboxValues">
