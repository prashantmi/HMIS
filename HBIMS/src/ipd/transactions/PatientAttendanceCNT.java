...
public class PatientAttendanceCNT extends CSRFGardTokenAction{
	...
	{		
		...
		PatientAttendanceDATA.getPatientList(request,fb,response);
		return null;
	}
	...
	{		
		...
		fb.setStrHospCode(request.getSession().getAttribute("HOSPITAL_CODE").toString());
		fb.setStrSeatId(request.getSession().getAttribute("SEATID")	.toString());
		PatientAttendanceDATA.getPatientStatusView(request,fb,response);		
		return null;
	}
