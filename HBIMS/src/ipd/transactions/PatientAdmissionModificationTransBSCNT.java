...
public class  PatientAdmissionModificationTransBSCNT extends CSRFGardTokenAction{
	...
	//	saveToken(request);
		...
		formBean.setStrHospCode(request.getSession().getAttribute("HOSPITAL_CODE").toString());
		formBean.setStrSeatId(request.getSession().getAttribute("SEATID").toString());
		PatientAdmissionModificationTransDATA.initPatientAdmission_BS(formBean);
		return mapping.findForward(target);
	...
	public ActionForward ADMCANCEL(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) throws HisException 
	{
		ActionForward acFwd = new ActionForward();
		PatientAdmissionModificationTransFB formBean = (PatientAdmissionModificationTransFB)form;
		acFwd.setPath("/ipd/transactions/PatientAdmissionCancellationTransBSCNT.cnt?hmode=GO&strCrNo="+formBean.getStrPatientCrNo()+"&isSingleMenu=1");
		acFwd.setContextRelative(true);
		return acFwd;
	}
	
	
	public ActionForward ADMNEW(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) throws HisException 
	{
		ActionForward acFwd = new ActionForward();
		PatientAdmissionModificationTransFB formBean = (PatientAdmissionModificationTransFB)form;
		acFwd.setPath("/ipd/transactions/PatientAdmissionTransBSCNT.cnt?hmode=unspecified&isSingleMenu=1");
		acFwd.setContextRelative(true);
		return acFwd;
	}
	...
	{
		ActionForward acFwd = new ActionForward();
		acFwd.setPath("/ipd/transactions/NewBornBabyTransBSCNT.cnt?hmode=unspecified&isSingleMenu=1");
		acFwd.setContextRelative(true);
		return acFwd;
	}
