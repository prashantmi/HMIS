...
//	System.out.println("vo.getStrUnitCode"+vo.getStrUnitCode());		
	...
	public static void insert(IssueTransFB formBean, HttpServletRequest request) {
	...
	public static void inserttemp(IssueTransFB formBean, HttpServletRequest request) {
...
//			    System.out.println("mcu.getStrDoseFrqFlg()==>"+mcu.getStrDoseFrqFlg());
	...
	public static void insertWithoutCrNo(IssueTransFB formBean, HttpServletRequest request) 
	{
		int debugPoint = 1;
		IssueTransBO bo  			  = null;
		IssueTransVO vo				  = null;
...
//			}			
			...
			{	
					...
					if (vo.getStrMsgType().equals("1")) 
					{
						...
						formBean.getWsIssueDetails().beforeFirst();
						//String str = IssueTransHLP.getIssueRptForDotMatrix(formBean);
...
public static void save(IssueTransFB _IssueTransFB,HttpServletRequest request,HttpServletResponse response)
{
	 /* Declaring Variables */
       String strmsgText = "";
	   IssueTransVO vo = null;
	   ...
	   try
	   {
		    ...
		    vo=new IssueTransVO();
		   	...
		   	/* Value set in Value Object */
		   	vo.setStrHospitalCode(request.getSession().getAttribute("HOSPITAL_CODE").toString());
		   	vo.setStrStoreId(_IssueTransFB.getStrStoreId());
		   	vo.setStrIssueNo(_IssueTransFB.getStrHiddenIssueNo());
		   	vo.setStrCrNo(_IssueTransFB.getCrNo());
		   	//vo.setStrItemCategoryNo(request.getParameter("itemCatNo"));
		   //System.out.println("Flag==>"+request.getParameter("strFlagVal"));
		//   	vo.setStrFlagVal(request.getParameter("strFlagVal"));
		   /*	if(request.getParameter("strFlagVal").equals("1"))
            ...
            /* Calling BO method */
		    bo.save(vo);		
		    _IssueTransFB.setStoreId(vo.getStrStoreId());
		    if(vo.getStrMsgType().equals("1"))
		    {
		    //_IssueTransFB.setStrNormalMsg("In-Sufficent Stock Available In Store");
		    _IssueTransFB.setStrErrMsg("In-Sufficent Stock Available In Store");
		    //_IssueTransFB.setStrWarningMsg("In-Sufficent Stock Available In Store");
		    //_IssueTransFB.setStrstockflg("1");
		    }else{
		    	//_IssueTransFB.setStrstockflg("2");
		    }
		   
	   }catch(Exception _err){
		   _err.printStackTrace();
		   _IssueTransFB.setStrNormalMsg("Data Saved Successfully");
		   strmsgText = _err.getMessage();
			HisException eObj = new HisException("MMS", "IssueTransDATA->getViewDtl()", strmsgText);
			_IssueTransFB.setStrErrMsg("ERROR####Application Error [ERROR ID : " + eObj.getErrorID() + "], Contact System Administrator! ");
			//_IssueTransFB.setStrMsgType("1");
