...
public class LotsDetailTransUTL extends TransInterface {
	...
	public String getMainQuery(String cmb[]) {
		...
		String strHospitalCode = httpSession.getAttribute("HOSPITAL_CODE")
				.toString();
		if (cmb != null) {
							/*brMainQuery.append("SELECT DISTINCT c.HSTNUM_STORE_ID||'@'||c.HSTNUM_AGENDA_NO||'@'||HSTNUM_ITEM_ID"+
							"||'@'||c.HSTNUM_ITEMBRAND_ID||'@'||c.GNUM_HOSPITAL_CODE||'^'||c.HSTNUM_AGENDA_NO||'^'||c.HSTDT_AGENDA_DATE"+
							"||'^'||decode(c.HSTNUM_CONDEMNATION_TYPE,1,'Crushed',2,'Burn',3,'Auction','-')"+
							" DATA,c.HSTNUM_AGENDA_NO FROM (Select A.HSTNUM_STORE_ID, A.HSTNUM_AGENDA_NO,"+
							" B.HSTNUM_ITEM_ID,B.HSTNUM_ITEMBRAND_ID,B.GNUM_HOSPITAL_CODE,A.SSTNUM_ITEM_CAT_NO,"+
							" A.HSTNUM_AGENDA_STATUS,A.HSTDT_AGENDA_DATE,A.HSTNUM_CONDEMNATION_TYPE FROM"+
							" HSTT_AGENDA_DTL A,HSTT_AGENDA_ITEM_DTL B"+
							" WHERE B.HSTNUM_AGENDA_NO=A.HSTNUM_AGENDA_NO"+
							" AND B.GNUM_HOSPITAL_CODE=A.GNUM_HOSPITAL_CODE"+
							" AND B.HSTNUM_STORE_ID=A.HSTNUM_STORE_ID"+
							"	AND A.GNUM_ISVALID=1) C"+
							" WHERE C.HSTNUM_STORE_ID=#1#"+
							" AND c.SSTNUM_ITEM_CAT_NO=#2#"+
							" AND c.HSTNUM_AGENDA_STATUS=#3#"+
							" AND c.GNUM_HOSPITAL_CODE="+strHospitalCode.trim());*/
							brMainQuery.append("SELECT HSTNUM_STORE_ID||'@'||HSTNUM_AGENDA_NO||'@'||GNUM_HOSPITAL_CODE||'^'||HSTNUM_AGENDA_NO||'^'||TO_DATE(HSTDT_AGENDA_DATE,'DD-Mon-yyyy')||'^'||");
							brMainQuery.append(" MMS_MST.GET_CONDEMN_TYPE_DTL(GNUM_HOSPITAL_CODE,HSTNUM_CONDEMNATION_TYPE) ");
							brMainQuery.append("	 DATA FROM HSTT_AGENDA_DTL");
							brMainQuery.append("	 where 	 GNUM_ISVALID=1 AND HSTNUM_STORE_ID=#1#");
							brMainQuery.append("	 AND SSTNUM_ITEM_CAT_NO=#2#");
							brMainQuery.append("AND HSTNUM_AGENDA_STATUS=#3#");
			...
			if(cmb[2].equals("2")){
							...
							brMainQuery.append(" MMS_MST.GET_CONDEMN_TYPE_DTL(GNUM_HOSPITAL_CODE,HSTNUM_CONDEMNATION_TYPE)");
							brMainQuery.append("	 DATA FROM HSTT_AGENDA_DTL");
							brMainQuery.append("	 where 	 GNUM_ISVALID=0 AND HSTNUM_STORE_ID=#1#");
							brMainQuery.append("	 AND SSTNUM_ITEM_CAT_NO=#2#");
							//brMainQuery.append("AND HSTNUM_AGENDA_STATUS=#3#");
							brMainQuery.append(" AND GNUM_HOSPITAL_CODE="+strHospitalCode);
			}
		} else {
					brMainQuery.append("SELECT HSTNUM_STORE_ID||'@'||HSTNUM_AGENDA_NO||'@'||GNUM_HOSPITAL_CODE||'^'||HSTNUM_AGENDA_NO"+
					"||'^'||HSTDT_AGENDA_DATE||'^'||"+
					" MMS_MST.GET_CONDEMN_TYPE_DTL(GNUM_HOSPITAL_CODE,HSTNUM_CONDEMNATION_TYPE) DATA FROM "+
					" HSTT_AGENDA_DTL WHERE GNUM_HOSPITAL_CODE=0");
			}
	...
	public String[] getColumnHeader() {
		String[] strColumnHeader = { "Lot No.", "Date","Condemnation Type"};
	...
	public String getJsFiles() {
		String files = "../../mms/js/lotsDetailDesk.js";
		return files;
	...
	public String[] getUserDefinedButtons() {
		String[] strButtons = { "Lots@validateLots()@0@FFC007@glyphicon-equalizer",
				"Cancel@validateCancel()@1@bb0000@glyphicon-remove",
				"View@validateView()@1@007bb6@glyphicon-fullscreen" };
		return strButtons;
	}
