...
public class SampleRegisterPurTransUTL extends TransInterface
{
	...
	{
		String transactionName = "On Line Sample Register";
	...
	public String getMainQuery(String cmb[]) 
	{
		...
		if (cmb != null) {
			String strMainQuery="select c.HSTNUM_SAMPLE_RECNO||'@'||c.HSTNUM_ITEM_ID||'@'||c.HSTNUM_ITEMBRAND_ID||'@'||"+
			 ...
			 " c.HSTSTR_BATCH_SL_NO||'@'||c.HSTNUM_APPROVAL_STATUS||'@'||c.HSTSTR_TENDER_NO||'@'||c.HSTSTR_QUOTATION_NO||'@'||c.GNUM_HOSPITAL_CODE||'^'||"+
			 
				" TO_CHAR(c.HSTDT_SAMPLE_RECDATE,'DD-Mon-YYYY')||'^'||c.HSTSTR_TENDER_NO||'^'||c.item_name"+
				
				" ||'^'||c.HSTSTR_BATCH_SL_NO"+
				
				" ||'^'||c.suppname||'^'||c.CommiteeType||'^'||c.approval_status AS DATA FROM "+
				...
				String cond1="c.GNUM_HOSPITAL_CODE="+httpSession.getAttribute("HOSPITAL_CODE").toString();
		...
		else
		{
			String strMainQuery="select c.HSTNUM_SAMPLE_RECNO||'@'||c.HSTNUM_ITEM_ID||'@'||c.HSTNUM_ITEMBRAND_ID||'@'||"+
			 ...
			 " c.HSTSTR_BATCH_SL_NO||'@'||c.HSTNUM_APPROVAL_STATUS||'@'||c.GNUM_HOSPITAL_CODE||'^'||"+
				...
				" b.HSTDT_SAMPLE_RECDATE,b.HSTSTR_TENDER_NO,"+" mms_mst.get_item_dtl(1,A.GNUM_HOSPITAL_CODE,a.HSTNUM_ITEM_ID) as item_name,"+
	...
	{
		String search_field[] = {"3", "c.item_name","5","c.suppname"};
		return search_field;
	}
	/**
	...
	public String[] getColumnHeader() 
	{
		...
		//String[] strColumnHeader = { "Recieved Date","Tendor No.", "Item Name", "Batch No.","Supplier Name"};
	...
	public String[] getComboQuery() 
	{
	   ...
	   comboQuery[0] = "select HSTNUM_STORE_ID||'/'||HSTNUM_STORETYPE_ID,HSTSTR_STORE_NAME"+
						" FROM HSTT_STORE_MST A where  GNUM_ISVALID = 1 and  GNUM_HOSPITAL_CODE="+httpSession.getAttribute("HOSPITAL_CODE").toString()+
						" AND EXISTS"+
						   "("+
						    "SELECT 'X'"+
	   ...
	   if(cmbVal==null)
	   	...
	   	comboQuery[2] = ("0^Active#1^Returned#2^Condemned");
		return comboQuery;
	...
	/*
	 * chk >> key1@key2|extra_info
	 * define the delete data for every delete query in same order as defined in delete query
	...
	{
		//String[] status = {"Autrin Cap", "2", "brown", "Expired","aLkol", "2", "blue", "Alkol"};
		String[] status = {"Rejected", "6", "#FA6D62", "Rejected"};//,"Approved","9","#38B625","Approved"};
	...
	public String[] getUserDefinedButtons() {
		...
		String[] strButtons = {"Recieve@validateRecieve(document.forms[0],'RECIEVE')@0@ea4c89@glyphicon-share",
	...
	{
		String orderBy[] = {"3", "c.item_name","5","c.suppname"};
		//String orderBy[] = {};
		return orderBy;
	} 
}
