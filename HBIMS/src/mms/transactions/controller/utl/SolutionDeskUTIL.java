...
public class SolutionDeskUTIL extends TransInterface {
	...
	{
		String masterName = "Solution Desk";
		return masterName;
	}
	...
	public String getMainQuery(String cmb[]) 
	{
		...
		MmsConfigUtil mmscofigutil = new  MmsConfigUtil(httpSession.getAttribute("HOSPITAL_CODE").toString());
		 StringBuffer brMainQuery = new StringBuffer(500);
			...
			String tempQry = mms.qryHandler_mms.getQuery(2, "select.soldesktrans.main.0");
	...
	/*
				...
				{				
                 ...
                 if (!cmb[0].equals("0"))
					...
					brMainQuery.append(" AND "+ mms.qryHandler_mms.getQuery(2,"select.soldesktrans.main.cond.2").replace("?", cmb[0]));
				
				
				if (!cmb[1].equals("0"))
					
					brMainQuery.append(" AND "+ mms.qryHandler_mms.getQuery(2,"select.soldesktrans.main.cond.0").replace("?", cmb[1]));
					...
					if (!cmb[2].equals("0")) 
						...
						//System.out.println("Combo 2 value:"+cmb[2]);	
					brMainQuery.append(" AND "
								+ mms.qryHandler_mms.getQuery(2,
										"select.soldesktrans.main.cond.1").replace("?", cmb[2]));
			...
			else
			{
				...
				brMainQuery.append(	" and "	+ mms.qryHandler_mms.getQuery(2,"select.soldesktrans.main.cond.1").replace("?","9"));
	...
	public String[] getSearchField() 
	{
		...
		String search_field[]={"1","c.HSTNUM_PROB_ID","5","HSTSTR_PROB_SUB"};
		return search_field;
	}
	...
	{
		String[] strComboHeader = {"0^0","Department","0^1","Drug Warehouse", "1","Problem Status"};
	...
	public String[] getColumnHeader() 
	{
		...
		String[] strColumnHeader = { "Issue_No", "Menu Name" , "Issue Raised Date","Status","Subject"};
	...
	comboQuery[0] = "select HSTSTR_DEPT_CODE,HSTSTR_DEPT_NAME from HSTT_HLPDESKDEPT_MST A WHERE GNUM_HOSPITAL_CODE= "+httpSession.getAttribute("HOSPITAL_CODE").toString()+" AND GNUM_ISVALID=1"+
						" AND EXISTS"+
						   "("+
						    "SELECT 'X'"+
						    ...
						    " AND UPPER(gstr_table_name) = UPPER('HSTT_HLPDESKDEPT_MST')"+
						    " AND UPPER(gstr_column_name) = UPPER('HSTSTR_DEPT_CODE')"+
						    " AND P.gnum_hospital_code = A.gnum_hospital_code"+
						  " AND gnum_column_value = A.HSTSTR_DEPT_CODE"+
						 " AND P.gnum_seatid = Pkg_Usermgmt.fun_getseatid("+strSeatId+				 
						 ",A.GNUM_HOSPITAL_CODE))" ; 
	
	comboQuery[1] = "select HSTNUM_STORE_ID,HSTSTR_STORE_NAME from HSTT_STORE_MST A where  GNUM_ISVALID = 1  AND  GNUM_HOSPITAL_CODE ="+
		...
		comboQuery[2] = "1^To Be Solved#2^Solved";
	...
	{
		String files="../../mms/js/SolutionDeskTrans.js";
	...
	*/
	...
	public String[] getUserDefinedButtons() 
	{
		...
		//if(res.getString("DRUG_INVENTORY").equals("1"))
		{
	       ...
	       String[] strButtons = {"Solution@viewAcknowledge()@1@8AC14A@glyphicon-console",
	    		   "View@viewsolution()@1@007bb6@glyphicon-fullscreen"};
	       return strButtons;
		}
		/*
		else
	...
	{
		String orderBy[] = {"1","c.HSTNUM_PROB_ID","3","c.GDT_ENTRY_DATE"};
