...
{	
	...
	public static String getDrugDetails(WebRowSet wb,TransferRequestTransFB formBean,String index) 
	{
		...
		{
			_e.printStackTrace();
			formBean.setStrMsgString("TransferRequestTransHLP.getDrugDetails() --> "+ _e.getMessage());
	...
	public static String getTransferDetails(TransferRequestTransFB formBean) 
	 {
		...
		if (ws != null && ws.size() != 0)
		{
			 ...
			 {
				 sb.append("<a name='pg1' id='pg1" + i + "'  STYLE='cursor:pointer;cursor:pointer;' class='pg-normal' title='Record Indexing' onClick='GetIndex1(\""+i+"\",\""+totalLayer+"\")'>" + (i+start-1)
						+ "</a>|&nbsp;");
			 }
			 sb.append("</td></tr>");
			 sb.append("</table>");		
...
//			 for (int i = 1; i <= totalLayer; i++)
//			 {
//				sb.append("<a STYLE='cursor:pointer;cursor:pointer;' color:'blue' onClick='GetIndex1(\"").append(i).append("\",\"").append(totalLayer).append("\")'>" ).append( (i+start-1)
//						).append( "</a> &nbsp;");
//			 }
//			sb.append("</td>");
//			sb.append("</tr>");
//			sb.append("</table>");
		    ...
		    for (int i = 1; i <= totalLayer; i++) 
		    {
			 if (i <= totalLayer) 
			 {
				...
				if (i == 1) 
				{
					sb.append("<div id='DivId1" ).append(i).append( "' style='display:block'>");
				} 
				else
				{
					sb.append("<div id='DivId1" ).append(i).append( "' style='display:none'>");
				}
				...
				for (int j = 0; j < REC_PER_PAGE; j++) 
				{
					/*
					 1- Transfer No
					 2- Transfer Date
					 3- Transfer Qty with Unit Name
					 4- Recived Date
					 5- Recived Qty with Unit Name	
					 6- Store ID		
	...
	public static String getTransferOrderDetails(TransferRequestTransFB formBean) 
	 {
		StringBuffer sb = new StringBuffer("");
		int start = 1;
		final int REC_PER_PAGE = 50;
			 ...
			 if (i <= totalLayer) 
			 {
				...
				for (int j = 0; j < REC_PER_PAGE; j++) 
				{
					/*
					 ...
					 2- Order Date
					 3- Demand Store Name
					 4- Order Qty with Unit Name
					 5- Transfer Qty with Unit Name
					 6- Ack Qty with Unit Name
					 7-Hidden Value 
					  ReqNo||'^'||Transfer Store ID||'^'||Transfer Req No||'^'||Transfer Date
					  ||'^'||Transfer remarks||'^'||Ack Date||'^'||ACK_REMARKS||'^'||STATUS||'^'||Demand Store ID
                   ...
                   */
					...
					sb.append("<input type='radio' name='strDOrderNo' onclick='disOldDate(this,"+count+")' >");
