	...
	public static String getDebitNoteList(DupSupplierReturnFromTransVO vo) {
		...
		try
		{
			...
			if (wb != null && wb.size() > 0) 
			{
					...
					br.append("<TR>");										
					br.append("<td width='5%' id='check"+i+"'><input class='form-control' type='radio' title='Print Debit Note' name='strdebitNote' value='"+strHiddenValue+ "' id='strdebitNote' onclick='onSelectRadioButton(\""+strHiddenValue+"\",\""+ i + "\");'></td>");
					br.append("</td>");						
					br.append("<TD WIDTH='25%' colspan='1' CLASS=''>"+ wb.getString(1) + "</TD>");
					/*
