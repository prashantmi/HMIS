 ...
 * 
 */
public class TaskMstUTL  implements MasterInterface
{
	...
	{
		String[] columnHdr = { "Task Name", "Effective From Date" };
		return columnHdr;
	}
	...
	{		
		...
		strComboQuery[0] = bmed.qryHandler_bmed.getQuery(1,"taskMst.enggItemtype.combo.0");
		strComboQuery[0] = strComboQuery[0].replace("?", Config.SUPER_USER_HOSPITAL_CODE);
		
		strComboQuery[1] = bmed.qryHandler_bmed.getQuery(1,"taskMst.enggItemSubtype.combo.1");
	...
	{		
		...
		strDeleteQuery[0] = bmed.qryHandler_bmed
				.getQuery(1, "taskMst.delete.0").replace("?", seatId);
		strDeleteQuery[0] = strDeleteQuery[0].concat(" WHERE "
				+ bmed.qryHandler_bmed.getQuery(1, "taskMst.delete.cond.0"));
	...
	{
		return "/HEMMS_ODISHA/WebContent/bmed/js/taskMst.js";
	}
	...
	public String getMainQuery(String[] cmb)
	{
		...
		brMainQuery.append(bmed.qryHandler_bmed.getQuery(1, "taskMst.main")
					.replace("?", strHosCode)).append(" AND " + bmed.qryHandler_bmed.getQuery(1,"taskMst.main.cond.1")
							.replace("?", "1"));
		...
		if (cmb != null) 
		{
			...
			brMainQuery.append(" AND "+bmed.qryHandler_bmed.getQuery(1, "taskMst.main.cond.2").replace("?", "0"));
		...
		{	
			...
			brMainQuery.append(" AND "+bmed.qryHandler_bmed.getQuery(1, "taskMst.main.cond.3").replace("?","0"));
	...
	{
		String strMasterName = "Task Master";
	...
	{
		String strOrderBy[] = { "1", " UPPER(HEMSTR_TASK_NAME) " };
		return strOrderBy;
	}
	...
	{
		String strSearchField[] = { "1", "HEMSTR_TASK_NAME" };
	...
	{		
		String strViewQuery = bmed.qryHandler_bmed.getQuery(1,"taskMst.view");
