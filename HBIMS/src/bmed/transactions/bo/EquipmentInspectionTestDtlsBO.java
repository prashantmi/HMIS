...
public class EquipmentInspectionTestDtlsBO 
{
	/* used in equipment Test or Inspection entry. */
	/*public void initializeTestInspectionDtl(TestDtlVO testDtlVO_p,WarrantyDtlVO warrantyDtlVO_p, HemtItemMcDtlVO hemtItemMcDtlVO_p) throws Exception {

		HisDAO hisDao = null;
		String strWarrantySlNo = null;
		String strMcSlNo = null;
		String strEnggItemTypeId = null;
		String strEnggItemSubTypeId= null;

		try {
			 Transaction Start 
			hisDao = new HisDAO("bmed", "EquipmentInspectionTestDtlsBO");

			testDtlVO_p.setStrMode("1");
			//System.out.println("Before calling getDATA in BO");
			TestDtlDAO.getData(testDtlVO_p, hisDao);
			//System.out.println("After calling getDATA in BO");
			if (testDtlVO_p.getWrsData() != null) 
			{
				if (testDtlVO_p.getWrsData().next()) 
				{
					strWarrantySlNo = testDtlVO_p.getWrsData().getString("HEMNUM_WARRANTY_SLNO");
					      strMcSlNo = testDtlVO_p.getWrsData().getString(	"HEMNUM_MC_SLNO");
					      strEnggItemTypeId = testDtlVO_p.getWrsData().getString("HEMNUM_ENGG_ITEM_TYPE_ID");
					      strEnggItemSubTypeId = testDtlVO_p.getWrsData().getString("HEMNUM_ENGG_ITEM_SUB_TYPE_ID");
				}
				testDtlVO_p.getWrsData().beforeFirst();
				TestDtlDAO.getDataSingleRow(testDtlVO_p);
			}
			System.out.println("inside method");
			
			 * A Test or Inspection can be carried out on an equipment which is either in warranty or maintenance
			 * contract, not both. Either one of this can be null, not both.
			...
			} else if (strMcSlNo != null && strEnggItemTypeId !=null && strEnggItemSubTypeId !=null) {
			...
			throw new Exception(
					"EquipmentInspectionTestDtlsBO.initializeTestInspectionDtl(TestDtlVO)-->"
							+ e.getMessage());
		} finally {
			 Closing Transaction 
			if (hisDao != null) {
	 ...
	 used in equipment Test or Inspection entry Parameter multi-row value retrieval. 
	public void initializeTestInspectionDtlMultiRow(TestDtlVO testDtlVO_p) throws Exception {
	...
	//	List listOutput=new ArrayList();
		try {
			...
			testDtlVO_p.setStrMode("2");
			//System.out.println("Before calling getDATA in BO");
			TestDtlDAO.getData(testDtlVO_p, hisDao);
			//System.out.println("After calling getDATA in BO");
			while(testDtlVO_p.getWrsData()!=null && testDtlVO_p.getWrsData().next())
			{
				listInput.add(testDtlVO_p.getWrsData().getString(1));
				listOutput.add(testDtlVO_p.getWrsData().getString(2));
			}

			testDtlVO_p.setStrParameterInput((String[])listInput.toArray(new String[0]));
			testDtlVO_p.setStrParameterOutput((String[])listOutput.toArray(new String[0]));
			
		} catch (Exception e) {
			throw new Exception(
					"EquipmentInspectionTestDtlsBO.initializeTestInspectionDtlMultiRow(TestDtlVO)-->"
							+ e.getMessage());
		} finally {
			 Closing Transaction 
			if (hisDao != null) {
...
//				gbltItemBrandMstVO.setStrMode("5");
		...
		public String getStockDetails(String strHospitalCode_p,String strItemBrandId_p,String strStoreId_p,String strMode_p) throws Exception 
		{
			...
			try 
			{
				...
				{	
//				   gblItemCurrStockMstVO.setStrMode("2");
				   gblItemCurrStockMstVO.setStrMode("5");	// For HEMMS_ODISHA_BMED
				}
				else
				{
...
//				storeMstVO.setStrMode("3");
				...
				storeMstVO.setStrMode("5"); // FOR HEMMS_ODISHA_BMED
				storeMstVO.setStrSeatid(strSeatid_p);
				storeMstVO.setStrDeptCode(strDeptCode_p);
...
/*		public void getPrintData(TestDtlVO vo) throws Exception
		{
			TestDtlDAO.getPrintData(vo);
			TestDtlDAO.getItemOthrsData(vo);
			TestDtlDAO.getPrvsRsultData(vo);
			if (vo.getStrMsgType().equals("1")) {
				String strErr = vo.getStrMsgString();
				vo.setStrMsgString("EquipmentInspectionTestDtlsBO.getPrintData() --> " + strErr);
			}
