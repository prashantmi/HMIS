...
public class DiscountApprovalTransBSDATA {
	public static void initClientDetail(String strChk,
		...
		try {
			...
			HisException eObj = new HisException("Billing",
					"DiscountApprovalTransBSDATA->initClientDetail()", strmsgText);
	...
	public static String AjaxResponse(DiscountApprovalTransFB formBean,
		...
		System.out.println("modName---------------------------------------------->"+strValmode);
		strValmode=strValmode.replace("_","^");
		...
		String strTemp[] = strValmode.split("@");
		...
		try {
			if (hlp_opt_sel == 1)
				tariffDetails = DiscountApprovalTransHLPNew.getTariffDetails_BS(wb,
						vo);
			if (hlp_opt_sel == 2)
				tariffDetails = DiscountApprovalTransHLPNew.getIPDBillSetlmnt(wb,
						vo);
		} catch (Exception e) {
			strmsgText = "billing.transactions.RefundApprovalTransBSDATA.TariffDtl(vo) --> "
					+ e.getMessage();
			HisException eObj = new HisException(
					"Billing",
					"RefundApprovalTransaction->RefundApprovalTransBSDATA->TariffDtl()",
					strmsgText);
			formBean.setStrErrMsg("Application Error [ERROR ID : "
					+ eObj.getErrorID() + "], Contact System Administrator! ");
			formBean.setStrMsgType("1");
	...
	public static void getRsnRmk(DiscountApprovalTransFB formBean) 
	{
		...
		{
			strmsgText = e.getMessage();
			HisException eObj = new HisException("Billing","DiscountApprovalTransBSDATA->getRsnRmk()", strmsgText);
	...
	public static void initOnlineReq(String strChk,
		...
		try {
			...
			strmsgText = e.getMessage();
			HisException eObj = new HisException("Billing",
					"DiscountApprovalTransBSDATA->initOnlineReq()", strmsgText);
		...
		try {
			...
			try {
				new HisException("Billing",
						"DiscountApprovalTransBSDATA->PopUpResponse()",
						strmsgText);
	...
	public static boolean getInsertData(DiscountApprovalTransFB formBean,HttpServletRequest request) 
	{
		...
		{
			 e.printStackTrace();
			strmsgText = "billing.transactions.DiscountApprovalTransBSDATA.getInsertData(vo) --> "+ e.getMessage();
