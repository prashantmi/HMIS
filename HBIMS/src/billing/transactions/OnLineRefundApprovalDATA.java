...
public class OnLineRefundApprovalDATA
{
	...
	//	boolean retVal = true;
		...
		try 
		{
	      ...
	      hisutil = new HisUtil("transaction", "OnLineRefundApprovalDATA");
	      ...
	      voObj.setStrSeatId(formBean.getStrSeatId());
			...
			{
				strTodayRefundApprovalDtls  = OnLineRefundApprovalHLP.todayRefundApprovalDtls(formBean);
				      strRefundRequestDtls  = OnLineRefundApprovalHLP.refundRequestDtls(formBean);
			...
			HisException eObj = new HisException("Billing",
					"OnLineRefundApprovalDATA->getOnLineRefundApprovalDtls()", msgStr);
			...
			HttpServletResponse response, OnLineRefundApprovalFB formBean) 
	{
		...
		try {
			...
			if (vo.getStrMsgType().equals("1")) 
			{
				...
				strRes =  OnLineRefundApprovalHLP.refundRequestDtls(formBean);   
		...
		catch (Exception e) 
		{
			...
			try {
				HisException eObj = new HisException("Billing",
						"OnLineRefundApprovalDATA->GO()", strmsgText);
				...
				new HisException("Billing", "OnLineRefundApprovalDATA->GO()",
			...
			HttpServletResponse response, OnLineRefundApprovalFB formBean) 
	{
		...
		try {
			...
			if (vo.getStrMsgType().equals("1")) 
			{
			    ...
			    strRes =  OnLineRefundApprovalHLP.tarrifDtls(formBean,strSearchMode);   
			    response.setHeader("Cache-Control", "no-cache");
				response.getWriter().print(strRes);
	...
	public static boolean  SAVE(OnLineRefundApprovalFB formBean,HttpServletRequest request)
	{
		...
		try 
		{
		  ...
		  catch (Exception e) 
		  {
	        e.printStackTrace(); 
	        retValue = false;
			strmsgText = "OnLineRefundApprovalDATA.INSERT(vo) --> "
					+ e.getMessage();
			HisException eObj = new HisException("purchase",
					"OnLineRefundApprovalDATA->INSERT()", strmsgText);
