...
public class RefundApprovalTransCNT extends CSRFGardTokenAction {
	public String bsid = "";
	...
	public ActionForward GO(ActionMapping mapping, ActionForm form,
		...
		RefundApprovalTransDATA.getRsnRmk(formBean);
		RefundApprovalTransDATA.getBillDtls(formBean,request);
	...
	public ActionForward TrfDtl(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) 
	{
		...
		String sid = "";
		formBean.setStrHospitalCode(request.getSession().getAttribute("HOSPITAL_CODE").toString());
		sid = RefundApprovalTransDATA.TariffDtl(formBean, request);
		...
		{
			new HisException("Billing","RefundApprovalTransCNT","RefundApprovalTransCNT.TrfDtl()-->" + e.getMessage());
		}
		return null;
	}
	...
	public ActionForward PopUp(ActionMapping mapping, ActionForm form,
		...
		response.setHeader("Cache-Control", "no-cache");
		try {
			sid = RefundApprovalTransDATA.popUpShw(request);
			response.getWriter().print(sid);
		} catch (Exception e) {
			new HisException("Refund Approval Transaction",
					"RefundApprovalTransCNT",
					"RefundApprovalTransCNT.PopUp()-->" + e.getMessage());
		}
		return null;
	}
			...
			throws Exception, SQLException
	{
		...
		formBean.setStrHospitalCode(request.getSession().getAttribute("HOSPITAL_CODE").toString());
		retValue = RefundApprovalTransDATA.insertData(form,request, response);
		if(retValue)
		{	
