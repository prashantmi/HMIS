...
public class DiscountApprovalTransHLPNew {
...
//				System.out.println("Net Cost--->"+wb.getString(27));
		...
		*/		
		...
		{
		    e.printStackTrace();
			throw new Exception("DiscountApprovalTransHLPNew.getTariffDetails()-->"+ e.getMessage());
		}
	...
	public static String getTariffDetails_BS(WebRowSet wb,DiscountApprovalTransVO vo) throws Exception 
	{
		StringBuffer sBuffer = new StringBuffer("");
		String costUnit = null;
		try 
		 ...
		 if (wb != null && wb.size() > 0) 
		  {
			...
			sBuffer.append("<p class='subHeaders'>Discount Details</p>");
			sBuffer.append("<table class='table'>");
			sBuffer.append("<thead>");
			sBuffer.append("<tr><th>#</th>");
			sBuffer.append("<th>Tariff Name</div></th>");
			sBuffer.append("<th>Qty</th>");
			sBuffer.append("<th>Rate/Unit</th>");
			sBuffer.append("<th>Last Discount/unit</th>");
			sBuffer.append("<th>Discount/unit</th>");
			sBuffer.append("<th>Discount Type</th>");
			sBuffer.append("<th>Add with Prev Amt</th>");
			sBuffer.append("<th>Discount Amt</th>");
			sBuffer.append("</tr>");
...
//				System.out.println("Net Cost--->"+wb.getString(27));
		...
		*/		
				...
				*/
				sBuffer.append("<input type='hidden' name='strhiddTrfAddDis' id='strhiddTrfAddDis"+ i + "' value='0'>");
				sBuffer.append("<tr><td style='width:1%;'> "+"<input type='checkbox' name='trfDisApproval' id='trfDisApproval"+i+ "' value='" + i + "' onClick='new_dis_add(this,"+i+","+app_id+");'></td>");
				sBuffer.append("<td style='width:38%;'>");
				sBuffer.append(TariffName);
				sBuffer.append("</td>");
				sBuffer.append("<td style='width:4%;'>");
				sBuffer.append(qtyUnitName);
				sBuffer.append("</td style='width:9%;'>");
				sBuffer.append("<td style='width:5%;'> ");
				sBuffer.append(costUnitName);
				sBuffer.append("</td>");
				sBuffer.append("<td style='width:12%;'>");
				sBuffer.append("<a name='lastDis' id='lastDis" + i + "' value='" + LstDis + "' STYLE='CURSOR:POINTER;CURSOR:HAND;color:blue'  onClick='myFunc1(this," + i + ");'>" + LstDis + "</a>");
				...
				sBuffer.append("</td>");
				sBuffer.append("<td style='width:9%;'> ");
				sBuffer.append("<input type='textbox' class='form-control' name='opd_discount' id='opd_discount"+i+"'value='0' " +
								" maxlength='10' " +
								" onKeyUp='onEnterDiscount("+i+","+strTrfAmt+","+strReqQty+","+strQtyBaseVal+","+strDiscountUnit+","+strDiscountType+","+strRateBaseVal+");'" + 
								" readOnly ='readOnly' onkeypress='return validateData(event,7);'>");
				sBuffer.append("</td>");
				sBuffer.append("<td style='width:11%;' >");
			
				
				sBuffer.append("<select name='opd_discountType' class='browser-default custom-select' id ='opd_discountType"+i+"'" +
							//	" onChange='clickCmb("+i+");'" + 
						" onChange='onEnterDiscount("+i+","+strTrfAmt+","+strReqQty+","+strQtyBaseVal+","+strDiscountUnit+","+strDiscountType+","+strRateBaseVal+");'" + 
								" disabled='disabled'>" +
								" <option value='1'>Fixed</option>" +
								" <option value='2'>Percentage</option></select>");
				sBuffer.append("</td>");
				sBuffer.append("<td align='center' style='width:11%;'>" +
								"<input type='checkbox'" +
								"name='trfAdd_Dis' id='trfAdd_Dis"+i+"'" +
							//	"value='0'  onClick= 'addDiscountAmt("+i+");'" +
								"value='0'  onClick= 'onEnterDiscount("+i+","+strTrfAmt+","+strReqQty+","+strQtyBaseVal+","+strDiscountUnit+","+strDiscountType+","+strRateBaseVal+");'" +
				...
				sBuffer.append("<td style='width:8%;'>");
				sBuffer.append("<input type='hidden'  name='disAmt' id='disAmt" + i + "' value='0' >");
				//sBuffer	.append("<input type='hidden' name='strIpdAndReqType' id='strIpdAndReqTypeID' value='"+wb.getString(33)+"'></td>");
				sBuffer.append("<div id='disAmtDivId"+i+"'>0.00</div>");
				...
				sBuffer.append("<div class='col-sm-9'></div>");
				sBuffer.append("<div class='col-sm-2'>Total Discount Amount</div>");
				sBuffer.append("<div class='col-sm-1'><div id='totDisAmtDivId'>0.00</div><input type='hidden' name='totDisAmt' id='totDisAmt1' value=''></div>");
				sBuffer.append(" </div> ");
		  }
		 else
		   {
...
//			System.out.println("Net Cost--27->"+wb.getString(27));
	 ...
	 catch (Exception e) 
	 {
			...
			new HisException("Discount Approval Trans",
					"DiscountApprovalTransHLPNew.getIPDBillSetlmnt()-->", e
							.getMessage());
	 }
