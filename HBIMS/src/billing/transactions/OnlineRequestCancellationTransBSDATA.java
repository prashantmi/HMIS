 ...
 * 
 */
public class OnlineRequestCancellationTransBSDATA {
			...
			OnlineRequestCancellationTransFB formBean) 
	{
		...
		try 
		{
            ...
            String strPatView = PatientDtlHLPNew.patientDtl(formBean.getStrCrNo(),
					true);
			...
			{
				result = OnlineRequestCancellationTransHLPNew.onlineRequestDetails(vo);
				if (vo.getStrMsgType().equals("1")) 
				{
					throw new Exception(vo.getStrMsgString());
				}
		 ...
		 catch (Exception e) 
		 {
			...
			strmsgText = "billing.transactions.OnlineRequestCancellationTransBSDATA.onlineRequestDetails(vo) --> "
				+ e.getMessage();
	...
	public static void cancelledByCmb(OnlineRequestCancellationTransFB formBean) {
		...
		try {
			...
			strmsgText = "billing.transactions.OnlineRequestCancellationTransBSDATA.cancelledByCmb(vo) --> "
				+ e.getMessage();
			HisException eObj = new HisException("Billing",
	...
	public static void cancelReasonCmb(OnlineRequestCancellationTransFB formBean) {
		...
		try {
			...
			strmsgText = "billing.transactions.OnlineRequestCancellationTransBSDATA.cancelReasonCmb(vo) --> "
				+ e.getMessage();
			HisException eObj = new HisException("Billing",
	...
	public static void getPopUp(HttpServletRequest request,
		...
		try {
			...
			{
				strPopUpDtls = OnlineRequestCancellationTransHLPNew.getPopUpInfo(vo,index);
				if (vo.getStrMsgType().equals("1")) 
				{
					throw new Exception(vo.getStrMsgString());
				}
			...
			strmsgText = "billing.transactions.OnlineRequestCancellationTransBDDATA.getPopUp(vo) --> "
				+ e.getMessage();
			HisException eObj = new HisException("Billing",
	...
	public static void getPopUpData(HttpServletRequest request,
		...
		try {
			...
			strPopUpDtls = OnlineRequestCancellationTransHLPNew.getPopUpInfo2(vo);
			...
			strmsgText = "billing.transactions.OnlineRequestCancellationTransBSDATA.getPopUp(vo) --> "
				+ e.getMessage();
			HisException eObj = new HisException("Billing",
				"OnlineRequestCancellation->getPopUp()", strmsgText);
			...
			HttpServletResponse response)
	{
		...
		{
			strmsgText = "billing.transactions.OnlineRequestCancellationTransBSDATA.insertData(vo) --> "
				+ e.getMessage();
			HisException eObj = new HisException("Billing",
