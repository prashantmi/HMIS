...
import org.apache.struts.action.ActionForm;
	...
	public class IpdBillReOpenTransDATA {
		...
		public static void getPatDtls(IpdBillReOpenTransFB formBean,
			...
			try {
				...
				vo.setStrHospitalCode(strHospitalCode);
					...
					strPatientDtls = hisglobal.tools.hlp.PatientDtlHLP
				...
				strmsgText = "billing.transactions.IpdBillReOpenTransDATA.getPatDtls(vo) --> "
						+ e.getMessage();
				HisException eObj = new HisException(
						"Billing",
						"IpdBillReOpenTransaction->IpdBillReOpenTransDATA->getPatDtls()",
						strmsgText);
		...
		public static void getCrNo(IpdBillReOpenTransFB formBean,
			...
			try {
			   ...
			   bo.getCrNo(vo);
				...
				strmsgText = "billing.transactions.IpdBillReOpenTransDATA.getCrNo(vo) --> "
						+ e.getMessage();
				HisException eObj = new HisException(
						"Billing",
						"IpdBillReOpenTransaction->IpdBillReOpenTransDATA->getCrNo()",
						strmsgText);
		...
		public static void getBillDetails(IpdBillReOpenTransFB formBean,HttpServletRequest request) 
		{
			...
			{
				e.printStackTrace();
				strmsgText = "billing.transactions.IpdBillReOpenTransDATA.getBillDetails(vo) --> "+ e.getMessage();
				HisException eObj = new HisException("Billing","IpdBillReOpenTransaction->IpdBillReOpenTransDATA->getBillDetails()",strmsgText);
		...
		public static boolean saveData(ActionForm form,HttpServletRequest request, HttpServletResponse response) 
		{
			...
			try 
			{
				...
				e.printStackTrace();
				fretValue = false;
				strmsgText = "billing.transactions.IpdBillReOpenTransDATA.saveData(vo) --> "
						+ e.getMessage();
				HisException eObj = new HisException("Billing",
						"IpdBillReOpenTransDATA->saveData()", strmsgText);
				formBean.setStrErrMsg("Application Error [ERROR ID : "
						+ eObj.getErrorID() + "], Contact System Administrator! ");
				formBean.setStrMsgType("1");
				eObj = null;
