...
public class DiscountRecommendationTransDAO {

	public void getTariffDtl(DiscountRecommendationTransVO vo) {
		...
		try {
			dao = new HisDAO("billing",
					"transactions.DiscountRecommendationTransDAO .getTariffDtl()");
			nprocIndex = dao.setProcedure(proc_name);

			dao.setProcInValue(nprocIndex, "reqno ", strTemp[0]);
			
			dao.setProcInValue(nprocIndex, "deptCode ", vo.getStrDepartmentCode());
			...
			e.printStackTrace();
			vo.setStrMsgString("DiscountRecommendationTransDAO.getTariffDtl() --> "
					+ e.getMessage());
			vo.setStrMsgType("1");
	...
	/*
	 * public WebRowSet getIPDBill_Set(DiscountRecommendationTransVO vo, String
	 ...
	 * HisDAO("billing","transactions.DiscountRecommendationTransDAO.getIPDBill_Set()");
	 ...
	 * (Exception e) {
	 * vo.setStrMsgString("DiscountRecommendationTransDAO.getIPDBill_Set() --> "+
	 * e.getMessage()); vo.setStrMsgType("1"); } finally {
	 * 
	 * if (dao != null) { dao.free(); dao = null; } } return ws; }
	 */
	public WebRowSet getPopUpDtl(DiscountRecommendationTransVO vo) {
		...
		try {
			dao = new HisDAO("billing",
					"transactions.DiscountRecommendationTransDAO.getPopUpDtl()");
			nprocIndex = dao.setProcedure(proc_name);
			...
			vo.setStrMsgString("DiscountRecommendationTransDAO.getPopUpDtl() --> "
					+ e.getMessage());
			vo.setStrMsgType("1");
		} finally {
			if (dao != null) {
	...
	// public WebRowSet getDiscountRsn(DiscountRecommendationTransVO vo)
	// {
	// WebRowSet ws = null;
	// HisDAO dao = null;
	// int nQryIndex;
	...
	// {
	// dao = new
	// HisDAO("billing","transactions.DiscountRecommendationTransDAO.getDiscountRsn()");
	...
	// catch (Exception e)
	// {
	// vo.setStrMsgString("DiscountRecommendationTransDAO.getDiscountRsn() --> "+
	// e.getMessage());
	// vo.setStrMsgType("1");
	// }
	// finally
	...
	public WebRowSet getDiscountRsn(DiscountRecommendationTransVO VO) {
		...
		try {
			...
			dao = new HisDAO("billing",
					"transactions.DiscountRecommendationTransDAO.getDiscountRsn()");
			...
			VO
					.setStrMsgString("transactions.DiscountRecommendationTransDAO.getDiscountRsn()--> "
							+ e.getMessage());
	...
	public WebRowSet getDiscountBy(DiscountRecommendationTransVO vo) {
		WebRowSet ws = null;
		String proc_name = "";
		...
		try {
			dao = new HisDAO("billing",
					"transactions.DiscountRecommendationTransDAO .getDiscountBy()");
			nprocIndex = dao.setProcedure(proc_name);
			dao
					.setProcInValue(nprocIndex, "hosp_code", vo
							.getStrHospitalCode()); // New Value
			...
			vo.setStrMsgString("DiscountRecommendationTransDAO.getDiscountBy() --> "
					+ e.getMessage());
			vo.setStrMsgType("1");
		} finally {
			if (dao != null) {
	...
	public void getRsnRmk(DiscountRecommendationTransVO vo) {
		...
		try {
			...
			vo.setStrMsgString("DiscountRecommendationTransDAO.getRsnRmk() --> "
					+ e.getMessage());
			vo.setStrMsgType("1");
	...
	public static void callingFunctionAppSlNo(DiscountRecommendationTransVO VO,
			int i, HisDAO dao) {
		...
		try {
			//dao = new HisDAO("BillingModule", "DiscountRecommendationTransDAO");
			funcIndex = dao
			...
			VO
					.setStrMsgString("DiscountRecommendationTransDAO.callingFunctionView() --> "
							+ e.getMessage());
			VO.setStrMsgType("1");
		}
	...
	/* ---------------Calling of Function for View Charges--------------------- */
	public static void callingFunctionAppSlNo1(DiscountRecommendationTransVO VO,HisDAO dao) {
		//HisDAO dao = null;
		// WebRowSet ws = null;
		// Declearing Variable
		String retVal = null;
	...
	public static boolean getInsertDataForFinalAdjustment(
			DiscountRecommendationTransVO vo) {
		// Decleration
		HisDAO dao = null;
		...
		try {
			...
			if (vo.getApprovalId()[0].equals("0")
				...
				if (pKeyDao.getStrErr() != null
					...
					billreqtrfDao.setStrDepartmentCode(vo.getStrDepartmentCode());
					
					
					billreqtrfDao.update4(dao);
			...
			vo
					.setStrMsgString("DiscountRecommendationTrans-->DiscountRecommendationTransDAO.getInsertDataForFinalAdjustment() --> "
							+ e.getMessage());
	...
	public static boolean getInsertDataProcForServices(
		...
		try {
			...
			for (int i = 0; i < vo.getTrfDisApproval().length; i++) {
				...
				tempChkVal = Integer.parseInt(vo.getTrfDisApproval()[i].replace("^", "#").split("#")[0]); // get
			...
			vo.setStrMsgString("DiscountRecommendationTransDAO.getInsertDataProc() --> "
					+ vo.getStrMsgString());
			vo.setStrMsgType("1");
			...
			try {
				...
				vo
						.setStrMsgString("DiscountRecommendationTransDAO.getInsertDataProc() --> "
								+ e1.getMessage());
				vo.setStrMsgType("1");
			}
