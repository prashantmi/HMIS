...
public class RefundApprovalTransDATA {
	...
	public static void getBillDtls(RefundApprovalTransFB formBean,HttpServletRequest request) 
	{
		...
		try 
		{
			...
			{
				strPatientDtls = hisglobal.tools.hlp.PatientDtlHLP.patientDtl(vo.getStrCrNo(),true);
				...
				else
				{
					...
					billDetails = RefundApprovalTransHLP.getBillDetails(vo);
			...
			strmsgText = "billing.transactions.RefundApprovalTransDATA.getBillDtls(vo) --> "
					+ e.getMessage();
			HisException eObj = new HisException(
					"Billing",
					"RefundApprovalTransaction->RefundApprovalTransDATA->getBillDtls()",
					strmsgText);
	...
	public static void getRsnRmk(RefundApprovalTransFB formBean) {
		...
		try {
			...
			strmsgText = "billing.transactions.RefundApprovalTransDATA.getRsnRmk(vo) --> "
					+ e.getMessage();
			HisException eObj = new HisException(
					"Billing",
					"RefundApprovalTransaction->RefundApprovalTransDATA->getRsnRmk()",
					strmsgText);
			formBean.setStrErrMsg("Application Error [ERROR ID : "
					+ eObj.getErrorID() + "], Contact System Administrator! ");
			formBean.setStrMsgType("1");
	...
	public static String TariffDtl(RefundApprovalTransFB formBean,HttpServletRequest request) 
	{
		...
		try 
		{
			...
			{
				tariffDetails = RefundApprovalTransHLP.getTariffDetails(vo);
				// tariffDetails = null;
		...
		catch (Exception e) 
		{
			...
			HisException eObj = new HisException("Billing","RefundApprovalTransDATA->TariffDtl()", strmsgText);
	...
	public static String popUpShw(HttpServletRequest request) {
		...
		try {
			...
			vo.setStrValmode(strValmode);
			res = RefundApprovalTransHLP.popUpShw(vo);
	...
	public static boolean insertData(ActionForm form,
		...
		try {
			...
			fretValue = false;
			strmsgText = "billing.transactions.RefundApprovalTransDATA.insertData(vo) --> "
					+ e.getMessage();
			HisException eObj = new HisException("Billing",
					"RefundApprovalTransDATA->insertData()", strmsgText);
			formBean.setStrErrMsg("Application Error [ERROR ID : "
					+ eObj.getErrorID() + "], Contact System Administrator! ");
			formBean.setStrMsgType("1");
			eObj = null;
