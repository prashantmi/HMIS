...
*/
public final class PatientLFAccountDAO {
	...
	private final String strProcName = "pkg_bill_dml.dml_hblt_pat_lf_account_dtl";// "pkg_bill_dml.dml_hblt_pataccount_dtl";
...
//	Methods starts from here
	...
	public void insert(HisDAO dao) throws Exception {
		...
		try {
			...
			//set the value
			//Input Value
			System.out.println("strAccountStatus >>> DAO::"+strAccountStatus);
			if(strAccountStatus.equals("1"))
			{dao.setProcInValue(nInsertedIndex,"modval","1",1);
System.out.println("Its mode 1");
			}
			else
			{dao.setProcInValue(nInsertedIndex,"modval","2",1);
			System.out.println("Its mode 2");
			}
			System.out.println("strPatAccNo:::"+strPatAccNo);
			dao.setProcInValue(nInsertedIndex,"hblnum_pataccount_no",strPatAccNo,2);
			dao.setProcInValue(nInsertedIndex,"sblnum_chargetype_id","1",3);
			...
			System.out.println("gnum_patient_cat_code__-------"+strCatCode.split("\\^")[0]);
			
			dao.setProcInValue(nInsertedIndex,"gnum_patient_cat_code",strCatCode.split("\\^")[0],10);
			...
			dao.setProcOutValue(nInsertedIndex,"err",1,34);
...
public void update5(HisDAO dao) throws Exception {
		...
		try {
			...
			dao.setProcInValue(nUpdatedIndex,"modval","7");
			dao.setProcInValue(nUpdatedIndex,"hblnum_pataccount_no",strPatAccNo);
			dao.setProcInValue(nUpdatedIndex,"hosp_code",strHospitalCode);  
			dao.setProcInValue(nUpdatedIndex,"is_bill_final_flag",isBillFinal);
