...
public class ClientChargeMstUTL implements MasterInterface {
	...
	public String getMasterName() {
		...
		String masterName = " Client Charge Master";
		return masterName;
	}

	public String[] getColumnHeader() {

		String[] columnHdr = {  "Tariff Code", "Tariff Name","Patient Category Name","Organization","Rate / Unit",
				 "Validity(From-To)" };
		return columnHdr;
	}
	...
	public String[] getComboQuery() {
		...
		comboQuery[2] = billing.qryHandler_billing.getQuery(1, "gbl.creditcat.2").replace("?", hosCode);
		comboQuery[3] = billing.qryHandler_billing.getQuery(1,"gbl.org.2").replace("?", hosCode);
		comboQuery[4] = billing.qryHandler_billing.getQuery(1,"gbl.ipdchargetype.1").replace("?", hosCode);
	...
	public String getMainQuery(String[] cmb) 
	{
		...
		brMainQuery.append(billing.qryHandler_billing.getQuery(1, "select.clientchargeMst.0").replace("?", hosCode)).append(" AND "+ billing.qryHandler_billing.getQuery(1,"select.clientchargeMst.cond.0").replace("?", "8"));
				
		if (cmb != null) 
		{
			if (!cmb[0].equals("0")) 
			{
				brMainQuery.replace(brMainQuery.lastIndexOf("8"), brMainQuery.lastIndexOf("8") + 1, cmb[0]);				
			}
			if (!cmb[1].equals("0")) 
			{				
				brMainQuery.append(" AND "+ billing.qryHandler_billing.getQuery(1,"select.clientchargeMst.cond.3").replace("?","6"));				
				brMainQuery.replace(brMainQuery.lastIndexOf("6"), brMainQuery.lastIndexOf("6") + 1, cmb[1]);
			}
			if (!cmb[2].equals("0")) 
			{				
				brMainQuery.append(" AND "+ billing.qryHandler_billing.getQuery(1,"select.clientchargeMst.cond.1").replace("?","9"));				
				brMainQuery.replace(brMainQuery.lastIndexOf("9"), brMainQuery.lastIndexOf("9") + 1, cmb[2]);
			}			
			if (!cmb[3].equals("0")) 
			{
				brMainQuery.append("AND ").append(billing.qryHandler_billing.getQuery(1,"select.clientchargeMst.cond.4").replace("?", "3"));
				brMainQuery.replace(brMainQuery.lastIndexOf("3"), brMainQuery.lastIndexOf("3") + 1, cmb[3]);
			}			
			if (!cmb[4].equals("0")) 
			{
				brMainQuery.append("AND ").append(billing.qryHandler_billing.getQuery(1,"select.clientchargeMst.cond.2").replace("?", "7"));
				brMainQuery.replace(brMainQuery.lastIndexOf("7"), brMainQuery.lastIndexOf("7") + 1, cmb[4]);
			}
		}
	...
	public String[] getComboHeader() {
		...
		String[] comboHdr = { "0", "Hospital Service", "0", "Group Name", "0",
				"Patient Category", "0","Organization","0", "Ward Type" };
		return comboHdr;
	}

	public String getViewQuery() {
		
		return qryHandler_billing.getQuery(1, "select.clientchargeMst.11");
		// return null;
	}
