...
import billing.masters.bo.BOPatientCategoryAndPaymentModeMst;
...
import billing.masters.controller.hlp.PatientCategoryAndPaymentMappingMstHLP;

public class PatientCategoryAndPaymentModeDATA extends GenericData {
	...
	public static void GETPAYMENTMODEMAPPING(HttpServletRequest request,
			HttpServletResponse response, MasterInterface masterObj) {
		PrintWriter out = null;
		BOPatientCategoryAndPaymentModeMst bo=null;
		VOPatientCategoryAndPaymentModeMst vo=null;
		WebRowSet wb=null;
		JSONObject  payMapping=new JSONObject();
		Map count=new HashedMap();

		try {
			vo=new VOPatientCategoryAndPaymentModeMst();
			bo=new BOPatientCategoryAndPaymentModeMst();
			out = response.getWriter();
			vo.setStrHospitalCode(request.getSession().getAttribute("HOSPITAL_CODE").toString());

			vo.setStrCategory(request.getParameter("catCode"));
			bo.getPaymentModeMapping(vo);
			out.print(PatientCategoryAndPaymentMappingMstHLP.getMappingData(vo));
		    ...
		    if(wb.size()!=0) {
				while(wb.next()) {
					...
					if(payMapping.containsKey(wb.getString(2)))
					{
						if(count.containsKey(wb.getString(2)))
							count.put(wb.getString(2), count.get(wb.getString(2))+"@"+wb.getString(3));
						else
							count.put(wb.getString(2),payMapping.get(wb.getString(2))+"@"+wb.getString(3));
					}else
					payMapping.put(wb.getString(2),wb.getString(3));
					...
					if(payMapping.containsKey(wb.getString(2)))
					{
						System.out.println("RRAY SIZE:::::::::::::::::::::::::::::::"+array.size());
						if(array.size()>0) {
							if(payMapping.ge.equals(wb.getString(2))) {
								array.add(wb.getString(3));
							}								
							else {
								array = new JSONArray();
								array.add(wb.getString(3));
						...
						payMapping.put(wb.getString(2),array);
					}
				...
				for(Object key:count.keySet()) {
					...
					String temp=(String) count.get(key);
					  
					 for(int i=0;i<temp.split("@").length;i++)
						 array.add(temp.split("@")[i]);
					 payMapping.put(key,array);
				}
			} out.print(payMapping);
			*/
			
					
		} catch (Exception e) {
			e.printStackTrace();
			vo.setStrMsg(e.getMessage());
			//vo.setStrMsgType("1");
			HisException eObj = new HisException(masterObj.getMasterName(),
					"GenericData-->GETPAYMENTMODEMAPPING()", vo.getStrMsg());
			out.println("ERROR####Application Error [ERROR ID : "
					+ eObj.getErrorID() + "], Contact System Administrator! ");
			eObj = null;
		} finally {
	...
	public static void GETPAYMENTMODEMAPPINGJSON(HttpServletRequest request,
			HttpServletResponse response, MasterInterface masterObj) {
		PrintWriter out = null;
		BOPatientCategoryAndPaymentModeMst bo=null;
		VOPatientCategoryAndPaymentModeMst vo=null;
