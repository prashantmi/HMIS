...
public class LocalTariffMstVO extends GenericFormBean {
	...
	/*Variable Add by Amit Kr on Date 20 - April - 2010 */
	...
	/**
	 * @param strUndefinedCharges the strUndefinedCharges to set
	 */
	...
	/**
	 * @param strCostType the strCostType to set
	 */
	...
	public String getAddGroupCombo() {
		...
		HisUtil hisutil = new HisUtil("Billing", "VOTariffMst");
		try {
			String hosCode = this.getStrHospitalCode();

			// System.out.println("this.getStrHospitalCode()-0->"+this.getStrHospitalCode());

			grpStr = billing.qryHandler_billing.getQuery(1,
					"select.tariffMst.1").replace("?", hosCode);

			// grpStr = grpStr.concat(" and
			// ").concat(billing.qryHandler_billing.getQuery(1,
			// "select.tariffMstGrpCmb.cond.1").replace("?",this.isPackage));
			// System.out.println("comboQUERY"+grpStr);

			comboValues = hisutil.getOptionValue(grpStr, this.getGroupId(),
					"0^Select Value");

		} catch (Exception e) {
			errMsg = "Billing.VOTariffMst() -->" + e.getMessage();
			new HisException("billing", "VOTariffMst.getAddGroupCombo()",
					errMsg);
		}
		return comboValues;
	}
	...
	public String getAddDefaultUnitComboWithPkg() {
		...
		int nqryIndex = 0;
		WebRowSet wb = null;
		HisDAO dao = new HisDAO("billing", "VOTariffMst");
		try {
			String hosCode = this.getStrHospitalCode();
			strDefaultUnitId = BillConfigUtil.DEFAULT_UNIT_ID;
			// System.out.println("strDefaultUnitId with
			// Pkg-->>>>>"+strDefaultUnitId);
			strquery = billing.qryHandler_billing.getQuery(1,
					"select.tariffunitname.1").replace("?", hosCode);
			strquery = strquery.concat(" and ").concat(
					billing.qryHandler_billing.getQuery(1,
							"select.defaulttariffunitname.cond.1").replace("?",
							strDefaultUnitId));
			...
			if (this.getDefaultUnit() == null || this.getDefaultUnit() == "") {
				comboValues2 = hisutil.getOptionValue(wb, "0",
						"0^Select Value", false);
			} else {
				comboValues2 = hisutil.getOptionValue(wb,
						this.getDefaultUnit(), "0^Select Value", false);
			}

			/*
			 * if (this.getDefaultUnit() == null || this.getDefaultUnit() ==
			 * ""){ comboValues2=
			 * hisutil.getOptionValue(billing.qryHandler_billing .getQuery(1,
			 * "select.tariffMst.2").replace("?", hosCode), "0", "0^Select
			 * Value");
			 * 
			 * }else{ comboValues2 =
			 * hisutil.getOptionValue(billing.qryHandler_billing .getQuery(1,
			 * "select.tariffMst.2").replace("?", hosCode), this
			 * .getDefaultUnit(), "0^Select Value"); }
			 */

			// System.out.println("in vo unti combo----------- :
			// "+comboValues2);
		} catch (Exception e) {
			new HisException("billing", "VOTariffMst.getAddDefaultUnitCombo()",
					e.getMessage());
		}
	...
	public String getAddPackageGroupCombo() {
		...
		try {
			...
			if (this.getGroupId() == null || this.getGroupId() == "")
				comboValues1 = hisutil.getOptionValue(
						billing.qryHandler_billing.getQuery(1,
								"select.tariffMst.1").replace("?", hosCode),
						"0", "0^Select Value");
				...
				comboValues1 = hisutil.getOptionValue(
						billing.qryHandler_billing.getQuery(1,
								"select.tariffMst.1").replace("?", hosCode),
						this.getGroupId(), "0^Select Value");

		} catch (Exception e) {
			new HisException("billing",
					"VOTariffMst.getAddPackageGroupCombo()", e.getMessage());
		}
		return comboValues1;
	}
	...
	public String getAddDefaultUnitCombo() {
		...
		try {
			...
			// System.out.println("By DefaultUnitId without
			// Pkg-->>>>>"+strModuleId);
			strquery = billing.qryHandler_billing.getQuery(1,
					"select.tariffunitname.1").replace("?", hosCode);
			strquery = strquery.concat(" and ").concat(
					billing.qryHandler_billing.getQuery(1,
							"select.tariffunitname.cond.1").replace("?",
							strModuleId));
	...
	public String getAddServiceCombo() {
		...
		try {
			...
			if (this.getServiceName() == null || this.getServiceName() == "") {
				comboValues3 = hisutil
						.getOptionValue(billing.qryHandler_billing.getQuery(1,
								"select.tariffMst.30").replace("?", hosCode),
								this.getServiceName() + "^"
			...
			new HisException("billing", "VOTariffMst.getAddServiceCombo()", e
					.getMessage());
		}
		return comboValues3;
	}
	...
	public String getAddTariffCombo() {
		...
		try {
			...
			dao = new HisDAO("Billing", "VOTariffMst");
			...
			if (wb != null) {
				...
				if (wb.size() > 0 && wb.next()) {
					...
					qry = billing.qryHandler_billing.getQuery(1, strTrfQry
							.trim());
			...
			if (!qry.equals("")) {
				...
				//System.out.println("this.getTariffId() = " + this.getTariffId());
			...
			// System.out.println("Tariff Id : "+this.getTariffId());

			if (wb != null) {

				val1 = "0";
				comboValues = hisutil.getOptionValue(wb, this.getTariffId(),
						"0^Select Value", false);
			} else {
			...
			new HisException("billing", "VOTariffMst.getAddTariffCombo()", e
					.getMessage());
		}
	...
	public String getAddDepartmentCombo() {
		...
		try {
			...
			comboValues5 = hisutil.getOptionValue(billing.qryHandler_billing
					.getQuery(1, "select.tariffMst.5").replace("?", hosCode),
					this.getTempDept(), "0^Select Value");
		} catch (Exception e) {
			new HisException("billing", "VOTariffMst.getAddDepartmentCombo()",
					e.getMessage());
		}
	...
	public String getMultiRowData() {
		...
		String rowdata = "";
		try {
			rowdata = data.getMultiRowData(this);
		} catch (Exception e) {
	...
	public String getCurrentDate() {
		HisUtil util = new HisUtil("billing", "VOTariffMst");
		String strCurDate = "";
		try {
			strCurDate = util.getASDate("dd-MMM-yyyy");
		} catch (Exception e) {
			new HisException("Billing", "VOTariffMst.getCurrentDate -->", e
					.getMessage());
		}
		return strCurDate;
	}
