/*$(document).ready(function(){
	$('input[name="chk"]').addClass("disableAddButtonCls");
...
function callComboAdd(form1, mode) {
	...
	with (form1) {
		if (combo[0].value == "0") {
			...
			alert("Please Select Dossier Name");
			...
			if(temp.length>0){
				alert("The dossier with same name already exists. Please use modify "+ 
				"option to make changes in the existing dossier or add items to a new dossier.");
			}
			else{
...
function modifySupplierMst() {
	if (checkForModify() == false)
		...
		var cmbVal1= document.forms[0].combo[1].options[document.forms[0].combo[1].selectedIndex].value;
		...
		document.forms[0].comboValue.value = cmbVal1;
...
function modifyDossierItemMst() {
	if (checkForModify() == false)
		...
		var cmbVal1= document.forms[0].combo[2].options[document.forms[0].combo[2].selectedIndex].value;
...
function dossierView() {
	...
	var cmbVal3= document.forms[0].combo[1].options[document.forms[0].combo[1].selectedIndex].value;
	...
	var heg_width = "width=900,height=500,left=200,top=200";

	var myPopup = window.open(createFHashAjaxQuery("DossierMstCNT.cnt?hmode=" + mode + "&chk="+ chk + "&cmbVal3=" + cmbVal3), 'popupWindow', heg_width);// +"&cnt_page="+cnt_page+"&masterName="+document.forms[0].masterName.value,'popupWindow',heg_width,
	// 'scrollbars=yes,ALIGN=CENETER,left=100,top=100');

}

function dossierItemView() {
	...
	var cmbVal3= document.forms[0].combo[2].options[document.forms[0].combo[2].selectedIndex].value;
	...
	var heg_width = "width=1200,height=500,left=100,top=100";
	//alert(cmbVal3);
	var myPopup = window.open(createFHashAjaxQuery("DossierItemMstCNT.cnt?hmode=" + mode + "&chk="+ chk + "&cmbVal3=" + cmbVal3), 'popupWindow', heg_width);// +"&cnt_page="+cnt_page+"&masterName="+document.forms[0].masterName.value,'popupWindow',heg_width,
	// 'scrollbars=yes,ALIGN=CENETER,left=100,top=100');
...
function itemBrandMstComboAdd(form1) {
	...
	with (form1) {
		...
		/* if (combo[0].value == "0") {
			alert("Please Select Item Kit Name");
			document.forms[0].combo[0].focus();
			return;
		} else */
		if (combo[0].value == "0") {
...
function itemView() {
	...
	var myPopup = window.open(createFHashAjaxQuery("DossierItemBrandMstCNT.cnt?hmode=" + mode + "&chk="+ chk + "&cmbVal3=" + cmbVal3), 'popupWindow', heg_width);// +"&cnt_page="+cnt_page+"&masterName="+document.forms[0].masterName.value,'popupWindow',heg_width,
	// 'scrollbars=yes,ALIGN=CENETER,left=100,top=100');
	...
	if (document.forms[0].combo[0].value == "0") {
		...
		var cmbValIndex = document.forms[0].combo[0].options[document.forms[0].combo[0].selectedIndex].value;
		var cmbValIndex2 = document.forms[0].combo[1].options[document.forms[0].combo[1].selectedIndex].value;
		var cmbValIndex3 = document.forms[0].combo[2].options[document.forms[0].combo[2].selectedIndex].value;
		
		var cmbVal4 = "";
		var comboValue = cmbVal + "^" + cmbVal2 + "^" + cmbVal3 + "^" + cmbVal4;
		var myPopup = window.open(createFHashAjaxQuery("DossierItemBrandMstCNT.cnt?hmode=" + mode + "&chk=" + chk
				+ "&comboValue=" + comboValue + "&cmbVal1=" + cmbVal + "&cmbVal2="
				+ cmbVal2 + "&cmbVal3=" + cmbVal3 + "&cmbValIndex=" + cmbValIndex 
				+ "&cmbValIndex2=" + cmbValIndex2 + "&cmbValIndex3=" + cmbValIndex3), 'popupWindow', heg_width);
		// +"&cnt_page="+cnt_page+"&masterName="+document.forms[0].masterName.value,'popupWindow',heg_width,
		// 'scrollbars=yes,ALIGN=CENETER,left=100,top=100');
...
*
*/
function deleteDossierMstRecords()
{
	var combo1;
	var prevNext	=	null;
	var totalChk	=	0;
	...
	var len			= 	0;
	var no_of_combo	=	0;
	var abc="org.apache.struts.taglib.html.TOKEN";
	...
	var retValue=confirm("Deleting a dossier will delete the items mapped to it as well.\n\nAre You Sure you want to delete the dossier?");	 	
	...
	var searchColumn	=	null;
	var rowNum			=	minrow;
	var TOKEN_VALUE     = document.getElementsByName(abc)[0].value;
	...
	*/	
	...
	//xmlHttp.setRequestHeader("struts.token.name", "token");
	
	xmlHttp.setRequestHeader("TOKEN_VALUE", TOKEN_VALUE);
	...
	xmlHttp.onreadystatechange = function()
	{
		if(xmlHttp.readyState ==4 )
		{
			if(xmlHttp.status == 200 || xmlHttp.status=='complete' )
			{
				...
				responseData	=	response.split("####");
				//alert("responseData"+response.split("$$$$")[1]);
				//alert("main "+response.split("$$$$")[1]);
				//check for error
				...
				else
				{
					...
					document.getElementById("searchid").innerHTML	= 	responseData[5];
					document.getElementById("footer").style.display	=	'block';					
					document.getElementsByName(abc)[0].value=response.split("$$$$")[1];					
