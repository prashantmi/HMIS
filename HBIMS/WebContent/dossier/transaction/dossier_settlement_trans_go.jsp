...
<script language="Javascript" src="/HBIMS/dossier/js/dossier_settlement_trans.js"></script>
		...
		if(conf)
			{
			   if($("#requestMode").val()=='OT'){    //Added By Ranjit for Dossier OT Integration
				  ...
				  $.ajax({
					     url:'/HBIMS/dossier/transaction/DossierSettlementCNT.cnt?hmode=INSERTOT',
...
function getDossierOT(){
	...
	document.getElementById("dossierId").innerHTML="";
	  ...
	  $.ajax({
          url: "/HBIMS/dossier/transaction/DossierSettlementCNT.cnt?hmode=getOTDossierRadio",
          type: "POST",
          cache: false,
          data: {
...
function rejectDossier(otReqNo,opCode,dossCode){
	  ...
	  $.ajax({
          url: "/HBIMS/dossier/transaction/DossierSettlementCNT.cnt?hmode=rejectDossier",
          type: "POST",
          cache: false,
          data: {
        	  otReqNo:$("#otPacReqNo").val(),
