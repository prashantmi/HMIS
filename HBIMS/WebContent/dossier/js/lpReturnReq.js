...
function SAVERETURN(){
	...
	if(!(document.forms[0].strReturnQty)){
		...
		return false;
			...
			var hisValidator = new HISValidator("DossierDeskBean"); 
		  ...
		  if(retVal){
			  if(!(document.forms[0].strQtyText1))
				  retVal=chkValReturn();
			  else
				{
					  //alert(document.getElementsByName("strQtyText1").length);
					 // alert(document.getElementById('strQtyText11-1').value);
					  for(var i=1;i<document.getElementsByName("strQtyText1").length;i++)
					  {
						 // alert(document.getElementById('strQtyText11-'+i).value);
						  if(document.getElementById('strQtyText11-'+i).value == '' || document.getElementById('strQtyText11-'+i).value =='0')
						  {
							  alert("Please Enter Qty of Extra Item(s)");
							  document.getElementById('strQtyText11-'+i).focus();
							  return false;
						  }
						  
					  }
					  totamt=document.forms[0].strNetCost1.value;
					  var accBal=document.forms[0].strBillingHiddenValue.value;
						var r=parseInt(((accBal.split('^')[1]))-totamt);
