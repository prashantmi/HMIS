 ...
 function cancelSampleSent() 
  {
	...
	hisValidator.addValidation("strLabId",   "dontselect=0","Please Select a Lab Name");    	
	...
	if (retVal) 
	{
		    ...
		    var hiddObj = document.getElementsByName("strCheckHidValue");
	        ...
	        if(!res=="")
	         {
		           alert("You Are Going To Cancel  [ "  + (len - countChk) +"  ] Records" );
...
{          
      if(obj.value.split("^")[1]!='0')
      {
	             ...
	             var x,y ;
				  ...
				  {
				   // alert("Select Name:::::"+obj.value.split("^")[0]+"::Item name::::"+itemNameObj[i].value.split("^")[0]);
					 if( (obj.value.split("^")[0] == itemNameObj[i].value.split("^")[0]) && (obj.value.split("^")[1] == batchNoObj[i].value.split("^")[0]))
					 {					    
					     x = divIndexObj[i].value;
					     y = itemNameObj[i].value.split("^")[1];
...
function cancel()
{
	 	document.forms[0].hmode.value="INITNEWSAMPLESENT";
		document.forms[0].submit();
}
function cancel1()
{
...
function pageResetMethod()
{
	...
	objVal = document.getElementById("newSampleSentDivId");
    objVal.innerHTML="";
  ...
  */
  function cancelViewPage() 
  {
  	document.forms[0].strStoreId.disabled=false;
	    document.forms[0].strItemCategoryId.disabled=false;
	    document.forms[0].strLabId.disabled=false;
	    document.forms[0].strSearchNameType.disabled=false;
  ...
  function validate1() 
  {
	...
	if(saveObj.style.display == "" || saveObj.style.display == "block" || saveObj.style.display == "Block" || saveObj.style.display == "BLOCK")
	{
		saveObj.style.display = "none"; 
		    ...
		    hisValidator.addValidation("strDSampleIssueQty", "req","Issue Quantity is a Mandatory Field");
			hisValidator.addValidation("strDSampleCodeNumber", "req","Secret Code Number is a Mandatory Field");
			...
			if (retVal) 
			{
				    ...
				    var chkObj = document.getElementsByName("strDSampleIssueQty");
					var len = chkObj.length;
					var countChk = 0;
					
					for ( var i = 0; i < (len-1); i++)
					{
						if (chkObj[i].value=='0') countChk = countChk + 1;
					}
				
					if (countChk == (len-1)) 
					{
						alert("Enter Issue Qty for atleast one drug !!");
						chkObj[0].focus();
...
{
	var issueQty = document.getElementById("strDSampleIssueQty" + index).value;
	if (issueQty != "") 
	{				
		...
		if (parseFloat(avlQtyBaseVal) < parseFloat(issueQty)) 
		{
			...
			document.getElementById("strDSampleIssueQty" + index).value = "0";			
			return false;
		}
...
function CompairWithAvlQty(index,mode)
{
                var inHandQty = document.getElementById("inHandQty"+index).value;
                var  issueQty = document.getElementById("strDSampleIssueQty"+index).value;
                 var  inHand  = parseInt(inHandQty);
                 var   issue  = parseInt(issueQty);
                if(issue > inHand)
                {
                  alert("Issue Quantity should not be greater than available Quantity!!!");
                  document.getElementById("strDSampleIssueQty"+index).value = '0';
                  return false;
                }
...
function getItemCategorys(strCombo) 
{
	    ...
	    else
	    {
	    	document.getElementById("itemCategoryDivId").innerHTML ="<select name='strItemCategoryId' class='comboNormal'>" +
 		    ...
 		    document.getElementsByName("strLabId")[0].value="0";
...
{		
   if(document.getElementsByName("strStoreId")[0].value=="0")
   {
	...
	if(document.getElementsByName("strLabId")[0].value=="0")
	{
		alert("Please Select Lab Name From Combo");
		return false;
	}
	
	if(document.getElementsByName("strSearchNameType")[0].value=="0")
	{
		alert("Please Select Status Combo");
		return false;
	}
	...
	else
	{
	    ...
	    //document.forms[0].strSearchNameType.disabled=true;
		...
		+"&labId="+document.forms[0].strLabId.value+"&searchType="+document.forms[0].strSearchNameType.value;
...
function getQcDrugList() 
{
    ...
    if(document.forms[0].strStoreId.value!='0' && document.forms[0].strItemCategoryId.value!='0' && document.forms[0].strLabId.value!='0')
    {
	...
	else
	{
	  ...
	  {
	  	alert("Please Select HQ Name!!!!");
	  	document.forms[0].strStoreId.focus();
	  	
	    return false;
	  }
	  else
	  {
	  	if(document.forms[0].strItemCategoryId.value =='0')
	    {
	  	   alert("Please Select Category!!!!");
	  	   document.forms[0].strItemCategoryId.focus();
	       return false;
	    }
	    else
	    {
	    	alert("Please Select Lab Name!!!!");
	    	document.forms[0].strLabId.focus();
	        return false;
	    }
	  }
...
function getItemSelectPopup()
{
	   ...
	   var StoreCmb    = document.forms[0].strStoreId.value;
	   var ItemCatgCmb = document.forms[0].strItemCategoryId.value;
	   var LabCmb      = document.forms[0].strLabId.value;
	   
	   if(StoreCmb!=0 && ItemCatgCmb!=0 && LabCmb!=0)
       {	
		    var strModeVal 					= "3" ; 
			var strRequestType              = "73";
			var strItemCategory 			= ItemCatgCmb;
			var strFromStoreId 				= StoreCmb;
			var strMultiRowCompArray 		= new Array('itemParamValue','itemCalcValue','itemUserValue','strDSampleIssueQty','strDSampleCodeNumber');
			var strMultiRowCompTypeArray 	= new Array('t','t','t','t','t');
			var testFunction                = "";
			var arg                         = "";  		
			var unitMode = "";  // only base unit  1 Means Only Base Unit Show 0 Means All Unit Show in Unit Combo
			var strUserInfo                 = document.forms[0].strResendFlag.value;
					
			var strMultiRowFetchDataArray 	= new Array('1','11','4','12');		    
		    var layerIndex = "1";
	        searchItemsWithUserFunction ( strModeVal , strItemCategory , strRequestType, strFromStoreId, strMultiRowCompArray , strMultiRowCompTypeArray , strMultiRowFetchDataArray , layerIndex, testFunction,arg,strUserInfo,unitMode);
