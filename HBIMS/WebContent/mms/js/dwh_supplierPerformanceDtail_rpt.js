...
{	
	...
	else
	{
		...
		var retFlag = confirm("Whether Complete Payment Detail Required !!");
		if(retFlag)
			retFlag = "1";
		else
			retFlag = "0";
		
	    var myPopup = window.open("SupplierPerformanceDtailRptCNT.cnt?hmode="+mode+ "&supplierId="+ suppId+"&groupCode="+grpId+"&drugCode="+drugId.split("^")[1]+"&fromDate="+document.forms[0].strFromDate.value+"&toDate="+document.forms[0].strToDate.value+"&suppName="+suppName+"&drugName="+drugName+"&strCheckHidValue="+strCheckHidValue+"&poNumber="+poNumber+"&flag=" + retFlag, 'popupWindow', heg_width);
	}
}
