...
{ 
    ...
    {
    	var url="ReturnFromTransCNT.cnt?hmode=ISSUEDETAILS&issueNo="+document.forms[0].strIssueNumber.value;	
...
function getIssueDetailsBasedOnPatientNameOrCrNo()
{
	 var hisValidator = new HISValidator("returnFromTransBean");
...
//	hisValidator.addValidation("strFromDate", "dtltet=${returnFromTransBean.strCtDate}" , "From Date should be Less than Equal to Current Date");
  ...
  var retVal = hisValidator.validate();
          ...
          if(retVal)
          {
                       document.forms[0].hmode.value = "GETISSUEDETAILS";
                       document.forms[0].submit();
           }else
           {
           		return false;
...
function getPatDtlsBasedOnPatientIssueNo()
{

	
		 var hisValidator = new HISValidator("returnFromTransBean");
	
		if(document.forms[0].strPatientDetails)
		{
			...
			for(var i=0;i<document.getElementsByName("strPatientDetails").length;i++)
			{
				if(document.getElementsByName("strPatientDetails")[i].checked==true)
				{
					var arrCrAndIssueNo=document.getElementsByName("strPatientDetails")[i].value.split("^");
					
					strTempCrNo=arrCrAndIssueNo[0];
					strTempIssueNo=arrCrAndIssueNo[1];
				...
				if(count==0)
				{
					alert("Select Atleast One Of The Issue Details And Then Click Ok");	
					return false;
				}
				else
				{
					document.forms[0].strIssueNumber.value=strTempIssueNo;
					document.forms[0].strCrNo.value=strTempCrNo;
					document.forms[0].crNo.value = document.forms[0].strCrNo.value;
					document.getElementsByName("flagWithoutCrNo")[0].value='1';
				//	document.forms[0].strStoreId.value = document.forms[0].strId.value;
