...
function savedata() 
{
	       ...
	       var hisValidator = new HISValidator("SolutionDeskTransBean");
	       if(document.forms[0].strApproved.checked == true && document.forms[0].strRejected.checked == false )
	       {
	        hisValidator.addValidation("strprobsol", "req", "Please Enter the Solution");
	        var retVal = hisValidator.validate();
		    hisValidator.clearAllValidations();
		    }
		    if(document.forms[0].strRejected.checked = true && document.forms[0].strApproved.checked == false)
		    {
	        hisValidator.addValidation("strremarks", "req", "Please Enter the Reason in Remarks for Reject the Problem");
	        var retVal = hisValidator.validate();
		    hisValidator.clearAllValidations();
		    }
...
/* Value Pass in Web Row Set
	...
	{
	 var mode="DOWNLOAD";
	var url="SolutionDeskCNT.cnt?hmode="+mode+"&hiddenValue="+document.getElementById("strHidVal"+index).value;
	//alert(url);
	//ajaxFunction(url,"1");  
	url = (url.replace(/%/g, '@$@$'));
