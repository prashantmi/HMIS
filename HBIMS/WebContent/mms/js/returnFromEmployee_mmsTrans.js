/* Java Script for Return From Employee */
...
function goFunc()  
{
	var hisValidator = new HISValidator("returnFromEmployeeBean");

	hisValidator.addValidation("strStoreId" , "dontselect=0" , "Select Drug Warehouse Name");
	hisValidator.addValidation("strItemCategoryNo" , "dontselect=0" , "Select Item Category");
	hisValidator.addValidation("strEmpNo" , "req" , "Employee No. is a Mandatory Field" );
	hisValidator.addValidation("strEmpNo" , "minlen=14" , "Employee No. must be of 14 Digits" );
	...
	document.forms[0].empNo.value = document.forms[0].strEmpNo.value;
...
{     
   ...
   {
    document.getElementById("EmployeeDivId").style.display="block";
   ...
   else
   {
    ...
    document.getElementById("EmployeeDivId").style.display="none";
...
function itemCategoryCombo()
{
   ...
   var url="ReturnFromEmployeeTransCNT.cnt?hmode=ITEMCATEGORY&storeid="+document.forms[0].strStoreId.value;
                                  
   ajaxFunction(url,"1");
   }else{
        objVal= document.getElementById("itemCategoryId");
		  objVal.innerHTML = "<select name ='strItemCategoryNo' class='comboNormal' id='strItemCategoryNo'><option value='0'>Select Value</option></select>";
   }
} 
...
{ 
    ...
    var url="ReturnFromEmployeeTransCNT.cnt?hmode=ISSUEDETAILS&issueNo="+document.forms[0].strIssueNo.value;
    ajaxFunction(url,"2");
...
function getAjaxResponse(res,mode)
{
	...
	{	
	   ...
	   if(temp1[0] == "ERROR")
	   {
        ...
        {
          objVal= document.getElementById("itemCategoryId");
		  objVal.innerHTML = "<select name ='strItemCategoryNo' class='comboNormal' id='strItemCategoryNo'>" + res + "</select>";
        }
    }
 ...
 function quantityUnitValue(index)
{
	if(document.getElementById('strReturnQtyUnitId'+index).value!='0'){
		...
		if(returnValue>parseFloat(balQty))
		{
			...
			if(isNaN(cost)){
				document.getElementById('strTotalCostDivId'+index).innerHTML = "0.00";
					document.getElementById('strTotalCost'+index).value = "0.00";
			}else{			
			...
			if(isNaN(netCost)){
				document.getElementById('strNetCostDivId').innerHTML = "0.00";
				document.getElementById('strNetCost').value = "0.00";
			}else{
				document.getElementById('strNetCostDivId').innerHTML = "Rs. "+netCost;
...
function openItemName(obj,index)
{
        ...
        var objVal4 = document.getElementById("6");
...
function validate1()
{
     ...
     hisValidator.addValidation("strStockStatusCode","dontselect=0","Select Stock Status Code" );
     hisValidator.addValidation("strRecommendDate", "date","Recommend Date is a mandatory field");
	 //hisValidator.addValidation("strRecommendDate", "dtgtet=${returnFromEmployeeBean.strCtDate}" , "Recommend Date should be Equal to Current Date");
