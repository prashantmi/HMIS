...
{
	var url = "TransferDemandReqTransCNT.cnt?hmode=TRANSFERDTL&strHiddenValue="+ document.getElementById("strHiddenValue"+index).value;
	ajaxFunction(url, "12");
...
function drugDtl(obj,index) 
{
   ...
   {	
	var url = "TransferDemandReqTransCNT.cnt?hmode=TRANSFERITEMDTL&strTransferNo="+ document.getElementById("strTransferNo"+index).value+"^"+index+"^"+document.getElementById("strTransferStoreId"+index).value;
  	ajaxFunction(url, "13");
   }
   else
   {
...
function getSubGrpAndGenericItem()
{
	...
	objVal.innerHTML = "<select name ='strSubGroupCode' class='comboMax'   onChange = 'ajaxItemBrandName();' >Select Value</select>";
	   ...
	   var url = "TransferDemandReqTransCNT.cnt?hmode=SUBGRPNAMECMB&GroupId="+ document.forms[0].strGroupId[document.forms[0].strGroupId.selectedIndex].value + "&storeId="+ document.forms[0].strStoreId.value;
       ajaxFunction(url, "9");
...
function getUnitName() 
{
  ...
  {	
   ...
   document.getElementById("unitNameId").innerHTML=document.forms[0].strItemBrandId[document.forms[0].strItemBrandId.selectedIndex].value.split("^")[1];
   document.getElementById("avlaibleQtyId").innerHTML="Avl Qty. ="+document.forms[0].strItemBrandId[document.forms[0].strItemBrandId.selectedIndex].value.split("^")[2]+"  "+document.forms[0].strItemBrandId[document.forms[0].strItemBrandId.selectedIndex].value.split("^")[1];
  }
  else
  {
  	document.getElementById("DrugNameId").innerHTML="";
