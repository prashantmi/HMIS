...
function validate(){
	...
	var hisValidator = new HISValidator("fsnxyzAnalysisRpt");
	...
	if(retVal){
		...
		var dateDifference = dateDiff(document.forms[0].strFromDate.value,document.forms[0].strToDate.value);
			
		var daysDiff = ((document.forms[0].strYnDays2.value)-(document.forms[0].strYnDays1.value));
		//alert(dateDifference);
		//alert(daysDiff);
		dateDifference = dateDifference.split(' ')[0];
		
		 	
		if(parseInt(dateDifference) < parseInt(daysDiff)){
			
				alert("Date Difference should be greater than equal to \n days defined for Y/N in configuration file");
				return false;
			}
	}
...
function getItemCatCmb(){ 
		...
		if(document.forms[0].strStoreId.value!=0){
			var url ="FSNXYZAnalysisRptCNT.cnt?hmode=ITEMCATCMB&storeId="+document.forms[0].strStoreId.value;
	 		ajaxFunction(url,"1");
	 	}else{
...
<html:form action="/reports/FSNXYZAnalysisRptCNT" method="post">
	
<div class="errMsg" id="errMsg"><bean:write name="fsnxyzAnalysisRpt" property="strErrMsg"/></div>
<div class="normalMsg" id="normalMsg"><bean:write name="fsnxyzAnalysisRpt" property="strNormalMsg"/></div>
<div class="warningMsg" id="warningMsg"><bean:write name="fsnxyzAnalysisRpt" property="strWarningMsg"/></div>


	<tag:tab tabLabel="FSN / XYZ Analysis"
	...
	<tr class="HEADER">
		...
		<tr>
			...
			<td width="50%" colspan="2" class="CONTROL">
				<select name="strStoreId" class="comboNormal" onchange="getItemCatCmb();">
					<bean:write name="fsnxyzAnalysisRpt" property="strStoreValues" filter="false" />
				</select>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td class="CONTROL" colspan="2"><dateTag:date name="strFromDate"
				value="${fsnxyzAnalysisRpt.strCurrentDate}" /></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td width="50%" colspan="2" class="CONTROL">
			<html:checkbox property="strIsFooter" name="fsnxyzAnalysisRpt" value="1"></html:checkbox>
			</td>
...
<input type="hidden" name="strCurrentDate" value="${fsnxyzAnalysisRpt.strCurrentDate}"/>
<input type="hidden" name="strYnDays1" value="${fsnxyzAnalysisRpt.strYnDays1}"/>
<input type="hidden" name="strYnDays2" value="${fsnxyzAnalysisRpt.strYnDays2}"/>
</html:form>
