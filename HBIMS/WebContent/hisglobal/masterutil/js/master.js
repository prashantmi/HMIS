...
function addLegends(){
	$('.nextPrevTag').closest('td').prepend('<div align="left" style="float: left;"><font style="font-size: 13px;"><b>Legends: </b><span>Click on <a style="text-decoration:none;" href="javascript:;" onclick="$(\'.prevPageBtn\').click();">...prev</a> and <a style="text-decoration:none;" href="javascript:;" onclick="$(\'.nextPageBtn\').click();">more...</a> to switching pages. </span></font></div>');
}
...
function newDesk(){ 
	var table = $('#mainDataTableId').dataTable( {   
        /*"scrollCollapse": true, 
        "jQueryUI":       true,
        "paging":         true,   
        "ordering": true,   
...
function userInterface()
{
	...
	generateReport();
	//COMMENTED THIS AJAX FUNCTION TO REMOVE LOGIC OF REPORT INTERFACE. SHOW ALL DATA ALWAYS IRRESPECTIVE OF REPORT INTERFACE
	/*xmlHttp.onreadystatechange = function()
	{
		if(xmlHttp.readyState ==4 )
		{
...
{ 
	...
	var strNoOfRecordPerPages="99999" ;
...
function edit(mode)
{
	if(checkForModify()==true)
	   {
		//document.forms[0].hmode.value=mode;
		document.forms[0].hmode.value="MODIFY_NEW";////FOR SECURITY GENERATE TOKEN FROM LIST PAGE
		document.forms[0].submit();
		}
}
...
{
	//document.forms[0].hmode.value=mode;
	document.forms[0].hmode.value="ADD_NEW";//FOR SECURITY GENERATE TOKEN FROM LIST PAGE 
	document.forms[0].submit();
...
function openPopUp_master(url,width,height,index,cntrlObj)
{
	...
	if(flag == 0) {
		...
		child = window.open(url, "popupWindow", windowFeatures);
		child.focus();
		
		//openURLInPopup(url,width,height,left,top);
		
		//openURLInPopup(url,700,400,50,50);
...
function deleteRecordsOnRow(mode,obj2)
{
	...
	var obj = document.getElementsByTagName("img");
	 ...
	 {
	 	if(obj[i].src.indexOf("trash.png") >-1)
	 	{
	 		...
	 		var retEval=eval(deleteRecords("DELETE",this));
	 		if(retEval==false)
	 			break;
...
function assignClassToButtons()
{
	...
	var obj = document.getElementsByTagName("img");
	 for(var i = obj.length-1 ; i >= 0; i--)
	 {
	 	...
	 	||obj[i].src.indexOf("btn-rpt.png") >-1)
	 	{
...
//added on 17/5/2018 for Complaint Escalation
function view1()
{
	document.getElementById('message').style.display='none';

	if(checkForView1()==false) return false;
...
//added on 17/5/2018 for Complaint Escalation
function checkForView1(){
	var totalChk=0;
	var combo1=document.forms[0].combo[0].value;
	var combo2=document.forms[0].combo[1].value;
	//alert(combo1);
	//alert(combo2);
	...
	else
	{
		if(combo1==0)
			{
				alert("please selecet Engineering Item Type");
				if(combo1 !=0 && combo2==0)
					alert("please selecet Engineering Item Sub Type");
				location.reload();
				return false;
...
/*
 * To display view function added by swapnil
 */
function  viewBS()
{
     document.getElementById('message').style.display='none';
	...
	*/
	...
	var url = "../../hisglobal/masterutil/master/view_mstTemp_gblNew.jsp?mode="+mode+"&chk="+chk+"&cnt_page="+cnt_page+"&masterName="+document.forms[0].masterName.value;

	var params= document.forms[0].action + "?hmode=VIEWDATABS&chk="+chk;
	$('#fetchviewdata').load(params);
