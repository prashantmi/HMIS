...
<head>
<meta charset=utf-8>
<title>Single Window Admission</title>
...
<script src="../../ipd/js/patientAdmissionModiBS.js"></script>
...
<body onload="view();showPatientListingWindowforPageBS('8',document.forms[0].strCrNo);onchangeCountry(); checkMsg();" onUnload="">

<html:form action="/transactions/PatientAdmissionModificationTransBSCNT" method="post" styleId="form1">

<fieldset form="form1">
	
  <legend style="width:auto; margin-bottom: 0px;font-size: 1.5rem;font-weight: 400;margin-left:10px;color: #666;" id='nonPrintableLegend'>Single Window Admission</legend>
...
<!-- 	<button  type="button" id="admitButton" class="float-right btn btn-outline-success mt-1 btn-circle" tabindex='2' onClick="return validate1();">					
  ...
  <div class="legend3" id='nonPrintableLegend3'>
 	<logic:equal name="patientAdmissionModiTransBean" property="strAdmissionChargeAtCounter" value="1">	
		<font color='red'>Admission Fee Deposited: <i class="fa fa-rupee-sign"  title="admFee"></i>
			<bean:write property="strAdmissionChargeValue" name="patientAdmissionModiTransBean" filter="false" />			
		</font>
			<input type='hidden' name='strAdmissionChargeValue' value="${patientAdmissionModiTransBean.strAdmissionChargeValue}" />
			<input type='hidden' name='strAdmissionCharge' value="${patientAdmissionModiTransBean.strAdmissionChargeValue}" />		
	</logic:equal>
  </div>
 
	<div  class="viewport" id="nonPrintable">
		<div class="container-fluid">
					<div class="alert alert-danger alert-dismissible fade show" id="errMsg"  style="display:none;"><bean:write name="patientAdmissionModiTransBean" property="strMsgString"/></div>
					<div class="alert alert-success alert-dismissible fade show" id="normalMsg" style="display:none;"><bean:write name="patientAdmissionModiTransBean" property="strMsg"/></div>
					<div class="alert alert-warning alert-dismissible fade show"  id="wrnID" style="display:none;"><bean:write name="patientAdmissionModiTransBean" property="strWarningMsg"/></div>			  	<div class="row justify-content-center" >
								<div class="col-md-12">												
							  ...
							  <pDtl:patDtlNew crNo="${patientAdmissionModiTransBean.strCrNo}" address="false"></pDtl:patDtlNew>
							  </div>
		...
		<div class="prescriptionTile" id="newModificationFormId" style="display: none">
			<p class="subHeaders"><i class="fas fa-university" style="font-size: 26px;">&nbsp;</i>Patient Address Details</p>	          	
		      	<div id="newAddressModiId"><bean:write name="patientAdmissionModiTransBean" property="strAddressModi" filter="false" /></div>
			    	...
			    	<div class="col-sm-12 col-md-12">	 
						<div id="wardDivId" style="display: none">
					 		<div class="" style="/*max-width: 1230px;*/">
					 			<div class=""><bean:write property="strDeptUnitWardRoomCatConsulatant" name="patientAdmissionModiTransBean" filter="false" /></div>
							</div>
						</div>
			...
			<div  id="emrgencyDivId" style="display:none">
		        <button type="button" class="btn btn-info btn-sm btnbg" onclick='emgDiv();' data-toggle="collapse" data-target="#emgid" ><i class="fa fa-ambulance"></i>&nbsp;&nbsp;Emergency Contact Details</button>
			   	<div id="emgid" class="collapse">
				   	<br><br>	     
					<div id="emgAddressDiv" style="display: block;"><bean:write name="patientAdmissionModiTransBean" property="strEmgAddress" filter="false"/></div>
				</div>	
		        <div id="PatientOccId"></div>
			    <div id="PatientOccDtl" style="display: none"><bean:write name="patientAdmissionModiTransBean" property="occupationDetailValues" filter="false" /></div>	 			 	 	
		 	</div>
		...
		<input type="hidden" name="isSingleMenu" value="${patientAdmissionModiTransBean.isSingleMenu}">
						                 ...
						                 <button  type="button" id="savebutton" class="btn btn-success btn-sm"  onClick="return validate1();" name="patientAdmissionModiTransBean"  data-toggle="modal" data-target="#validateModal">Save</button>					
...
<div class="modal fade" id="printModal" role="dialog" >
  <div class="modal-dialog modal-lg" role="document" style="">
	...
	<logic:equal name="patientAdmissionModiTransBean"  property="strSaveFlag" value="1">
		<tiles:insert  page="/ipd/transactions/PatientAdmissionTransBSCNT.cnt?hmode=PRINTSLIP&strCrNo=${patientAdmissionModiTransBean.strPatientCrNo}&strAdmNo=${patientAdmissionModiTransBean.strAdmNo}&duplicateMode=1"/>
	</logic:equal>



					<%-- <logic:notEqual value="1" name="patientAdmissionModiTransBean" property="strSaveFlag">
