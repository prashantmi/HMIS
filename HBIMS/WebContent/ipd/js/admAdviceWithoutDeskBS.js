		...
		if(document.forms[0].strWard.selectedIndex != 0)
		{
		    ...
		    var temp=document.forms[0].strWard.value.split("^");
       		...
       		for(var i=0;i<temp.length-1;i++)
       			temp1+="@"+temp[i+1];
			var url='AdmissionAdviceTransCNT.cnt?hmode='+hmode+'&wardCode='+temp1+'&bDate='+document.forms[0].strPropAdmissionDate.value+'&deptname='+document.forms[0].strDepartment.value+'&sexCode='+document.forms[0].strSex.value+'&age='+document.forms[0].strAge.value+'&treatmentCategory='+document.forms[0].strPrimaryCategory.value
...
function checkDuplicate()
{
		if(document.forms[0].strWard.value!=0)
		{
			...
			var url="/HBIMS/ipd/transactions/AdmissionAdviceTransCNT.cnt?hmode="+mode+"&wardCode="+temp1+"&deptUnitCode="+document.forms[0].strUnitValue.value+"&crno="+document.getElementsByName("patCrNo")[0].value;
			//alert(url);
...
function getOfflineUnit()
{
		if(document.forms[0].strDepartmentValue.value!="0")
		{
	    	...
	    	"<option value='0'>Select Value</select>";
}
function getAdvanceAmountOnCatChange()
{
			...
			var temp=document.forms[0].strWard.value.split("^");
       		...
       		for(var i=0;i<temp.length-1;i++)
       			temp1+="@"+temp[i+1];
			var url="/HBIMS/ipd/transactions/AdmissionAdviceTransCNT.cnt?hmode="+mode+"&puk="+document.forms[0].strCrNo.value+"&catcode="+document.forms[0].strTreatment.value+"&wardCode="+temp1;
			//alert(url);
		    ajaxFunction(url,"6");
	        }
}
...
{    
		...
		//var url="/HBIMS/ipd/transactions/AdmissionAdviceTransCNT.cnt?hmode="+mode+"&puk="+document.getElementsByName("patCrNo")[0].value+"&catcode="+document.forms[0].strTreatment.value+"&wardType="+document.getElementsByName("strWard")[0].value.split("^")[1];
	     var url="/HBIMS/ipd/transactions/AdmissionAdviceTransCNT.cnt?hmode="+mode+"&wardCode="+temp1+"&catcode="+document.forms[0].strTreatment.value;
		ajaxFunction(url,"7");
}
...
function checkMsg()
{
	...
	if (err.innerHTML != "") {
		...
		err.innerHTML="<i class='far fa-thumbs-up'></i>"+"&nbsp;&nbsp;&nbsp;<strong>Success!</strong>&nbsp;&nbsp;"+nor.innerHTML+"<button type='button' class='close' data-dismiss='alert'>&times;</button>";
		err.style.display = "";
