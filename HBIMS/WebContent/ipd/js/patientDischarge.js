...
{ 
	...
	var reqDate=dischargeDate.split(" ")[0];
	console.log(reqDate);
	  var arr=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
	  var i=0;
	  for (i=0;i<11;i++)
		  {
		  	if(arr[i]==reqDate.split("")[1])
		  		break;
		  }
	  //console.log(reqDate.split("-")[2],i,reqDate.split("-")[0]);
	var dt=new Date(reqDate.split("-")[2],i,reqDate.split("-")[0]);
	var today=new Date();
	
  
	
	
	console.log(compareDate(reqDate,today.getDate()+'-'+(today.toString()).split(" ")[1]+'-'+today.getFullYear()));
	//var dt1=new Date(dt.getFullYear(),)
	console.log(dt);
	console.log(today);
	/*alert("dt"+dt);
	alert("today"+today);*/
	
	//console.log(dateDiff("01-01-2020","01-01-2020"));
	var retValues=compareDate(reqDate,today.getDate()+'-'+(today.toString()).split(" ")[1]+'-'+today.getFullYear());
	console.log(today.getDate()+'-'+(today.toString()).split(" ")[1]+'-'+today.getFullYear());
	console.log(retValues.mode);
	if(retValues.mode == 2)
	{
		alert("Discharge Date can not be greater than Current Date");
	    return false;				                 
	}
	var admissionDate=(document.getElementsByName("bedallotdtme")[0].value).split("/")[0];
	var admissionTime=(document.getElementsByName("bedallotdtme")[0].value).split("/")[1];
	...
	if(strDisDateTime < strAdmDateTime)
		{
		alert("Invalid Discharge Date/ Discharge Date can't be less than Bed Allocation Date");
			return false;
		}
	...
	if(document.forms[0].strDischargeType.value==2)//---Offline
	{
		...
		hisValidator.addValidation("strTransferUnit", "dontselect=0", "Discharge Type is Mandatory Field." );
		//alert("4");
...
function getDisBy()
{
            ...
            {   
               ...
               //alert(document.forms[0].strCurrentDeptUnitRoom.value);
               ...
               var temp=document.forms[0].strCurrentDeptUnitRoom.value.split("^");
          	  ...
          	  var url="PatientFinalDischargeCNT.cnt?hmode="+mode+"&curDept="+temp1;
		       //alert(url);
		       ajaxFunction(url,"10");
		    } 
}
...
function printReport(){
	...
	if(document.forms[0].strSaveStatus.value == "3")
	{
		...
		document.getElementById("normalMsg").innerHTML="Discharge Summary(Absconded Form) Report Successfully Generated!!";
		document.forms[0].submit();	
	}
