...
function initializeValues(){
	  ...
	  if(${chargeBean.strIsPackage} == "1"){
		...
		document.getElementById("packDetailsId").style.display="none";  
...
function calcTotalAmount(index){
	...
	function enterAmount(index){
			...
			if(isNaN(sec) || sec=="") sec = "0";
					...
					total = manipulateValue(parseFloat(adv),parseFloat(sec),0);
					document.getElementById("strPackTotalCost"+index).value = total;
					retVal = true;
			...
			if(isNaN(sec) || sec=="") sec = "0";
					...
					total = manipulateValue(parseFloat(adv),parseFloat(sec),0);
					document.forms[0].strPackTotalCost.value = total; //document.forms[0].strPackTotalCost.value = total;
					retVal = true;
				calcGrandTotal(0);
			}
		return retVal;
	...
	function calcGrandTotal(index){
		...
		if(index == 1){
			totObj = document.getElementsByName("strPackTotalCost");
			len = totObj.length ;
		}else{
...
function calculateArr(_these){
	...
	//alert("Resultant Product Cost= "+ document.getElementsByName("strProductCostArr")[getIndex(_these)].value);
	var nTariffCost = document.getElementsByName("strTariffCostArr")[getIndex(_these)].value;
	//alert("Resultant Tariff Cost= "+ document.getElementsByName("strTariffCostArr")[getIndex(_these)].value);
	procCost = nProcCost==""?"0":nProcCost;
	tarrifCost = nTariffCost==""?"0":nTariffCost;
	//document.getElementsByName("strProductCostArr")[getIndex(_these)].value = nProcCost;
	//document.getElementsByName("strTariffCostArr")[getIndex(_these)].value = nTariffCost;
...
function calcAll(_these){
	var strElement="";
	var strCheckBox = document.getElementsByName("strCheckBox");
	if(!strCheckBox[getIndex(_these)].checked){
		alert("Please select the record to modify");
		return;
		}
	
	var nHikeAmount = document.getElementsByName("strHike")[getIndex(_these)].value;
	//alert("nHikeAmount= "+nHikeAmount);
	var strPerOrFix = document.getElementsByName("strHikeType")[getIndex(_these)].value;
	//alert("strPerOrFix= "+strPerOrFix);
	var strUpOrDown = document.getElementsByName("strPriceType")[getIndex(_these)].value;
	//alert("strUpOrDown= "+strUpOrDown);
	
	 //strCheckBox[getIndex(_these)].checked="true";    // added by pawan Kumar b n
	//alert("strCheckBox.length= "+strCheckBox.length);
	//for(var i=0;i<strCheckBox.length;i++)
				//alert("strCheckBox["+i+"]= "+strCheckBox[i]);
	//alert("strCostType= "+document.getElementsByName("strCostType")[getIndex(_these)].value);
	...
	//alert("strArr.length="+strArr.length);
	var gIndex=getIndex(_these);
	for(var nTmpI=0; nTmpI<strArr.length; nTmpI++,gIndex++){
		
	//alert("strPkChargeTypeArr["+nTmpI+"]= "+document.getElementsByName("strPkChargeTypeArr")[nTmpI].value);
	//alert("(getIndex(_these)+1)="+getIndex(_these));
	//alert("gIndex= "+gIndex);
	//alert("nTmpI= "+nTmpI);
		//if(document.getElementsByName("strPkChargeTypeArr")[nTmpI].value!=gIndex)
		//{
			//continue;
			//}
			//alert("!strCheckBox[nTmpI].checked = "+ strCheckBox[nTmpI].checked);
		//if(!(strCheckBox[nTmpI].checked)
		//{
			//continue;
			//}
			if(strCheckBox[gIndex].checked)
			{
		if(strUpOrDown==1){
		//alert("inside strUpOrDown=1");
			if(strPerOrFix==1)
			{
			//alert("inside Per or Fix=1");
				strArr[nTmpI].value = eval(strArr[nTmpI].value) + (eval(strArr[nTmpI].value)*(nHikeAmount/100));
				//alert("strArr[nTmpI].value="+strArr[nTmpI].value);
				}
			else
			{
			//alert("inside Per or Fix =0");
				strArr[nTmpI].value = eval(strArr[nTmpI].value) + eval(nHikeAmount);
				//alert("strArr[nTmpI].value="+strArr[nTmpI].value);
				}
		}else{
		//alert("inside strUpOrDown=0");
			if(strPerOrFix==1)
			{
				//alert("inside Per or Fix=1");
				strArr[nTmpI].value = eval(strArr[nTmpI].value) - (eval(strArr[nTmpI].value)*(nHikeAmount/100));
...
//alert("Inside Save Modify Logic");
	//var strCheckBox = document.getElementsByName("strCheckBox");
	//var arr=new Array(strCheckBox.length);
...
function resetAll(_these){
	var strCheckBox = document.getElementsByName("strCheckBox");
	//for(var nTmpI = 0; nTmpI <3; nTmpI++){
	//	if(document.getElementsByName("strPkChargeTypeArr")[nTmpI].value!=(getIndex(_these)+1))
		//	continue;
		//alert("strCheckBox["+getIndex(_these)+"]= "+strCheckBox[getIndex(_these)].value.split("^")[nTmpI]);
		//}
		document.getElementsByName("strProductCostArr")[getIndex(_these)].value = strCheckBox[getIndex(_these)].value.split("^")[0];
		document.getElementsByName("strTariffCostArr")[getIndex(_these)].value = strCheckBox[getIndex(_these)].value.split("^")[1];
		document.getElementsByName("strTotalCostArr")[getIndex(_these)].value = strCheckBox[getIndex(_these)].value.split("^")[2];
	//}
}
...
<html:form action="/masters/CNTChargeMst.cnt" name="chargeBean"
			...
			<table width="100%" border="0" align="center">
				<tr class="HEADER">
					...
					<div align="right"><input type=checkbox 
						value='1' disabled="disabled" name="strIsPackage">
					Package</div>
					</td>
				</tr>
			</table>
	...
	<table class="TABLEWIDTH" border="0" align="center">
		...
		<tr>
			...
			<div style="display: none" id="grpName"><select
				name="strGroupCombo" onChange="tariffCombo();" disabled="disabled">
		...
		<tr>
			...
			<div style="display: none" id="trfId"><select
				name="strTariffCombo" disabled="disabled">
	...
	<table class="TABLEWIDTH" border="0" align="center">
		<tr>
			...
			<td class="CONTROL"> 		
					...
					<html:select property="strPackHospService" value="${chargeBean.strPackHospService}" disabled="true" >
							<bean:write name="chargeBean" property="strHospitalServiceCombo" filter="false" />
						</html:select>
		 <!--  	</select> --><div id="a4" style="display:none" ></div></td>
		</tr>
		<tr>
			<td class="LABEL"><font color="red">*</font>Group Name</td>
			<td class="CONTROL" colspan="3">
			<div id="grpName1"> <!--  <select name="a3" onChange="packTariffCombo();"
				disabled="disabled"> -->
				<select name="strPackGroupName" disabled="disabled" >
						<bean:write name="chargeBean" property="groupCombo" filter="false" />
						</select>
				
			<!-- </select> --> </div><div id="a3" style="display:none"></div>
			</td> 
		</tr>
		<tr>
			...
			<div id="trfId1"> <select name="strPackTariffName" 
			  ...
			  </select> </div><div id="a1" style="display:none"></div>
			</td>
			<td class="LABEL" width="16%"><font color="red">*</font>Unit</td>
			<td class="CONTROL" width="16%">
			<div id="unitId1">  <select name="strPackUnit" disabled="disabled"> 
				<bean:write name="chargeBean" property="unitCombo" filter="false" />
			</select></div><div id="a2" style="display:none"></div>
			</td>
		</tr>
		<tr>
			<td class="LABEL">Ward Type</td>
			<td class="CONTROL" colspan="3">
			<div id="wardId1"> <select name="strPackWardType" disabled="disabled"> 
				<bean:write name="chargeBean" property="wardCombo" filter="false" />
			  </select>  </div><div id="a5" style="display:none"></div>
			</td>
		</tr>
	</table>
	...
	<table class="TABLEWIDTH" border="0" align="center">
		...
		<tr>
			...
			Category</td>
			<td class="multiLabel" width="16%" style="display: none"><font color="red">*</font>Procedure
			Cost</td>
		...
		<tr>
			<td class="multiControl"><bean:write name="chargeBean" property="strPatCatName"
					filter="false" />
					<input type=hidden name=strPatientCategoryMod value="${chargeBean.strPatientCategoryMod }">
				</td>
			<td class="multiControl" style="display: none"><input type="text" class="txtFldSmall"
				name="strPackProcdCost" maxlength="9"
				value="${chargeBean.strPackProcdCost}"
				onkeypress="return validateData(event,7);"
				onkeyup="enterAmount('0');"></td>
			<td class="multiControl"><input type="text" class="txtFldSmall"
				name="strPackTariffCost" maxlength="9"
				value="${chargeBean.strPackTariffCost}"
				onkeypress="return validateData(event,7);"
				onkeyup="enterAmount('0');"></td>
			<td class="multiControl"><input type="text" class="txtFldSmall"
				readonly="readonly" name="strPackTotalCost" maxlength="9"
				value="${chargeBean.strPackProcdCost + chargeBean.strPackTariffCost}"
				onkeypress="return validateData(event,7);"></td>
			<!-- <td class="multiControl"><input type="checkbox"
				name="strPackIsAdvance"></td> -->
			<td class="multiControl"><html:checkbox name="chargeBean"
				property="strPackIsRefundable" value="1"></html:checkbox></td>
			<td class="multiControl"><html:checkbox name="chargeBean"
				property="strPackIsAdvance" value="1"></html:checkbox></td>
		</tr>
	</table>
	<table class="TABLEWIDTH" border="0" align="center">
		<tr class="HEADER">
		...
		<tr>
			...
			<td class="multiLabel" width="15%">Procedure Cost/unit(<img src='/AHIMS/hisglobal/images/INR.png'>)</td>
			<td class="multiLabel" width="15%">Tariff Cost/unit(<img src='/AHIMS/hisglobal/images/INR.png'>)</td>
			<td class="multiLabel" width="15%">Total Cost/unit(<img src='/AHIMS/hisglobal/images/INR.png'>)</td>
			<td class="multiLabel" width="10%">Refundable</td>
		</tr>
	</table>
	<div id="packDetailsId" style="display: none"><bean:write name="chargeBean"
