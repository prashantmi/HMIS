...
<head>
<meta charset=utf-8>
<title>Client Charge Master Modify Page</title>
...
function tariffCombo(){	
	...
	var mode="TRFCMB";
	var url1="CNTClientChargeMst.cnt?hmode="+mode+"&service="+temp;
	ajaxFunction(url1,"2");
}
...
function unitCombo(){
	...
	var mode="UNTCMB";
	var url1="CNTClientChargeMst.cnt?hmode="+mode+"&service="+temp[1];
...
function packGroupCombo(){
	...
	var temp = document.clientChargeBean.strPackHospService[document.clientChargeBean.strPackHospService.selectedIndex].value;
...
function packTariffCombo(){	
	var temp = document.clientChargeBean.strPackGroupName[document.clientChargeBean.strPackGroupName.selectedIndex].value;
	var mode="TRFCMB1";
	var url1="CNTClientChargeMst.cnt?hmode="+mode+"&service="+temp;
	ajaxFunction(url1,"6");
}
...
function packUnitCombo(){
	...
	var temp = document.clientChargeBean.strPackTariffName[document.clientChargeBean.strPackTariffName.selectedIndex].value.split('^');
...
function packageDetails(){
	var temp = document.clientChargeBean.strPackTariffName[document.clientChargeBean.strPackTariffName.selectedIndex].value.split('^');
	var mode="PACKDET";
	var url1="CNTClientChargeMst.cnt?hmode="+mode+"&service="+temp[0];
...
function packWardCombo(){
		...
		var temp1 = document.clientChargeBean.strPackHospService[document.clientChargeBean.strPackHospService.selectedIndex].value;	
		var mode="WRDCMB";
		var url="CNTClientChargeMst.cnt?hmode="+mode+"&service1="+temp1;	
...
function initializeValues(){
	  ...
	  if("${clientChargeBean.strIsPackage}" == '1'){
	  document.clientChargeBean.strIsPackage.checked = true;
		// TEST////
...
function submitData(mode){
	...
	var hisValidator = new HISValidator("clientChargeBean");
		
	document.clientChargeBean.strIsPackage.disabled = false;
	var isPackageVal = document.clientChargeBean.strIsPackage.checked;
	document.clientChargeBean.strIsPackage.disabled = true;
	
	
	if(isPackageVal)
	{
		document.clientChargeBean.strIsPackage.value = "1";
	...
	{
		document.clientChargeBean.strIsPackage.value = "0";		
	...
	hisValidator.addValidation("strEffectiveFrm","dtgtet=${clientChargeBean.strCurrentDate}" ,"Please Select Effective From Date Greater Than Or Equal To Current Date.");
	}
...
function calcTotalAmount(index){
	...
	function checkUpdateMode(radioObj){
		...
		if(radioObj.value == 0){
			...
			document.forms[0].strEffectiveFrm.value = "${clientChargeBean.strEffectiveFrm}";
...
<html:form action="/masters/CNTClientChargeMst.cnt" name="clientChargeBean"
	type="billing.masters.controller.fb.ClientChargeMstFB">
	<div class="errMsg" align="center"><bean:write name="clientChargeBean"
		property="strErrMsg" /></div>
	<div class="normalMsg" id='normalMsg' align="center"><bean:write
		name="clientChargeBean" property="strNormalMsg" /></div>
	<div class="warningMsg" align="center"><bean:write
		name="clientChargeBean" property="strWarningMsg" /></div>
	...
	<table class="TABLEWIDTH" border="0" align="center">
		<tr class="HEADER">
			...
			<table width="100%" border="0" align="center">
				<tr class="HEADER">
					<td width="50%">
					<div align="left">Client Charge Master&gt;&gt;Modify</div>
					</td>
					<td width="50%">
					<div align="right"><input type=checkbox 
						value='${clientChargeBean.strIsPackage}' disabled="disabled" name="strIsPackage">
					Package</div>
					</td>
				</tr>
			</table>
	...
	<table class="TABLEWIDTH" border="0" align="center">
		...
		<tr>
			...
			<select name="strGroupCombo" onChange="tariffCombo();" disabled="disabled" class='comboNormal'>
				<bean:write name="clientChargeBean" property="groupCombo" filter="false" />
			</select></div>
		...
		<tr>
			...
			<div style="display: none" id="trfId"><select
				...
				<bean:write name="clientChargeBean" property="tariffCombo" filter="false" />
			</select></div>
			...
			<div style="display: none" id="unitId"><select
				...
				<bean:write name="clientChargeBean" property="unitCombo" filter="false" />
			</select></div>
			<div id="divUnitId"></div>
			</td>
		</tr>
	</table>
	<bean:write name="clientChargeBean" property="strModifyDtl" filter="false" />
	</div>
	...
	<table class="TABLEWIDTH" border="0" align="center">
		...
		<tr>
			...
			<div id="grpName1" style="display:none"><select name='a3' onChange="packTariffCombo();"
				...
				<bean:write name="clientChargeBean" property="groupCombo1" filter="false" />
		...
		<tr>
			...
			<div id="trfId1" style="display:none"><select name='a1' onChange="packUnitCombo();"
				disabled="disabled" class='comboNormal'>
				<bean:write name="clientChargeBean" property="tariffCombo1" filter="false" />
		...
		<tr>
			...
			<div id="wardId1" style="display:none"><select name='a5' disabled="disabled" class='comboNormal'>
				<bean:write name="clientChargeBean" property="wardCombo" filter="false" />
			</select></div><div id="a5"></div>
			</td>
		</tr>  
	</table> -->
 <div id="forPackage2" style="display:none">
	<table class="TABLEWIDTH" border="0" align="center">
	
		<tr class="TITLE"><td colspan="7">Current Charges</td></tr>
		<tr>
			<td class="multiLabel" width="20%"><font color="red">*</font>Patient Category</td>
			<td class="multiLabel" width="20%"><font color="red">*</font>Organization</td>
			<td class="multiLabel" width="20%" style="display: none"><font color="red">*</font>Procedure Cost</td>
			<td class="multiLabel" width="20%"><font color="red">*</font>Tariff Cost</td>
			<td class="multiLabel" width="20%">Total Cost</td>
			<!-- <td class="multiLabel" width="16%">Is-Advance</td> 
			<td class="multiLabel" width="16%">Is-Refundable</td>
			<td class="multiLabel" width="16%">Is-Advance</td>  -->
		</tr>
		<tr>
			<td class="multiControl"><bean:write name="clientChargeBean" property="strPatCatName" filter="false" />
					<input type=hidden name=strPatientCategoryMod value="${clientChargeBean.strPatientCategoryMod }">
				</td>
				<td class="multiControl"><bean:write name="clientChargeBean" property="strOrgName" filter="false" /></td>
			<td class="multiControl" style="display: none"><input type="text" class="txtFldSmall" name="strPackProcdCost" maxlength="9" value="${clientChargeBean.strPackProcdCost}"
				onkeypress="return validateData(event,7);" onkeyup="enterAmount('0');"></td>
			
			<td class="multiControl"><input type="text" class="txtFldSmall" name="strPackTariffCost" maxlength="9" value="${clientChargeBean.strPackTariffCost}"
				onkeypress="return validateData(event,7);" onkeyup="enterAmount('0');"></td>
			<td class="multiControl"><input type="text" class="txtFldSmall" readonly="readonly" name="strPackTotalCost" maxlength="9"
				value="${clientChargeBean.strPackProcdCost + clientChargeBean.strPackTariffCost}" onkeypress="return validateData(event,7);"></td>
			<!-- <td class="multiControl"><input type="checkbox" name="strPackIsAdvance"></td> 
			<td class="multiControl"><html:checkbox name="clientChargeBean" property="strPackIsRefundable" value="1"></html:checkbox></td>
			<td class="multiControl"><html:checkbox name="clientChargeBean" property="strPackIsAdvance" value="1"></html:checkbox></td> -->
		</tr>
	</table>
	</div>
<!-- 	<table class="TABLEWIDTH" border="0" align="center">
	...
	<div id="packDetailsId"><bean:write name="clientChargeBean"
	...
	<table class="TABLEWIDTH" border="0" align="center">
		<tr>
			...
			<td class="CONTROL" colspan="3"><textarea rows="2"
				name="strRemarks"><bean:write name="clientChargeBean"
	...
	<input type="hidden" name="chk" value="${clientChargeBean.chk[0]}">
	<input type="hidden" name="strGroupNameHidden"
		value="${clientChargeBean.strGroupName}">
	<input type="hidden" name="strUnitHidden" value="${clientChargeBean.strUnit}">
	<input type="hidden" name="strTempCategory"
		value="${clientChargeBean.strPatientCategory[0]}">
	<input type="hidden" name="hospitalService"
		value="${clientChargeBean.hospitalService }">
	<input type="hidden" name="strPackHospService"
		value="${clientChargeBean.strPackHospService }">
	<input type="hidden" name="strGroupName"
		value="${clientChargeBean.strGroupName }">
	<input type="hidden" name="strPackGroupName"
		value="${clientChargeBean.strPackGroupName }">
	<input type="hidden" name="strTariffName"
		value="${clientChargeBean.strTariffName }">
	<input type="hidden" name="strPackTariffName"
		value="${clientChargeBean.strPackTariffName }">
	<input type="hidden" name="strUnit" value="${clientChargeBean.strUnit }">
	<input type="hidden" name="strPackUnit"
		value="${clientChargeBean.strPackUnit }">
	<input type="hidden" name="strWardType"
		value="${clientChargeBean.strWardType}">
	<input type="hidden" name="strPackWardType"
		value="${clientChargeBean.strPackWardType}">
	<input type="hidden" name="strPatientCategoryMod"
		value="${clientChargeBean.strPatientCategoryMod}">
	<input type="hidden" name="strPackPatientCategory"
		value="${clientChargeBean.strPackPatientCategory}">
...
showLabel();
if("${clientChargeBean.strIsPackage}"=="1")
	document.getElementsByName("clientChargeBean")[0].checked = true;
