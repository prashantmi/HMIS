...
function chooseOption(){
	if(document.chargeBean.strSelectOption[0].checked){
		...
		if(document.chargeBean.hospitalService[document.chargeBean.hospitalService.selectedIndex].value != 2){
...
function submitData(mode){	
	...
	if(document.chargeBean.strSelectOption[0].checked){
		...
		if(document.chargeBean.strTempPatientCategory.value == document.chargeBean.strCopyPatCat.value){
			alert("Please Choose Different Patient Categories");
			return false;
		}
	}
	
	hisValidator.addValidation("strEffectiveFrm","dtgtet="+"${chargeBean.strEffectiveFrm}","Please Select Effective From Date Greater Than Or Equal To Current Date.");	
	...
	if(retVal){
		...
		if(obj[0].value == "+"){
			document.chargeBean.strNewProcCost.value = document.chargeBean.strNewProcCost.value/100;
		} else {
			document.chargeBean.strNewProcCost.value = -(document.chargeBean.strNewProcCost.value/100);
		}
		if(obj[1].value == "+"){
			document.chargeBean.strNewTrfCost.value = document.chargeBean.strNewTrfCost.value/100;
		} else {
			document.chargeBean.strNewTrfCost.value = -(document.chargeBean.strNewTrfCost.value/100);
		}
		if(!document.chargeBean.strSelectOption[0].checked){
			if(obj[2].value == "+"){
				document.chargeBean.strCopyProcCost.value = document.chargeBean.strCopyProcCost.value/100;
			} else {
				document.chargeBean.strCopyProcCost.value = -(document.chargeBean.strCopyProcCost.value/100);
			}
			if(obj[3].value == "+"){
				document.chargeBean.strCopyTrfCost.value = document.chargeBean.strCopyTrfCost.value/100;
			} else {
			document.chargeBean.strCopyTrfCost.value = -(document.chargeBean.strCopyTrfCost.value/100);
			}		
		}
...
<body>
<html:form action="/masters/CNTChargeMst.cnt"
	name="chargeBean" type="billing.masters.VOChargeMst">
	<div class="errMsg" align="center"><bean:write name="chargeBean" property="strErrMsg"/> </div>
	<div class="normalMsg" id="normalMsg" align="center"><bean:write name="chargeBean" property="strNormalMsg"/></div>
	...
	<table class="TABLEWIDTH" border="0" align="center">
		<tr class="HEADER">
			<td colspan="4">Charge Master&gt;&gt; Batch Update</td>
		</tr>
		<tr>
	...
	<table class="TABLEWIDTH" border="0" align="center">
		<tr>
			...
			<td class="CONTROL" width="80%"><dateTag:date name="strEffectiveFrm"
				value="${chargeBean.strEffectiveFrm}"></dateTag:date></td>
		</tr>
		<tr>
	...
	<div align="center"><img src="../../hisglobal/images/ok.gif" style="cursor:pointer;cursor:hand;" title="Update Batch" 
