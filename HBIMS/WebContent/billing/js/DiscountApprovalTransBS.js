...
{   
	      ...
	      if(document.forms[0].CrNo.value!="")
          {
		    ...
		    {  
	           ...
	           var strchk = document.forms[0].strChk[0].value;
	           
	           var temp = strchk.split("^");
	   		   var temp1 = temp[0];
	   		   for (var i = 0; i < temp.length - 1; i++)
	   			temp1 += "_" + temp[i + 1];
	           //alert(temp1);
	           var url="DiscountApprovalTransBSCNT.cnt?hmode="+mode+"&modName="+temp1;
	           //alert(url);
		       ajaxFunction(url,"1");
...
{ 
  ...
  // alert("REQ_NO+SBLNUM_BSERVICE_ID+HBLNUM_PATACCOUNT_NO+HiddenValue::"+document.forms[0].strTempVal.value);
   ...
   var url="DiscountApprovalTransBSCNT.cnt?hmode="+mode+"&modName="+document.forms[0].strTempVal.value;
  // alert(url);
  // alert(url);
   var temp = url.split("^");
	   var temp1 = temp[0];
...
function myFunc(obj,index)
{
	   ...
	   //    alert("Popup_Id for Tariff Dtl::"+document.forms[0].strPopUpId.value);
	        ...
	        var url="DiscountApprovalTransBSCNT.cnt?hmode="+mode+"&modName="+document.forms[0].strTempVal.value+"&popUpId="+document.forms[0].strPopUpId.value;
	    	ajaxFunction(url,"2"); 
}
...
///////////////////for selecting a record to cancel/////////////////
	...
	{
	   var mode="Cancel_hlp";
	   var url="DiscountApprovalTransBSCNT.cnt?hmode="+mode;
	   ajaxFunction(url,"3");
	}
