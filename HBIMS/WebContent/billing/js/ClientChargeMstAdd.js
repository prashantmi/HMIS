...
{	
		if(document.clientChargeBean.strIsPackage.checked)
		{
	...
	function unitCombo()
	{
		...
		if(temp.length > 1)
		{
			var url1="CNTClientChargeMst.cnt?hmode="+mode+"&service="+temp[2];
		}
		else
		{
			var url1="CNTClientChargeMst.cnt?hmode="+mode+"&service=0000";
		}
		ajaxFunction(url1,"3");
	}
...
/*
	...
	function getPreviousDetails(){
		...
		var url = "";
		if(document.clientChargeBean.strIsPackage.checked){
			url="CNTClientChargeMst.cnt?hmode="+mode+"&hService="+document.forms[0].strPackHospService.value+"&tariffId="+document.forms[0].strPackTariffName.value+"&wardType="+document.forms[0].strPackWardType.value;	
		}else{
			url="CNTClientChargeMst.cnt?hmode="+mode+"&hService="+document.forms[0].hospitalService.value+"&tariffId="+document.forms[0].strTariffName.value+"&wardType="+document.forms[0].strWardType.value;
		}
	ajaxFunction(url,"10");
	}
 ...
 // for(var v=0;v<4000;v++){}
			...
			document.getElementById("strPatientCategorydiv#delIndex#").innerHTML = "<select name='strPatientCategory' class='comboNormal' id='strPatientCategory#delIndex#' onchange='changeOrgCombo();'>"+res+"</select>";
		...
		{
			document.getElementById("divstrOrg").innerHTML = "<select name='strOrg' class='comboNormal' id='strOrg'>"+res+"</select>";
		//	document.getElementById("strPackPatientCategorydiv").innerHTML = "<select name='strPackPatientCategory' class='comboNormal' >"+res+"</select>";
			// add1stRow();
		}
	}
...
function changeCatCombo(_these)
{
	...
	if(document.clientChargeBean.strIsPackage.checked)
	{
		url="CNTClientChargeMst.cnt?hmode="+mode+"&hService="+document.forms[0].strPackHospService.value+"&tariffId="+document.forms[0].strPackTariffName.value.split("^")[0]+"&groupId="+document.forms[0].strPackGroupName.value+"&wardType="+document.forms[0].strWardType.value;
	}
	else
	{
		url="CNTClientChargeMst.cnt?hmode="+mode+"&hService="+document.getElementsByName("hospitalService")[0].value+"&tariffId="+document.getElementsByName("strTariffName")[0].value.split("^")[0]+"&groupId="+document.getElementsByName("strGroupName")[0].value+"&wardType="+document.forms[0].strWardType.value;
	}
	ajaxFunction(url,"11");
}

function changeOrgCombo()
{
	//add1stRow();
	//alert(document.getElementsByName("strPatientCategory")[0].value);
	var mode = "GETORGCOMBO";
	if(document.clientChargeBean.strIsPackage.checked)
	{
		url="CNTClientChargeMst.cnt?hmode="+mode+"&patcat="+document.forms[0].strPackPatientCategory.value+"&tariffId="+document.forms[0].strPackTariffName.value.split("^")[0]+"&groupId="+document.forms[0].strPackGroupName.value+"&wardType="+document.forms[0].strWardType.value;
	}
	else
	{
		url="CNTClientChargeMst.cnt?hmode="+mode+"&patcat="+document.getElementsByName("strPatientCategory")[0].value;
	}
	ajaxFunction(url,"12");
}
...
function add1stRow()
{
	...
	document.getElementsByName("rowIndex1")[0].value = 0;
	document.getElementsByName("rowLength1")[0].value = 0;
	addRows(new Array('strPatientCategory','strOrg','strTariffCost','strTotalCost'),new Array('s','s','t'),'1','1','R');
}

function addMoreRows()
{
	addRows(new Array('strPatientCategory','strOrg','strTariffCost'),new Array('s','s','t'),'1','1','R');
}