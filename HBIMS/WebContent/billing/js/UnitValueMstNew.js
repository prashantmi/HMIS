...
function combo1(mode)
 { 
	 ...
	 if(mode=="FROMUNITVAL")
	 {
	  url="CNTUnitValueMstBS.cnt?hmode="+mode+"&modName="+document.forms[0].strmoduleName.value;
	  ajaxFunction(url,"1");
	 }
	else
	if(mode=="TOUNITVAL")
	{
	 ...
	 url="CNTUnitValueMstBS.cnt?hmode="+mode+"&modName="+document.forms[0].strmoduleName.value+"&fromunit="+frmUnitId;
	 ajaxFunction(url,"2");
	}
  }
  function getAjaxResponse(res,mode)
  {
	...
	objVal.innerHTML = "<select name ='strfromUnit' class='form-control' onChange ='combo1(\"TOUNITVAL\");'>" + res + "</select>";
	}
	else if(mode=="2")
	{
	var objVal = document.getElementById("toUnitId");
	objVal.innerHTML = "<select name='strtoUnit' class='form-control'>" + res + "</select>";
	}
  }
