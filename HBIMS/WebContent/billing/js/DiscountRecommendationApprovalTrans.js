 ...
 */
 function showSearchWindow() {
 	...
 	if(document.forms[0].strDepartment[document.forms[0].strDepartment.selectedIndex].value != 0){

		showPatientListingWindowWithDeptCode('10',document.forms[0].strCrNo,'setSelectedCrNo', document.forms[0].strDepartment[document.forms[0].strDepartment.selectedIndex].value);
 		
 	}else{
 		
 		alert("Please Select a Department");
 		document.forms[0].strDepartment.focus();
 		return false;
  ...
  */
  function getUnitList(deptObj) {
  	
  		if(deptObj.value == '0'){
  			
  			document.getElementById("unitDivId").innerHTML = "<select name='strUnit' class='comboNormal'><option value='0'>Select Value</option></select>";
  			
  		}else{
  			
  			var url="DiscountRecommendationApprovalTransCNT.cnt?hmode=GETUNIT&deptCode="+deptObj.value;
...
function onEnterDiscount(index,discountLimit, rateUnit)
{
	var flag = 1;
	...
	var strDiscountUnit = disUnitObj.value;
 	var strDiscountType = disTypeObj[disTypeObj.selectedIndex].value; 
 	 	
 	if(strDiscountUnit == "" || strDiscountUnit == null) strDiscountUnit = "0";
 	strDiscountUnit = parseFloat(strDiscountUnit);
 	
 	var rateUnitVal = 	parseFloat(rateUnit);
 	var discountLimitVal = parseFloat(discountLimit);
 	
 	var allowedDiscountAmt = (rateUnitVal * discountLimitVal) / 100;
 	var fixedDiscountAmt = parseFloat("0");
 	var perDiscountAmt = parseFloat("0");
 	var reqDiscountUnit = parseFloat(strDiscountUnit);
 	
 	var calDiscountAmt = parseFloat("0"); 
 	 	
 	if(strDiscountType = 1){
 		
 		fixedDiscountAmt = reqDiscountUnit;
 		
 		if(fixedDiscountAmt > rateUnit){
 			
 			alert("Discount cannot be greater than Rate");
 		
 		disUnitObj.value = 0;
 		
 		onEnterDiscount(index,discountLimit, rateUnit);
 		return false;
 			
 		}
 		
 		if(fixedDiscountAmt > allowedDiscountAmt){
 			
 			
 			alert("Discount cannot be greater than Discount Limit");
 		...
 		calDiscountAmt = fixedDiscountAmt;
 	...
 	if(strDiscountType == 2){
 		...
 		perDiscountAmt = (rateUnit * reqDiscountUnit) / 100;
 		
 		
 		if(perDiscountAmt > allowedDiscountAmt){
 		...
 		calDiscountAmt = perDiscountAmt;
 		
 	}
 	
 	
 	calDiscountAmt = roundValue(calDiscountAmt , 2);
 	
 	document.getElementById("disAmtDivId"+index).innerHTML = calDiscountAmt;
 	document.getElementById("disAmt"+index).value = calDiscountAmt;
...
function goFunc()                //  for CR No. field validation
{
	...
	if(document.forms[0].strDepartment[document.forms[0].strDepartment.selectedIndex].value != 0){
		...
		var hisValidator = new HISValidator("discountRecommendationApprovalTransBean");  
...
{   
          ...
          {
            if(document.forms[0].CrNo.value.length > 1)
		    {	
	           ...
	           var mode="BId";
	           var url="DiscountRecommendationApprovalTransCNT.cnt?hmode="+mode+"&modName="+document.forms[0].strChk[0].value+"&deptCode="+document.forms[0].strDepartment[document.forms[0].strDepartment.selectedIndex].value;
...
{ 
  ...
  // alert("REQ_NO+SBLNUM_BSERVICE_ID+HBLNUM_PATACCOUNT_NO+HiddenValue::"+document.forms[0].strTempVal.value);
   ...
   var url="DiscountRecommendationApprovalTransCNT.cnt?hmode="+mode+"&modName="+document.forms[0].strTempVal.value+"&deptCode="+document.forms[0].strDepartment[document.forms[0].strDepartment.selectedIndex].value;
...
/**
 * showBalanceLimitDtlsPopup
 * @param {String} appendedDetails 
 */
 function showBalanceLimitDtlsPopup(pWindow, appendedDetails) {
 	
 	var dtls = appendedDetails.split('^');
 	
 	document.getElementById("11").innerHTML = dtls[0];
 	document.getElementById("22").innerHTML = dtls[1];
 	document.getElementById("33").innerHTML = dtls[2];
 	document.getElementById("44").innerHTML = dtls[3];
 	
 	 display_popup_menu(pWindow,"discountDtl2","300","");
...
function myFunc(obj,index)
{
	   ...
	   //    alert("Popup_Id for Tariff Dtl::"+document.forms[0].strPopUpId.value);
	        ...
	        var url="DiscountRecommendationApprovalTransCNT.cnt?hmode="+mode+"&modName="+document.forms[0].strTempVal.value+"&popUpId="+document.forms[0].strPopUpId.value;
	    	ajaxFunction(url,"2"); 
}
      ...
      var temp = res.split("####");
           ...
           if(mode=="3")
            {
		      ...
		      getUnitList(document.forms[0].strDepartment);
	   ...
	   if(mode == '4')
       {
	    ...
	    if(mode == '5'){
	    	...
	    	document.getElementById("unitDivId").innerHTML = "<select name='strUnit' class='comboNormal'>"+res+"</select>";
	    	
	    	
	    	document.forms[0].strUnit.value = document.forms[0].strUnitId.value;
...
///////////////////for selecting a record to cancel/////////////////
	...
	{
	   var mode="Cancel_hlp";
	   var url="DiscountRecommendationApprovalTransCNT.cnt?hmode="+mode;
	   ajaxFunction(url,"3");
	}
