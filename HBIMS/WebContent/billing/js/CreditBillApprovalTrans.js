//for setting waived amt in div showing total waived amt
function setTotalWavedAmt()
{
	//alert("setTotalWavedAmt called...");
	
	if(document.getElementById("hlp_opt_sel")==null)
	{
		//no operation..
	}
	else
	{
		if(document.getElementsByName('opd_discount').length == 1)//ipd advance case..jz 1 row..
		{
			document.getElementById('totDisAmtDivId').innerHTML=document.getElementsByName('opd_discount')[0].value;
		}
		
	}
}
//  for CR No. field validation
function goFunc()                
{

  
		var hisValidator = new HISValidator("creditBillApprovalTransBean");  
	      ...
	      if(document.forms[0].CrNo.value!="")
          {
		    ...
		    {  
              ...
              // alert("ajax sent..");
               ...
               //if no online requests are there..den no further processing can be done in action cz req param 'strChk' vl be missing..
	           var mode="BId";
	           var url="CreditBillApprovalTransCNT.cnt?hmode="+mode+"&modName="+document.forms[0].strChk[0].value;
		       ajaxFunction(url,"1");
		     } 
...
function validateFunc1()  
 {
	...
	else
	{
	    ...
	    if(retVal)
		{
		        document.getElementById("patdtltld").style.display="none";
                document.getElementById("disBnR").style.display="none";
                document.getElementsByName('finalCRNo')[0].value=document.forms[0].strCrNo.value;
                document.forms[0].strCrNo.value="";
		    	document.forms[0].hmode.value="INSERT";
		    	//alert('final cr no being sent is::'+document.getElementsByName('finalCRNo')[0].value);
		    	document.forms[0].submit();
		}
		else
		{
 ...
 function ValidateServcie()
 {
      ...
      if(document.getElementsByName('hlpBServiceID')[0].value == '19')
      {
      	
      	//it's ipd case is to be handled differently..
      	//jz 1 row will be there in ipd case..
      	var discountAmt = document.getElementById("opd_discount0").value;
 	    var creditLetterNo = document.getElementById("strCreditLetterNo0").value;
 	    var creditLetterDate = document.getElementById("strCreditLetterDate#0").value;
 	    
 	    if(parseInt(discountAmt) <= 0)
		{
			alert("Waiver Amount should be greater than Zero!!");
			retVal = false;
			return;
		}
		
		if(!checkAmount(discountAmt,'9,2'))
		{
			alert("Incorrect Amount Format. Atmost 7 digits before and 2 digit after decimal are allowed.");
			retVal = false;
			return;
		}
						
		if(creditLetterNo.trim().length == 0)
		{
			alert("Enter Credit Letter No.");
			retVal = false;
			return;
		}

		if(creditLetterDate.trim().length == 0)
		{
			alert("Enter Credit Letter Date.");
			retVal = false;
			return;
		}
						
		//credit letter date should be less than = current date
		var tempYear=creditLetterDate.split('-')[2];
		var tempMon=creditLetterDate.split('-')[1];
		var tempDate=creditLetterDate.split('-')[0];
		var numeralValMonth=0;//jan
		var arrMon=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
						
		for(var t=0;t<arrMon.length;t++)
		{
			if(arrMon[t]==tempMon)
			{
				numeralValMonth=t;
		...
		var tempDate=new Date(tempYear,numeralValMonth,tempDate);
		var currDate=new Date();
						
		if(tempDate.getTime() > currDate.getTime())
		{
			alert("Credit Letter Date cannot be greater than Current Date.");
			retVal = false;
			return;
    ...
    var arrInputs = document.getElementsByName('uploadedFile[0]');
            ...
            if (sFileName.length > 0) {
                ...
                for (var j = 0; j < _validFileExtensions.length; j++) {
                    ...
                    if (sFileName.substr(sFileName.length - sCurExtension.length, sCurExtension.length).toLowerCase() == sCurExtension.toLowerCase()) {
                     ...
                     if(parseInt(oInput.files[i].size)> parseInt(document.getElementsByName('MAX_FILE_SIZE')[0].value ))
               		{
               			 //alert("Maximum 2MB files are allowed.");
	                     blnValid = false;
						 break;
               		}
 	   ...
 	   //  var opdDis  = document.getElementsByName("opd_discount");
 	     ...
 	     // alert("inside if chkObj.length="+chkObj.length)
 	      	...
 	      	for(var i = 0 ; i < length; i++)
 	     	{
 	     	        ...
 	     	        var creditLetterNo = document.getElementById("strCreditLetterNo"+i).value;
 	     	        var creditLetterDate = document.getElementById("strCreditLetterDate#"+i).value;
					...
					if(chkObj.checked ==  true )
					{
						...
						//alert("changedd.........."+document.getElementById("strCreditLetterDate#"+i).readonly);
						if(creditLetterDate.trim().length == 0)
						{
							alert("Enter Credit Letter Date.");
							retVal = false;
						...
						if(creditLetterDate != "")
						{
							var retVal=compareDate(creditLetterDate,document.forms[0].strTempCtDate.value);
							if(retVal.mode==2)
							{
							...
							var datediffVar=dateDiff(creditLetterDate,document.forms[0].strTempCtDate.value);
    ...
    var arrInputs = document.getElementsByName('uploadedFile['+i+']');
    for (var i = 0; i < arrInputs.length; i++) {
        var oInput = arrInputs[i];
        if (oInput.type == "file") {
            var sFileName = oInput.value;
 	...
 	//alert("values::"+document.getElementById("strCreditLetterDate#0").value);
       var hisValidator = new HISValidator("creditBillApprovalTransBean");  
 	
 	   hisValidator.clearAllValidations;
 	   hisValidator.addValidation("strRmk", "dontselect=0","Waiver By is a mandatory field");
      // hisValidator.addValidation("opd_discount", "amount=5,2","Waiver Amount should be in correct format(00.00)");
      // hisValidator.addValidation("strEmployeeId","req", "Employee Id is a Mandatory Field" );
     //  hisValidator.addValidation("strEmployeeName","req", "Employee Name is a Mandatory Field" );
     //  hisValidator.addValidation("strRalationId", "dontselect=0","Relation With Patient is a mandatory field");
    //   hisValidator.addValidation("strCardValidity","date", "Card Validity is a Mandatory Field" );
   		
   		if(document.getElementsByName('strCardValidity')[0].value.length > 0)
   		{
   			hisValidator.addValidation("strCardValidity","dtgtet="+document.forms[0].strCurrentDate.value,"Card Validity should be Greater Than Or Equal To Current Date");
   		}
       ...
       retVal = hisValidator.validate();
 	   ...
 	   if(retVal)
 	   {
 	   		//done such that value could go into FB..
 	   		for(var i = 0 ; i < length; i++)
 	     	{
 	     		document.getElementById("strCreditLetterDate#"+i).readonly=false;
 	     	}
 	   }
 	 return retVal;
 ...
 function onEnterDiscount(index,strTrfRate,strReqQty,strQtyBaseVal,strPrevDiscountUnit,strPrevDiscountType,strRateBaseVal)
{
    ...
    if(parseFloat(objDisUnit.value) > 0 && objDisAdd.disabled == false && objDisAdd.checked == true)
    {
 	...
 	if (strDiscountType == 2)
 	{
 		...
 		{
 			alert("Waiver can not be greater than 100%");
 			disUnitObj.value = "0.00";
 			flag = 0;
 		}
 	}
 	...
 	if (flag == 1)
 	{
	 	...
	 	{
	 		alert("Waiver Amt can not be greater than tariff total amount !!");
	 		disUnitObj.value = "0.00";
	 		discountAmt = 0;
	 	}
	 }
	...
	document.getElementById("discGivenAmt"+index).value=discountAmt;
	//alert("value written.............");
...
{ 
  ...
  // alert("REQ_NO+SBLNUM_BSERVICE_ID+HBLNUM_PATACCOUNT_NO+HiddenValue::"+document.forms[0].strTempVal.value);
   ...
   var url="CreditBillApprovalTransCNT.cnt?hmode="+mode+"&modName="+document.forms[0].strTempVal.value;
   ajaxFunction(url,"1");
}
var pWindow ="";
  ...
  */
  ...
  if(obj.checked)
  {
     ...
     */
   	...
   	//by default calculate at 100% discount
   /*	var strTrfAmt=document.getElementById('tariffCost').value;
   	var strReqQty=document.getElementById('qty').value;
   	var strQtyBaseVal=document.getElementById('tariffCost').value;
   	var strDiscountUnit=document.getElementById('tariffCost').value;
   	var strDiscountType=document.getElementById('tariffCost').value;
   	var strRateBaseVal=document.getElementById('tariffCost').value;
   	
   	onEnterDiscount("+i+","+strTrfAmt+","+strReqQty+","+strQtyBaseVal+","+strDiscountUnit+","+strDiscountType+","+strRateBaseVal+");
 */
  }
 else
 {
