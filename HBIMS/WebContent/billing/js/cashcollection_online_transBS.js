			...
			if(normalMsgVal.search(document.forms[0].strTempBillNo.value) != -1)
			{
					...
					if(!confirmFlag)
					{
						...
						else
						{
							...
							var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode+"&billNo="+billNo+"&recNo="+recNo;
			...
			if(checkCrdef(document.getElementById("strCrNoId"))==false)//Complete CR No Entered
			{
				...
				if(document.forms[0].currentState.value == '1')
				{
					...
					if(onLineRadioObj.length > 0)
					{
						...
						var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode+"&crNo="+document.forms[0].strCrNo.value+"&chargeTypeId="+tempVal[3]+"&bServiceId="+tempVal[4]+"&reqNo="+tempVal[0]+"&admNo="+tempVal[10]+"&accNo="+tempVal[11]+"&treatCat="+tempVal[14]+"&reqFor="+tempVal[15]+"&reqType="+tempVal[13]+"&billNo="+tempVal[7]+"&grpId="+tempVal[2]+"&catCode="+tempVal[8];
						ajaxFunction(url,"1");
	 ...
	 */
	 function reprintContents() {
	 		...
	 		if(gblPrintLimitVal != 0){
	 			...
	 			var hmode = "REPRINT"; 
				var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode;
	...
	function getBillService(){
	 ...
	 document.forms[0].currentBserviceId.value = document.forms[0].strOffLineBillingService[document.forms[0].strOffLineBillingService.selectedIndex].value;
	 	...
	 	if(document.forms[0].currentReceiptType.value == '2'){
			...
			var hmode = "BILLDTLS"; 
			var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode+"&hService="+document.forms[0].strOffLineHospitalService[document.forms[0].strOffLineHospitalService.selectedIndex].value+"&requestType="+document.forms[0].strOffLineRequestType[document.forms[0].strOffLineRequestType.selectedIndex].value;
	 ...
	 function showReconcilPopup(hidVal,parentPos){
			...
			var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode+"&hidVal="+hidVal;
	 ...
	 */
	 function getOnlineThirdPartyAmtView() {
	 	...
	 	var hmode = "ONLINETHIRDPARTYAMTDTL"; 
			var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode+"&strChargeTypeId="+gblOnLineHService+"&strRequestTypeId="+gblReqType+"&strBServiceId="+gblBServiceId+"&clientStatus="+clientStatus;
...
function getParticularsDetails(obj,grpId,acctNo,index) 
{
 ...
 function fetchPopUpData(grpId,acctNo,index)
  { 
     ...
     var url="CashCollectionOnlineTransBSCNT.cnt?hmode="+mode+"&modName="+modeVal;
	  ...
	  */
	  function showRefundTariffPopup(parent,hidVal) {
	  	...
	  	refundTariffPopupParent = parent;
			...
			var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode+"&hidValue="+hidVal;
	 ...
	 /**
		 ...
		 */
		 function getRefundTariffUnit(unitId,baseValue,index) {
		 	var hmode = "TRFUNIT"; 
			var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode+"&unitId="+unitId+"&baseValue="+baseValue+"&rowIndex="+index;
 ...
 */
 function showBillListingWindow(userJsFuncName) {
 	...
 	if(document.forms[0].strBillNo.value == ""){
				...
				var myWindow = window.open(createFHashAjaxQuery('CashCollectionOnlineTransBSCNT.cnt?hmode='+hmode+"&usrFuncName="+userJsFuncName,'popupWindow',featuresList)); 
  ...
  */
  function fetchBillList(fromRows,blockSet) {
  	...
  	var url = "CashCollectionOnlineTransBSCNT.cnt?hmode="+hmode+"&searchString="+convString+"&fromRow="+fromRows+"&rowPerPage="+gblRowPerPage+"&ctBlockSet="+blockSet;
   ...
   */
   ...
   function hideAll(endVal){
	 ...
	 * findServiceandSave
	 */
	 window.findBillServices = function () 
	 {	 
		 if(!showAlert())
	 		return false;
		 	...
		 	//		confirmLogic();
		 		...
		 		//	return false;
					/*$( function() {
		 			 ...
		 			 if(confirmBS("Are You Sure to Save it?"))
		 			 {
		 			 	confirmTrue();
		 			 	return false;
		 			 }
