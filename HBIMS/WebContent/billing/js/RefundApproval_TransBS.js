 ...
 */
 function groupCheck(chkObj) {
    ...
    document.forms[0].strBillDate.value= chkObj.value.split("^")[17];
     ...
     var strchk = chkObj.value;
     
     var temp = strchk.split("|");
		   var temp1 = temp[0];
		   for (var i = 0; i < temp.length - 1; i++)
			temp1 += "_" + temp[i + 1];
     var url="RefundApprovalTransBSCNT.cnt?hmode="+mode+"&modName="+temp1;
     ajaxFunction(url,"1");
 }
 ...
 */
  ...
  */
  function popUp(obj,index,id) {
 	 ...
 	 var j;
     var mode="PopUp";
     var url="RefundApprovalTransBSCNT.cnt?hmode="+mode+"&modName="+obj.value;
     ajaxFunction(url,"4");
 } 
...
function myFunc(obj,index,id)
{
 ...
 var url="RefundApprovalTransBSCNT.cnt?hmode="+mode+"&modName="+linkVal;
 ajaxFunction(url,"2"); 
}
