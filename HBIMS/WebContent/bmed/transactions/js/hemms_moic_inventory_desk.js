	...
	if(check[3].value=="10")
 	{
 		...
 		document.getElementsByName("chk")[3].value = "-1";
 	}
}
...
function chkUserDefinedFunc(these){
	...
	var verifiedCount=0,modifyCount=0;
	var check=document.getElementsByName("chk");
	...
	if((modifyCount>=1 && count<=0)){
		enableButton("Modify Request");
	}
	else{
		disableButton("Modify Request");
	}
	
	if((modifyCount>=1 && verifiedCount>=1))
	{
		disableButton("Verify");
		disableButton("Modify Request");
	}
	
/*	if(parseInt(checkCount)>=1){
		enableButton("Verify");
		enableButton("Modify Request");
	}
	else{
...
function verifyInventory(mode) 
{
	if(chkIsVerified())
	{
		alert("Inventory already Verified");
		return;
	}
	document.forms[0].hmode.value = mode;
	document.forms[0].submit();
}
function modifyRequest(mode) 
{
	var verfVal = document.forms[0].strChk.value.split("@")[8];
 	verfVal = verfVal.split("$")[0];
 	if(parseInt(verfVal) == 0)
 	{
 		alert("Inventory not Verified");
 		return;
 	}
 	if(parseInt(verfVal) >= 2)
 	{
 		alert("Modification request already sent");
 		return ;
 	}
