...
{	
	...
	try
	{
		...
		if(checkCount=='1')
		{
			...
			{
				enableButton("Assignment");
				disableButton("Down Time");
				disableButton("Up Time");
				enableButton("View");
				return;
			}
			else if(document.getElementById("comboid2").value =="2")
			 {
			 	disableButton("Assignment");
				enableButton("Down Time");
				disableButton("Up Time");
				enableButton("View");
				return;
			 }
...
function buttonLogicsOnClick1(modeNo, mode , display)
{
	...
	try
	{
		if(mode=='Assignment' || mode=='Assignment'){
	    if(checkCount == 0)
	    {
	 ...
	 else if(mode=='DownTime' || mode=='DownTime'){
	   if(checkCount == 0)
	    {
	    	alert("Please Select Record !");
	    }
		...
		else if(mode=='UpTime' || mode=='UpTime'){
	    if(checkCount == 0)
	    {
	    	alert("Please Select Record !");
	    }
	     else
	     {
			...
			if(checkCount=='1')
			{
				if(mode=="MODIFY")
				{
					var cmb = document.getElementsByName("combo");
					if(cmb[2].value=="2")
						add(mode);
					else
						alert("Please Select a Manpower Assigned Record to modify.");
						return false;
				}
				if(mode=="VIEW")
					add(mode);
