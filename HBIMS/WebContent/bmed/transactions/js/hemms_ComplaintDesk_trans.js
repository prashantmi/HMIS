...
function buttonLogicsOnClick1(modeNo, mode , display)
{
	...
	if(mode=="RAISING")
	{
 	...
 	else if(checkCount=='1')
 	   {
 		if(mode=="VIEW")
			{
				var URL="ComplaintDeskACTION.cnt?hmode="+mode+"&chk="+checkedVal; 
		...
		{
			if(mode=="ORIGINALPRINT")
			{
			
				alert('1');
				var URL="ComplaintDeskACTION.cnt?hmode="+mode+"&chk="+checkedVal;  
				//alert("URL::"+URL);  	
				Child = window.open(URL,'popupWindow','status=yes,scrollbars=yes,height=500,width=1000,left=20,top=10');
			}
 			else{
   	 	   	  add(mode);
   	 	   	  } 	
