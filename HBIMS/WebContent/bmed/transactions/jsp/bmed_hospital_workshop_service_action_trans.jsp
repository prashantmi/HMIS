...
<html:form name="hospitalWorkshopDeskFB"
	...
	<table class="TABLEWIDTH" style="display: none;">
		<tr>
			<td colspan="4" class="LABEL"><html:radio name="hospitalWorkshopDeskFB"
				property="strItemOrNonItemMode" value="1"
				onclick="changeViewMode();" /> Item <!-- html:radio name="hospitalWorkshopDeskFB" property="strItemOrNonItemMode" value="0" onclick="changeViewMode();" / -->
			<!-- Non-Item --></td>
		</tr>
	</table>
	<div class="line">
	...
	<table class="TABLEWIDTH" id="attendDetailsTable"
		...
		<tr style="display: none;">
			...
			<td width="25%" class="CONTROL">${hospitalWorkshopDeskFB.strEngineerAddress}</td>
			<input type="hidden" name="strEngineerAddress"
				value="${hospitalWorkshopDeskFB.strEngineerAddress}" />
		</tr>
		<tr>
			<td width="25%" class="LABEL">Service Engineer Name</td>
			<td width="25%" class="CONTROL">${hospitalWorkshopDeskFB.strEngineerName}</td>
			...
			<td width="25%" class="CONTROL">${hospitalWorkshopDeskFB.strMobileNo}</td>
			<input type="hidden" name="strEngineerAddress"
				value="${hospitalWorkshopDeskFB.strMobileNo}" />
		</tr>
		...
		<tr>
			...
			<input type="hidden" name="strAttendDate"
				value="${hospitalWorkshopDeskFB.strAttendDate}" />
			<td width="25%" class="LABEL">Attend Time</td>
			<td width="25%" class="CONTROL">${hospitalWorkshopDeskFB.strAttendTime}</td>
		...
		<tr>
			...
			<td class="CONTROL" rowspan="2">${hospitalWorkshopDeskFB.strActualProblemDesc}</td>
			<input type="hidden" name="strActualProblemDesc"
				value="${hospitalWorkshopDeskFB.strActualProblemDesc}" />
			<td class="LABEL">Remarks</td>
			<td class="CONTROL" rowspan="2">${hospitalWorkshopDeskFB.strAttendRemarks}</td>
	...
	<table class="TABLEWIDTH">
		<tr style="display: none;">
			...
			<div id="strVendorServiceEngId"><select
				...
				<bean:write name="hospitalWorkshopDeskFB" property="strServiceEngNameCombo"
					filter="false" />
			</select> <html:hidden property="strServiceEngNameCombo"
				value="${hospitalWorkshopDeskFB.strServiceEngNameCombo}" /></div>
			</td>
			...
			<td width="25%" class="CONTROL"><input type="text"
				...
				value="${hospitalWorkshopDeskFB.strCommunicateIdContactId}"
		...
		<tr style="display: none;">
			...
			<td width="25%" class="CONTROL"><dateTag:date
				name="strAttendDate" value="${hospitalWorkshopDeskFB.strAttendDate}"></dateTag:date>
			</td>
		...
		<tr style="display: none;">
			...
			Closing</td>
			<td width="25%" class="CONTROL"><dateTag:date
				name="strClosingDate" value="${hospitalWorkshopDeskFB.strClosingDate}"></dateTag:date>
			</td>
			...
			<td width="25%" class="CONTROL"><input type="text"
				...
				value="${hospitalWorkshopDeskFB.strClosingTime}"
				onkeypress='return checkTime(event,this);' /> (HH:MM 24Hr)</td>
		</tr>
	...
	<table class="TABLEWIDTH" id="SparePartMultiRowId">
		...
		<tr>
			<td width="19%" class="CONTROL_TD_NEW">${hospitalWorkshopDeskFB.strStoreNameCombo}
			<input type="hidden" id="strWorkshopName" name="strWorkshopName"
				value="${hospitalWorkshopDeskFB.strWorkshopName}" /></td>

			<td width="19%" class="LABEL">
			<div id="GroupNameDiv" align="center"><select
				name="strGroupName" id="strGroupName" OnChange='getEquipNameCmb();'
				title="Select Group Name">
				<bean:write name="hospitalWorkshopDeskFB" property="strGroupNameCombo"
					filter="false" />
			</select></div>
			</td>
			<td width="19%" class="LABEL">
			<div id="EquipmentNameDiv" align="center"><select
				...
				<bean:write name="hospitalWorkshopDeskFB" property="strEquipNameCombo"
					filter="false" />
			</select></div>
			</td>
			<td width="19%" class="LABEL">
			<div align="center" id="SparePartNameDiv"><select
				...
				<bean:write name="hospitalWorkshopDeskFB" property="strSparePartNameOptions"
					filter="false" />
			</select></div>
			</td>
			...
			<div id="statusDiv" align="center"><select name="strStatus"
				...
				<bean:write name="hospitalWorkshopDeskFB" property="strSparePartStatusOptions"
					filter="false" />
			</select></div>
			</td>
	...
	<table class="TABLEWIDTH">
		...
		<tr>
			...
			Description</td>
			<td width="25%" class="CONTROL"><html:textarea name="hospitalWorkshopDeskFB"
		...
		<tr id="dateAndTimeId" style="display: none;">
			...
			<td width="25%" class="CONTROL"><dateTag:date name="strFromDate"
				value="${hospitalWorkshopDeskFB.strFromDate}"></dateTag:date></td>
	...
	<table class="TABLEWIDTH">
		...
		<tr style="display: none;">
			...
			<div id="strVerifiedById"><select name="strVerifiedBy"
				...
				<bean:write name="hospitalWorkshopDeskFB" property="strVerifiedByOptions"
					filter="false" />
			</select></div>
			<html:hidden property="strVerifiedByOptions"
				value="${hospitalWorkshopDeskFB.strVerifiedByOptions}" /></td>
		...
		<tr style="display: none;">
			...
			<td width="25%" class="CONTROL"><dateTag:date
				name="strVerifyDate" value="${hospitalWorkshopDeskFB.strVerifyDate}"></dateTag:date>
			</td>
	...
	<input type="hidden" name="strEmpName" value="${hospitalWorkshopDeskFB.strEmpName}" />
	<input type="hidden" name="strItemId" value="${hospitalWorkshopDeskFB.strItemId}" />
	<input type="hidden" name="strStoreId" value="${hospitalWorkshopDeskFB.strStoreId}" />
	<input type="hidden" name="strEnggItemTypeId"
		value="${hospitalWorkshopDeskFB.strEnggItemTypeId}" />
	<input type="hidden" name="strEnggItemSubTypeId"
		value="${hospitalWorkshopDeskFB.strEnggItemSubTypeId}" />
	<input type="hidden" name="strComplaintId"
		value="${hospitalWorkshopDeskFB.strComplaintId}" />
	<input type="hidden" name="strPageFlag"
		value="${hospitalWorkshopDeskFB.strPageFlag}" />
	<input type="hidden" name="strDesignation" value="" />
	<input type="hidden" name="strOldDepartmentCode"
		value="${hospitalWorkshopDeskFB.strDeptCode}" />
	...
	<input type="hidden" name="strComplaintStatus" id="strComplaintStatus"
		value="${hospitalWorkshopDeskFB.strComplaintStatus}">
	...
	<html:hidden property="strSparePartNameOptions"
		value="${hospitalWorkshopDeskFB.strSparePartNameOptions}" />
	<html:hidden property="strManufactureNameOptions"
		value="${hospitalWorkshopDeskFB.strManufactureNameOptions}" />
	<html:hidden property="strSparePartStatusOptions"
		value="${hospitalWorkshopDeskFB.strSparePartStatusOptions}" />
	<input type="hidden" name="strRetValue"
		value="${hospitalWorkshopDeskFB.strRetValue}" />
	<input type="hidden" name="strMsgString"
		value="${hospitalWorkshopDeskFB.strMsgString}" />

	<input type="hidden" name="strUploadFileId" value="" />
	<input type="hidden" name="strAttendedContactPerson" value="" />
	<html:hidden name="hospitalWorkshopDeskFB" property="strSerialNo" value="0" />
	<html:hidden name="hospitalWorkshopDeskFB" property="strSparePartsFlag" value="0" />
