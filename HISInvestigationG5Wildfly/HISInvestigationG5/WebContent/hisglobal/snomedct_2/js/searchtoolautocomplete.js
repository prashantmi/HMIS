...
$(document)
		.ready(
				function() {
					...
					divDialogForm = '<div id="dialog-form" style="width: 600px; display:none;">'
							+ dialogFormHTML + '</div>';
...
function loadResultsListauto(state, semantictag, acceptability, returnlimit, call, elmtid) {
	//alert("inside load result");
	$('#reccnt').show();
	$('#reccount').show();
	$('#norec').hide();
	$('#msg3chars').hide();
	...
	if (document.getElementById("conceptdiv_"+elmtid))
		$("#conceptdiv_"+elmtid).remove();
	...
	if (displaySearch == true) {
		...
		if (dataValue.trim() == '') {
		/*	$("#dialog-form_"+elmtid).dialog("option", "height", 160);*/
			if (document.getElementById("conceptdiv_"+elmtid))
				$("#conceptdiv_"+elmtid).remove();
			$('#reccnt').hide();
			$('#reccount').hide();
			...
			$('#msg3chars').hide();
			$('#nosuggestion').hide();
			var txtBox = document.getElementById("txt-snomed-ct-search_"+elmtid);
			txtBox.focus();
			return;
		}
				...
				.ajax({
					...
					success : function(data, textStatus, jqXHR) {
						...
						/*	$("#dialog-form_"+elmtid).dialog("option", "height", 160);*/
							...
							$('#nosuggestion').hide();
							var txtBox = document
									.getElementById("txt-snomed-ct-search_"+elmtid);
							txtBox.focus();
							return;
						}
						...
						htmlData = '';
						htmlData += '<div class="concept" id="conceptdiv_'+elmtid+'"  style="display:block; float:inherit; z-index:2; border-style:groove;border-radius:6px; position:absolute;  margin-right:28%; background:#E5E5E6; width:40%;" >';
						...
						/*$("#dialog-form_"+elmtid).dialog("option", "height", "500");*/
						htmlData += '</table>';
						htmlData += '</div>';
						if (document.getElementById("conceptdiv_"+elmtid))
							$("#conceptdiv_"+elmtid).remove();
						$("#dialog-form_"+elmtid).append(htmlData);
 ...
 * 
 */
function selectSNOMEDCTauto(state_IN, semantictag_IN, acceptability_IN, returnlimit_IN,
		elmtid,callback) {

	//alert(elmtid+','+semantictag_IN);
	...
	$("#dialog-form_"+elmtid).click(
			function(e) {
				if (e.target.id == "srcImg") {
					// document.getElementById("txt-snomed-ct-search_"+elmtid).value="fev";
					var elem = document.getElementById("txt-snomed-ct-search_"+elmtid);
					...
					if (dataValue.length >= 3) {
						...
						$('#msg3chars').hide();
						$('#nosuggestion').hide();
						loadResultsListauto(elem.stateParam, elem.semantictagParam,
								elem.acceptabilityParam, elem.returnlimitParam, callback,elmtid);
					} else {
						if (document.getElementById("conceptdiv_"+elmtid))
							$("#conceptdiv_"+elmtid).remove();
						$('#msg3chars').show();
...
*/	displaySearch = false;
	...
	/*$("#dialog-form_"+elmtid).dialog("option", "height", 160);*/
/*	$("#dialog-form_"+elmtid).html(dialogFormHTML);*/
/*	jQuery('.ui-dialog button:nth-child(1)').button('disable');
	...
	//$("#dialog-form_"+elmtid).dialog("open");
			...
			.autocomplete(
					{
						...
						source : function(request, response) {
                           ...
                           // alert(dataValue);
							var servURL = "";
							if (dataValue == '') {
							/*	$("#dialog-form_"+elmtid).dialog("option", "height",
										160);*/
								if (document.getElementById("conceptdiv_"+elmtid))
									$("#conceptdiv_"+elmtid).remove();
								var txtBox = document
							...
							servURL += "?term="
									...
									+ "&state=" + elem.stateParam + "&semantictag="
									+ elem.semantictagParam + "&acceptability="
									+ elem.acceptabilityParam + "&returnlimit=5";
							console.log(servURL);
						...
						select : function(event, ui) {
							...
							if (dataValue.length >= 3) {
								...
								$("#msg3chars").hide();
								$('#nosuggestion').hide();
								document.getElementById("txt-snomed-ct-search_"+elmtid).value = ui.item.value;
								if (document.getElementById("conceptdiv_"+elmtid))
									$("#conceptdiv_"+elmtid).remove();
...
function selectValue(value, callback) {
	...
	$("#dialog-form_"+elmtid).dialog("close");
