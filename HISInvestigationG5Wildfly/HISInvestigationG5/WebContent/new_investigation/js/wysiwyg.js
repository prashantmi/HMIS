 ...
 *
 * $Id: wysiwyg.js,v 1.2 2010/06/14 05:27:22 siddharth Exp $
 * $Revision: 1.2 $
 *
...
var WYSIWYG = {
	...
	Settings: function() {
		...
		// Images Directory
		this.ImagesDir = "/HISInvestigationG5/hisglobal/images/";
		
		// Popups Directory
		this.PopupsDir = "/HISInvestigationG5/new_investigation/js/popups/";
		
		// CSS Directory File
		this.CSSFile = "/HISInvestigationG5/hisglobal/css/wysiwyg.css";		
	...
	"print":		  ['Print', 			   'Print',       	 	 	'print_on.gif',			  'print_on.gif'],
	"removeformat":   ['RemoveFormat',         'Strip Word HTML',    	'remove_format.gif',      'remove_format_on.gif'],
	"delete":         ['Delete',               'Delete',             	'delete_on.gif',     	  'delete_on.gif'],
	//"save": 		  ['Save', 				   'Save document',         'saveoff.gif', 			  'save_on.gif'],
	...
	//"canned": 	  ['canned', 			   'Canned File',   'C.png', 		  'C.png']
	},
	...
	attach: function(id, settings) {
		...
		CKEDITOR.replace( id );
		//var id1 = '1011000315062610000901#null#template#100071006';
		//var editor = CKEDITOR.instances.id;
		//alert(id);
		//alert(editor);
		//window.onload = editor.setData("<p>dfdf</p>");
		/*if(id != "all") {	
	...
	_display: function(n, settings) {
		...
		if(textarea == null) {
			HISAlert('info','',"No textarea found with the given identifier (ID: " + n + ").");
			return;
		}
		
		// Validate browser compatiblity
		if(!WYSIWYG_Core.isBrowserCompatible()) {
			if(this.config[n].NoValidBrowserMessage != "") { HISAlert('info','',this.config[n].NoValidBrowserMessage); }
			return;
		}
		...
		var iframe = '<table cellpadding="0" cellspacing="0" border="0" style="width:' + currentWidth + '; height:' + currentHeight + ';" class="tableTextareaEditor"><tr><td valign="top" onclick="callOnFocus(document.getElementById(\"'+n+'\"))">\n'
	    + '<iframe frameborder="0" id="wysiwyg' + n + '" class="iframeText" style="width:' + ifrmWidth + ';height:' + ifrmHeight + ';"></iframe>\n'
	    + '</td></tr></table>\n';
		//HISAlert('info','',iframe);
	...
	_generate: function(n, settings) {
		...
		textarea.style = "width:560px;height:200px;";						
		// Hide the textarea 
		textarea.style.display = 'none'; 
	    ...
	    var editor = "";
	    editor += '<div id="wysiwyg_div_' + n + '" style="width:' + currentWidth  +';" onclick="callOnFocus(document.getElementsByName(\''+n+'\')[0])">';
		...
		for (var j = 0; j < this.config[n].Toolbar.length;j++) { 
	 	...
	 	editor += '</td></tr><tr onclick="callOnFocus(document.getElementsByName(\''+n+'\')[0])"><td valign="top" onclick="callOnFocus(document.getElementsByName(\''+n+'\')[0])">\n';
		// Create iframe which will be used for rich text editing
	    ...
	    if(this.config[n].StatusBarEnabled) {
		    editor += '<tr><td class="wysiwyg-statusbar" style="height:10px;" id="wysiwyg_statusbar_' + n + '" onclick="callOnFocus(document.getElementsByName(\''+n+'\')[0])">&nbsp;</td></tr>';    
		}
	    editor += '</table>';
	    editor += '</div>';
	...
	disable: function(n) {
		...
		if(editor == null) {
			HISAlert('info','',"No editor found with the given identifier (ID: " + n + ").");
			return;
		}
	...
	execCommand: function(n, cmd, value) {
		...
		if(this.viewTextMode[n] && !cmdValid) {
			HISAlert('info','',"You are in TEXT Mode. This feature has been disabled.");
		  	return;
		} 
		...
		switch(cmd) {
			...
			case "ForeColor":
				...
				var rgb = this.getEditorWindow(n).document.queryCommandValue(cmd);
			  	...
			  	window.open(this.config[n].PopupsDir + 'select_color.html?color=' + currentColor + '&command=' + cmd + '&wysiwyg=' + nstr, 'popup', 'location=0,status=0,scrollbars=0,width=210,height=165,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
			break;
			...
			case "InsertImage": 
				...
				window.open(imagePopupFile + '?wysiwyg=' + nstr, 'popup', 'location=0,status=0,scrollbars=0,resizable=0,width=' + imagePopupWidth + ',height=' + imagePopupHeight + ',top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
			break;
			...
			case "InsertTable": 
				...
				window.open(this.config[n].PopupsDir + 'create_table.html?wysiwyg=' + nstr, 'popup', 'location=0,status=0,scrollbars=0,resizable=0,width=500,height=260,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
			break;
			...
			case "Help":
				...
				window.open(this.config[n].PopupsDir + 'about.html?wysiwyg=' + nstr, 'popup', 'location=0,status=0,scrollbars=0,resizable=0,width=400,height=350,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
			break;
			...
			case "Preview":
				...
				window.open(this.config[n].PopupsDir + 'preview.html?wysiwyg=' + nstr,'popup', 'location=0,status=0,scrollbars=1,resizable=1,width=' + this.config[n].PreviewWidth + ',height=' + this.config[n].PreviewHeight + ',top=' + popupPosition.top + ',left='+popupPosition.left).focus();
			break;
			...
			case "canned":
				...
				if(n.length>20)
				{
					urlStr="/HISInvestigationG5/new_investigation/transaction/resultEntryPOPUP.cnt?hmode=GETCANNEDFILE&selectedLab="+n.substring(3,8)+"&hospitalCode="+n.substring(0,3)+"&callingMode=editor&eName="+nstr+"&wysiwyg="+nstr;
				}
				else
				  urlStr="/HISInvestigationG5/new_investigation/transaction/resultEntryPOPUP.cnt?hmode=GETCANNEDFILE&callingMode=editor&eName="+nstr+"&wysiwyg="+nstr;
				document.forms[0].currentElement=document.getElementById(n);
				var child_1=window.open (urlStr,"mywindow","location=1,status=0,scrollbars=0,width=400,height=300");
				child_1.moveTo(300, 300);
			break;
			...
			case "Save":
			    ...
			    if(form == null) {
			    	HISAlert('info','',"Can not submit the content, because no form element found.");
			    	return;
			    }
			   // form.submit();
	...
	updateTextArea: function(n) {	
		...
		////HISAlert('info','','updateTextArea '+this.getEditor(n));
	    if(this.getEditor(n)!=null)
	    {
...
var WYSIWYG_Table = {
	...
	_enableHighlighting: function(node) {
		...
		if(style == null) style = " ";
		//HISAlert('info','',"ENABLE: ELM = " + node.tagName + "; STYLE = " + style);
	...
	_disableHighlighting: function(node) {
		...
		//HISAlert('info','',"DISABLE: ELM = " + node.tagName + "; STYLE = " + style);
