...
function validateFinalSubmit(){
    ...
    {
          alert("Select atleast one Canned");
	...
	for(var i=0;i<document.getElementsByName("userCannedCode").length;i++)
		{
		
		var k=0;
		for(k=i+1;k<document.getElementsByName("userCannedCode").length;k++)
			if(document.getElementsByName("userCannedCode")[i].value.toUpperCase()==document.getElementsByName("userCannedCode")[k].value.toUpperCase())
			{	alert("Duplicate Canned Code Exists. Enter Unique Values.");
				document.getElementsByName("userCannedCode")[k].focus();
				return false;
			}
		
		
		
		if(document.getElementsByName("userCannedCode")[i].value=="")
{
			  alert("Enter the Code Value");
	          document.getElementsByName("userCannedCode")[i].focus();
	          return false;
...
function moveRightSingle()
{
	...
	document.getElementById("cannedCodeInput").style.display="";
...
function AddRowToTable(cannedName)
{ 
		
	    var nRow=0;
	  	var tableObj=document.getElementById('cannedCodeInput');
		...
		td1.innerHTML="<div align='center'><font color='#000000' size='2' face='Verdana, Arial, Helvetica, sans-serif'>"+cannedName+"<bean:message key=MandatoryValue />&nbsp; </font> </div>";
	  	td1.className='tdfont';
	  	td1.colspan="1";
	  	
	  	
	  	td2.innerHTML="<div align='center'>"+" <input type='text' name='userCannedCode' id='userCannedCode"+index+"'  value='' maxlength='10' size='60' style='width:200px;' /></div>"
