...
// 	var objXHR = {url: "/HISInvestigationG5/new_investigation/antibioticprocesss.cnt?hmode="+_mode+"&organismCode="+organismCode, sync:true, postData: "", handleAs: "text",async:false,
// 		load: function(data) 
// 		{
// 			//alert(data);
// 			labTestCodeArray = data;
...
/* 
	$.ajax({url: "/HISInvestigationG5/new_investigation/antibioticprocesss.cnt?hmode="+_mode+"&organismCode="+organismCode,
		async: true,
...
<body onload="call()">
<html:form action="/antibioticprocesss">
	<html:hidden name="invAntibioticProcessFB" property="hmode" /> 
		<html:hidden name="invAntibioticProcessFB" property="counterrr" /> 
	
	<html:hidden name="invAntibioticProcessFB" property="growthname" /> 
	
	
	<html:hidden name="invAntibioticProcessFB" property="organismnamee" />  
	     <html:hidden name="invAntibioticProcessFB" property="currentPage" />
	  <html:hidden name="invAntibioticProcessFB" property="showStatus" />
	  <html:hidden name="invAntibioticProcessFB" property="requisitionDNo" />
	  <html:hidden name="invAntibioticProcessFB" property="testParaCode" />   
	  <html:hidden name="invAntibioticProcessFB" property="flag" />
	...
	<%
			...
			List<invAntibioticProcessVO> lstPatVO12221=null;
			/* List<invAntibioticProcessVO> getantiwholelist=null; */
				...
				ResultEntryVO vo=lstPatVO11321.get(i);
				 ...
				 if(lstPatVO12212!=null)
				 {
					...
					ResultEntryVO vo1=vo11.getParameterCombo().get(k1);
					 ...
					 if(vo11.getGetantibioticlistdynamic().size()>0)
						{
						 for(int k1=0;k1<vo11.getGetantibioticlistdynamic().size();k1++)
						 {
						 invAntibioticProcessVO vo122=vo11.getGetantibioticlistdynamic().get(k1);
					
						 if((reqDno21121.equals(vo122.getReqDno())) && (testParaCod21121.equals((vo122.getTestParaCode()))))
						 {
							 lstPatVO12221=vo11.getGetantibioticlistdynamic();
							 
							 for(int i1=0;i1<lstPatVO12221.size();i1++)
							 {
								 invAntibioticProcessVO getantilist=lstPatVO12221.get(i1);
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="0">
			    ...
			    <td width="25%" class="tdfont">
			        <div align="right">
						...
						<html:hidden name="invAntibioticProcessFB" property="organismName" />
				     </div>
			      </td>
			      <td width="25%" class="tdfont">
				 ...
				 <html:select name="invAntibioticProcessFB" property="organismCode"    tabindex="1"  onchange="getantibiotic(this)">
				       					<html:option value="-1">Select value</html:option>	
				 	   					<html:options collection="<%=InvestigationConfig.ORGANISM_COMBO%>" property="value" labelProperty="label"/>
				      				</html:select>
				      				
				      				
				      				<%-- <html:hidden name="invAntibioticProcessFB" property="organismCode"/> --%>
				      				</span>
				  </div>
			     ...
			     <td width="25%" class="tdfont">
			      ...
			      &nbsp;&nbsp;
					   <span class="custom-dropdown small">
				 <html:select name="invAntibioticProcessFB" property="growthCode"    tabindex="1"  onchange="">
				       					<html:option value="-1">Select value</html:option>
...
<%-- 			   <his:ButtonToolBarTag>
			...
			<%} %>
			    ...
			    <%
			    boolean flag=false; %>
			   	  <logic:equal name="invAntibioticProcessFB" property="showStatus" value="0"> 
  	 <%
				//Pagination Logic
					PaginationFB fbPage= new PaginationFB();
					pageContext.setAttribute("fbPagination",fbPage);
					fbPage.setCurrentPage(((invAntibioticProcessFB)request.getAttribute("invAntibioticProcessFB")).getCurrentPage());
			...
			<tr>
					<td width="10%" align="left"  >	
						<b> <font color="#000000" size="2" 
							...
							<%
							 List<invAntibioticProcessVO> preffered=(List<invAntibioticProcessVO>)session.getAttribute(InvestigationConfig.ANTIBIOTIC_COMBO_PREFFREDED);
                              String prefferedd="";
							 if(preffered!=null && preffered.size()>0)
							 {
								 for(int i=0;i<preffered.size();i++)
								 {
								 invAntibioticProcessVO vo=preffered.get(i);
			...
			<table   width="100%" bgcolor="#EBEBEB" cellspacing="0" style="border-spacing: 0;">
					<%
					String anticode="";
					 List<invAntibioticProcessVO> lstPatVO=(List<invAntibioticProcessVO>)session.getAttribute(InvestigationConfig.ANTIBIOTIC_COMBO);
					 int i,size=0;
					...
					if(j<size)
									{
						invAntibioticProcessVO voPat=lstPatVO.get(j);
			 		...
			 		String reqnoo=(String)session.getAttribute("reqn1o");
						...
						<input type="checkbox" checked="checked" id="s#<%=j %>" name="chkInfo" value="2" onclick="chkselect(this)"/></font>
						 	...
						 	<%} %>
						 	<%-- <html:checkbox  id="<%=j %>" property="chkInfo" name="invAntibioticProcessFB" value="1" onclick="chkselect(this)"></html:checkbox> --%>
	   ...
	   <%} %>
				  ...
				  <% 
					...
					<%}}}  }%>
		<logic:notEmpty name="<%=InvestigationConfig.list_in_sessionhyperlinkdata%>">
	...
	<img class="button" src='<his:path src="/hisglobal/images/Add.png"/>' id="gob"  style="cursor: pointer" onkeypress="if(event.keyCode==13) adddetails('1')" onclick="adddetails('1')" tabindex="1">
		    ...
		    <div id="pagbtn" style="display: none">		
		    	<logic:empty name="<%=InvestigationConfig.list_in_sessionhyperlinkdata%>">
	...
	<img class="button" src='<his:path src="/hisglobal/images/Add.png"/>' id="gob"  style="cursor: pointer" onkeypress="if(event.keyCode==13) adddetails('1')" onclick="adddetails('1')" tabindex="1">
			...
			List<invAntibioticProcessVO> lstPatVO1222=null;
			/* List<invAntibioticProcessVO> getantiwholelist=null; */
				...
				ResultEntryVO vo=lstPatVO1132.get(i);
				 ...
				 if(lstPatVO12212!=null)
				 {
					...
					ResultEntryVO vo1=vo11.getParameterCombo().get(k3);
					 ...
					 if(vo11.getGetantibioticlistdynamic().size()>0)
						{
						 for(int k33=0;k33<vo11.getGetantibioticlistdynamic().size();k33++)
						 {
							 
						 
						 invAntibioticProcessVO vo122=vo11.getGetantibioticlistdynamic().get(k33);
					
						 if((reqDno2112.equals(vo122.getReqDno())) && (testParaCod2112.equals((vo122.getTestParaCode()))))
						 {
							 lstPatVO1222=vo11.getGetantibioticlistdynamic();
							 
							 for(int i1=0;i1<lstPatVO1222.size();i1++)
							 {
								 invAntibioticProcessVO getantilist=lstPatVO1222.get(i1);
			...
			boolean flag1=false; %>
			 ...
			 <%
				//Pagination Logic
					...
					fbPage1.setCurrentPage(((invAntibioticProcessFB)request.getAttribute("invAntibioticProcessFB")).getCurrentPage());
     	 ...
     	 <%
			...
			<table   width="100%" bgcolor="#EBEBEB" cellspacing="0" style="border-spacing: 0;">
					<%
					 ...
					 List<invAntibioticProcessVO> lstPatVO11=(List<invAntibioticProcessVO>)session.getAttribute(InvestigationConfig.ANTIBIOTIC_COMBO);
