...
public class externalInvestigationCaptureDAO extends DataAccessObject implements InvestigationEssentialDAOi
{
	public externalInvestigationCaptureDAO(JDBCTransactionContext _tx)
	{
		super(_tx);
	}
	...
	*/
	...
	public void insertRequisitionDtl(InvestigationRequistionVO voLabTest, UserVO _userVO)
	{
		...
		catch (HisRecordNotFoundException e)
		{
	  	...
	  	// insert in requisition dtl table for external capture

		public void insertRequisitionDtl(externalInvestigationCaptureVO external_vo, UserVO _userVO) {

			String query = "";
			Map populateMAP = new HashMap();
			Sequence sq = new Sequence();
			String filename = InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
			String queryKey ="INSERT.HIVT_EXTERNAL_INVESTIGATION_DTL";
			...
			try {
				...
				populateMAP.put(sq.next(),external_vo.getExternalLabCode());
				populateMAP.put(sq.next(),external_vo.getLabCode()); 
				populateMAP.put(sq.next(),external_vo.getLabName()); 
				populateMAP.put(sq.next(),external_vo.getParameterCode()); 
				populateMAP.put(sq.next(),external_vo.getParameterName());
				populateMAP.put(sq.next(),external_vo.getSampleCode()); 
				populateMAP.put(sq.next(),external_vo.getSampleName()); 
				populateMAP.put(sq.next(),external_vo.getTestCode()); 
				populateMAP.put(sq.next(),external_vo.getTestName()); 
				populateMAP.put(sq.next(),_userVO.getHospitalCode());
				populateMAP.put(sq.next(),external_vo.getResult());
				populateMAP.put(sq.next(),external_vo.getPatCrNo()); 
				populateMAP.put(sq.next(),_userVO.getSeatId());
				populateMAP.put(sq.next(),external_vo.getFileName());
		...
		//using to get all global labs for external capture 
		public List getLabNames(UserVO _UserVO)
		{
			...
			String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
			String queryKey="SELECT.GLOBALLAB_NAMES_EXTERNALPROCESS.HIVT_LABORATORY_MST";
	...
	/*	public void insertHivtRequsitionTestMandatoryDtl(InvestigationRequistionVO voLabTest, UserVO _userVO) {
		...
		public  List<LabTestVO> getAptByDesk(InvestigationSearchVO searchVO ,UserVO _UserVO)
		{
			...
			Sequence sq = new Sequence();
			String cond1="and HIVDT_REQUISITION_DATE>=TO_DATE(?,'dd-Mon-YYYY') and HIVDT_REQUISITION_DATE<=TO_DATE(?,'dd-Mon-YYYY')";	
			String cond2="and HIVDT_REQUISITION_DATE>=TO_DATE(sysdate,'dd-Mon-YYYY') and HIVDT_REQUISITION_DATE<=TO_DATE(sysdate,'dd-Mon-YYYY')";
			populateMAP.put(sq.next(), _UserVO.getHospitalCode());
			populateMAP.put(sq.next(), _UserVO.getHospitalCode());
			populateMAP.put(sq.next(), InvestigationConfig.ADVISED_TEST_STATUS);
			
			
			if(searchVO.getFromDate().equals("")==false)
			{
		...
		public  List<LabTestVO> getAppointment(String reqNo, String crNo,UserVO _UserVO)
		{
			...
			// String condition2=" AND TRUNC(hivdt_appointment_date) >= TRUNC(TO_DATE(SYSDATE,'DD-Mon-YYYY'))  AND TRUNC(hivdt_appointment_date) <= TRUNC(TO_DATE(SYSDATE,'DD-Mon-YYYY')))";
			//	String condition3="and  TRUNC(hivdt_appointment_date) >= TRUNC(TO_DATE(\'"+objSampleCollectionVO.getFromDate()+"\','DD-Mon-YYYY')) AND TRUNC(hivdt_appointment_date) <= TRUNC(TO_DATE(\'"+objSampleCollectionVO.getToDate()+"\','DD-Mon-YYYY')))";
		...
		public void saveAppointmentDetail(InvestigationRequistionVO searchVO ,UserVO _UserVO)
		{
			...
			try
			{
				...
				// UPDATE hivt_requisition_dtl set hivdt_appointment_date=TO_DATE(?),hivstr_appointment_time=?,hapnum_app_ref_no=?,hivnum_reqdtl_status=? where hivtnum_req_dno=? and gnum_hospital_code=?
		...
		public  List<LabTestVO> searchLabTestSample(InvestigationSearchVO searchVO ,UserVO _UserVO)
		{
			...
			List<LabTestVO> lstLabVO=null;
			String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
			String queryKey = "SELECT.LAB_TEST_SAMPLE";
	...
	//get external lab names from external lab master
		public List getExternalLabNames(UserVO _UserVO)
		{
			String query="";
			Map populateMap= new HashMap();
			ResultSet rs=null;
			List<String> lstLabNames=new ArrayList<String>();
			String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
			String queryKey="SELECT.EXTERNAL_LAB_NAMES.HIVT_EXTERNAL_LABORATORY_MST";
		...
		public List getSampleCombo_external(UserVO _UserVO)
		{
			String query="";
			Map populateMap= new HashMap();
			ResultSet rs=null;
			List lstSample=new ArrayList();
			String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
			String queryKey="SELECT.SAMPLES_EXTERNAL.HIVT_SAMPLE_MST";
		...
		//using for external capture
		public List getTestNames_external(UserVO _UserVO)
		{
			String query="";
			Map populateMap= new HashMap();
			ResultSet rs=null;
			List<String> lstTestNames=new ArrayList<String>();
			String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
			String queryKey="SELECT.TEST_COMBO_NAMES_EXTERNALPROCESS.HIVT_TEST_MST";
		...
		//using for external capture
		public List getParameterNames_external(UserVO _UserVO)
		{
			String query="";
			Map populateMap= new HashMap();
			ResultSet rs=null;
			List<String> lstTestNames=new ArrayList<String>();
			String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_TRANSACTIONDAO;
			String queryKey="SELECT.PARAMETER_COMBO_NAMES.HIVT_PARAMETER_MST";
		...
		public void insertRequisitionDtl(InvestigationRequistionVO voLabTest,
				UserVO _userVO, String prioirtyAll) {
