...
public class MacroMstDAO extends DataAccessObject implements MacroMstDAOi
{
	...
	public void createMacro(MacroMasterVO macromaster_VO, UserVO _UserVO)
	{
		...
		String queryKey = "INSERT.HIVT_MACRO_MST";
		try
		{
		...
		try
		{
			//populateMAP.put(sq.next(), macromaster_VO.getMacroCode());
			populateMAP.put(sq.next(), _UserVO.getSeatId());
			populateMAP.put(sq.next(), InvestigationConfig.IS_VALID_ACTIVE);
			populateMAP.put(sq.next(), macromaster_VO.getMacroText());
			populateMAP.put(sq.next(), macromaster_VO.getRemarks());
			
		}
		catch (Exception e)
		{
			throw new HisApplicationExecutionException("macromaster_DAO.populateMAP::" + e);
		}
		try
		{
	...
	public void fetchMacro(MacroMasterVO macromaster_VO, UserVO _UserVO)
	{
		...
		ResultSet rs=null;
		String filename= InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
		String queryKey="SELECT.HIVT_MACRO_MST";
...
//		populateMap.put(sq.next(),macromaster_VO.getMacroText());
		...
		try
		{
			...
			else
			{
				...
				while(rs.next())
				{
					HelperMethods.populateVOfrmRS(macromaster_VO ,rs);
				}
			}
		}
		catch(Exception e)
	...
	public void updateMacro(MacroMasterVO macromaster_VO, UserVO _UserVO)
	{
		...
		Sequence sq = new Sequence();
		String filename = InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
		String queryKey ="UPDATE.HIVT_MACRO_MST";
		try
		{
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		}
	...
	public String checkDuplicateMacro(MacroMasterVO macromaster_VO, UserVO _UserVO)
	{
		...
		Sequence sq = new Sequence();
		String filename = InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
		String queryKey ="CHECKDUPLICATE.HIVT_MACRO_MST";
		try
		{
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		}
		...
		try
		{
			populateMAP.put(sq.next(), macromaster_VO.getMacroText().trim());
	...
	public String checkDuplicateModifyMacro(MacroMasterVO macromaster_VO,UserVO _UserVO)
	{
		...
		Sequence sq = new Sequence();
		String filename = InvestigationConfig.QUERY_FILE_FOR_INVESTIGATION_MASTERSDAO;
		String queryKey ="CHECKDUPLICITY_MODIFY.HIVT_MACRO_MST";
		try
		{
			query = HelperMethodsDAO.getQuery(filename, queryKey);
		}
