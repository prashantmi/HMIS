...
import new_investigation.masters.controller.data.TestMandatoryGlobalMstDATA;
...
import hisglobal.exceptions.HisMandatoryFieldEmptyException;
...
public class TestMandatoryGlobalMstUTIL extends ControllerUTIL implements MasterInterface {
	...
	{
		String masterName = "Test Mandatory Global Master";
		return masterName;
	}
	...
	public static boolean fetchTestMandatory(TestMandatoryMstFB fb, HttpServletRequest _request)
	{
		...
		try
		{
			...
			mp=TestMandatoryGlobalMstDATA.fetchTestMandatory(testMandatoryMstVO, userVO);
	...
	public static boolean getMandatory(TestMandatoryMstFB fb, HttpServletRequest _request)
	{
		...
		try
		{
			...
			mp=TestMandatoryGlobalMstDATA.getMandatory(testMandatoryMstVO, userVO);
			WebUTIL.setMapInSession(mp, _request);
	...
	public static boolean saveTestMandatoryMst(TestMandatoryMstFB fb, HttpServletRequest _request)
	{
		...
		List oldMappedList=TestMandatoryGlobalMstUTIL.getOldMappedList(fb, _request);			
			
		//getting the New mapped list
		
		List newMappedList=TestMandatoryGlobalMstUTIL.getNewMappedList(fb, _request);
		...
		if(oldMappedList==null)
			...
			insertList=TestMandatoryGlobalMstUTIL.getCompareList(newMappedList, oldMappedList);	
			
			
			//getting the insert  list
			deleteList=TestMandatoryGlobalMstUTIL.getCompareList(oldMappedList,newMappedList);	
			
			//getting the insert MasterVO
			if(insertList!=null)				
				insert_testMandatoryMstVO=TestMandatoryGlobalMstUTIL.getLabCollectionAreaVO(insertList, fb);
			
			//getting the insert MasterVO
			if(deleteList!=null)				
				delete_testMandatoryMstVO=TestMandatoryGlobalMstUTIL.getLabCollectionAreaVO(deleteList, fb);
...
*/			
			TestMandatoryGlobalMstDATA.saveTestMandatory(insert_testMandatoryMstVO,delete_testMandatoryMstVO, userVO);
		...
		catch(HisMandatoryFieldEmptyException e)
		{
			saveFlag=false;
			System.out.println(e.getMessage());
			objStatus.add(Status.ERROR_DA, "", "Select Atleast One Mandatory");	
		}
		catch (HisDataAccessException e)
		{
			saveFlag=false;
...
return testMandatoryMstVO;
...

    {
...

          {
                ...
                mp.putAll(TestMandatoryGlobalMstDATA.fetchTestMandatory(testMandatoryMstVO,userVO));
                mp.putAll(TestMandatoryGlobalMstDATA.getMandatory(testMandatoryMstVO, userVO));
                WebUTIL.setMapInSession(mp, _request);
                fb.setTestCode(testCode);
                testMandatoryMstVO.setTestCode(testCode);
                objStatus.add(Status.NEW);
